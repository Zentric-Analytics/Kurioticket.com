...
The worker is the only authorized creator of Preview native builds. Expo GitHub build triggers and repository workflows that invoke `eas build` must remain disabled. Every cycle compares exact-SHA EAS history with durable action IDs. An unowned build is persisted as an ownership incident and rejected unless its Expo project, platform, Preview profile, application identifier, source SHA, native fingerprint, version/build number, terminal artifact, and any iOS submission all match exactly.

A fully verified finished artifact may be adopted with `npm run preview-release:adopt-native -- --platform ios --sha <current-dev-sha> --build-id <eas-build-id> --submission-id <eas-submission-id>`; Android omits `--submission-id`. Adoption is current-`dev`-SHA-only, active-mode-only, transactionally rejects conflicting IDs or delivered build numbers, and records `SAFE_VERIFIED_ADOPTION`. Missing historical provider objects are retained as `REMOTE_OBJECT_UNAVAILABLE`, never deleted or silently recreated. Ownership, release, and notification reconciliation are isolated so one platform cannot suppress eligible final notifications for the other.

If the currently delivered Android baseline remains fingerprint-compatible but its exact EAS build object has been permanently removed, an owner-approved installability backfill uses `npm run preview-release:recover-native -- --platform android --sha <current-dev-sha> --artifact-backfill true`. The command first proves the delivered build ID and fingerprint, receives the provider's exact unavailable-object response, preserves the former action as `REMOTE_OBJECT_UNAVAILABLE`, reserves one durable replacement identity, and then uses the normal frozen-credential Preview build and baseline-advancement path. It fails closed while the original artifact is retrievable and is not available for iOS or Production.

Release planning calculates native fingerprints through the pinned EAS CLI with `fingerprint:generate --build-profile preview`. This loads the same EAS `preview` environment and inline profile variables used by `eas build`; raw local `@expo/fingerprint` execution is forbidden because dynamic app config may otherwise resolve differently. During an ownership incident, an owner-approved canonical replacement uses `npm run preview-release:recover-native -- --platform <android|ios> --sha <current-dev-sha>`. It requires a rejected incident, corrects a stale pre-fix planned fingerprint only before any owned remote build exists, reserves the fingerprint identity transactionally before EAS creation, and reuses the normal build, submission, TestFlight, delivered-pointer, and notification paths. Rerunning the command reconciles the durable reservation rather than creating another build.
