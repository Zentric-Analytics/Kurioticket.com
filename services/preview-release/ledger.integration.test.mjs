...
test("PostgreSQL canonical native projection is numeric, monotonic, and platform-independent", { skip: !connectionString }, async () => {
  ...
  try {
    await ledger.pool.query("DROP TABLE IF EXISTS preview_delivered_native_state, preview_release_action, preview_release CASCADE; DROP SEQUENCE IF EXISTS preview_release_progression_order_seq CASCADE");
