...
const routeLabel = (alert: AccountPriceAlert) => alert.origin && alert.origin.trim().toLowerCase() !== alert.destination.trim().toLowerCase() ? `${alert.origin} → ${alert.destination}` : alert.destination;
...
function AlertCard({ alert, t }: { alert: AccountPriceAlert; t: Record<string, string> }) {
  const startKey=alert.type==="HOTEL"?"checkIn":alert.type==="CAR"?"pickupDate":"departureDate"; const endKey=alert.type==="HOTEL"?"checkOut":alert.type==="CAR"?"dropoffDate":"returnDate";
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-5 text-start shadow-sm sm:p-6">
      ...
      <dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2">
        <div><dt className="font-semibold text-slate-500">{alert.type==="CAR"?"Pickup date/time":alert.type==="HOTEL"?"Check-in":"Departure date"}</dt><dd className="mt-1 text-slate-950">{formatDate(dateValue(alert,startKey))}{alert.type==="CAR"&&dateValue(alert,"pickupTime")?` · ${dateValue(alert,"pickupTime")}`:""}</dd></div>
        {dateValue(alert,endKey) && <div><dt className="font-semibold text-slate-500">{alert.type==="CAR"?"Return date/time":alert.type==="HOTEL"?"Check-out":"Return date"}</dt><dd className="mt-1 text-slate-950">{formatDate(dateValue(alert,endKey))}{alert.type==="CAR"&&dateValue(alert,"dropoffTime")?` · ${dateValue(alert,"dropoffTime")}`:""}</dd></div>}
