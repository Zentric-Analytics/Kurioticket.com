...
export function emailFor(input: BuildNotification) {
  ...
  let action = "";
  let details = "";
  if (success && input.platform === "android" && input.buildUrl) {
    const installUrl = input.buildDetailsUrl || input.buildUrl;
    action = `<p><strong>Installation:</strong> Open the install page on your Android device, then tap <strong>Install</strong> to download the APK.</p><p style="margin:24px 0"><a href="${escapeHtml(installUrl)}" style="display:inline-block;background:#0f172a;color:#fff;text-decoration:none;padding:12px 18px;border-radius:10px;font-weight:700">Install Android Preview</a></p>`;
    if (input.buildDetailsUrl) {
      details = `<p style="font-size:13px;color:#64748b">If the install page does not work on your device, <a href="${escapeHtml(input.buildUrl)}">try the direct APK download</a>.</p>`;
    }
  ...
  const androidInstall = success && input.platform === "android" && input.buildUrl
    ? `\nInstallation: Open this page on your Android device, then tap Install to download the APK: ${input.buildDetailsUrl || input.buildUrl}${input.buildDetailsUrl ? `\nDirect APK fallback: ${input.buildUrl}` : ""}`
    : "";
