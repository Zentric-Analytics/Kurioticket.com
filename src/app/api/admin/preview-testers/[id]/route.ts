...
export async function PATCH(request: Request, context: Context) {
  ...
  const previousRoles = normalizeTeamAccessRoles(previous.roles);
  ...
  const roles = action === "update" ? normalizeTeamAccessRoles(body.roles) : previousRoles;
  const updateResult = await getPrisma().previewTester.updateMany({
    where: { id, updatedAt: expectedUpdatedAt },
    data: action === "update"
      ? { roles, allowGoogleSignIn: true, allowStagingEmail: true, expiresAt, reason }
      : action === "reactivate"
  ...
  await writeAdminAuditLog({
    ...
    metadata: { previousStatus: previous.status, nextStatus: tester.status, previousRoles, nextRoles, reason: reason ?? tester.reason },
    request,
  });
  return NextResponse.json({ tester: { ...tester, roles: nextRoles } });
}
