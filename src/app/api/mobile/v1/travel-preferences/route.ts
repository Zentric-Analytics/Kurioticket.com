...
type Dependencies = { session: (request: Request) => Promise<Session>; prisma: Parameters<typeof getTravelPreferencesForUser>[0] };
type Session = { user: { id: string; status: string } } | null;
export function createMobileTravelPreferencesHandlers(deps: Dependencies = { session: getMobileSession, prisma: getPrisma() }) {
