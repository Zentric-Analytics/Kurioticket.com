...
export async function POST(request: Request) {
  try {
    ...
    if (Number.isFinite(contentLength) && contentLength > 32_768) return NextResponse.json({ error: MOBILE_PASSKEY_GENERIC_ERROR }, { status: 400 });
    ...
    if (error instanceof MobilePasskeyAuthenticationError) return NextResponse.json({ error: MOBILE_PASSKEY_GENERIC_ERROR, code: error.code }, { status: error.code === "CHALLENGE_EXPIRED" ? 400 : 401 });
