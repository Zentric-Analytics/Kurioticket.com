...
type RequestCodeDependencies = {
  canUseCredentials: typeof canUseStagingCredentials;
  findUser: (email: string) => Promise<{ name: string | null } | null>;
  sendCode: typeof sendEmailVerificationCode;
};

const defaultDependencies: RequestCodeDependencies = {
  canUseCredentials: canUseStagingCredentials,
  findUser: (email) => getPrisma().user.findUnique({ where: { email }, select: { name: true } }),
  sendCode: sendEmailVerificationCode,
};

export function createRequestCodeHandler(dependencies: RequestCodeDependencies = defaultDependencies) {
  return async function POST(request: Request) {
    const body = await request.json().catch(() => null);
    const parsed = emailSchema.safeParse(body?.email);
    if (!parsed.success) return NextResponse.json({ error: "Enter a valid email address." }, { status: 400 });
    if (!(await dependencies.canUseCredentials(parsed.data))) return NextResponse.json({ error: "Preview access is restricted." }, { status: 403 });
    try {
      checkAuthRateLimit({ action: "mobile-request-code", email: parsed.data, request, limit: 5, windowMs: 15 * 60_000 });
      const user = await dependencies.findUser(parsed.data);
      const result = await dependencies.sendCode({ email: parsed.data, name: user?.name, action: "mobile-auth", enforceCooldown: true });
...
export const POST = createRequestCodeHandler();
