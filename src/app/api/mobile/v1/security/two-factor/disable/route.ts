...
export function createDisableHandler(deps:Dependencies=defaults){return async(request:Request)=>{const auth=await deps.authenticate(request);if(!auth)return mobileUnauthorized();
 ...
 try{const hasExplicitCredential=Boolean(parsed.data.code||parsed.data.password);let verified=!hasExplicitCredential&&hasRecentReauthentication(auth);if(parsed.data.code)verified=await deps.verify({userId:auth.user.id,code:parsed.data.code});if(!verified&&parsed.data.password){const hash=await deps.passwordHash(auth.user.id);verified=Boolean(hash&&await deps.compare(parsed.data.password,hash));}if(!verified)return NextResponse.json({error:"Unable to verify that request."},{status:403});
