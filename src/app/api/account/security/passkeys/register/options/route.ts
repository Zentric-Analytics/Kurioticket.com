...
export async function POST(request: Request) {
  ...
  if (onboarding) {
    const user = await prisma.user.findUnique({ where: { id: session.user.id }, select: { emailVerified: true, status: true, passkeys: { where: { revokedAt: null }, select: { id: true }, take: 1 } } });
    if (!user?.emailVerified || user.status !== "ACTIVE" || user.passkeys.length) return NextResponse.json({ error: "Passkey setup is available after your email is verified." }, { status: 403 });
  } else {
