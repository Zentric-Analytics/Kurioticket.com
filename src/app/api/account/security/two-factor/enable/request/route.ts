...
export async function POST(request: Request) {
  ...
  try { checkAuthRateLimit({ action: "two-factor-enable-request", email: session.user.email || undefined, request, limit: 5, windowMs: 15 * 60 * 1000 }); }
  catch (error) { if (error instanceof AuthRateLimitError) return NextResponse.json({ error: "Too many requests. Please wait and try again." }, { status: 429, headers: { "Retry-After": String(error.retryAfterSeconds) } }); throw error; }
  ...
  try { const result = await sendTwoFactorCode({ userId: user.id, email: user.email, name: user.name, purpose: "enable" }); return NextResponse.json({ ok: true, cooldownSeconds: result.cooldownSeconds }); }
