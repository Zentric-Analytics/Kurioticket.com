...
import { DealsGuidedHandoffBoundaryV2 } from "@/components/results/deals/DealsGuidedHandoffBoundaryV2";
      ...
      <main className="flex-1 bg-surface-muted/40">
        <section className="border-b border-border bg-white">
          <div className="page-shell max-w-5xl py-7 sm:py-10">
            {mode === "guided-v2" ? (
              <DealsGuidedHandoffBoundaryV2
                key={buildDealsSearchFingerprint(search)}
                search={search}
              />
            ) : mode === "guided" ? (
              <DealsGuidedHandoffClient
                key={buildDealsSearchFingerprint(search)}
                search={search}
              />
            ) : mode === "legacy" ? (
