...
export default function DealsPage() {
  ...
  const dealsPageRef = useRef<HTMLElement>(null);
  const searchWrapperRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const dealsPage = dealsPageRef.current;
    const searchWrapper = searchWrapperRef.current;
    if (!dealsPage || !searchWrapper) return;

    let animationFrame = 0;

    const setSearchHeight = (height: number) => {
      if (!Number.isFinite(height) || height <= 0) return;

      if (animationFrame) cancelAnimationFrame(animationFrame);
      animationFrame = requestAnimationFrame(() => {
        dealsPage.style.setProperty("--deals-search-inside", `${height * 0.62}px`);
        dealsPage.style.setProperty("--deals-search-outside", `${height * 0.38}px`);
        animationFrame = 0;
      });
    };

    setSearchHeight(searchWrapper.getBoundingClientRect().height);

    const observer = new ResizeObserver((entries) => {
      const entry = entries[0];
      setSearchHeight(entry?.contentRect.height ?? searchWrapper.getBoundingClientRect().height);
    });
  ...
  return <><AppHeader /><main ref={dealsPageRef} className="flex-1 bg-slate-50 pb-12">
    <section className="relative overflow-hidden overflow-visible border-b border-slate-200 bg-[#F6F9FC] pb-12 shadow-[0_18px_45px_rgba(15,23,42,0.06)] sm:pb-[calc(var(--deals-search-inside,0px)+3rem)]">
