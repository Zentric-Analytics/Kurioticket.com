...
export default function DealsPage() {
  ...
  return <><AppHeader /><main ref={dealsPageRef} className="flex-1 bg-slate-50 pb-12" style={{ "--deals-search-inside": "0px", "--deals-search-outside": "0px" } as CSSProperties}>
    <section className="relative overflow-hidden border-b border-slate-200 bg-[#F6F9FC] pb-12 shadow-[0_18px_45px_rgba(15,23,42,0.06)] sm:overflow-visible sm:pb-0 lg:h-[390px]">
      <div className="absolute inset-0 overflow-hidden"><Image src={dealsHeroImage} alt="" fill priority sizes="100vw" className="object-cover object-[center_52%] lg:object-[center_48%]" /><div className="absolute inset-0 bg-gradient-to-br from-slate-950/25 via-slate-100/20 to-sky-100/10 lg:hidden" /><div className="absolute inset-0 bg-[linear-gradient(115deg,rgba(248,250,252,0.82)_0%,rgba(248,250,252,0.5)_45%,rgba(248,250,252,0.12)_75%)] lg:bg-[linear-gradient(90deg,rgba(248,250,252,0.96)_0%,rgba(248,250,252,0.77)_34%,rgba(248,250,252,0.18)_62%,rgba(248,250,252,0)_78%)]" /></div>
      <div className="page-shell relative z-10 pt-8 sm:pb-[calc(2.5rem+var(--deals-search-inside))] sm:pt-14 lg:pt-[72px]"><h1 className="max-w-4xl text-balance text-3xl font-semibold tracking-tight text-slate-800 sm:text-4xl lg:max-w-[470px] lg:text-[44px] lg:font-extrabold lg:leading-[1.08] lg:tracking-[-0.035em] lg:text-[#10233d]">{t("deals.heroTitle")}</h1><p className="mt-3 max-w-2xl text-base font-medium leading-7 text-slate-700 lg:mt-5 lg:text-[17px] lg:font-normal lg:text-slate-600">{t("deals.heroSubtitle")}</p></div>
      <div ref={dealsSearchRef} className="page-shell relative z-20 mt-8 sm:absolute sm:inset-x-0 sm:bottom-0 sm:mt-0 sm:translate-y-[38%] lg:translate-y-[56%]"><div className="mx-auto max-w-[1280px]"><Suspense fallback={<div className="min-h-80 rounded-3xl bg-white/90" />}><DealsSearchForm presentation="desktop-landing" /></Suspense></div></div>
    </section>
