...
import { isDealsJourneyStage, normalizeDealsJourneyCarId, normalizeDealsJourneyFlightId, normalizeDealsJourneyHotelId } from "@/lib/deals/dealsJourneyRoutes";
...
export default async function DealsJourneyPage({ params, searchParams }: { params: Promise<{ stage: string }>; searchParams: Promise<Query> }) {
  ...
  return <><AppHeader flushDesktopBottom hideDesktopTravelNav /><DealsJourneyShell key={contextKey} stage={stage} search={search} invalid={invalid} hotelId={hotelId} flightId={flightId} carId={carId} /><Footer /></>;
}
