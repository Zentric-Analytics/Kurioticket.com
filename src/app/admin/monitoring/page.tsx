...
export default async function AdminMonitoringPage() {
  const session = await requireAdminSession("/admin/monitoring");
  const role: AdminRole = session.user.role === "SUPPORT" || session.user.role === "USER" ? session.user.role : "ADMIN";
  const destinations = getAdminHubDestinations("monitoring", role);

  return (
    <AdminPageShell title="Monitoring" description="Review search activity, provider handoffs and admin audit trails." eyebrow="Admin navigation">
