...
const adminModules = [
  { href: "/admin/users", title: "Users", icon: Users, body: "Search users, review status, suspend/reactivate accounts, and inspect roles." },
  { href: "/admin/providers", title: "Providers", icon: Activity, body: "Monitor Duffel and track paused/future provider integrations." },
  { href: "/admin/searches", title: "Searches", icon: Plane, body: "Review recent flight metasearch activity and result counts." },
  { href: "/admin/redirects", title: "Redirects", icon: Gauge, body: "Inspect future external booking handoff logs." },
  { href: "/admin/support", title: "Support", icon: LifeBuoy, body: "Review support tickets and operational queues." },
  { href: "/admin/logs", title: "Logs", icon: ListChecks, body: "Audit sensitive admin actions and account changes." },
  { href: "/admin/system", title: "System", icon: ServerCog, body: "Check safe environment, auth, database, and runtime status." },
  { href: "/admin/settings", title: "Settings", icon: Settings, body: "Read-only operational settings and active/paused systems." },
];
  ...
  return (
    ...
    >
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard label="Duffel health" value={duffel.connected ? "Healthy" : duffel.configured ? "Attention" : "Not configured"} />
        <MetricCard label="Database" value={system.databaseConnected ? "Connected" : system.databaseConfigured ? "Configured" : "Missing"} />
        ...
        {adminModules.map((module) => (
          <Link key={module.href} href={module.href} className="block rounded-xl focus-ring">
            <Card className="h-full p-4 transition hover:-translate-y-0.5 hover:border-teal hover:shadow-lg">
              ...
              <span className="mt-4 inline-block text-sm font-bold text-teal-dark">Open {module.title} →</span>
            </Card>
          </Link>
        ))}
      </div>
      ...
      <Card className="mt-6 p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <StatusPill tone="good">Active: NextAuth</StatusPill>
          <StatusPill tone="good">Active: PostgreSQL</StatusPill>
          <StatusPill tone="good">Active: Duffel foundation</StatusPill>
          <StatusPill>Paused: Travelpayouts</StatusPill>
          <StatusPill>Paused: Stripe premium</StatusPill>
          <StatusPill>Paused: Resend alerts</StatusPill>
          <StatusPill>Paused: OpenAI premium features</StatusPill>
        </div>
      </Card>
    </AdminPageShell>
  );
