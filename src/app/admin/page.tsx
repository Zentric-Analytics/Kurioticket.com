...
export default async function AdminPage() {
  ...
  return (
    ...
    >
      <section aria-labelledby="needs-attention-heading" className="border-y border-[#DDE7F0] py-5">
        ...
        <div data-admin-home-attention-rail="flat" className="mt-4">
          {attentionIssues.length > 0 ? (
            <div className="grid divide-y divide-[#DDE7F0] md:grid-cols-2 md:divide-x md:divide-y-0 2xl:grid-cols-4">
      ...
      <section aria-labelledby="at-a-glance-heading" className="mt-10">
        ...
        <div data-admin-home-metric-rail="flat" className="mt-4">
          <div className="grid grid-cols-2 divide-x divide-y divide-[#DDE7F0] border-y border-[#DDE7F0] md:grid-cols-3 xl:grid-cols-6 xl:divide-y-0">
      ...
      <section aria-label="Search Activity and Service Status" className="mt-10 grid items-start gap-8 xl:grid-cols-[minmax(0,1.18fr)_1px_minmax(360px,0.82fr)] xl:gap-8">
        <section data-admin-home-surface="search-activity" className="relative min-h-[17rem] overflow-hidden p-0" aria-labelledby="search-activity-heading">
          ...
          <div className="relative z-10">
            ...
            {searchHealth.hasLogs ? (
              <>
                <div data-admin-home-search-metrics="icon-divider-rail" className="mt-5 grid grid-cols-1 divide-y divide-[#DDE7F0] sm:grid-cols-3 sm:divide-x sm:divide-y-0">
                  ...
                  {searchHealth.topProducts.length > 0 ? (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {searchHealth.topProducts.map((product) => (
                        <AdminStatusBadge key={product.label} tone="info">{formatProductSearchLabel(product.label)} {product.count}</AdminStatusBadge>
                      ))}
                    </div>
                  ) : (
        ...
        <div data-admin-home-layout-divider="search-status" aria-hidden="true" className="hidden h-full w-px bg-[#DDE7F0] xl:block" />

        <section data-admin-home-surface="service-status" className="relative min-h-[17rem] overflow-hidden p-0 xl:pl-0" aria-labelledby="service-status-heading">
          <ServicePanelDecoration />
          <div className="relative z-10">
            <PanelHeading id="service-status-heading" icon={Gauge}>Service Status</PanelHeading>
            <div data-admin-home-service-groups="provider-system" className="mt-5 grid gap-5 2xl:grid-cols-[minmax(0,0.9fr)_auto_minmax(0,1.1fr)] 2xl:gap-6">
              ...
              <div data-admin-home-service-divider="responsive" aria-hidden="true" className="h-px bg-[#DDE7F0] 2xl:h-auto 2xl:w-px" />
              <div className="min-w-0">
      ...
      <section aria-labelledby="recent-admin-activity-heading" className="mt-10">
        ...
        <div className="mt-4"><RecentActivityList items={activity} /></div>
      </section>
    </AdminPageShell>
  );
}
...
function HeroRouteArtwork() {
  return (
    <svg data-admin-home-hero-artwork="route-lines" aria-hidden="true" className="pointer-events-none hidden h-32 w-full max-w-lg text-[#004BB8] opacity-70 md:block" viewBox="0 0 620 160" fill="none">
      <path d="M8 120C120 48 206 128 318 78C415 34 472 0 612 20" stroke="currentColor" strokeWidth="1.5" opacity="0.24" />
      <path d="M44 132C178 78 243 98 345 66C448 33 506 52 610 84" stroke="#93C5FD" strokeWidth="1" opacity="0.26" />
...
function OverviewMetric({ label, value, hint, icon }: { label: string; value: string | number; hint?: string; icon: MetricIcon }) {
  ...
  return <div data-admin-home-metric-item="flat" className="min-w-0 px-3 py-5 sm:px-4"><div className="flex items-start gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#EEF4FF] text-[#004BB8] ring-1 ring-[#DDE7F0]"><Icon className="h-5 w-5" aria-hidden="true" /></span><div><p className="text-[13px] font-semibold text-slate-600">{label}</p><p className="mt-1 text-[29px] font-extrabold leading-none tracking-tight text-[#021C2B]">{value}</p>{hint ? <p className="mt-1 text-xs font-medium text-slate-500">{hint}</p> : null}</div></div></div>;
}
...
function PanelMetric({ label, value, icon: Icon, tone }: { label: string; value: string | number; icon: LucideIcon; tone: "blue" | "amber" | "rose" }) {
  ...
  return <div data-admin-home-search-metric="flat-icon" className="min-w-0 py-4 sm:px-5 sm:first:pl-0 sm:last:pr-0"><div className="flex items-start gap-3"><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full ring-1 ${toneClass}`}><Icon className="h-4 w-4" aria-hidden="true" /></span><div className="min-w-0"><p className="text-[13px] font-semibold leading-5 text-slate-600">{label}</p><p className="mt-1 text-[29px] font-extrabold leading-none tracking-tight text-[#021C2B]">{value}</p></div></div></div>;
}
...
function AttentionRow({ issue }: { issue: AttentionIssue }) {
  ...
  return <div data-admin-home-attention-item="flat" className="flex min-w-0 items-start gap-3 py-4 pr-5 md:px-5 md:first:pl-0"><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${iconClassName}`}><Icon className="h-5 w-5" aria-hidden="true" /></span><div className="min-w-0"><p className="text-sm font-semibold leading-5 text-[#021C2B]">{issue.message}</p><TextLink href={issue.href} className="mt-2">{issue.linkLabel}</TextLink></div></div>;
}
...
function StatusRow({ label, status, tone, icon }: { label: string; status: string; tone: StatusTone; icon: string }) {
  ...
  return <div data-admin-home-status-row="flat" className="flex min-h-11 items-center justify-between gap-5 border-b border-[#DDE7F0]/70 py-2.5 text-sm last:border-b-0"><span className="flex min-w-0 flex-1 items-center gap-2 font-semibold text-slate-700"><Icon className="h-4 w-4 shrink-0 text-[#315078]" aria-hidden="true" />{label}</span><span className="shrink-0 whitespace-nowrap"><AdminStatusBadge tone={tone}>{status}</AdminStatusBadge></span></div>;
}
...
function RecentActivityList({ items }: { items: Array<{ id: string; title: string; detail: string; timestamp: string }> }) {
  ...
  return <div data-admin-home-timeline="true" className="border-y border-[#DDE7F0]"><div>{items.map((item) => { const Icon = activityIconFor(item.title); return <div key={item.id} className="group grid grid-cols-[2.25rem_minmax(0,1fr)] gap-x-4 gap-y-1 border-b border-[#DDE7F0] py-3.5 pr-0 text-sm transition-colors last:border-b-0 hover:bg-white/35 sm:grid-cols-[2.25rem_minmax(0,1fr)_auto] sm:gap-x-6"><div data-admin-home-timeline-icon-column="fixed" className="relative flex w-9 justify-center"><div aria-hidden="true" className="pointer-events-none absolute bottom-[-0.875rem] top-0 w-px bg-[#004BB8]/20" /><span className={`relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full ring-4 ring-[#F7F6F2] ${activityToneClass(item.title)}`}><Icon className="h-4 w-4" aria-hidden="true" /></span></div><div className="min-w-0"><p className="text-sm font-semibold leading-5 text-[#021C2B]">{humanizeAuditAction(item.title)}</p><p className="mt-1 break-words text-[13px] leading-5 text-slate-600">{item.detail}</p></div><p className="col-start-2 text-xs font-semibold leading-5 text-slate-500 sm:col-start-3 sm:text-right">{item.timestamp}</p></div>; })}</div></div>;
}

function TextLink({ href, className = "", children }: { href: string; className?: string; children: React.ReactNode }) {
  return <Link href={href} className={`focus-ring inline-flex min-h-11 items-center text-sm font-semibold text-[#004BB8] hover:text-[#021C2B] ${className}`}>{children}</Link>;
}
...
function formatProductSearchLabel(label: string) {
  const normalized = label.toLowerCase();
  if (normalized === "flight" || normalized === "flights") return "Flights";
  if (normalized === "hotel" || normalized === "hotels") return "Hotels";
  if (normalized === "car" || normalized === "cars") return "Cars";
  return normalized.charAt(0).toUpperCase() + normalized.slice(1);
}

function activityToneClass(action: string) {
  const normalized = action.toLowerCase();
  if (normalized.includes("delete") || normalized.includes("remove") || normalized.includes("suspend")) return "bg-rose-50 text-rose-600";
  if (normalized.includes("restore") || normalized.includes("resolve") || normalized.includes("refresh") || normalized.includes("success")) return "bg-emerald-50 text-emerald-600";
  if (normalized.includes("config") || normalized.includes("warning") || normalized.includes("credential") || normalized.includes("provider")) return "bg-amber-50 text-amber-600";
  return "bg-[#EEF4FF] text-[#004BB8]";
}

function activityIconFor(action: string): LucideIcon {
  const normalized = action.toLowerCase();
  if (normalized.includes("delete") || normalized.includes("remove") || normalized.includes("suspend")) return Trash2;
  if (normalized.includes("restore") || normalized.includes("resolve") || normalized.includes("refresh") || normalized.includes("success")) return RotateCcw;
  if (normalized.includes("config") || normalized.includes("warning") || normalized.includes("credential") || normalized.includes("provider")) return Settings2;
  return Activity;
}
