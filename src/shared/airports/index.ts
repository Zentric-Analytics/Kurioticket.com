/** Platform-neutral airport catalogue and deterministic local search.
 * No framework, server, Node, browser, or localization dependencies belong here.
 */
...
const airportSeeds: readonly AirportSeed[] = [
  ...
  ["ENU", "Enugu", "Akanu Ibiam International Airport", "NG", 6.4743, 7.5619],
...
const airportCountry = (countryCode: string, country?: string) => country || countryNames[countryCode] || countryCode;
...
export const airports: readonly AirportOption[] = airportSeeds.map((seed, index) => {
...
export const AIRPORT_RESULT_LIMIT = 8;

export function normalizeAirportSearchText(value: string): string {
  return value.normalize("NFKD").replace(/\p{M}/gu, "").trim().toLocaleLowerCase().replace(/\s+/g, " ");
}

function airportSearchRank(airport: AirportOption, query: string): number {
  const code = normalizeAirportSearchText(airport.code);
  const fields = [airport.city, airport.airport, airport.name ?? "", airport.country ?? "", airport.countryCode ?? ""].map(normalizeAirportSearchText);
...
export function searchAirports(query: string, limit = AIRPORT_RESULT_LIMIT): AirportOption[] {
  const normalizedQuery = normalizeAirportSearchText(query);
  const safeLimit = Math.max(0, Math.floor(limit));
  const indexed = airports.map((airport, index) => ({ airport, index }));

  if (!normalizedQuery) {
    return indexed
      .sort((a, b) => (b.airport.priority ?? 0) - (a.airport.priority ?? 0) || a.index - b.index)
      .slice(0, safeLimit)
      .map(({ airport }) => airport);
  }

  return indexed
    .map(({ airport, index }) => ({ airport, index, rank: airportSearchRank(airport, normalizedQuery) }))
