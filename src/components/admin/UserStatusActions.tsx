...
type UserActionsMenuContextValue = {
  activeUserId: string | null;
  setActiveUserId: (userId: string | null) => void;
};

const UserActionsMenuContext = createContext<UserActionsMenuContextValue | null>(null);

export function UsersActionsMenuProvider({ children }: { children: ReactNode }) {
  const [activeUserId, setActiveUserId] = useState<string | null>(null);

  return <UserActionsMenuContext.Provider value={{ activeUserId, setActiveUserId }}>{children}</UserActionsMenuContext.Provider>;
}

export function UserStatusActions({ userId, email, role, status, isSelf, isProtectedAdmin }: UserStatusActionsProps) {
  const menuController = useContext(UserActionsMenuContext);
  ...
  const open = menuController?.activeUserId === userId;
  ...
  useEffect(() => {
    if (!open || !menuController) return;

    function closeOnEscape(event: KeyboardEvent) {
      if (event.key !== "Escape") return;
      menuController?.setActiveUserId(null);
  ...
  return (
    <div ref={buttonRef} className="relative inline-flex justify-end">
      <AdminButton type="button" size="sm" variant="secondary" aria-haspopup="menu" aria-expanded={open} onClick={() => { setMessage(""); menuController?.setActiveUserId(open ? null : userId); }}>
        Manage
        <ChevronDown className="h-4 w-4" aria-hidden="true" />
      </AdminButton>
      {open && typeof document !== "undefined" ? createPortal(
        <>
          <button type="button" className="fixed inset-0 z-40 cursor-default" aria-label="Close user actions" onClick={() => menuController?.setActiveUserId(null)} />
