...
type Tester = { id: string; email: string; status: string; allowGoogleSignIn: boolean; allowStagingEmail: boolean; expiresAt: Date | null; approvedAt: Date | null; updatedAt: Date; reason: string | null; approvedByAdmin: { email: string | null; name: string | null } | null };
...
export function PreviewTesterRegistry({ testers }: { testers: Tester[] }) {
  ...
  async function change(id: string, action: string) {
    ...
    setError("");
    const response = await fetch(`/api/admin/preview-testers/${id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ action }) });
  ...
  async function update(formData: FormData) {
    ...
    setError("");
    const response = await fetch(`/api/admin/preview-testers/${id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "update", allowGoogleSignIn: formData.get("allowGoogleSignIn") === "on", allowStagingEmail: formData.get("allowStagingEmail") === "on", expiresAt: formData.get("expiresAt") || null, reason: formData.get("reason") }) });
    if (!response.ok) { const result = await response.json(); setError(result.error || "Unable to update tester."); return; }
    router.refresh();
  }
  return <div className="grid gap-6">
    ...
    <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white"><table className="w-full min-w-[1400px] text-left text-sm"><thead><tr><th className="p-3">Email</th><th>Status</th><th>Permissions / expiry / reason</th><th>Approved by</th><th>Approved</th><th>Updated</th><th>Actions</th></tr></thead><tbody>{testers.map((tester) => <tr className="border-t align-top" key={tester.id}><td className="p-3">{tester.email}</td><td className="p-3">{tester.expiresAt && new Date(tester.expiresAt) <= new Date() ? "EXPIRED" : tester.status}</td><td className="p-3"><form action={update} className="grid gap-2"><input type="hidden" name="id" value={tester.id} /><label><input type="checkbox" name="allowGoogleSignIn" defaultChecked={tester.allowGoogleSignIn} /> Google Sign-In</label><label><input type="checkbox" name="allowStagingEmail" defaultChecked={tester.allowStagingEmail} /> Staging email</label><input type="datetime-local" name="expiresAt" defaultValue={tester.expiresAt ? new Date(tester.expiresAt).toISOString().slice(0, 16) : ""} className="rounded border px-2 py-1" /><input name="reason" maxLength={500} defaultValue={tester.reason || ""} placeholder="Reason" className="rounded border px-2 py-1" /><Button type="submit">Save</Button></form></td><td className="p-3">{tester.approvedByAdmin?.name || tester.approvedByAdmin?.email || "Unknown"}</td><td className="p-3">{tester.approvedAt ? new Date(tester.approvedAt).toLocaleString() : "—"}</td><td className="p-3">{new Date(tester.updatedAt).toLocaleString()}</td><td className="flex gap-2 p-2"><Button type="button" onClick={() => change(tester.id, "reactivate")}>Activate</Button><Button type="button" onClick={() => change(tester.id, "suspend")}>Suspend</Button><Button type="button" onClick={() => change(tester.id, "revoke")}>Revoke</Button></td></tr>)}</tbody></table></div>
  </div>;
}
