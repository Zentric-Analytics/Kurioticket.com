  ...
  return (
    <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-4 md:p-5">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold text-slate-900 md:text-lg">Recent searches</h2>
      ...
      <div className="flex snap-x gap-3 overflow-x-auto pb-1 md:grid md:grid-cols-3 md:gap-4 md:overflow-visible lg:grid-cols-4">
        {entries.map((entry) => (
          <Link
            key={entry.id}
            href={entry.href}
            className="focus-ring group relative min-h-[200px] min-w-[260px] snap-start overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 transition-colors hover:border-slate-300 hover:bg-white md:min-w-0"
          >
            <img src={resolveCardImage(entry)} alt={entry.label} className="h-28 w-full object-cover" loading="lazy" />
            <button
              type="button"
