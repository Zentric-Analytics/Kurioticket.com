...
export type DealsSearchFormProps = {
  ...
  onCancel?: () => void; onDraftChange?: (search: DealsSearch) => void; warning?: ReactNode;
};

export function DealsSearchForm({ initialSearch, variant = "landing", onSubmitSearch, onCancel, onDraftChange, warning }: DealsSearchFormProps = {}) {
  ...
  const searchDealsButton = <div className="flex w-full flex-col-reverse gap-2 sm:w-auto sm:flex-row">{variant === "results" && <button type="button" onClick={onCancel} className="min-h-11 rounded-xl border border-slate-300 px-6 text-sm font-extrabold text-slate-700 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2">{t("deals.results.editor.cancel")}</button>}<button type="submit" disabled={submitting} aria-busy={submitting} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#004BB8] px-8 text-sm font-extrabold text-white shadow-lg shadow-blue-900/20 hover:bg-[#021C2B] disabled:opacity-70 sm:w-auto"><Search className="h-4 w-4" />{t(variant === "results" ? "deals.results.editor.update" : "deals.searchButton")}</button></div>;

  return <form onSubmit={submit} noValidate className={`mx-auto w-full max-w-[1120px] rounded-3xl border border-slate-200 bg-white p-4 sm:px-6 sm:py-4 lg:py-3 ${variant === "landing" ? "shadow-[0_18px_46px_rgba(15,23,42,0.12)]" : ""}`}>
