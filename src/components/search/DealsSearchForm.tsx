...
export function DealsSearchForm() {
  ...
  const flightDatesSummary = useMemo(() => { const departure = parseIsoDate(search.flightDepartureDate); const returning = parseIsoDate(search.flightReturnDate); return departure && returning && returning >= departure ? formatFlightsDateSummary(departure, returning, calendarLocale) : t("travelDates"); }, [calendarLocale, search.flightDepartureDate, search.flightReturnDate, t]);
  ...
  const openFlightDates = () => { resetFlightDatesDraft(); const departure = parseIsoDate(search.flightDepartureDate); setVisibleFlightMonth(departure ? new Date(departure.getFullYear(), departure.getMonth(), 1) : new Date(todayLocal.getFullYear(), todayLocal.getMonth(), 1)); if (window.matchMedia("(max-width: 639px)").matches) setMobileFlightDatesOpen(true); else setFlightDatesOpen(true); };
