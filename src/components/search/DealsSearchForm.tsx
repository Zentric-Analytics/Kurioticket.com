...
export function DealsSearchForm() {
  ...
  const selectDraftHotelDate = (date: Date) => { if (isBeforeToday(date)) return; const selected = toIsoDate(date); const checkIn = parseIsoDate(draftHotelCheckIn); if (!checkIn || draftHotelCheckOut) { setDraftHotelCheckIn(selected); setDraftHotelCheckOut(""); } else if (selected <= draftHotelCheckIn) { setDraftHotelCheckIn(selected); setDraftHotelCheckOut(""); } else setDraftHotelCheckOut(selected); };
