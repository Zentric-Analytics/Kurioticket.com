...
import { ArrowRightLeft, BedDouble, Calendar, Car, Check, ChevronDown, MapPin, Minus, Plane, Plus, Search, UserRound, X } from "lucide-react";
...
import { applyFlightDestination, applySharedDates, applySharedDestination, customizeInheritedField, relinkInheritedField, swapFlightAirports, transitionDealsMode } from "@/lib/deals/dealsSearchSynchronization";
...
export function DealsSearchForm({ initialSearch, variant = "landing", onSubmitSearch, onDraftChange, warning, pending = false }: DealsSearchFormProps = {}) {
  ...
  const swapDealsFlightAirports = () => {
    ...
    setSearch((current) => {
      ...
      return swapFlightAirports(current, newDestination);
    });
  ...
  const commitFlightDates = (mobile = false) => { const departure = parseIsoDate(draftFlightDepartureDate); if (!departure || isBeforeToday(departure)) return; const normalizedDeparture = toIsoDate(departure); if (search.flightTripType === "one-way") setSearch((current) => applySharedDates(current, { start: normalizedDeparture, end: current.sharedTravelEndDate })); else { const returning = parseIsoDate(draftFlightReturnDate); if (!returning || isBeforeToday(returning) || returning < departure) return; setSearch((current) => applySharedDates(current, { start: normalizedDeparture, end: toIsoDate(returning) })); } if (mobile) mobileFlightDatesCommittedRef.current = true; else { setFlightDatesOpen(false); restoreFlightDatesFocus(); } };
  const setDealsFlightTripType = (nextTripType: DealsFlightTripType) => { setSearch((current) => ({ ...current, flightTripType: nextTripType, flightReturnDate: nextTripType === "round-trip" ? current.sharedTravelEndDate : "" })); setDraftFlightReturnDate(nextTripType === "round-trip" ? search.sharedTravelEndDate : ""); };
  ...
  const travelerSummary = useMemo(() => {
    ...
    const travellers = search.flightAdults + search.flightChildren + search.flightInfants;
    const parts = [part(travellers, t("deals.travelerSingular"), t("deals.travelerPlural"))];
    if (included.hotel) parts.push(part(search.hotelRooms, t("roomSingular"), t("roomPlural"))); return parts.join(separator);
  }, [included.hotel, locale, search.flightAdults, search.flightChildren, search.flightInfants, search.hotelRooms, t]);
  ...
  const normalizeTravelersDraft = () => { const adults = Math.max(1, Math.min(9, draftAdults)); const children = Math.max(0, Math.min(9 - adults, draftChildren)); const infants = Math.max(0, Math.min(adults, 9 - adults - children, draftInfants)); return { adults, children, infants, rooms: Math.max(1, Math.min(6, draftHotelRooms)) }; };
  ...
  const closeProductPickers = (product: DealsProduct) => {
    ...
    if (product === "car") { setMobileCarLocation(null); setCarDatesOpen(false); setMobileCarDatesOpen(false); setCarTimesOpen(false); setMobileCarTimesOpen(false); setMobileCarDriverAgeOpen(false); }
  };
  ...
  const chooseAirport = (kind: "origin" | "destination", option: AirportOption) => {
    ...
    setSearch((current) => kind === "destination" ? applyFlightDestination(current, option.city, text, option.code.toUpperCase()) : { ...current, [textKey]: text, [codeKey]: option.code.toUpperCase() });
  ...
  const travelersPicker = <div className="w-full space-y-4">
    ...
    {included.hotel ? <div className="space-y-3 rounded-2xl border border-slate-200 p-3"><div className="flex items-center justify-between"><span className="font-extrabold">{t("rooms")}</span><span className="flex items-center gap-2"><button type="button" disabled={draftHotelRooms <= 1} onClick={() => setDraftHotelRooms((value) => Math.max(1, value - 1))} className="focus-ring h-10 w-10 rounded-full border">−</button><span>{draftHotelRooms}</span><button type="button" disabled={draftHotelRooms >= 6} onClick={() => setDraftHotelRooms((value) => Math.min(6, value + 1))} className="focus-ring h-10 w-10 rounded-full border">+</button></span></div><label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={draftHotelPetFriendly} onChange={(event) => setDraftHotelPetFriendly(event.target.checked)} />{t("petFriendly")}</label></div> : null}
  </div>;
  ...
  return <form onSubmit={submit} noValidate className={`mx-auto w-full max-w-[1120px] bg-white p-4 sm:px-4 sm:py-3 ${variant === "landing" ? "rounded-3xl border border-slate-200 shadow-[0_18px_46px_rgba(15,23,42,0.12)] sm:px-6 lg:py-3" : ""}`}>
    ...
    {included.car && <section aria-labelledby="deals-car-heading" className="border-t border-slate-200 py-4 sm:py-3 lg:py-2">
      ...
      <div className={`${connectedShell} sm:grid-cols-2 lg:grid-cols-3`}>
        ...
        <div className={connectedSegment}><span className={label}>{t("deals.returnLocation")}</span>{search.carReturnToDifferentLocation ? <div className="relative hidden sm:block"><input ref={carReturnLocationRef} value={search.carReturnLocation} aria-label={t("deals.returnLocation")} onChange={(event) => setSearch((current) => ({ ...current, carReturnLocation: event.target.value, carReturnToDifferentLocation: Boolean(event.target.value.trim()) }))} className={`${field} ${connectedField}`} /></div> : null}<button type="button" ref={carReturnLocationLauncherRef} aria-haspopup="dialog" aria-expanded={mobileCarLocation === "return"} aria-controls="deals-car-mobile-return-location" onClick={() => openCarLocation("return")} className={`${field} ${connectedField} ${search.carReturnToDifferentLocation ? "sm:hidden" : ""} flex items-center justify-between text-start`}><span className="truncate">{search.carReturnToDifferentLocation ? search.carReturnLocation : t("deals.sameAsPickup")}</span><ChevronDown className="h-4 w-4" aria-hidden="true" /></button>{search.carReturnToDifferentLocation ? <button type="button" onClick={() => setSearch((current) => ({ ...current, carReturnToDifferentLocation: false, carReturnLocation: "" }))} className="focus-ring mt-1 text-xs font-bold text-[#004BB8]">{t("deals.useSameAsPickup")}</button> : null}</div>
    ...
    <section aria-labelledby="deals-travellers-heading" data-deals-travellers-row className="border-t border-slate-200 py-4 sm:py-3 lg:py-2"><h2 id="deals-travellers-heading" className="mb-2 flex items-center gap-2 text-base font-extrabold text-[#021C2B]"><UserRound className="h-5 w-5 text-[#004BB8]" aria-hidden="true" />{t("deals.travellersRow")}</h2><div className="grid items-end gap-3 sm:grid-cols-[minmax(0,1fr)_auto]"><div><span className={label}>{t("deals.travellersRooms")}</span><button ref={travelersLauncherRef} type="button" aria-expanded={travelersOpen || mobileTravelersOpen} aria-haspopup="dialog" aria-controls={mobileTravelersOpen ? "deals-mobile-travellers" : "deals-desktop-travellers"} onClick={() => travelersOpen ? dismissDesktopTravelers() : openTravelers()} className={`${field} flex items-center justify-between text-start`}><span className="truncate">{travelerSummary}</span><ChevronDown className="h-4 w-4" /></button></div>{searchDealsButton}</div></section>
    {warning}
