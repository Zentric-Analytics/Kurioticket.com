...
import { applySharedDates, customizeInheritedField, relinkInheritedField, transitionDealsMode } from "@/lib/deals/dealsSearchSynchronization";
...
export function DealsSearchForm({ initialSearch, variant = "landing", onSubmitSearch, onDraftChange, warning, pending = false }: DealsSearchFormProps = {}) {
  ...
  const commitTravelers = (mobile = false) => { const normalized = normalizeTravelersDraft(); setSearch((current) => ({ ...current, flightAdults: normalized.adults, flightChildren: normalized.children, flightInfants: normalized.infants, flightCabinClass: normalized.cabin, hotelAdults: normalized.adults, hotelChildren: normalized.children })); if (mobile) mobileTravelersCommittedRef.current = true; else { setTravelersOpen(false); restoreTravelersFocus(); } };
  ...
  return <form onSubmit={submit} noValidate className={`mx-auto w-full max-w-[1120px] bg-white p-4 sm:px-4 sm:py-3 ${variant === "landing" ? "rounded-3xl border border-slate-200 shadow-[0_18px_46px_rgba(15,23,42,0.12)] sm:px-6 lg:py-3" : ""}`}>
    ...
    <HotelDestinationMobilePicker open={hotelDestinationMobileOpen} value={search.hotelDestination} titleId="deals-hotel-mobile-destination-title" inputId="deals-hotel-mobile-destination-input" launcherRef={hotelDestinationMobileLauncherRef} onChange={(value) => { dirty.current.hotelDestination = true; setSearch((current) => customizeInheritedField(current, "stayDestination", value)); }} onClear={() => { dirty.current.hotelDestination = true; setHotelSuggestions([]); }} onClose={() => setHotelDestinationMobileOpen(false)} />
