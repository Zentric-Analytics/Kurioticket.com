...
import { ArrowRightLeft, BedDouble, Calendar, Car, Check, ChevronDown, MapPin, Minus, Plane, Plus, Search, Users, X } from "lucide-react";
...
import {
  buildDealsResultsUrl, createDefaultDealsSearch, dealsProductOrder,
  getIncludedProducts, parseDealsSearchParams, tryToggleDealsProduct, validateDealsSearch,
  type DealsFlightTripType, type DealsPackageMode, type DealsSearch, type DealsProduct,
} from "@/lib/deals/dealsSearchParams";
import { applySharedDates, applySharedDestination, customizeInheritedField, relinkInheritedField, setFlightTripType, swapFlightAirports, transitionDealsMode } from "@/lib/deals/dealsSearchSynchronization";
...
const productOptions = { hotel: { label: "hotels", Icon: BedDouble }, flight: { label: "flights", Icon: Plane }, car: { label: "cars", Icon: Car } } as const;
...
export type DealsSearchFormProps = {
  initialSearch?: DealsSearch; variant?: "landing" | "results"; onSubmitSearch?: (search: DealsSearch) => void;
  onDraftChange?: (search: DealsSearch) => void; warning?: ReactNode; pending?: boolean;
};

export function DealsSearchForm({ initialSearch, variant = "landing", onSubmitSearch, onDraftChange, warning, pending = false }: DealsSearchFormProps = {}) {
  ...
  const [search, setSearch] = useState<DealsSearch>(() => initialSearch ?? (params.size ? parseDealsSearchParams(params) : createDefaultDealsSearch()));
  ...
  const carDatesLauncherRef = useRef<HTMLButtonElement>(null); const carTimesLauncherRef = useRef<HTMLButtonElement>(null);
  const mobileCarDatesCommittedRef = useRef(false); const mobileCarTimesCommittedRef = useRef(false);
  ...
  useEffect(() => { onDraftChange?.(search); }, [onDraftChange, search]);

  useEffect(() => {
    if (variant === "results" || !dealsIncludesFlights(search.mode) || search.flightOriginText.trim() || search.flightOriginCode.trim() || flightOriginUserInteractedRef.current || flightDefaultOriginRequestedRef.current) return;
  ...
  }, [locale, search.flightOriginCode, search.flightOriginText, search.mode, variant]);

  const swapDealsFlightAirports = () => {
    flightOriginUserInteractedRef.current = true;
    setSearch((current) => swapFlightAirports(current, getAirportByCode(current.flightOriginCode)?.city ?? current.flightOriginText));
  ...
  const commitFlightDates = (mobile = false) => { const departure = parseIsoDate(draftFlightDepartureDate); if (!departure || isBeforeToday(departure)) return; const normalizedDeparture = toIsoDate(departure); const returning = search.flightTripType === "round-trip" ? parseIsoDate(draftFlightReturnDate) : null; if (search.flightTripType === "round-trip" && (!returning || isBeforeToday(returning) || returning < departure)) return; setSearch((current) => applySharedDates(current, { start: normalizedDeparture, end: returning ? toIsoDate(returning) : current.sharedTravelEndDate })); if (mobile) mobileFlightDatesCommittedRef.current = true; else { setFlightDatesOpen(false); restoreFlightDatesFocus(); } };
  const setDealsFlightTripType = (nextTripType: DealsFlightTripType) => { setSearch((current) => setFlightTripType(current, nextTripType)); setDraftFlightReturnDate(nextTripType === "round-trip" ? search.sharedTravelEndDate : ""); };
  ...
  const commitHotelDates = (mobile = false) => { const checkIn = parseIsoDate(draftHotelCheckIn); const checkOut = parseIsoDate(draftHotelCheckOut); if (!checkIn || !checkOut || isBeforeToday(checkIn) || isBeforeToday(checkOut) || checkOut <= checkIn) return; const normalizedCheckIn = toIsoDate(checkIn); const normalizedCheckOut = toIsoDate(checkOut); setSearch((current) => getIncludedProducts(current.mode).flight ? customizeInheritedField(current, "stayDates", { start: normalizedCheckIn, end: normalizedCheckOut }) : applySharedDates(current, { start: normalizedCheckIn, end: normalizedCheckOut })); if (mobile) mobileHotelDatesCommittedRef.current = true; else { setHotelDatesOpen(false); restoreHotelDatesFocus(); } };
  ...
  const travelerSummary = useMemo(() => {
    const count = search.flightAdults + search.flightChildren + search.flightInfants;
    const travellers = `${count} ${t(count === 1 ? "deals.travellerSingular" : "deals.travellerPlural")}`;
    return included.hotel ? `${travellers}, ${search.hotelRooms} ${t(search.hotelRooms === 1 ? "roomSingular" : "roomPlural")}` : travellers;
  }, [included.hotel, search.flightAdults, search.flightChildren, search.flightInfants, search.hotelRooms, t]);
  const restoreTravelersFocus = () => requestAnimationFrame(() => travelersLauncherRef.current?.focus({ preventScroll: true }));
  const resetTravelersDraft = useCallback(() => { setDraftAdults(search.flightAdults); setDraftChildren(search.flightChildren); setDraftInfants(search.flightInfants); setDraftHotelRooms(search.hotelRooms); setDraftHotelPetFriendly(search.hotelPetFriendly); }, [search.flightAdults, search.flightChildren, search.flightInfants, search.hotelPetFriendly, search.hotelRooms]);
  ...
  const normalizeTravelersDraft = () => { const maximum = included.flight ? 9 : 12; const adults = Math.max(1, Math.min(maximum, draftAdults)); const children = Math.max(0, Math.min(maximum - adults, draftChildren)); const infants = Math.max(0, Math.min(adults, maximum - adults - children, draftInfants)); return { adults, children, infants, rooms: Math.max(1, Math.min(6, draftHotelRooms)) }; };
  const commitTravelers = (mobile = false) => { const normalized = normalizeTravelersDraft(); setSearch((current) => ({ ...current, flightAdults: normalized.adults, flightChildren: normalized.children, flightInfants: normalized.infants, hotelAdults: normalized.adults, hotelChildren: normalized.children, hotelRooms: normalized.rooms, hotelPetFriendly: draftHotelPetFriendly })); if (mobile) mobileTravelersCommittedRef.current = true; else { setTravelersOpen(false); restoreTravelersFocus(); } };
  ...
  const closeCarMobilePickers = () => { setMobileCarLocation(null); setMobileCarDatesOpen(false); setMobileCarTimesOpen(false); };
  ...
  const commitCarDates = (mobile = false) => { const pickup = parseIsoDate(draftCarPickupDate); const returning = parseIsoDate(draftCarReturnDate); if (!pickup || !returning || isBeforeToday(pickup) || isBeforeToday(returning) || returning < pickup) return; setSearch((current) => customizeInheritedField(current, "carDates", { start: toIsoDate(pickup), end: toIsoDate(returning) })); if (mobile) mobileCarDatesCommittedRef.current = true; else { setCarDatesOpen(false); restoreCarFocus(carDatesLauncherRef); } };
  ...
  const closeProductPickers = (product: DealsProduct) => {
    if (product === "flight") { setFlightOriginOpen(false); setFlightDestinationOpen(false); setFlightMobileAirport(null); setFlightDatesOpen(false); setMobileFlightDatesOpen(false); setTravelersOpen(false); setMobileTravelersOpen(false); }
    if (product === "hotel") { setHotelDestinationOpen(false); setHotelDestinationMobileOpen(false); setHotelDatesOpen(false); setMobileHotelDatesOpen(false); }
    if (product === "car") { setMobileCarLocation(null); setCarDatesOpen(false); setMobileCarDatesOpen(false); setCarTimesOpen(false); setMobileCarTimesOpen(false); }
  };
  const toggleProduct = (product: DealsProduct) => {
    ...
    if (!result.changed) { setProductSelectionMessage(t("deals.productSelector.minimumTwo")); return; }
  ...
  const chooseAirport = (kind: "origin" | "destination", option: AirportOption) => {
    ...
    setSearch((current) => kind === "destination" ? { ...applySharedDestination(current, option.city, text), flightDestinationCode: option.code.toUpperCase() } : { ...current, [textKey]: text, [codeKey]: option.code.toUpperCase() });
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); if (submitting) return; const found = validateDealsSearch(search); setErrors(found); if (Object.keys(found).length) { requestAnimationFrame(() => { firstError.current?.scrollIntoView({ behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth", block: "center" }); firstError.current?.focus({ preventScroll: true }); }); return; } if (variant === "results" && onSubmitSearch) { onSubmitSearch(search); return; } setSubmitting(true); start(); router.push(buildDealsResultsUrl(search)); };
  ...
  const travelersPicker = <div className="w-full space-y-4">
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
      {([
        ...
        const total = draftAdults + draftChildren + draftInfants; const partyMaximum = included.flight ? 9 : 12; const canDecrease = count > minimum; const canIncrease = total < partyMaximum && (key !== "infants" || draftInfants < draftAdults);
        ...
        return <div key={key} className="flex items-center justify-between gap-3 border-b border-slate-100 px-3 py-3 last:border-b-0">
          ...
          <span className="flex shrink-0 items-center gap-2">
            ...
            <button type="button" aria-label={ariaName("deals.increaseCountAria")} disabled={!canIncrease} onClick={() => { if (total >= (included.flight ? 9 : 12)) return; if (key === "adults") setDraftAdults((current) => current + 1); else if (key === "children") setDraftChildren((current) => current + 1); else if (draftInfants < draftAdults) setDraftInfants((current) => current + 1); }} className="focus-ring inline-flex h-11 w-11 items-center justify-center rounded-full border border-slate-300 bg-white text-slate-700 hover:border-[#004BB8] hover:text-[#004BB8] disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-300 sm:h-10 sm:w-10"><Plus className="h-4 w-4" aria-hidden="true" /></button>
          </span>
        </div>;
      })}
    </div>
    {included.hotel ? <div className="space-y-3 rounded-2xl border border-slate-200 p-3"><div className="flex items-center justify-between"><span className="font-extrabold">{t("rooms")}</span><span className="flex items-center gap-2"><button type="button" aria-label={t("deals.decreaseRooms")} disabled={draftHotelRooms <= 1} onClick={() => setDraftHotelRooms((value) => Math.max(1, value - 1))} className="focus-ring h-10 w-10 rounded-full border">−</button><span>{draftHotelRooms}</span><button type="button" aria-label={t("deals.increaseRooms")} disabled={draftHotelRooms >= 6} onClick={() => setDraftHotelRooms((value) => Math.min(6, value + 1))} className="focus-ring h-10 w-10 rounded-full border">+</button></span></div><label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={draftHotelPetFriendly} onChange={(event) => setDraftHotelPetFriendly(event.target.checked)} />{t("petFriendly")}</label></div> : null}
  </div>;
  ...
  return <form onSubmit={submit} noValidate className={`mx-auto w-full max-w-[1120px] bg-white p-4 sm:px-4 sm:py-3 ${variant === "landing" ? "rounded-3xl border border-slate-200 shadow-[0_18px_46px_rgba(15,23,42,0.12)] sm:px-6 lg:py-3" : ""}`}>
    ...
    {included.flight && <section aria-labelledby="deals-flight-heading" className="border-t border-slate-200 py-4 sm:py-3 lg:py-2"><div><h2 id="deals-flight-heading" className="mb-0.5 flex items-center gap-2 text-base font-extrabold text-[#021C2B]"><Plane className="h-5 w-5 text-[#004BB8]" />{t("deals.flightRow")}</h2><div role="radiogroup" aria-label={t("tripType")} className="mb-1 inline-flex items-center gap-3 rounded-lg px-0.5 py-1 sm:gap-1 sm:rounded-full sm:bg-transparent sm:p-0.5">{(["round-trip", "one-way"] as const).map((value) => <button key={value} type="button" role="radio" aria-checked={search.flightTripType === value} onClick={() => setDealsFlightTripType(value)} onKeyDown={(event) => { if (!["ArrowRight", "ArrowLeft", "ArrowDown", "ArrowUp"].includes(event.key)) return; event.preventDefault(); setDealsFlightTripType(value === "round-trip" ? "one-way" : "round-trip"); }} className={`focus-ring group inline-flex min-h-8 items-center gap-2 rounded-lg px-1.5 py-1 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-100/70 hover:text-slate-950 sm:min-h-9 sm:flex-none sm:justify-center sm:px-3.5 sm:py-2 sm:font-bold ${search.flightTripType === value ? "bg-[#004BB8]/8 text-[#004BB8] ring-1 ring-[#004BB8]/10 sm:bg-[#004BB8]/8 sm:text-[#004BB8] sm:shadow-none" : ""}`}><span aria-hidden="true" className={`flex h-4 w-4 shrink-0 items-center justify-center rounded-full border bg-white transition-colors ${search.flightTripType === value ? "border-[#004BB8]" : "border-slate-300 group-hover:border-slate-400"}`}><span className={`h-1.5 w-1.5 rounded-full bg-[#004BB8] transition-opacity ${search.flightTripType === value ? "opacity-100" : "opacity-0"}`} /></span><span>{t(value === "round-trip" ? "roundTrip" : "oneWay")}</span></button>)}</div></div><div className={`${connectedShell} lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)_minmax(0,0.8fr)]`}>
      ...
      <div className={`${connectedSegment} sm:border-e sm:border-b sm:border-slate-200 lg:border-b-0 lg:last:border-e-0`}><label className={label} htmlFor="deals-flight-cabin">{t("deals.cabinClass")}</label><select id="deals-flight-cabin" value={search.flightCabinClass} onChange={(event) => update("flightCabinClass", event.target.value as DealsSearch["flightCabinClass"])} className={`${field} ${connectedField}`}><option value="economy">{t("economy")}</option><option value="business">{t("business")}</option><option value="first">{t("first")}</option></select></div>
    </div>{errorBlock("flight")}</section>}
    {included.hotel && <section aria-labelledby="deals-hotel-heading" className="border-t border-slate-200 py-4 sm:py-3 lg:py-2"><h2 id="deals-hotel-heading" className="mb-2 flex items-center gap-2 text-base font-extrabold text-[#021C2B] lg:mb-1"><BedDouble className="h-5 w-5 text-[#004BB8]" />{t("deals.stayRow")}</h2><div className={`${connectedShell} sm:grid-cols-2`}><div ref={hotelDestinationWrapRef} className={`${connectedSegment} sm:border-e sm:border-b sm:border-slate-200 lg:border-b-0 ${hotelDestinationOpen ? "sm:z-20 sm:bg-[#004BB8]/8 sm:ring-1 sm:ring-inset sm:ring-[#004BB8]/20" : ""}`} data-deals-hotel-destination><label className={label} htmlFor="deals-hotel-destination">{t("deals.destination")}</label><div className="relative hidden sm:block"><input ref={hotelDestinationInputRef} id="deals-hotel-destination" role="combobox" aria-autocomplete="list" aria-controls="deals-hotel-destination-listbox" aria-expanded={hotelDestinationOpen} aria-activedescendant={hotelDestinationOpen && hotelSuggestions[hotelDestinationHighlight] ? `deals-hotel-destination-option-${hotelDestinationHighlight}` : undefined} value={search.hotelDestination} placeholder={t("hotelSearchDestinationPlaceholder")} onFocus={() => openHotelDestination()} onChange={(event) => { setSearch((current) => getIncludedProducts(current.mode).flight ? customizeInheritedField(current, "stayDestination", event.target.value) : applySharedDestination(current, event.target.value)); setHotelDestinationHighlight(0); openHotelDestination(); }} onKeyDown={(event) => { if (event.key === "Escape") return setHotelDestinationOpen(false); if (event.key === "ArrowDown" || event.key === "ArrowUp") { event.preventDefault(); openHotelDestination(); if (hotelSuggestions.length) setHotelDestinationHighlight((current) => (current + (event.key === "ArrowDown" ? 1 : -1) + hotelSuggestions.length) % hotelSuggestions.length); } else if (event.key === "Enter" && hotelDestinationOpen && hotelSuggestions[hotelDestinationHighlight]) { event.preventDefault(); const option = hotelSuggestions[hotelDestinationHighlight]; setSearch((current) => getIncludedProducts(current.mode).flight ? customizeInheritedField(current, "stayDestination", option.searchValue) : applySharedDestination(current, option.searchValue)); setHotelDestinationOpen(false); setHotelDestinationHighlight(0); } }} className={`${field} ${connectedField} pe-10`} autoComplete="off" />{search.hotelDestination ? <button type="button" aria-label={t("clearDestination")} onMouseDown={(event) => event.preventDefault()} onClick={() => { setSearch((current) => getIncludedProducts(current.mode).flight ? customizeInheritedField(current, "stayDestination", "") : applySharedDestination(current, "")); setHotelSuggestions([]); setHotelDestinationLoading(false); setHotelDestinationHighlight(0); hotelDestinationInputRef.current?.focus(); }} className="focus-ring absolute end-1.5 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full text-slate-500 hover:bg-slate-100"><X className="h-4 w-4" aria-hidden="true" /></button> : null}</div><button ref={hotelDestinationMobileLauncherRef} type="button" aria-haspopup="dialog" aria-expanded={hotelDestinationMobileOpen} onClick={() => openHotelDestination(true)} className={`${field} flex items-center justify-between gap-2 text-start sm:hidden`}><span className={`min-w-0 truncate ${search.hotelDestination ? "text-slate-900" : "text-slate-400"}`}>{search.hotelDestination || t("hotelSearchDestinationPlaceholder")}</span><ChevronDown className="h-4 w-4 shrink-0 text-slate-500" aria-hidden="true" /></button><DealsDestinationPopover open={hotelDestinationOpen} anchorRef={hotelDestinationWrapRef} width={436} marker="hotel">{hotelSuggestionContent}</DealsDestinationPopover>{getIncludedProducts(search.mode).flight && !search.stayDestinationLinked ? <button type="button" onClick={() => { setSearch((current) => relinkInheritedField(current, "stayDestination")); }} className="focus-ring mt-1 rounded-md px-1 text-xs font-bold text-[#004BB8] hover:underline">{t("deals.useMainDestination")}</button> : null}</div><div className={`${connectedSegment} sm:border-e sm:border-b sm:border-slate-200 lg:border-b-0 lg:last:border-e-0 ${hotelDatesOpen ? "sm:z-20 sm:bg-[#004BB8]/8 sm:ring-1 sm:ring-inset sm:ring-[#004BB8]/20" : ""}`}><span className={label}>{t("deals.travelDates")}</span><button ref={hotelDatesLauncherRef} type="button" aria-expanded={hotelDatesOpen || mobileHotelDatesOpen} aria-haspopup="dialog" aria-controls={mobileHotelDatesOpen ? "deals-hotel-mobile-dates" : "deals-hotel-desktop-dates"} aria-label={t("chooseTravelDates")} onClick={() => hotelDatesOpen ? dismissDesktopHotelDates(true) : openHotelDates()} className={`${field} ${connectedField} flex items-center justify-between gap-2 text-start`}><span className="min-w-0 truncate">{hotelDatesSummary}</span><Calendar aria-hidden="true" className="h-4 w-4 shrink-0 text-slate-500" /></button>{getIncludedProducts(search.mode).flight && !search.stayDatesLinked ? <button type="button" onClick={() => { setSearch((current) => relinkInheritedField(current, "stayDates")); }} className="focus-ring mt-1 rounded-md px-1 text-xs font-bold text-[#004BB8] hover:underline">{t("deals.useMainTravelDates")}</button> : null}</div></div>{errorBlock("hotel")}</section>}
    {included.car && <section aria-labelledby="deals-car-heading" className="border-t border-slate-200 py-4 sm:py-3 lg:py-2">
      <h2 id="deals-car-heading" className="mb-2 flex items-center gap-2 text-base font-extrabold text-[#021C2B] lg:mb-1"><Car className="h-5 w-5 text-[#004BB8]" />{t("deals.carRow")}</h2>
      <div className={`${connectedShell} sm:grid-cols-2 lg:grid-cols-3`}>
        <div className={connectedSegment}><label className={label} htmlFor="deals-car-pickup">{t("deals.pickup")}</label><div className="relative hidden sm:block"><input id="deals-car-pickup" ref={carPickupLocationRef} value={search.carPickupLocation} placeholder={t("carsSearch.pickupLocationPlaceholder")} autoComplete="off" onChange={(event) => { setSearch((current) => customizeInheritedField(current, "carPickup", event.target.value)); }} className={`${field} ${connectedField} truncate pe-10`} />{search.carPickupLocation ? <button type="button" aria-label={t("carsSearch.clearPickupLocation")} onClick={() => { setSearch((current) => customizeInheritedField(current, "carPickup", "")); carPickupLocationRef.current?.focus(); }} className="focus-ring absolute end-1.5 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full text-slate-500"><X className="h-4 w-4" /></button> : null}</div><button ref={carPickupLocationLauncherRef} type="button" aria-haspopup="dialog" onClick={() => openCarLocation("pickup")} className={`${field} flex items-center justify-between sm:hidden`}><span className="truncate">{search.carPickupLocation || t("carsSearch.pickupLocationPlaceholder")}</span><ChevronDown className="h-4 w-4" /></button>{!search.carPickupLinked ? <button type="button" onClick={() => { setSearch((current) => relinkInheritedField(current, "carPickup")); }} className="focus-ring mt-1 text-xs font-bold text-[#004BB8]">{t("deals.useMainDestination")}</button> : null}</div>
        <div className={connectedSegment}><span className={label}>{t("deals.returnLocation")}</span><button type="button" ref={carReturnLocationLauncherRef} aria-haspopup="dialog" aria-expanded={mobileCarLocation === "return"} onClick={() => openCarLocation("return")} className={`${field} ${connectedField} flex items-center justify-between text-start`}><span className="truncate">{search.carReturnToDifferentLocation ? search.carReturnLocation : <>{t("deals.sameAsPickup")}</>}</span><ChevronDown className="h-4 w-4" aria-hidden="true" /></button>{search.carReturnToDifferentLocation ? <button type="button" onClick={() => setSearch((current) => ({ ...current, carReturnToDifferentLocation: false, carReturnLocation: "" }))} className="focus-ring mt-1 text-xs font-bold text-[#004BB8]">{t("deals.useSameAsPickup")}</button> : null}</div>
        <div className={`${connectedSegment} ${carDatesOpen ? "sm:z-20 sm:bg-[#004BB8]/8" : ""}`}><span className={label}>{t("deals.travelDates")}</span><button ref={carDatesLauncherRef} type="button" aria-haspopup="dialog" aria-expanded={carDatesOpen || mobileCarDatesOpen} onClick={() => carDatesOpen ? dismissCarDates(true) : openCarDates()} className={`${field} ${connectedField} flex items-center justify-between text-start`}><span className="truncate">{carDatesSummary}</span><Calendar className="h-4 w-4" /></button>{!search.carDatesLinked ? <button type="button" onClick={() => { setSearch((current) => relinkInheritedField(current, "carDates")); }} className="focus-ring mt-1 text-xs font-bold text-[#004BB8]">{t("deals.useMainTravelDates")}</button> : null}</div>
      </div>
      <details className="mt-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2"><summary className="focus-ring cursor-pointer text-sm font-bold text-[#004BB8]">{t("deals.carOptions")} · {carTimesSummary} · {carDriverAgeLabel(search.carDriverAge)}</summary><div className="mt-3 grid gap-3 sm:grid-cols-2"><button ref={carTimesLauncherRef} type="button" onClick={() => openCarTimes()} className={`${field} text-start`}>{t("carsSearch.pickupReturnTimeLabel")}: {carTimesSummary}</button><select value={search.carDriverAge} onChange={(event) => update("carDriverAge", event.target.value)} aria-label={t("carsSearch.driverAgeLabel")} className={field}>{driverAgeOptions.map((age) => <option key={age} value={age}>{carDriverAgeLabel(age)}</option>)}</select></div></details>
      {errorBlock("car")}
    </section>}
    <section aria-labelledby="deals-travellers-heading" data-deals-travellers-row className="border-t border-slate-200 py-4 sm:py-3 lg:py-2"><h2 id="deals-travellers-heading" className="mb-2 flex items-center gap-2 text-base font-extrabold text-[#021C2B]"><Users className="h-5 w-5 text-[#004BB8]" aria-hidden="true" />{t("deals.travellersRow")}</h2><div className="grid items-end gap-3 sm:grid-cols-[minmax(0,1fr)_auto]"><div><span className={label}>{t("deals.travellersRooms")}</span><button ref={travelersLauncherRef} type="button" aria-expanded={travelersOpen || mobileTravelersOpen} aria-haspopup="dialog" aria-controls={mobileTravelersOpen ? "deals-mobile-travellers" : "deals-desktop-travellers"} onClick={() => travelersOpen ? dismissDesktopTravelers() : openTravelers()} className={`${field} flex items-center justify-between text-start`}><span className="truncate">{travelerSummary}</span><ChevronDown className="h-4 w-4" aria-hidden="true" /></button></div>{searchDealsButton}</div></section>
    {warning}
    {(["origin", "destination"] as const).map((kind) => { const textKey = kind === "origin" ? "flightOriginText" : "flightDestinationText"; const codeKey = kind === "origin" ? "flightOriginCode" : "flightDestinationCode"; const inputRef = kind === "origin" ? flightOriginMobileInputRef : flightDestinationMobileInputRef; const launcherRef = kind === "origin" ? flightOriginMobileLauncherRef : flightDestinationMobileLauncherRef; return <FlightMobilePickerShell key={kind} open={flightMobileAirport === kind} title={t(kind)} titleId={`deals-flight-mobile-${kind}-title`} launcherRef={launcherRef} onClose={() => setFlightMobileAirport(null)} contentClassName="px-4 py-5"><div className="space-y-4 overflow-x-hidden"><label className={label} htmlFor={`deals-flight-mobile-${kind}-input`}>{t(kind)}</label><div className="relative"><input ref={inputRef} id={`deals-flight-mobile-${kind}-input`} value={search[textKey]} placeholder={t("cityOrAirport")} autoComplete="off" onChange={(event) => { const value = event.target.value; if (kind === "origin") flightOriginUserInteractedRef.current = true; setSearch((current) => kind === "destination" ? { ...applySharedDestination(current, value), flightDestinationCode: /^[a-z]{3}$/i.test(value.trim()) ? value.trim().toUpperCase() : "" } : { ...current, [textKey]: value, [codeKey]: /^[a-z]{3}$/i.test(value.trim()) ? value.trim().toUpperCase() : "" }); if (kind === "origin") setFlightOriginHighlight(0); else setFlightDestinationHighlight(0); if (value.trim().length < 2) setAirportLists((all) => ({ ...all, [kind]: [] })); }} className={`${field} pe-10`} />{search[textKey] ? <button type="button" aria-label={t("clear")} onClick={() => { if (kind === "origin") flightOriginUserInteractedRef.current = true; setSearch((current) => kind === "destination" ? { ...applySharedDestination(current, ""), flightDestinationCode: "" } : { ...current, [textKey]: "", [codeKey]: "" }); setAirportLists((all) => ({ ...all, [kind]: [] })); inputRef.current?.focus(); }} className="focus-ring absolute end-1.5 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full text-slate-500 hover:bg-slate-100"><X className="h-4 w-4" aria-hidden="true" /></button> : null}</div><div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">{flightSuggestionContent(kind)}</div></div></FlightMobilePickerShell>; })}
    <HotelDestinationMobilePicker open={hotelDestinationMobileOpen} value={search.hotelDestination} titleId="deals-hotel-mobile-destination-title" inputId="deals-hotel-mobile-destination-input" launcherRef={hotelDestinationMobileLauncherRef} onChange={(value) => { setSearch((current) => getIncludedProducts(current.mode).flight ? customizeInheritedField(current, "stayDestination", value) : applySharedDestination(current, value)); }} onClear={() => { setHotelSuggestions([]); }} onClose={() => setHotelDestinationMobileOpen(false)} />
    ...
    <DealsFlightPopover open={travelersOpen} anchorRef={travelersLauncherRef}><div id="deals-desktop-travellers" role="dialog" aria-modal="false" aria-label={t("deals.travellersRooms")}>{travelersPicker}<div className="mt-4 flex justify-end border-t border-slate-100 pt-3"><button type="button" onClick={() => commitTravelers()} className="focus-ring min-h-10 rounded-xl bg-[#004BB8] px-5 text-sm font-extrabold text-white hover:bg-[#021C2B]">{t("done")}</button></div></div></DealsFlightPopover>
    <FlightMobilePickerShell open={mobileTravelersOpen} title={t("deals.travellersRooms")} titleId="deals-mobile-travellers" launcherRef={travelersLauncherRef} onClose={closeMobileTravelers} pickerMarker="shared-travellers" contentClassName="px-4 py-5" footer={(requestClose) => <div className="flex justify-end"><button type="button" onClick={() => { commitTravelers(true); requestClose(); }} className="focus-ring min-h-11 rounded-xl bg-[#004BB8] px-6 py-3 text-sm font-extrabold text-white hover:bg-[#021C2B]">{t("done")}</button></div>}>{travelersPicker}</FlightMobilePickerShell>
