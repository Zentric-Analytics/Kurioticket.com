...
import { ArrowRightLeft, BedDouble, Calendar, Car, Check, ChevronDown, MapPin, Minus, Plane, Plus, Search, X } from "lucide-react";
...
import { applySharedDates, applySharedDestination, customizeInheritedField, relinkInheritedField, transitionDealsMode } from "@/lib/deals/dealsSearchSynchronization";
...
export function DealsSearchForm({ initialSearch, variant = "landing", onSubmitSearch, onDraftChange, warning, pending = false }: DealsSearchFormProps = {}) {
  ...
  const dirty = useRef({ hotelDestination: !search.stayDestinationLinked, hotelDates: !search.stayDatesLinked, carLocation: !search.carPickupLinked, carDates: !search.carDatesLinked });
  ...
  const commitHotelGuests = (mobile = false) => { const adults = Math.max(1, Math.min(12, draftHotelAdults)); const children = Math.max(0, Math.min(12 - adults, draftHotelChildren)); const rooms = Math.max(1, Math.min(6, draftHotelRooms)); setSearch((current) => ({ ...current, hotelAdults: adults, hotelChildren: children, hotelRooms: rooms, hotelPetFriendly: draftHotelPetFriendly, flightAdults: adults, flightChildren: children })); if (mobile) mobileHotelGuestsCommittedRef.current = true; else { setHotelGuestsOpen(false); restoreHotelGuestsFocus(); } };
  ...
  const commitCarDates = (mobile = false) => { const pickup = parseIsoDate(draftCarPickupDate); const returning = parseIsoDate(draftCarReturnDate); if (!pickup || !returning || isBeforeToday(pickup) || isBeforeToday(returning) || returning < pickup) return; dirty.current.carDates = true; setSearch((current) => customizeInheritedField(current, "carDates", { start: toIsoDate(pickup), end: toIsoDate(returning) })); if (mobile) mobileCarDatesCommittedRef.current = true; else { setCarDatesOpen(false); restoreCarFocus(carDatesLauncherRef); } };
  ...
  const closeProductPickers = (product: DealsProduct) => {
    ...
    if (product === "hotel") { setHotelDestinationOpen(false); setHotelDestinationMobileOpen(false); setHotelDatesOpen(false); setMobileHotelDatesOpen(false); setHotelGuestsOpen(false); setMobileHotelGuestsOpen(false); }
  ...
  return <form onSubmit={submit} noValidate className={`mx-auto w-full max-w-[1120px] bg-white p-4 sm:px-4 sm:py-3 ${variant === "landing" ? "rounded-3xl border border-slate-200 shadow-[0_18px_46px_rgba(15,23,42,0.12)] sm:px-6 lg:py-3" : ""}`}>
    ...
    {included.car && <section aria-labelledby="deals-car-heading" className="border-t border-slate-200 py-4 sm:py-3 lg:py-2">
      ...
      <div className={`${connectedShell} sm:grid-cols-2 lg:grid-cols-3`}>
        <div className={connectedSegment}><label className={label} htmlFor="deals-car-pickup">{t("deals.pickup")}</label><div className="relative hidden sm:block"><input id="deals-car-pickup" ref={carPickupLocationRef} value={search.carPickupLocation} placeholder={t("carsSearch.pickupLocationPlaceholder")} autoComplete="off" onChange={(event) => { dirty.current.carLocation = true; setSearch((current) => customizeInheritedField(current, "carPickup", event.target.value)); }} className={`${field} ${connectedField} truncate pe-10`} />{search.carPickupLocation ? <button type="button" aria-label={t("carsSearch.clearPickupLocation")} onClick={() => { dirty.current.carLocation = true; setSearch((current) => customizeInheritedField(current, "carPickup", "")); carPickupLocationRef.current?.focus(); }} className="focus-ring absolute end-1.5 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full text-slate-500"><X className="h-4 w-4" /></button> : null}</div><button ref={carPickupLocationLauncherRef} type="button" aria-haspopup="dialog" onClick={() => openCarLocation("pickup")} className={`${field} flex items-center justify-between sm:hidden`}><span className="truncate">{search.carPickupLocation || t("carsSearch.pickupLocationPlaceholder")}</span><ChevronDown className="h-4 w-4" /></button>{!search.carPickupLinked ? <button type="button" onClick={() => { dirty.current.carLocation = false; setSearch((current) => relinkInheritedField(current, "carPickup")); }} className="focus-ring mt-1 text-xs font-bold text-[#004BB8]">{t("deals.useMainDestination")}</button> : null}</div>
        <div className={connectedSegment}><span className={label}>{t("deals.returnLocation")}</span>{search.carReturnToDifferentLocation ? <div className="relative hidden sm:block"><input ref={carReturnLocationRef} value={search.carReturnLocation} aria-label={t("deals.returnLocation")} onChange={(event) => update("carReturnLocation", event.target.value)} className={`${field} ${connectedField} pe-10`} /><button type="button" aria-label={t("deals.sameAsPickup")} onClick={() => setSearch((current) => ({ ...current, carReturnToDifferentLocation: false, carReturnLocation: "" }))} className="focus-ring absolute end-1 top-1/2 -translate-y-1/2 rounded-lg px-2 text-xs font-bold text-[#004BB8]">↩</button></div> : <button type="button" ref={carReturnLocationLauncherRef} aria-haspopup="dialog" onClick={() => { setSearch((current) => ({ ...current, carReturnToDifferentLocation: true })); openCarLocation("return"); }} className={`${field} ${connectedField} flex items-center justify-between text-start`}><span>{t("deals.sameAsPickup")}</span><ChevronDown className="h-4 w-4" /></button>}{search.carReturnToDifferentLocation ? <button type="button" onClick={() => setSearch((current) => ({ ...current, carReturnToDifferentLocation: false, carReturnLocation: "" }))} className="focus-ring mt-1 text-xs font-bold text-[#004BB8]">{t("deals.sameAsPickup")}</button> : null}</div>
        <div className={`${connectedSegment} ${carDatesOpen ? "sm:z-20 sm:bg-[#004BB8]/8" : ""}`}><span className={label}>{t("deals.travelDates")}</span><button ref={carDatesLauncherRef} type="button" aria-haspopup="dialog" aria-expanded={carDatesOpen || mobileCarDatesOpen} onClick={() => carDatesOpen ? dismissCarDates(true) : openCarDates()} className={`${field} ${connectedField} flex items-center justify-between text-start`}><span className="truncate">{carDatesSummary}</span><Calendar className="h-4 w-4" /></button>{!search.carDatesLinked ? <button type="button" onClick={() => { dirty.current.carDates = false; setSearch((current) => relinkInheritedField(current, "carDates")); }} className="focus-ring mt-1 text-xs font-bold text-[#004BB8]">{t("deals.useMainTravelDates")}</button> : null}</div>
      </div>
    ...
    <section aria-labelledby="deals-travellers-heading" data-deals-travellers-row className="border-t border-slate-200 py-4 sm:py-3 lg:py-2"><h2 id="deals-travellers-heading" className="mb-2 flex items-center gap-2 text-base font-extrabold text-[#021C2B]"><BedDouble className="h-5 w-5 text-[#004BB8]" />{t("deals.travellersRow")}</h2><div className="grid items-end gap-3 sm:grid-cols-[minmax(0,1fr)_auto]"><div><span className={label}>{t("deals.travellersRooms")}</span><button ref={travelersLauncherRef} type="button" aria-expanded={travelersOpen || mobileTravelersOpen} aria-haspopup="dialog" onClick={() => travelersOpen ? dismissDesktopTravelers() : openTravelers()} className={`${field} flex items-center justify-between text-start`}><span className="truncate">{travelerSummary}</span><ChevronDown className="h-4 w-4" /></button></div>{searchDealsButton}</div></section>
    {warning}
    ...
    <HotelDestinationMobilePicker open={hotelDestinationMobileOpen} value={search.hotelDestination} titleId="deals-hotel-mobile-destination-title" inputId="deals-hotel-mobile-destination-input" launcherRef={hotelDestinationMobileLauncherRef} onChange={(value) => { dirty.current.hotelDestination = true; setSearch((current) => getIncludedProducts(current.mode).flight ? customizeInheritedField(current, "stayDestination", value) : applySharedDestination(current, value)); }} onClear={() => { dirty.current.hotelDestination = true; setHotelSuggestions([]); }} onClose={() => setHotelDestinationMobileOpen(false)} />
    ...
    <DealsFlightPopover open={travelersOpen} anchorRef={travelersLauncherRef}><div id="deals-flight-desktop-travelers" role="dialog" aria-modal="false" aria-label={t("deals.travellersRooms")}>{travelersPicker}<div className="mt-4 flex justify-end border-t border-slate-100 pt-3"><button type="button" onClick={() => commitTravelers()} className="focus-ring min-h-10 rounded-xl bg-[#004BB8] px-5 text-sm font-extrabold text-white hover:bg-[#021C2B]">{t("done")}</button></div></div></DealsFlightPopover>
    <FlightMobilePickerShell open={mobileTravelersOpen} title={t("deals.travellersRooms")} titleId="deals-flight-mobile-travelers" launcherRef={travelersLauncherRef} onClose={closeMobileTravelers} contentClassName="px-4 py-5" footer={(requestClose) => <div className="flex justify-end"><button type="button" onClick={() => { commitTravelers(true); requestClose(); }} className="focus-ring min-h-11 rounded-xl bg-[#004BB8] px-6 py-3 text-sm font-extrabold text-white hover:bg-[#021C2B]">{t("done")}</button></div>}>{travelersPicker}</FlightMobilePickerShell>
