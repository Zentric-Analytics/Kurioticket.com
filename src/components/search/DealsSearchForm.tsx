...
export function DealsSearchForm({ initialSearch, variant = "landing", onSubmitSearch, onDraftChange, warning, pending = false }: DealsSearchFormProps = {}) {
  ...
  const dirty = useRef({ hotelDestination: Boolean(search.hotelDestination), hotelDates: Boolean(initialSearch) || params.has("hotelCheckIn"), carLocation: Boolean(search.carPickupLocation), carDates: Boolean(initialSearch) || params.has("carPickupDate") });
