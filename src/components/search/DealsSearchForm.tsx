...
import { BedDouble, Calendar, Car, ChevronDown, MapPin, Minus, Plane, Plus, Search, X } from "lucide-react";
...
export function DealsSearchForm() {
  ...
  return <form onSubmit={submit} noValidate className="mx-auto w-full max-w-[1120px] rounded-3xl border border-slate-200 bg-white p-4 shadow-[0_18px_46px_rgba(15,23,42,0.12)] sm:p-6">
    ...
    {included.flight && <section aria-labelledby="deals-flight-heading" className="border-t border-slate-200 py-5"><h2 id="deals-flight-heading" className="mb-4 flex items-center gap-2 text-base font-extrabold text-[#021C2B]"><Plane className="h-5 w-5 text-[#004BB8]" />{t("flights")}</h2><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {(["origin", "destination"] as const).map((kind) => { const textKey = kind === "origin" ? "flightOriginText" : "flightDestinationText"; const codeKey = kind === "origin" ? "flightOriginCode" : "flightDestinationCode"; const open = kind === "origin" ? flightOriginOpen : flightDestinationOpen; const loading = kind === "origin" ? flightOriginLoading : flightDestinationLoading; const highlight = kind === "origin" ? flightOriginHighlight : flightDestinationHighlight; const wrapRef = kind === "origin" ? flightOriginWrapRef : flightDestinationWrapRef; const inputRef = kind === "origin" ? flightOriginInputRef : flightDestinationInputRef; const launcherRef = kind === "origin" ? flightOriginMobileLauncherRef : flightDestinationMobileLauncherRef; return <div ref={wrapRef} className="relative min-w-0" key={kind} data-deals-flight-destination={kind}><label className={label} htmlFor={`deals-flight-${kind}`}>{t(kind)}</label><div className="relative hidden sm:block"><input ref={inputRef} id={`deals-flight-${kind}`} role="combobox" aria-autocomplete="list" aria-expanded={open} aria-controls={`deals-flight-${kind}-listbox`} aria-activedescendant={open && airportLists[kind][highlight] ? `deals-flight-${kind}-option-${highlight}` : undefined} value={search[textKey]} placeholder={t("cityOrAirport")} onFocus={() => openFlightAirport(kind)} onKeyDown={(event) => handleFlightKey(kind, event)} onChange={(event) => { const value = event.target.value; openFlightAirport(kind); setSearch((current) => ({ ...current, [textKey]: value, [codeKey]: /^[a-z]{3}$/i.test(value.trim()) ? value.trim().toUpperCase() : "" })); if (kind === "origin") setFlightOriginHighlight(0); else setFlightDestinationHighlight(0); if (value.trim().length < 2) setAirportLists((all) => ({ ...all, [kind]: [] })); }} className={`${field} pe-10`} autoComplete="off" />{search[textKey] ? <button type="button" aria-label={t("clear")} onMouseDown={(event) => event.preventDefault()} onClick={() => { setSearch((current) => ({ ...current, [textKey]: "", [codeKey]: "" })); setAirportLists((all) => ({ ...all, [kind]: [] })); if (kind === "origin") { setFlightOriginLoading(false); setFlightOriginHighlight(0); } else { setFlightDestinationLoading(false); setFlightDestinationHighlight(0); } inputRef.current?.focus(); }} className="focus-ring absolute end-1.5 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full text-slate-500 hover:bg-slate-100"><X className="h-4 w-4" aria-hidden="true" /></button> : null}</div><button ref={launcherRef} type="button" aria-haspopup="dialog" aria-expanded={flightMobileAirport === kind} onClick={() => openFlightAirport(kind, true)} className={`${field} flex items-center justify-between gap-2 text-start sm:hidden`}><span className={`min-w-0 truncate ${search[textKey] ? "text-slate-900" : "text-slate-400"}`}>{search[textKey] || t("cityOrAirport")}</span><ChevronDown className="h-4 w-4 shrink-0 text-slate-500" aria-hidden="true" /></button><DealsDestinationPopover open={open} anchorRef={wrapRef} width={390} marker={`flight-${kind}`}>{flightSuggestionContent(kind)}</DealsDestinationPopover>{loading ? <span className="sr-only" aria-live="polite">{t("searchingAirportsAndCities")}</span> : null}</div>; })}
