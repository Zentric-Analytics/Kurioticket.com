...
import { BedDouble, Car, ChevronDown, Minus, Plane, Plus, Search } from "lucide-react";
...
export function DealsSearchForm() {
  ...
  const syncFlightDates = (key: "flightDepartureDate" | "flightReturnDate", value: string) => setSearch((current) => ({ ...current, [key]: value, ...!dirty.current.hotelDates ? { [key === "flightDepartureDate" ? "hotelCheckIn" : "hotelCheckOut"]: value } : {}, ...!dirty.current.carDates ? { [key === "flightDepartureDate" ? "carPickupDate" : "carReturnDate"]: value } : {} }));
