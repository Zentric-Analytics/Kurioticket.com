...
import {
  ...
  getIncludedProducts, parseDealsSearchParams, synchronizeDealsSearchDefaults, tryToggleDealsProduct, validateDealsSearch,
...
const productOptions = { hotel: { label: "deals.journey.step.hotel", Icon: BedDouble }, flight: { label: "deals.journey.step.flight", Icon: Plane }, car: { label: "deals.journey.step.car", Icon: Car } } as const;
...
export function DealsSearchForm({ initialSearch, variant = "landing", onSubmitSearch, onDraftChange, warning, pending = false }: DealsSearchFormProps = {}) {
  ...
  const [search, setSearch] = useState<DealsSearch>(() => synchronizeDealsSearchDefaults(initialSearch ?? (params.size ? parseDealsSearchParams(params) : createDefaultDealsSearch())));
  ...
  const dirty = useRef({
    hotelDestination: Boolean(search.hotelDestination && search.flightDestinationText && !search.flightDestinationText.toLowerCase().includes(search.hotelDestination.toLowerCase())),
    hotelDates: Boolean(search.hotelCheckIn && (search.hotelCheckIn !== search.flightDepartureDate || search.hotelCheckOut !== search.flightReturnDate)),
    carLocation: Boolean(search.carPickupLocation && search.flightDestinationText && !search.flightDestinationText.toLowerCase().includes(search.carPickupLocation.toLowerCase())),
    carDates: Boolean(search.carPickupDate && (search.carPickupDate !== search.flightDepartureDate || search.carReturnDate !== search.flightReturnDate)),
  });
  ...
  const travelerSummary = useMemo(() => {
    ...
    if (included.hotel) parts.push(`${search.hotelRooms} ${t(search.hotelRooms === 1 ? "roomSingular" : "roomPlural")}`);
  ...
  const resetTravelersDraft = useCallback(() => { setDraftAdults(search.flightAdults); setDraftChildren(search.flightChildren); setDraftInfants(search.flightInfants); setDraftHotelRooms(search.hotelRooms); }, [search.flightAdults, search.flightChildren, search.flightInfants, search.hotelRooms]);
  ...
  const normalizeTravelersDraft = () => { const adults = Math.max(1, Math.min(9, draftAdults)); const children = Math.max(0, Math.min(9 - adults, draftChildren)); const infants = Math.max(0, Math.min(adults, 9 - adults - children, draftInfants)); return { adults, children, infants }; };
  const commitTravelers = (mobile = false) => { const normalized = normalizeTravelersDraft(); setSearch((current) => ({ ...current, flightAdults: normalized.adults, flightChildren: normalized.children, flightInfants: normalized.infants, hotelAdults: normalized.adults, hotelChildren: normalized.children, hotelRooms: Math.max(1, Math.min(6, draftHotelRooms)) })); if (mobile) mobileTravelersCommittedRef.current = true; else { setTravelersOpen(false); restoreTravelersFocus(); } };
  ...
  const travelersPicker = <div className="w-full space-y-4">
    ...
    {included.hotel ? <div className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-3 py-3"><span><span className="block font-extrabold text-slate-950">{t("rooms")}</span><span className="block text-xs font-medium text-slate-500">{t("hotelRoomsHelper")}</span></span><span className="flex items-center gap-2"><button type="button" aria-label={t("deals.decreaseCountAria").replace("{{label}}", t("rooms"))} disabled={draftHotelRooms <= 1} onClick={() => setDraftHotelRooms((current) => Math.max(1, current - 1))} className="focus-ring inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-300 disabled:opacity-40"><Minus className="h-4 w-4" aria-hidden /></button><span className="min-w-7 text-center font-extrabold tabular-nums">{draftHotelRooms}</span><button type="button" aria-label={t("deals.increaseCountAria").replace("{{label}}", t("rooms"))} disabled={draftHotelRooms >= 6} onClick={() => setDraftHotelRooms((current) => Math.min(6, current + 1))} className="focus-ring inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-300 disabled:opacity-40"><Plus className="h-4 w-4" aria-hidden /></button></span></div> : null}
  </div>;
  ...
  return <form onSubmit={submit} noValidate className={`mx-auto w-full max-w-[1180px] bg-white p-4 sm:px-6 sm:py-5 ${variant === "landing" ? "rounded-3xl border border-slate-200 shadow-[0_18px_46px_rgba(15,23,42,0.12)] sm:px-6 lg:py-3" : ""}`}>
    ...
    <section aria-labelledby="deals-travellers-heading" className="border-t border-slate-200 py-5">
      <h2 id="deals-travellers-heading" className="mb-2 flex items-center gap-2 text-base font-extrabold text-[#021C2B]"><span aria-hidden="true" className="flex h-5 w-5 items-center justify-center rounded-full bg-[#004BB8]/10 text-xs text-[#004BB8]">2</span>{t("deals.travelersRoomsLabel")}</h2>
      <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
        <div className={`${connectedSegment} rounded-xl border border-slate-200 bg-white ${travelersOpen ? "z-20 bg-[#004BB8]/8 ring-1 ring-[#004BB8]/20" : ""}`}><span className={label}>{t("deals.travelersRoomsLabel")}</span><button ref={travelersLauncherRef} type="button" aria-expanded={travelersOpen || mobileTravelersOpen} aria-haspopup="dialog" aria-controls={mobileTravelersOpen ? "deals-flight-mobile-travelers" : "deals-flight-desktop-travelers"} onClick={() => travelersOpen ? dismissDesktopTravelers() : openTravelers()} className={`${field} ${connectedField} flex items-center justify-between gap-2 text-start`}><span className="min-w-0 truncate">{travelerSummary}</span><ChevronDown aria-hidden="true" className={`h-4 w-4 shrink-0 text-slate-500 transition-transform ${travelersOpen || mobileTravelersOpen ? "rotate-180" : ""}`} /></button></div>
        {searchDealsButton}
      </div>
    </section>
    {warning}
    {(["origin", "destination"] as const).map((kind) => { const textKey = kind === "origin" ? "flightOriginText" : "flightDestinationText"; const codeKey = kind === "origin" ? "flightOriginCode" : "flightDestinationCode"; const inputRef = kind === "origin" ? flightOriginMobileInputRef : flightDestinationMobileInputRef; const launcherRef = kind === "origin" ? flightOriginMobileLauncherRef : flightDestinationMobileLauncherRef; return <FlightMobilePickerShell key={kind} open={flightMobileAirport === kind} title={t(kind)} titleId={`deals-flight-mobile-${kind}-title`} launcherRef={launcherRef} onClose={() => setFlightMobileAirport(null)} contentClassName="px-4 py-5"><div className="space-y-4 overflow-x-hidden"><label className={label} htmlFor={`deals-flight-mobile-${kind}-input`}>{t(kind)}</label><div className="relative"><input ref={inputRef} id={`deals-flight-mobile-${kind}-input`} value={search[textKey]} placeholder={t("cityOrAirport")} autoComplete="off" onChange={(event) => { const value = event.target.value; if (kind === "origin") flightOriginUserInteractedRef.current = true; setSearch((current) => ({ ...current, [textKey]: value, [codeKey]: /^[a-z]{3}$/i.test(value.trim()) ? value.trim().toUpperCase() : "", ...kind === "destination" && !dirty.current.hotelDestination ? { hotelDestination: value } : {}, ...kind === "destination" && !dirty.current.carLocation ? { carPickupLocation: value } : {} })); if (kind === "origin") setFlightOriginHighlight(0); else setFlightDestinationHighlight(0); if (value.trim().length < 2) setAirportLists((all) => ({ ...all, [kind]: [] })); }} className={`${field} pe-10`} />{search[textKey] ? <button type="button" aria-label={t("clear")} onClick={() => { if (kind === "origin") flightOriginUserInteractedRef.current = true; setSearch((current) => ({ ...current, [textKey]: "", [codeKey]: "", ...kind === "destination" && !dirty.current.hotelDestination ? { hotelDestination: "" } : {}, ...kind === "destination" && !dirty.current.carLocation ? { carPickupLocation: "" } : {} })); setAirportLists((all) => ({ ...all, [kind]: [] })); inputRef.current?.focus(); }} className="focus-ring absolute end-1.5 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full text-slate-500 hover:bg-slate-100"><X className="h-4 w-4" aria-hidden="true" /></button> : null}</div><div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">{flightSuggestionContent(kind)}</div></div></FlightMobilePickerShell>; })}
    <HotelDestinationMobilePicker open={hotelDestinationMobileOpen} value={search.hotelDestination} titleId="deals-hotel-mobile-destination-title" inputId="deals-hotel-mobile-destination-input" launcherRef={hotelDestinationMobileLauncherRef} onChange={(value) => { dirty.current.hotelDestination = true; setSearch((current) => ({ ...current, hotelDestination: value, ...!getIncludedProducts(current.mode).flight && !dirty.current.carLocation ? { carPickupLocation: value } : {} })); }} onClear={() => { dirty.current.hotelDestination = true; setHotelSuggestions([]); }} onClose={() => setHotelDestinationMobileOpen(false)} />
