...
export type DealsSearchFormProps = {
  ...
  onCancel?: () => void; onDraftChange?: (search: DealsSearch) => void; warning?: ReactNode; pending?: boolean;
};

export function DealsSearchForm({ initialSearch, variant = "landing", onSubmitSearch, onCancel, onDraftChange, warning, pending = false }: DealsSearchFormProps = {}) {
  ...
  const searchDealsButton = <div className="flex w-full flex-col-reverse gap-2 sm:w-auto sm:flex-row">{variant === "results" && <button type="button" disabled={pending} onClick={onCancel} className="min-h-11 rounded-xl border border-slate-300 px-6 text-sm font-extrabold text-slate-700 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 disabled:opacity-60">{t("deals.results.editor.cancel")}</button>}<button type="submit" disabled={submitting || pending} aria-busy={submitting || pending} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#004BB8] px-8 text-sm font-extrabold text-white shadow-lg shadow-blue-900/20 hover:bg-[#021C2B] disabled:opacity-70 sm:w-auto"><Search className="h-4 w-4" />{t(variant === "results" && pending ? "deals.results.editor.updating" : variant === "results" ? "deals.results.editor.update" : "deals.searchButton")}</button></div>;
