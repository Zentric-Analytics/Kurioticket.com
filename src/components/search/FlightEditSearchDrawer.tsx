...
export function FlightEditSearchDrawer({ open, initialValue, launcherRef, onClose, onSearch }: Props) {
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- Escape follows the current open drawer lifecycle.
  }, [open]);
  ...
  if (!open) return null;
  return <>
    <div role="dialog" aria-modal="true" aria-labelledby="flight-mobile-search-title" className="fixed inset-0 z-[10000] min-h-[100dvh] overflow-hidden overscroll-contain bg-slate-50 sm:hidden">
      <form onSubmit={(event) => { event.preventDefault(); if (canSearch) onSearch(draft); }} className="flex h-full min-h-0 w-full min-w-0 flex-col bg-slate-50">
        <div className="shrink-0 border-b border-slate-200/80 bg-white px-4 pb-2 pt-[calc(0.5rem+env(safe-area-inset-top))]"><div className="flex min-h-11 items-center justify-between gap-3"><h2 id="flight-mobile-search-title" className="text-xl font-bold leading-6 tracking-[-0.01em] text-slate-950">Edit flight search</h2><button type="button" aria-label="Close edit search" onClick={closeDrawer} className="inline-flex h-11 w-11 items-center justify-center rounded-[10px] text-slate-700 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#004BB8]/35"><X className="h-5 w-5" aria-hidden="true" /></button></div></div>
