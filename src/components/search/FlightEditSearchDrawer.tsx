...
type Props = {
  ...
  travelerPickerDensity?: "default" | "compact";
};
...
export function FlightEditSearchDrawer({ open, initialValue, launcherRef, onClose, onSearch, presentation = "fullscreen", travelerPickerDensity = "default" }: Props) {
    ...
    <div onPointerDown={(event) => { if (bottomSheet && event.target === event.currentTarget) closeDrawer(); }} data-flight-edit-presentation={presentation} className={`${bottomSheet ? `fixed inset-0 z-[10000] flex items-end overflow-hidden bg-slate-950/35 transition-opacity duration-200 sm:hidden ${isClosing || !hasEntered ? "opacity-0" : "opacity-100"}` : "fixed inset-0 z-[10000] min-h-[100dvh] overflow-hidden overscroll-contain bg-slate-50 sm:hidden"}`}>
      <form role="dialog" aria-modal="true" aria-labelledby="flight-mobile-search-title" onSubmit={(event) => { event.preventDefault(); if (canSearch) onSearch(draft); }} className={`flex min-h-0 w-full min-w-0 flex-col bg-slate-50 ${bottomSheet ? `h-[94dvh] max-h-[94dvh] overflow-hidden rounded-t-[22px] shadow-[0_-12px_36px_rgba(15,23,42,0.18)] transition-transform duration-200 ease-out ${isClosing || !hasEntered ? "translate-y-full" : "translate-y-0"}` : "h-full"}`}>
        ...
        <div className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto overscroll-contain px-4 py-4 pb-[calc(1.25rem+env(safe-area-inset-bottom))]"><div className="mx-auto flex w-full min-w-0 max-w-xl flex-col gap-3.5">
          ...
          {draft.tripType === "multi-city" ? <MultiCityFlightEditor legs={draft.legs} onChange={(legs) => setDraft((current) => ({ ...current, legs }))} minimumDate={today().toISOString().slice(0, 10)} presentation="homepage" onAirportValidityChange={setMultiCityAirportsValid} /> : <><div className="relative grid min-w-0 gap-3" data-mobile-route-fields><button ref={originRef} type="button" aria-haspopup="dialog" onClick={() => openMobilePickerWithKeyboard(() => setAirportPicker("origin"), "edit-flight-origin")} className={fieldClass} data-mobile-field="origin">{field("Origin", firstLeg.origin || "Choose origin", <MapPin className="h-5 w-5 text-slate-500" aria-hidden="true" />)}</button><button type="button" aria-label="Swap origin and destination" onClick={() => setDraft((current) => ({ ...current, legs: [{ ...firstLeg, origin: firstLeg.destination, destination: firstLeg.origin }, ...current.legs.slice(1)] }))} data-mobile-swap-control className="absolute left-1/2 top-1/2 z-10 inline-flex h-11 w-11 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-[#D8E1EC] bg-white text-[#004BB8]"><ArrowRightLeft className="h-5 w-5" aria-hidden="true" /></button><button ref={destinationRef} type="button" aria-haspopup="dialog" onClick={() => openMobilePickerWithKeyboard(() => setAirportPicker("destination"), "edit-flight-destination")} className={fieldClass} data-mobile-field="destination">{field("Destination", firstLeg.destination || "Choose destination", <MapPin className="h-5 w-5 text-slate-500" aria-hidden="true" />)}</button></div><button ref={datesRef} type="button" onClick={() => setDatePickerOpen(true)} className={fieldClass} data-mobile-field="dates" title={travelDatesDisplay ?? "Travel dates"} aria-label={`Travel dates: ${travelDatesDisplay ?? "Travel dates"}`}>{field("Travel dates", travelDatesDisplay ?? "Travel dates", <Calendar className="h-5 w-5 text-slate-500" aria-hidden="true" />)}</button></>}
