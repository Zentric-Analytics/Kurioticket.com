...
export function SearchTabs({
  ...
  const wrapper = useMemo(
    () =>
      cn(
        ...
        mobileHomepage
          ? tab === "cars"
            ? "rounded-[14px] bg-[#f8fafc] p-4 shadow-[0_16px_36px_rgba(15,23,42,0.12)] sm:rounded-[22px] sm:bg-white"
  ...
  const carsFieldCardClassName = cn(
    fieldCardClassName,
    mobileHomepage &&
      "rounded-none border-0 bg-transparent p-0 shadow-none ring-0 sm:rounded-2xl sm:border sm:border-slate-200 sm:bg-white sm:p-0.5 sm:shadow-[0_10px_28px_rgba(15,23,42,0.10)] lg:border-slate-200/90 lg:p-1 lg:shadow-[0_14px_34px_rgba(15,23,42,0.10)] lg:ring-1 lg:ring-slate-900/[0.02]",
  );
  ...
  const carsReturnLocationField = carsValues.returnToDifferentLocation ? (
    <div ref={carsDropoffFieldRef} className={cn("relative rounded-xl border border-slate-300 bg-white", mobileHomepage ? cn(hotelJoinedFieldClassName, carsMobileHomepageFieldClassName) : "px-4 py-2 sm:max-w-[50%]")} data-testid="cars-return-location-field">
      <label htmlFor="homepage-cars-dropoff" className={hotelFieldLabelClassName}>{translate("carsSearch.returnLocationLabel") || "Return location"}</label>
      <CarLocationAutocomplete id="homepage-cars-dropoff" name="dropoffLocation" value={carsValues.dropoffLocation} onValueChange={(value) => updateCarsValue("dropoffLocation", value)} placeholder={translate("carsSearch.returnLocationPlaceholder") || "Return city, airport or address"} presentation="responsive" inputClassName={cn(hotelFieldValueClassName, "h-8 w-full")} strings={carsLocationStrings} fieldAnchorRef={carsDropoffFieldRef} searchCardRef={carsSearchSurfaceRef} isOpen={carsOpenPicker === "dropoff"} onOpenChange={(open) => setCarsOpenPicker(open ? "dropoff" : null)} />
  ...
  if (mobileHomepage && tab === "flights") {
    ...
    const mobileProductTabs = (
      <div
        ...
        className="grid h-[46px] grid-cols-4 overflow-hidden rounded-xl border border-[#dee5ed] bg-[#fcfdfe] text-[15px] max-[359px]:text-[14px]"
      >
        {([
          ...
          return (
            <button
              ...
              className={cn(
                "focus-ring flex min-w-0 items-center justify-center gap-1.5 border-slate-200 px-0.5 font-medium text-slate-950 transition-colors max-[359px]:gap-1",
                index > 0 && "border-s",
                selected && "bg-[#eef5ff] text-[#075ee8]",
              )}
            >
              <Icon aria-hidden="true" className="h-5 w-5 shrink-0 max-[359px]:h-[18px] max-[359px]:w-[18px]" strokeWidth={1.8} />
  ...
  if (mobileHomepage && tab === "deals") {
    ...
    return (
      <section data-testid="mobile-homepage-deals-surface" className="rounded-[19px] border border-[#dee5ed] bg-[#f8fafc] p-[13px] shadow-[0_8px_22px_rgba(15,23,42,0.07)] sm:hidden">
        <div role="tablist" aria-label={translate("searchType") || "Search type"} data-testid="mobile-homepage-product-tabs" className="grid h-[46px] grid-cols-4 overflow-hidden rounded-xl border border-[#dee5ed] bg-[#fcfdfe] text-[15px] max-[359px]:text-[14px]">
          {products.map(([mode, Icon, label], index) => {
            const selected = tab === mode;
            return <button key={mode} type="button" role="tab" aria-selected={selected} onClick={() => { setCarsOpenPicker(null); setTab(mode); }} className={cn("focus-ring flex min-w-0 items-center justify-center gap-1.5 border-slate-200 px-0.5 font-medium text-slate-950 transition-colors max-[359px]:gap-1", index > 0 && "border-s", selected && "bg-[#eef5ff] text-[#075ee8]")}><Icon aria-hidden="true" className="h-5 w-5 shrink-0 max-[359px]:h-[18px] max-[359px]:w-[18px]" strokeWidth={1.8} /><span className="truncate">{label}</span></button>;
          })}
        </div>
      ...
      >
        <button
          ...
          className={cn(
            ...
            mobileHomepage ? "min-w-0 gap-1.5 px-0.5 text-[15px] font-medium max-[359px]:text-[14px] max-[389px]:gap-1" : "rounded-lg px-3 py-1.5",
        ...
        >
          <Plane className={mobileHomepage ? "h-5 w-5 shrink-0 max-[359px]:h-[18px] max-[359px]:w-[18px]" : "h-4 w-4"} strokeWidth={mobileHomepage ? 1.8 : undefined} />
          {t.flights}
        </button>
        ...
        <button
          ...
          className={cn(
            "focus-ring inline-flex items-center justify-center gap-2 text-sm font-semibold transition-colors",
            mobileHomepage ? "min-w-0 gap-1.5 border-s border-slate-200 px-0.5 text-[15px] font-medium max-[359px]:text-[14px] max-[389px]:gap-1" : "rounded-lg px-3 py-1.5",
            compactHero && !mobileHomepage && "lg:px-3.5 lg:py-2 lg:text-[15px]",
            tab === "hotels"
              ? mobileHomepage
                ? "bg-[#eef5ff] text-[#075ee8]"
        ...
        >
          {mobileHomepage ? <Building2 className="h-5 w-5 shrink-0 max-[359px]:h-[18px] max-[359px]:w-[18px]" strokeWidth={1.8} /> : <BedDouble className="h-4 w-4" />}
          {t.hotels}
        </button>
        ...
        >
          <CarFront className={mobileHomepage ? "h-5 w-5 shrink-0 max-[359px]:h-[18px] max-[359px]:w-[18px]" : "h-4 w-4"} strokeWidth={mobileHomepage ? 1.8 : undefined} />
          {t.cars}
        </button>
        ...
        <form onSubmit={onCarsSubmit} className={formClassName} noValidate>
          ...
          <div className={carsFieldCardClassName} data-testid="cars-joined-search-card">
            <div className={carsGridClassName} data-testid="cars-primary-row">
              ...
              <div className={cn(hotelSubmitWrapClassName, mobileHomepage && "mt-0.5 sm:mt-0")}>
                <Button type="submit" disabled={isCarsSearchDisabled} aria-busy={isCarsSubmitting} aria-label={translate("searchCars") || "Search cars"} className={cn(hotelSubmitButtonClassName, "whitespace-nowrap", mobileHomepage && "rounded-[11px] sm:rounded-xl lg:rounded-none lg:rounded-e-xl")}>
