      ...
      <div key={monthStart.toISOString()} className="w-full min-w-[260px]">
        ...
        <div className="grid grid-cols-7 gap-1.5">
          {slots.map((value, index) => {
            ...
            const isSelected =
              (departure && isSameDay(date, departure)) ||
              (returning && isSameDay(date, returning));
            const inRange =
              departure &&
              returning &&
              date > departure &&
            ...
            return (
              <button
                ...
                style={
                  isSelected ? { backgroundColor: BRAND_PURPLE } : undefined
                }
              >
                {value}
              </button>
      ...
      {tab === "flights" ? (
        <>
          <div className="mb-3 flex flex-wrap items-center gap-2 text-sm font-semibold text-slate-700">
            {(["round-trip", "one-way", "multi-city"] as TripType[]).map(
              (mode) => (
                <button
                  key={mode}
                  type="button"
          ...
          <form
            className={cn(
              "grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-[minmax(140px,1fr)_minmax(140px,1fr)_minmax(120px,1fr)_minmax(120px,1fr)_minmax(130px,1fr)_minmax(130px,1fr)_140px]",
              isCalendarOpen && "pb-[22rem] md:pb-[20rem]",
            )}
            onSubmit={(event) => {
            ...
            >
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className="focus-ring flex h-11 items-center rounded-lg border border-slate-300 px-3 text-left text-sm font-semibold text-slate-700"
                >
                ...
                <button
                  ...
                  className="focus-ring flex h-11 items-center rounded-lg border border-slate-300 px-3 text-left text-sm font-semibold text-slate-700 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {returnDate ||
                    (tripType === "one-way"
                      ? t.notNeeded || "Not needed"
              ...
              {isCalendarOpen ? (
                <div
                  className={cn(
                    "absolute top-full z-30 mt-2 w-[min(94vw,44rem)] rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_18px_50px_rgba(15,23,42,0.16)] md:p-5",
                    activeDateField === "departure" ? "left-0" : "right-0",
                ...
                >
                  <div className="mb-4 flex items-center justify-between">
                    <button
                      ...
                      className="focus-ring rounded-full border border-slate-200 px-2.5 py-1 text-slate-700 hover:bg-violet-50"
