      ...
      {tab === "flights" ? (
        <>
          ...
          <form className="relative grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-[minmax(140px,1fr)_minmax(140px,1fr)_minmax(120px,1fr)_minmax(120px,1fr)_minmax(130px,1fr)_minmax(130px,1fr)_140px]" onSubmit={(event) => {
            ...
            <div className="relative" ref={departureFieldRef}>
            ...
            <div className="relative" ref={returnFieldRef}>
            ...
            {isCalendarOpen ? (
              <div
                ref={datePickerRef}
                className={cn(
                  "absolute z-30 mt-2 w-[min(94vw,44rem)] rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_18px_50px_rgba(15,23,42,0.16)] md:p-5",
                  "md:translate-y-0",
                )}
                style={{
                  top: activeDateField === "departure" ? departureFieldRef.current?.offsetTop ? `${departureFieldRef.current.offsetTop + 52}px` : undefined : returnFieldRef.current?.offsetTop ? `${returnFieldRef.current.offsetTop + 52}px` : undefined,
                  left:
                    activeDateField === "departure"
                      ? departureFieldRef.current?.offsetLeft
                      : Math.max(0, (returnFieldRef.current?.offsetLeft || 0) - 320),
                }}
              >
