      ...
      {tab === "flights" ? (
        <div className="space-y-2">
          ...
          <div className="relative grid overflow-visible rounded-2xl border border-violet-100 bg-white shadow-sm lg:grid-cols-[1fr_1fr_1fr_1fr_1fr_auto]">
            ...
            {isCalendarOpen ? (
              <div className={`absolute top-[calc(100%+8px)] z-30 w-full rounded-2xl border border-violet-100 bg-white p-3 shadow-[0_16px_36px_rgba(15,23,42,0.16)] sm:p-4 lg:w-[560px] ${pickerTarget === "departure" ? "lg:left-[34%]" : "lg:left-[50%]"}`}>
...
function DateTriggerField({ label, value, subtext, isActive, onClick, buttonRef }: { label: string; value: string; subtext: string; isActive: boolean; onClick: () => void; buttonRef?: React.RefObject<HTMLButtonElement | null> }) {
  return (
    <button ref={buttonRef} type="button" onClick={onClick} className={`flex min-h-[72px] w-full flex-col justify-center border-t border-violet-100 px-3.5 py-2 text-left transition first:border-t-0 lg:border-l lg:border-t-0 lg:first:border-l-0 ${isActive ? "bg-violet-50/60" : "hover:bg-slate-50"}`}>
      <span className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-500">{label}</span>
