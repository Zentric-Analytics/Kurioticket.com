    ...
    return (
      <div key={monthStart.toISOString()} className="w-full min-w-[260px]">
        ...
        <div className="grid grid-cols-7 gap-1">
          {slots.map((value, idx) => {
            if (value < 1) return <span key={`empty-${idx}`} className="h-9" />;
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  !isPast && !isSelected && !inRange && "text-slate-700 hover:bg-violet-100",
      ...
      {tab === "flights" ? (
        <>
          ...
          <form className="grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-[minmax(140px,1fr)_minmax(140px,1fr)_minmax(120px,1fr)_minmax(120px,1fr)_minmax(130px,1fr)_minmax(130px,1fr)_140px]" onSubmit={(event) => {
            ...
            <div className="relative md:col-span-2 xl:col-span-2" ref={datePickerRef}>
              <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={() => openCalendar("departure")} className="focus-ring flex h-11 items-center rounded-lg border border-slate-300 px-3 text-left text-sm font-semibold text-slate-700">
                  {departureDate || t.selectDate || "Select date"}
                </button>
                <button type="button" onClick={() => openCalendar("return")} disabled={tripType === "one-way"} className="focus-ring flex h-11 items-center rounded-lg border border-slate-300 px-3 text-left text-sm font-semibold text-slate-700 disabled:cursor-not-allowed disabled:opacity-50">
                  {returnDate || (tripType === "one-way" ? t.notNeeded || "Not needed" : t.selectDate || "Select date")}
                </button>
              </div>

              {isCalendarOpen ? (
                <div className="absolute z-30 mt-2 w-full min-w-[300px] rounded-xl border border-slate-200 bg-white p-4 shadow-2xl xl:w-auto">
                  <div className="mb-3 flex items-center justify-between">
                    <button type="button" onClick={() => setCurrentMonth((prev) => addMonths(prev, -1))} className="focus-ring rounded-full border border-slate-200 px-2 py-1 text-slate-700 hover:bg-slate-100" aria-label="Previous month">←</button>
                    <button type="button" onClick={() => setCurrentMonth((prev) => addMonths(prev, 1))} className="focus-ring rounded-full border border-slate-200 px-2 py-1 text-slate-700 hover:bg-slate-100" aria-label="Next month">→</button>
                  </div>
