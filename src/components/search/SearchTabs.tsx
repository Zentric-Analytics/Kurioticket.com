...
export function SearchTabs({
  ...
  function onFlightSubmit(formData: FormData) {
    const params = new URLSearchParams({
      ...
      departureDate: compactHero
        ? departureDate
        : String(formData.get("departureDate") || ""),
      returnDate: compactHero
        ? returnDate
  ...
  return (
    <div
      className={`w-full rounded-xl border bg-white ${
        compactHero
          ? "border-[#6d28d9] shadow-[0_14px_45px_rgba(2,36,96,0.16)]"
          : "border-slate-100 shadow-[0_18px_60px_rgba(15,23,42,0.12)]"
      }`}
    >
      <div className="flex w-full max-w-xs overflow-hidden rounded-t-xl border-b border-slate-200 bg-white">
        <button
          type="button"
          className={`focus-ring flex h-16 flex-1 items-center justify-center gap-2 border-b-2 text-base font-extrabold ${
            tab === "flights"
      ...
      {tab === "flights" ? (
        <form
          ...
          className={`grid gap-6 ${
            compactHero ? "p-4 sm:p-5" : "p-5 sm:p-7"
          }`}
        >
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <button
              type="button"
              onClick={() => setTab("hotels")}
              className="focus-ring inline-flex items-center gap-2 text-sm font-extrabold text-[#6d28d9]"
            >
              <Hotel size={17} />
          ...
          <div
            className={`grid overflow-visible rounded-xl border bg-white ${
              compactHero
                ? "grid-cols-1 lg:grid-cols-[1.1fr_auto_1.1fr_1fr_1fr_1.1fr]"
                : "md:grid-cols-[1fr_auto_1fr] lg:grid-cols-[1fr_auto_1fr_1fr_1fr_1fr]"
            } ${compactHero ? "border-[#6d28d9]" : "border-slate-200"}`}
          >
            <SearchField
              label={t.from}
              name="origin"
              helper={t.cityAirport}
              placeholder={t.from}
            />

            <div
              className={`hidden items-center justify-center border-slate-200 px-2 ${
                compactHero ? "lg:flex lg:border-l" : "md:flex lg:border-r"
              }`}
            >
            ...
            {compactHero ? (
              <>
                <div
                  ref={calendarRef}
                  className="relative border-t border-slate-200 p-4 lg:border-l lg:border-t-0"
                >
                  <DateDisplay
                    label={t.departure}
                    value={departureDate}
                ...
                <div className="border-t border-slate-200 p-4 lg:border-l lg:border-t-0">
                  <DateDisplay
                    ...
                    value={returnDate}
                    helper={
                      tripType === "one-way" ? t.notNeeded : t.selectDate
                    }
                  ...
                  <input
                    type="hidden"
                    name="returnDate"
                    value={tripType === "one-way" ? "" : returnDate}
                  />
                </div>
              </>
            ) : (
            ...
            <label className="block border-t border-slate-200 p-4 lg:border-l lg:border-t-0">
              ...
              <select
                name="travelers"
                defaultValue="1"
                className="mt-2 w-full bg-transparent text-lg font-extrabold text-slate-950 outline-none"
              >
                <option value="1">{t.oneTraveler}</option>
                <option value="2">{t.twoTravelers}</option>
                <option value="3">{t.threeTravelers}</option>
                <option value="4">{t.fourTravelers}</option>
              </select>

              <select
                name="cabinClass"
                defaultValue="economy"
                className="mt-1 w-full bg-transparent text-sm font-semibold text-slate-600 outline-none"
              >
          ...
          <div
            className={`${
              compactHero ? "flex justify-end" : "flex justify-center"
            }`}
          >
            <Button
              size="lg"
              ...
              className={`w-full sm:w-auto ${
                compactHero
                  ? "h-14 rounded-xl bg-[#6d28d9] px-16 text-base font-black text-white transition-colors duration-200 hover:bg-[#5b21b6]"
                  : "bg-[#6d28d9] px-12 hover:bg-[#5b21b6]"
              }`}
            >
              <Search size={18} />
              {t.searchFlights}
        ...
        <form
          action={onHotelSubmit}
          className={`grid gap-6 ${
            compactHero ? "p-4 sm:p-5" : "p-5 sm:p-7"
          }`}
        >
          <div
            className={`grid overflow-hidden rounded-xl border bg-white md:grid-cols-2 lg:grid-cols-5 ${
              compactHero ? "border-[#6d28d9]" : "border-slate-200"
            }`}
          >
            <SearchField
          ...
          >
            <Button
              ...
              className={`w-full sm:w-auto ${
                compactHero
                  ? "h-14 rounded-xl bg-[#6d28d9] px-16 text-base font-black text-white hover:bg-[#5b21b6]"
                  : "bg-[#6d28d9] px-12 hover:bg-[#5b21b6]"
              }`}
            >
              <Search size={18} />
...
function FlightCalendar({
  ...
  return (
    <div className="absolute left-0 z-30 mt-3 w-[min(92vw,680px)] rounded-2xl border border-violet-100 bg-white p-3 shadow-2xl sm:p-5">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="focus-ring rounded-full border border-violet-100 p-1.5 text-slate-700 hover:bg-violet-50"
          >
            <ChevronLeft size={17} />
          </button>
...
function MonthGrid({
  ...
  return (
    <div>
      <div className="mb-2 text-center text-sm font-extrabold text-slate-900">
        ...
        {cells.map((date, index) => {
          ...
          return (
            <button
              ...
              style={isEdge ? { backgroundColor: PURPLE } : undefined}
            >
              {date.getDate()}
            </button>
          );
...
function DateDisplay({
  ...
  return (
    ...
    >
      ...
      <span className="mt-1 block h-10 text-lg font-extrabold uppercase text-slate-950">
  ...
  return (
    <label className="block border-t border-slate-200 p-4 first:border-t-0 md:border-l md:border-t-0 md:first:border-l-0">
      ...
      <Input
        ...
        disabled={disabled}
        required={!disabled}
        className="mt-1 h-10 rounded-none border-0 bg-transparent px-0 text-lg font-extrabold uppercase shadow-none placeholder:text-slate-400 focus-visible:shadow-none"
      />
