      ...
      {tab === "flights" ? (
        <div className="space-y-2">
          ...
          <div className="relative grid overflow-visible rounded-2xl border border-violet-100 bg-white shadow-sm lg:grid-cols-[1fr_1fr_1fr_1fr_1fr_auto]">
            ...
            <DateTriggerField label="DEPARTURE" value={format(departureDate, "MMM dd, yyyy")} subtext={format(departureDate, "EEEE")} isActive={isCalendarOpen && pickerTarget === "departure"} onClick={() => { setPickerTarget("departure"); setIsCalendarOpen(true); setActiveLocationField(null); setIsTravelerOpen(false); }} />
            <DateTriggerField label="RETURN" value={format(returnDate, "MMM dd, yyyy")} subtext={format(returnDate, "EEEE")} isActive={isCalendarOpen && pickerTarget === "return"} onClick={() => { setPickerTarget("return"); setIsCalendarOpen(true); setActiveLocationField(null); setIsTravelerOpen(false); }} />

            <TravelerField travelers={travelers} cabinClass={cabinClass} isOpen={isTravelerOpen} onToggle={() => { setIsTravelerOpen((v) => !v); setActiveLocationField(null); setIsCalendarOpen(false); }} onIncrement={() => setTravelers((v) => Math.min(9, v + 1))} onDecrement={() => setTravelers((v) => Math.max(1, v - 1))} onClassChange={setCabinClass} />
            ...
            {activeLocationField ? (
              <div className="absolute left-0 top-[calc(100%+8px)] z-30 w-full rounded-2xl border border-violet-100 bg-white p-2 shadow-[0_12px_30px_rgba(15,23,42,0.14)] lg:w-[40%]">
            ...
            {isCalendarOpen ? (
              <div className="absolute left-0 right-0 top-[calc(100%+8px)] z-30 rounded-2xl border border-violet-100 bg-white p-3 shadow-[0_16px_36px_rgba(15,23,42,0.16)] sm:p-4 lg:left-[25%] lg:right-[14%]">
...
function DateTriggerField({ label, value, subtext, isActive, onClick }: { label: string; value: string; subtext: string; isActive: boolean; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className={`flex min-h-[72px] w-full flex-col justify-center border-t border-violet-100 px-3.5 py-2 text-left transition first:border-t-0 lg:border-l lg:border-t-0 lg:first:border-l-0 ${isActive ? "bg-violet-50/60" : "hover:bg-slate-50"}`}>
      <span className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-500">{label}</span>
      <div className="mt-1 inline-flex items-center gap-2"><span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-violet-100 text-[#6d28d9]"><CalendarDays size={13} /></span><span className="text-[15px] font-semibold leading-tight text-slate-900">{value}</span></div>
  ...
  return (
    <div className="relative flex min-h-[72px] flex-col justify-center border-t border-violet-100 px-3 py-2 first:border-t-0 lg:border-l lg:border-t-0 lg:first:border-l-0">
      <button type="button" onClick={onToggle} className="text-left">
        ...
        <div className="mt-1 inline-flex items-center gap-2"><Users size={14} className="text-[#6d28d9]" /><span className="text-sm font-semibold text-slate-900">{travelers} Traveler{travelers > 1 ? "s" : ""}</span><ChevronDown size={14} className="text-slate-500" /></div>
      ...
      {isOpen ? (
        <div className="absolute left-0 top-[calc(100%+8px)] z-30 w-full min-w-[220px] rounded-2xl border border-violet-100 bg-white p-3 shadow-[0_12px_30px_rgba(15,23,42,0.14)]">
          <div className="flex items-center justify-between"><span className="text-sm font-semibold text-slate-700">Travelers</span><div className="inline-flex items-center gap-2"><button type="button" onClick={onDecrement} className="h-8 w-8 rounded-full border border-slate-200">-</button><span className="w-5 text-center text-sm font-semibold">{travelers}</span><button type="button" onClick={onIncrement} className="h-8 w-8 rounded-full border border-slate-200">+</button></div></div>
