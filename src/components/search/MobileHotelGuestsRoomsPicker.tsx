...
function Row({ icon, label, description, counter, compact }: { icon: ReactNode; label: string; description: string; counter: ReactNode; compact: boolean }) {
  return <div className={cn("flex items-center", compact ? "min-h-[86px] gap-2.5 px-3.5" : "min-h-[104px] gap-3 px-4")}>
    <span className={cn("flex shrink-0 items-center justify-center rounded-full bg-slate-100 text-[#075EE8]", compact ? "h-11 w-11" : "h-[52px] w-[52px]")}>{icon}</span>
...
export function MobileHotelGuestsRoomsPicker({ adults, children, rooms, petFriendly, strings, density = "default", onAdultsChange, onChildrenChange, onRoomsChange, onPetFriendlyChange }: Props) {
  ...
  return <div className="mx-auto w-full max-w-xl pb-2">
    ...
    <div className={cn("flex items-center rounded-xl border border-slate-200 bg-white", compact ? "mt-4 min-h-[74px] gap-2.5 px-3.5" : "mt-5 min-h-[88px] gap-3 px-4")}>
      <span className={cn("flex shrink-0 items-center justify-center rounded-full bg-slate-100", compact ? "h-11 w-11" : "h-[52px] w-[52px]")}><PawPrint aria-hidden="true" className={cn("text-[#075EE8]", compact ? "h-5 w-5" : "h-[22px] w-[22px]")} /></span>
      ...
      <button type="button" role="switch" aria-checked={petFriendly} aria-label={strings.petFriendly} onClick={() => onPetFriendlyChange(!petFriendly)} className={cn("focus-ring relative flex h-11 shrink-0 items-center justify-center", compact ? "w-[46px]" : "w-[52px]")}>
        <span className={cn("relative block rounded-full transition-colors", compact ? "h-[26px] w-[46px]" : "h-[30px] w-[52px]", petFriendly ? "bg-[#075EE8]" : "bg-slate-200")}><span className={cn("absolute top-[3px] rounded-full bg-white shadow-sm transition-[inset]", compact ? "h-5 w-5" : "h-6 w-6", petFriendly ? "end-[3px]" : "start-[3px]")} /></span>
      </button>
    </div>
  </div>;
}
