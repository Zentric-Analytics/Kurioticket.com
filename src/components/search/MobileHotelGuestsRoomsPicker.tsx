...
function Counter({ value, min, max, onChange, decreaseLabel, increaseLabel, compact }: { value: number; min: number; max: number; onChange: (value: number) => void; decreaseLabel: string; increaseLabel: string; compact: boolean }) {
  ...
  return <div data-hotel-counter className={cn("flex shrink-0 items-center", compact ? "gap-2" : "gap-3")}>
    {control("decrease")}
...
function Row({ icon, label, description, counter, compact }: { icon: ReactNode; label: string; description: string; counter: ReactNode; compact: boolean }) {
  return <div data-hotel-guest-row className={cn("flex items-center", compact ? "min-h-[80px] gap-2.5 px-3.5" : "min-h-[104px] gap-3 px-4")}>
    <span data-hotel-guest-icon className={cn("flex shrink-0 items-center justify-center rounded-full", compact ? "h-10 w-10 bg-[#075EE8]/[0.06] text-[#075EE8]" : "h-[52px] w-[52px] bg-slate-100 text-slate-800")}>{icon}</span>
    <span data-hotel-guest-copy className="min-w-0 flex-1">
...
export function MobileHotelGuestsRoomsPicker({ adults, childCount: children, rooms, petFriendly, strings, density = "default", onAdultsChange, onChildrenChange, onRoomsChange, onPetFriendlyChange }: Props) {
  ...
  return <div className="mx-auto w-full max-w-xl pb-2">
    <section data-hotel-guest-section aria-labelledby="mobile-hotel-guests-heading">
      ...
      <div data-hotel-guest-card className="divide-y divide-slate-200 overflow-hidden rounded-xl border border-slate-200 bg-white">
    ...
    <section data-hotel-guest-section aria-labelledby="mobile-hotel-rooms-heading" className={compact ? "mt-4" : "mt-7"}>
      ...
      <div data-hotel-guest-card className="overflow-hidden rounded-xl border border-slate-200 bg-white"><Row compact={compact} icon={<BedDouble aria-hidden="true" className={guestIconClassName} />} label={strings.rooms} description={strings.roomDescription} counter={<Counter value={rooms} min={1} max={6} onChange={onRoomsChange} decreaseLabel={strings.decrease(strings.rooms)} increaseLabel={strings.increase(strings.rooms)} compact={compact} />} /></div>
    </section>
    <div data-hotel-guest-card data-hotel-guest-row data-hotel-pet-row className={cn("flex items-center rounded-xl border border-slate-200 bg-white", compact ? "mt-4 min-h-[70px] gap-2.5 px-3.5" : "mt-5 min-h-[88px] gap-3 px-4")}>
      <span data-hotel-guest-icon className={cn("flex shrink-0 items-center justify-center rounded-full", compact ? "h-10 w-10 bg-[#075EE8]/[0.06]" : "h-[52px] w-[52px] bg-slate-100")}><PawPrint aria-hidden="true" className={cn(compact ? "h-5 w-5 text-[#075EE8]" : "h-[22px] w-[22px] text-slate-700")} /></span>
      <span data-hotel-guest-copy className="min-w-0 flex-1"><span className={cn("block font-bold text-slate-950", compact ? "text-[14px]" : "text-[15px]")}>{strings.petFriendly}</span><span className={cn("block text-[12px] font-medium text-slate-600", compact ? "mt-0.5 leading-[16px]" : "mt-1")}>{strings.petDescription}</span></span>
