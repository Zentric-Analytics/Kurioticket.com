...
function Counter({ value, minimum, maximum, label, strings, onChange, neutral }: { value: number; minimum: number; maximum: number; label: string; strings: Strings; onChange: (value: number) => void; neutral: boolean }) {
  const button = (direction: -1 | 1) => {
    const disabled = direction < 0 ? value <= minimum : value >= maximum;
    return <button type="button" disabled={disabled} aria-label={(direction < 0 ? strings.decrease : strings.increase)(label)} onClick={() => onChange(Math.max(minimum, Math.min(maximum, value + direction)))} className={`group focus-ring flex h-11 w-11 items-center justify-center rounded-full disabled:cursor-not-allowed disabled:text-slate-300 ${neutral ? "text-slate-900" : "text-[#075ee8]"}`}><span className={`flex h-10 w-10 items-center justify-center rounded-full border bg-white group-disabled:border-slate-200 ${neutral ? "border-slate-500" : "border-[#075ee8]/75"}`}>{direction < 0 ? <Minus aria-hidden="true" className="h-4 w-4"/> : <Plus aria-hidden="true" className="h-4 w-4"/>}</span></button>;
  };
...
export function MobilePackageTravelersRoomsPicker(props: Props) {
  ...
  return <div className="mx-auto w-full max-w-xl pb-2" data-mobile-package-travelers-rooms>
    <div className="overflow-hidden rounded-[11px] border border-slate-200 bg-white">
      <Row neutral={!includeHotel} icon={<UserRound aria-hidden="true" className="h-[22px] w-[22px]"/>} label={strings.adults} description={strings.adultDescription} counter={<Counter neutral={!includeHotel} value={adults} minimum={1} maximum={adults + (maximum - total)} label={strings.adults} strings={strings} onChange={(value) => { props.onAdultsChange(value); if (infants > value) props.onInfantsChange(value); }}/>} />
      <Row neutral={!includeHotel} icon={<ChildOutlineIcon/>} label={strings.children} description={strings.childDescription} counter={<Counter neutral={!includeHotel} value={children} minimum={0} maximum={children + (maximum - total)} label={strings.children} strings={strings} onChange={props.onChildrenChange}/>} />
      <Row neutral={!includeHotel} icon={<Baby aria-hidden="true" className="h-6 w-6"/>} label={strings.infants} description={strings.infantDescription} counter={<Counter neutral={!includeHotel} value={infants} minimum={0} maximum={Math.min(adults, infants + (maximum - total))} label={strings.infants} strings={strings} onChange={props.onInfantsChange}/>} />
    </div>
    {includeHotel ? <div className="mt-5 overflow-hidden rounded-[11px] border border-slate-200 bg-white">
      <Row icon={<BedDouble aria-hidden="true" className="h-[22px] w-[22px]"/>} label={strings.rooms} description={strings.roomDescription} counter={<Counter neutral={false} value={rooms} minimum={1} maximum={6} label={strings.rooms} strings={strings} onChange={props.onRoomsChange}/>} />
