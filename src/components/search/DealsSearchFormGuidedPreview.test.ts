...
test("guided preview is query gated to the landing variant only", () => {
  ...
  assert.doesNotMatch(form, /variant === "results"[\s\S]{0,200}guidedPreviewEnabled/);
  assert.doesNotMatch(form, /guidedPreviewEnabled[\s\S]{0,200}variant === "results"/);
});
...
test("normal submit remains on legacy Deals results and does not branch on preview", () => {
  const submit = form.match(/const submit = \(event: FormEvent\) => \{[\s\S]*?\n  \};/)?.[0] ?? "";
  ...
  assert.doesNotMatch(submit, /buildDealsJourneyUrl|getFirstDealsJourneyStage|guidedPreview/);
});

test("preview action uses shared validation and canonical guided route helpers", () => {
  const preview = form.match(/const previewGuidedJourney = \(\) => \{[\s\S]*?\n  \};/)?.[0] ?? "";
...
test("preview panel and action keep accessibility contracts", () => {
  assert.match(form, /<section[\s\S]*aria-labelledby="deals-guided-preview-title"/);
  assert.match(form, /aria-describedby="deals-guided-preview-description"/);
  assert.match(form, /aria-label=\{t\("deals\.guidedPreview\.accessibleName"\)\}/);
  assert.equal((form.match(/<h1/g) ?? []).length, 0);
  assert.match(form, /<button[\s\S]*type="button"[\s\S]*disabled=\{submitting \|\| pending\}/);
