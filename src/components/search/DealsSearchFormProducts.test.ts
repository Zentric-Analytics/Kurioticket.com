...
test("blocked toggles announce the minimum and successful hiding closes pickers without clearing values", () => {
  ...
  assert.doesNotMatch(form, /setCarDatesOpen\(false\)/);
  assert.doesNotMatch(
    form,
...
test("product selector keeps pressed blue pills without selected check marks", () => {
  ...
  assert.match(
    selector,
    /border-\[#004BB8\] bg-blue-50 text-\[#004BB8\] shadow-sm/,
  );
  assert.match(
    selector,
  ...
  assert.doesNotMatch(
    selector,
    /(?:selected|aria-pressed=\{selected\})[\s\S]{0,160}<(?:Circle|Badge|Dot)\b/,
  );
  assert.doesNotMatch(
    form,
    /import \{[\s\S]*\bCheck,?[\s\S]*\} from "lucide-react"/,
  );
});
...
test("selector behavior contracts keep the existing toggle flow and consumers", () => {
  ...
  assert.match(
    form,
    /if \(!result\.changed\) \{[\s\S]*setProductSelectionMessage\(t\("deals\.productSelector\.minimumTwo"\)\)/,
  );
  ...
  assert.match(
    modify,
    /<DealsSearchForm initialSearch=\{search\} variant="results"/,
  );
  assert.match(
    form,
