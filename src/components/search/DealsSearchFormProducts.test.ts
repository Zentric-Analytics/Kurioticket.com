...
test("one shared selector renders three pressed product buttons rather than package radios", () => { assert.match(form, /dealsProductOrder\.map/); assert.match(form, /data-deals-product=\{product\}/); assert.match(form, /type="button" aria-pressed=\{selected\}/); assert.match(form, /hotel: \{ label: "hotels"/); assert.match(form, /flight: \{ label: "flights"/); assert.match(form, /car: \{ label: "cars"/); assert.doesNotMatch(form, /type="radio" name="packageMode"/); assert.match(modify, /<DealsSearchForm/); });
