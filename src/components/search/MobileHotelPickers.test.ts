...
test("shared Hotel guests picker preserves approved sections and limits", () => {
  for (const token of [
    ...
    assert.match(guests, new RegExp(token));
...
test("shared Hotel guests picker offers compact density without changing its default", () => {
  ...
  for (const compactToken of [
    ...
    assert.ok(
      guests.includes(compactToken),
      `compact density should include ${compactToken}`,
    );
  }
...
test("compact Hotel guest visuals use blue active accents and neutral disabled controls", () => {
  ...
  assert.match(
    guests,
    /disabled \? "border-slate-200 text-slate-300" : "border-\[#075EE8\] text-\[#075EE8\]"/,
  );
...
test("compact Hotel guest density is scoped to approved mobile integrations", () => {
  ...
  assert.match(
    standalone,
    /density=\{mobileLandingPresentation \|\| mobileResultsSheet \? "compact" : undefined\}/,
  );
  assert.doesNotMatch(standalone, /density="compact"/);
});

test("Hotel results edit flow uses compact guest spacing and separate rounded field cards", () => {
  assert.match(
    standalone,
...
test("both mobile Hotel picker shells use the dedicated Guests & Rooms title", () => {
  for (const source of [home, standalone]) {
    assert.match(
      source,
      /title=\{(?:translate|t)\("hotelGuestsRooms\.mobileTitle"\)\}/,
    );
