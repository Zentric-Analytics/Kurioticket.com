...
export function VerifyLoginForm({
  ...
  async function resendCode() {
    ...
    if (!email || !resendPassword) {
      setError(t.verifyLoginEnterPassword);
      return;
    }
  ...
  return (
    <Card className="mx-auto w-full max-w-md p-5">
      <h1 className="text-2xl font-bold text-navy">{t.verifyLoginPageTitle}</h1>

      <p className="mt-2 text-sm text-muted">{t.verifyLoginSubtitle}</p>
      ...
      <form action={submit} className="mt-5 grid gap-4">
        ...
        <Button disabled={busy || code.length !== 6}>
          {busy ? t.verifyLoginVerifying : t.verifyLoginSubmit}
        </Button>
      </form>
      ...
      <div className="mt-4 grid gap-3 rounded-md border border-teal/20 p-3">
        ...
        <Field label={t.verifyLoginPasswordForResend}>
          <Input
            type="password"
        ...
        >
          {resending
            ...
            : cooldownSeconds > 0
              ? formatTranslation(t.verifyLoginResendIn, {
