...
export function ResetPasswordForm({ token = "" }: { token?: string }) {
  ...
  if (!hasToken) {
    return (
      <Card className="mx-auto w-full max-w-md p-5">
        <h1 className="text-2xl font-bold text-navy">{t.resetPasswordPageTitle}</h1>
        <p className="mt-4 text-sm text-muted">{invalidLinkMessage}</p>
        <p className="mt-4 text-sm text-muted">
          <Link className="font-semibold text-teal-dark" href="/auth/forgot-password">{t.resetPasswordRequestNewLink}</Link>
        </p>
      </Card>
    );
  }
  ...
  return (
    <Card className="mx-auto w-full max-w-md p-5">
      <h1 className="text-2xl font-bold text-navy">{t.resetPasswordNewPasswordTitle}</h1>
      <p className="mt-2 text-sm text-muted">{t.resetPasswordNewPasswordSubtitle}</p>
      ...
      <form action={submit} className="mt-5 grid gap-4">
        ...
        <Button disabled={loading || isPending}>{loading || isPending ? t.resetPasswordSubmitting : t.resetPasswordSubmit}</Button>
      </form>

      <p className="mt-4 text-sm text-muted">{t.resetPasswordRememberPrompt} <Link className="font-semibold text-teal-dark" href="/auth/signin">{t.resetPasswordLoginLink}</Link></p>
    </Card>
  );
}
