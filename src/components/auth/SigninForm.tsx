    ...
    <section className="mx-auto grid w-full max-w-6xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_30px_80px_rgba(15,23,42,0.16)] lg:grid-cols-[1.1fr_1fr]">
      <aside className="relative hidden min-h-[620px] lg:block">
        ...
        <div className="absolute inset-x-0 bottom-0 p-10 text-white">
          <p className="text-xs uppercase tracking-[0.22em] text-cyan-200/90">CurioTicket</p>
          <p className="mt-3 max-w-md text-3xl font-semibold leading-tight">
            Unlock curated fares, stays, and experiences designed for your next escape.
          </p>
        </div>
      </aside>
      ...
      <div className="flex min-h-[620px] items-center justify-center bg-gradient-to-b from-slate-50 to-white p-4 sm:p-8 lg:p-12">
        <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white/95 p-6 shadow-lg backdrop-blur sm:p-8">
          ...
          <p className="mt-2 text-sm text-slate-600">Continue planning your next journey.</p>

          <form action={submit} className="mt-8 grid gap-5" aria-live="polite">
            <Field label="Email">
              <Input
                name="email"
                type="email"
            ...
            <Field label="Password">
              <div className="relative">
                ...
                <button
                  ...
                  className="focus-ring absolute right-2 top-1/2 -translate-y-1/2 rounded-md px-3 py-1 text-xs font-semibold text-slate-600 hover:bg-slate-100 hover:text-slate-800"
          ...
          <p className="mt-6 text-sm text-slate-600">
            New to Curioticket?{" "}
            <Link className="font-semibold text-cyan-700 hover:text-cyan-800" href="/auth/signup">
              Create an account
            </Link>
          </p>
        </div>
