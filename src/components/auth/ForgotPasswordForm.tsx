...
export function ForgotPasswordForm() {
  ...
  async function submit(formData: FormData) {
    ...
    startTransition(() => {
      window.location.href = `/auth/reset-password?email=${encodeURIComponent(normalizedEmail)}`;
    });
  }
  ...
  return (
    <Card className="mx-auto w-full max-w-md p-5">
      ...
      <form action={submit} className="mt-5 grid gap-4">
        <Field label="Email">
          <Input name="email" type="email" autoComplete="email" required disabled={loading || isPending} placeholder="you@example.com" />
        </Field>
        ...
        <Button disabled={loading || isPending}>{loading || isPending ? "Sending..." : "Send reset code"}</Button>
      </form>
