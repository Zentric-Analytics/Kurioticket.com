...
export function OnboardingSecurityForm() {
  ...
  async function setupPasskey() {
    ...
    try {
      ...
      setStatus("Passkey added. Continuing to the homepage…");
  ...
  return <Card className="mx-auto w-full max-w-xl p-6">
    ...
    <p className="mt-3 text-sm leading-6 text-muted">Passkey setup is being finalized so verification continues directly into your device security prompt.</p>
    <p className="mt-3 rounded-xl bg-slate-50 p-3 text-sm text-slate-700">You can skip this optional step and use password or Google sign-in for now. We’ll make passkey setup available again from Security Settings when it is ready.</p>
    {status ? <p className="mt-4 text-sm font-semibold text-teal-dark" aria-live="polite">{status}</p> : null}
    <div className="mt-6 flex flex-col gap-3 sm:flex-row">
      <Button type="button" onClick={setupPasskey} disabled>{busy ? "Setting up…" : "Passkeys coming soon"}</Button>
      <Button type="button" variant="secondary" onClick={() => { window.location.href = "/"; }} disabled={busy}>Skip for now</Button>
    </div>
  </Card>;
}
