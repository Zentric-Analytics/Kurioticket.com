    ...
    <article className="relative w-full overflow-hidden rounded-2xl border border-[#D8E1EC] bg-white shadow-[0_12px_30px_-24px_rgba(15,23,42,0.55)] transition duration-200 hover:-translate-y-0.5 hover:border-[#CBD6E2] hover:shadow-[0_18px_38px_-26px_rgba(15,23,42,0.42)]">
      <div className="grid md:grid-cols-[250px_minmax(0,1fr)] lg:grid-cols-[250px_minmax(0,1fr)_205px] xl:grid-cols-[270px_minmax(0,1fr)_205px]">
        ...
        <div className="flex min-w-0 flex-col px-4 py-3">
          ...
          <ul className="mt-2 flex flex-wrap gap-x-3 gap-y-1.5 text-sm font-medium text-slate-600">
        ...
        <div className="col-span-full flex min-w-0 flex-col border-t border-[#E2E8F0] bg-slate-50/45 px-4 py-3 lg:col-span-1 lg:border-s lg:border-t-0 lg:bg-white lg:text-end">
          <div className="space-y-2">
            ...
            <div><p className="text-xs font-medium text-slate-500">Total</p><p className="whitespace-nowrap text-lg font-bold leading-7 tracking-[-0.01em] text-[#102A43] tabular-nums lg:text-xl" dir="ltr" title={totalDisplayPrice.title} aria-label={totalDisplayPrice.ariaLabel}>{totalDisplayPrice.formatted}</p>{offer.taxesAndFeesIncluded && <p className="mt-1 text-xs text-slate-500">Taxes and fees included</p>}</div>
          </div>
          <Link href={detailsHref} className="mt-3 inline-flex h-10 w-full items-center justify-center rounded-lg bg-[#004BB8] px-5 text-sm font-bold text-white transition hover:bg-[#021C2B] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#004BB8]/40 focus-visible:ring-offset-2 lg:mt-auto">View car</Link>
        </div>
      </div>
    </article>
  );
