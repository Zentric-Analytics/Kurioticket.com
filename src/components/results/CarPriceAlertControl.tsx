...
export function CarPriceAlertControl({ search, results }: { search: CarSearchParams; results: NormalizedCarResult[] }) {
  ...
  return <section aria-label={t("travel.account.carAlert.title")} className="rounded-xl border border-blue-100 bg-white p-3 shadow-sm sm:rounded-2xl sm:p-4">
    <div className="flex flex-wrap items-center justify-between gap-3 min-[360px]:flex-nowrap"><div className="flex min-w-0 items-center gap-2.5"><span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-blue-50 text-[#004BB8]"><Bell className="h-4 w-4" aria-hidden="true" /></span><div className="min-w-0"><h2 className="whitespace-nowrap text-sm font-bold text-slate-950 sm:text-base">{t("travel.account.carAlert.title")}</h2><p className="mt-1 hidden text-sm text-slate-600 sm:block">{t("travel.account.carAlert.body")}</p></div></div><button type="button" className="min-h-9 shrink-0 rounded-lg border border-[#004BB8]/20 bg-blue-50 px-3 text-sm font-bold text-[#004BB8] transition hover:border-[#004BB8]/35 hover:bg-blue-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#004BB8]/30 max-[359px]:ms-auto sm:min-h-11 sm:rounded-xl sm:border-transparent sm:bg-[#004BB8] sm:px-4 sm:text-white sm:hover:bg-[#003f9c]" onClick={() => { setStatus("idle"); setOpen(true); }}>{t("travel.account.carAlert.create")}</button></div>
