...
export function CarPriceAlertControl({ search, results }: { search: CarSearchParams; results: NormalizedCarResult[] }) {
  ...
  return <>
    ...
    {feedback ? <div role={feedback.startsWith("error") ? "alert" : "status"} aria-live="polite" className={`fixed bottom-[calc(max(env(safe-area-inset-bottom),12px)+12px)] left-4 right-4 z-[110] mx-auto flex max-w-md items-center gap-2.5 rounded-[14px] border-[0.5px] border-[#C8DFF7] bg-[#EDF6FF] px-[13px] py-[11px] shadow-[0_4px_16px_rgba(15,23,42,0.16)] ${snackbarLeaving ? "cars-price-alert-snackbar-leaving" : "cars-price-alert-snackbar-entering"}`}>
      <CheckCircle2 className={`h-5 w-5 shrink-0 ${feedback.startsWith("error") ? "text-rose-600" : "text-[#1769AA]"}`} aria-hidden="true" />
      ...
      {feedback === "active" ? <Link href="/price-alerts" className="inline-flex min-h-11 shrink-0 items-center px-1 text-[13.5px] font-bold text-[#1769AA] focus-visible:rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1769AA]/35">{t("carsResults.priceTracking.manage")}</Link> : null}
    </div> : null}
  </>;
}
