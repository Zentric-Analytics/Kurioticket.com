...
export function CarPriceAlertControl({ search, results }: { search: CarSearchParams; results: NormalizedCarResult[] }) {
  ...
  return <>
    ...
    {feedback ? <div role={feedback.startsWith("error") ? "alert" : "status"} aria-live="polite" className={`fixed bottom-[calc(max(env(safe-area-inset-bottom),12px)+12px)] left-4 right-4 z-[110] mx-auto flex max-w-md items-center gap-2.5 rounded-[14px] border-[0.5px] border-slate-300/80 bg-white px-[13px] py-[11px] shadow-[0_4px_16px_rgba(15,23,42,0.16)] ${snackbarLeaving ? "cars-price-alert-snackbar-leaving" : "cars-price-alert-snackbar-entering"}`}>
      <CheckCircle2 className={`h-5 w-5 shrink-0 ${feedback.startsWith("error") ? "text-rose-600" : "text-[#004BB8]"}`} aria-hidden="true" />
      <span className="flex min-w-0 flex-1 flex-col gap-px"><strong className="block text-[14px] font-bold leading-[19px] text-slate-950">{t(feedback === "active" ? "carsResults.priceTracking.active" : feedback === "paused" ? "carsResults.priceTracking.paused" : feedback === "error-start" ? "carsResults.priceTracking.errorStart" : "carsResults.priceTracking.errorPause")}</strong>{feedback === "active" ? <span className="block text-[12.5px] leading-[17px] text-slate-600">{t("carsResults.priceTracking.activeBody")}</span> : null}</span>
      {feedback === "active" ? <Link href="/price-alerts" className="inline-flex min-h-11 shrink-0 items-center px-1 text-[13.5px] font-bold text-[#004BB8] focus-visible:rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#004BB8]/35">{t("carsResults.priceTracking.manage")}</Link> : null}
    </div> : null}
  </>;
}
