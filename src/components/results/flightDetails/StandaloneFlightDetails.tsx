      ...
      <div className="mx-auto w-full max-w-[1470px] px-0 sm:px-6 lg:px-[34px]">
        ...
        <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,2.45fr)_minmax(310px,0.95fr)] lg:gap-7">
          <section className="min-w-0 border-y border-[#E2E8F0] bg-white p-4 sm:rounded-[13px] sm:border sm:p-6 sm:shadow-[0_3px_15px_rgba(15,23,42,0.045)] lg:p-6" aria-labelledby="flight-details-heading">
            ...
            <div role="radiogroup" aria-label="Available fares" className={`${fareChoices.length > 1 ? "flex snap-x snap-mandatory gap-3 overflow-x-auto pb-2 sm:grid sm:snap-none sm:overflow-visible sm:pb-0" : "grid gap-3"} ${fareChoices.length === 1 ? "lg:max-w-[336px]" : fareChoices.length === 2 ? "sm:grid-cols-2 lg:max-w-[684px]" : fareChoices.length === 3 ? "sm:grid-cols-2 md:grid-cols-3 lg:max-w-[1020px]" : "sm:grid-cols-2 xl:grid-cols-4"}`}>
              {fareChoices.map((fare, index) => {
                ...
                return <button key={fare.key} ref={(element) => { fareButtonRefs.current[index] = element; }} type="button" role="radio" aria-checked={selected} tabIndex={selected ? 0 : -1} onClick={() => selectFare(index)} onKeyDown={(event) => handleFareKeyDown(event, index)} className={`rounded-[10px] border p-3 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#075EE8]/40 ${fareChoices.length > 1 ? "w-[min(82vw,310px)] shrink-0 snap-start sm:w-auto sm:shrink" : "w-full"} ${selected ? "border-[1.5px] border-[#075EE8] bg-[#075EE8]/[0.02]" : "border-[#E2E8F0] bg-white hover:border-slate-300"}`}>
                  <div className="flex items-start gap-2.5"><span className={`mt-1 flex h-[18px] w-[18px] shrink-0 items-center justify-center rounded-full border-2 lg:h-4 lg:w-4 ${selected ? "border-[#075EE8]" : "border-slate-300"}`} aria-hidden="true"><span className={`h-2 w-2 rounded-full lg:h-1.5 lg:w-1.5 ${selected ? "bg-[#075EE8]" : "bg-transparent"}`} /></span><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-blue-50 text-[#075EE8]"><Luggage className="h-4 w-4" aria-hidden="true" /></span><div className="min-w-0"><p className="text-[13px] font-semibold text-slate-950">{fare.label}</p><p className="text-[20px] font-bold leading-5 text-[#075EE8] lg:text-[19px]" aria-label={price.ariaLabel}>{price.formatted}</p></div></div>
                  {compactTerms.length ? <ul className="mt-2 space-y-1 pl-6">{compactTerms.map(({ term, text }, termIndex) => <FareTerm key={`${term.category}-${term.legDirection || "trip"}-${term.text}-${termIndex}`} term={term} text={text} compact />)}</ul> : null}
                </button>;
              })}
            </div>

            <div className="mt-5" role="tablist" aria-label="Fare information">{fareTabs.map((tab, index) => <button key={tab.id} ref={(element) => { tabRefs.current[index] = element; }} id={`fare-tab-${tab.id}`} type="button" role="tab" aria-selected={activeTab === tab.id} aria-controls={`fare-panel-${tab.id}`} tabIndex={activeTab === tab.id ? 0 : -1} onClick={() => setActiveTab(tab.id)} onKeyDown={(event) => handleTabKeyDown(event, index)} className={`min-h-11 border-b-2 px-3 text-xs font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#075EE8]/35 sm:px-4 sm:text-sm ${activeTab === tab.id ? "border-[#075EE8] text-[#075EE8]" : "border-transparent text-slate-700 hover:text-slate-950"}`}>{tab.label}</button>)}</div>
...
function FareTerm({ term, text = term.text, compact = false }: { term: FlightDetailsFareChoice["distinguishingTerms"][number]; text?: string; compact?: boolean }) {
  ...
  return <li className={`flex items-start text-slate-700 ${compact ? "gap-1.5 text-[12px] leading-4 lg:text-[11px]" : "gap-2 text-[13px] leading-5"}`}><span className={`mt-0.5 flex shrink-0 items-center justify-center rounded-full border ${compact ? "h-4 w-4 lg:h-3.5 lg:w-3.5" : "h-4 w-4"} ${iconClass}`}><Icon className={compact ? "h-2.5 w-2.5 lg:h-2 lg:w-2" : "h-2.5 w-2.5"} aria-hidden="true" /></span>{text}</li>;
}
