          ...
          <section className="min-w-0 border-y border-[#E2E8F0] bg-white p-4 sm:rounded-[13px] sm:border sm:p-6 sm:shadow-[0_3px_15px_rgba(15,23,42,0.045)] lg:p-6" aria-labelledby="flight-details-heading">
            ...
            <div role="radiogroup" aria-label="Available fares" className={`min-w-0 ${fareChoices.length > 1 ? "flex snap-x snap-mandatory gap-3 overflow-x-auto pb-2 sm:grid sm:snap-none sm:overflow-visible sm:pb-0" : "grid gap-3"} ${fareChoices.length === 1 ? "max-w-[270px]" : fareChoices.length === 2 ? "sm:grid-cols-2 lg:max-w-[632px]" : fareChoices.length === 3 ? "sm:grid-cols-2 md:grid-cols-3 lg:max-w-[954px]" : "sm:grid-cols-2 xl:max-w-[1276px] xl:grid-cols-4"}`}>
              {fareChoices.map((fare, index) => {
                ...
                return <button key={fare.key} ref={(element) => { fareButtonRefs.current[index] = element; }} type="button" role="radio" aria-checked={selected} tabIndex={selected ? 0 : -1} onClick={() => selectFare(index)} onKeyDown={(event) => handleFareKeyDown(event, index)} className={`min-w-0 max-w-[310px] rounded-[10px] border p-3 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#075EE8]/40 ${fareChoices.length > 1 ? "w-[min(82vw,310px)] shrink-0 snap-start sm:w-full sm:shrink" : "w-[min(100%,270px)] max-w-[270px]"} ${selected ? "border-[1.5px] border-[#075EE8] bg-[#075EE8]/[0.02]" : "border-[#E2E8F0] bg-white hover:border-slate-300"}`}>
            ...
            <div className="mt-5 flex min-w-0 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:overflow-x-visible" role="tablist" aria-label="Fare information">{fareTabs.map((tab, index) => <button key={tab.id} ref={(element) => { tabRefs.current[index] = element; }} id={`fare-tab-${tab.id}`} type="button" role="tab" aria-selected={activeTab === tab.id} aria-controls={`fare-panel-${tab.id}`} tabIndex={activeTab === tab.id ? 0 : -1} onClick={() => setActiveTab(tab.id)} onKeyDown={(event) => handleTabKeyDown(event, index)} className={`min-h-11 shrink-0 whitespace-nowrap border-b-2 px-3 text-center text-[11px] font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#075EE8]/35 sm:px-4 sm:text-sm ${activeTab === tab.id ? "border-[#075EE8] text-[#075EE8]" : "border-transparent text-slate-700 hover:text-slate-950"}`}>{tab.label}</button>)}</div>
            <FarePanel activeTab={activeTab} fare={selectedFare} offer={selectedOffer} locale={locale} formatDealPrice={(deal) => formatDisplayPrice({ amount: deal.price, sourceCurrency: deal.currency, displayCurrency: selectedOption.currency, convertUsdEstimate: true, rates: currencyRates.rates, isFallbackRate: currencyRates.isFallback })} redirecting={redirecting} onViewDeal={continueToOffer} />
...
function FarePanel({ activeTab, fare, offer, locale, formatDealPrice, redirecting, onViewDeal }: { activeTab: FareTab; fare: FlightDetailsFareChoice | undefined; offer: FlightDetailsOffer; locale: string; formatDealPrice: (deal: FlightDetailsDeal) => ReturnType<typeof formatDisplayPrice>; redirecting: boolean; onViewDeal: (offerId: string) => void }) {
  ...
  if (activeTab === "deals") return <CompareDealsPanel deals={fare?.deals ?? []} formatPrice={formatDealPrice} redirecting={redirecting} onViewDeal={onViewDeal} />;
...
function CompareDealsPanel({ deals, formatPrice, redirecting, onViewDeal }: { deals: FlightDetailsDeal[]; formatPrice: (deal: FlightDetailsDeal) => ReturnType<typeof formatDisplayPrice>; redirecting: boolean; onViewDeal: (offerId: string) => void }) {
  return <section id="fare-panel-deals" role="tabpanel" aria-labelledby="fare-tab-deals" className="rounded-[10px] border border-[#E2E8F0] p-4 sm:p-5">
    ...
    {deals.length ? <><p className="mt-4 text-xs font-semibold text-slate-600">{deals.length} {deals.length === 1 ? "deal" : "deals"} available</p><ul className="mt-2 space-y-2">{deals.map((deal) => { const price = formatPrice(deal); return <li key={deal.key} className="flex min-h-11 min-w-0 items-center justify-between gap-3 rounded-[10px] border border-[#E2E8F0] bg-white px-3 py-2.5"><p className="min-w-0 break-words text-sm font-semibold text-slate-900">{deal.providerName}</p><div className="flex shrink-0 items-center gap-2"><p className="text-sm font-bold text-[#075EE8]" aria-label={price.ariaLabel}>{price.formatted}</p><button type="button" disabled={redirecting} onClick={() => onViewDeal(deal.offerId)} className="inline-flex min-h-11 items-center justify-center rounded-lg px-2 text-sm font-semibold text-[#075EE8] hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#075EE8]/35 disabled:opacity-50">View deal</button></div></li>; })}</ul></> : <p className="mt-4 text-sm text-slate-600">No live booking deals are available for this fare right now.</p>}
  </section>;
}
