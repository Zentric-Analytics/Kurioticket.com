  ...
  return (
    <section data-mobile-native-fare-information-deck className="mt-3 sm:hidden">
      <div className="-mx-2 overflow-x-auto border-b border-[#D8E1EC] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <div role="tablist" aria-label="Fare information" className="flex w-max min-w-full gap-[22px] px-2">
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`relative min-h-12 shrink-0 whitespace-nowrap px-0 text-[14px] leading-5 text-[#536B92] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#075EE8]/35 ${
                  selected ? "font-extrabold text-[#1A1A1A]" : "font-semibold"
                }`}
              >
                {tab.label}
                {selected ? <span className="absolute -bottom-px left-0 right-0 h-[3px] rounded-full bg-[#075EE8]" aria-hidden="true" /> : null}
              </button>
            );
          })}
        </div>
