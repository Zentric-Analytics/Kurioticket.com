...
import { buildDealsJourneyUrl, buildLegacyDealsResultsUrl, getFirstDealsJourneyStage, getNextDealsJourneyStage, getPreviousDealsJourneyStage, getRequiredDealsJourneyStageAt, type DealsJourneyStage } from "@/lib/deals/dealsJourneyRoutes";
...
export function DealsJourneyShell({ stage, search, invalid, hotelId, flightId, carId }: { stage: DealsJourneyStage; search: DealsSearch; invalid: boolean; hotelId: string | null; flightId: string | null; carId: string | null }) {
  ...
  const restartCurrentPreview = () => {
    removeDealsStagedJourneyPlan(); setPlanState(unresolvedDealsPlanState()); setPlanStatus("loading");
  ...
  const confirm = (product: DealsTripPlanProduct, selection: DealsTripPlanHotel | DealsTripPlanFlight | DealsTripPlanCar) => {
    ...
    if (!result.ok) {
      ...
      if (failure === "storage-read-unavailable") { setPlanStatus("confirmation-read-failure"); setError(t("deals.guided.confirmation.readError")); }
      else if (failure === "persistence-failed") { setPlanStatus("confirmation-persistence-failure"); setError(t("deals.guided.confirmation.saveError")); }
...
function GuidedSafeState({ title, body }: { title: string; body: string }) { return <div role="status" className="mt-6 rounded-2xl border border-amber-300 bg-white p-6"><h2 className="text-xl font-bold">{title}</h2><p className="mt-2 leading-7 text-slate-600">{body}</p></div>; }
