...
export function DealsJourneyShell({ stage, search, invalid, hotelId, flightId }: { stage: DealsJourneyStage; search: DealsSearch; invalid: boolean; hotelId: string | null; flightId: string | null }) {
  ...
  const requiredStage = resolved ? getRequiredDealsJourneyStage(stage, search.mode, plan, hotelId, flightId) : stage;
  useEffect(() => {
  ...
  const backHref = stage === "flight-details" ? buildDealsJourneyUrl("flight-results", search) : previous ? buildDealsJourneyUrl(previous, search) : buildLegacyDealsResultsUrl(search);
  ...
  return <main className="flex-1 overflow-x-clip bg-[#f6f8fb] pb-12">
    ...
    <div className="page-shell max-w-5xl pt-8 sm:pt-10">
      ...
      <section className="mt-7 min-w-0">
        ...
        {!resolved ? <div role="status" className="mt-6 min-h-36 animate-pulse rounded-2xl border border-slate-200 bg-white" aria-label={t("deals.guided.loading")} /> : requiredStage === stage && stage === "hotel-results" ? <DealsHotelResultsStage search={search} /> : requiredStage === stage && stage === "hotel-details" ? <DealsHotelDetailsStage search={search} hotelId={hotelId} plan={plan} confirming={confirmingHotel} confirmationError={confirmationError} onConfirm={confirmGuidedHotelSelection} /> : requiredStage === stage && stage === "flight-results" ? <DealsFlightResultsStage search={search} /> : requiredStage === stage && stage === "flight-details" ? <div data-deals-guided-flight-details-pending className="mt-6 rounded-2xl border border-blue-200 bg-white p-5 shadow-sm sm:p-8"><h2 className="text-xl font-extrabold text-slate-950">{t("deals.guided.flightDetails.pendingTitle")}</h2><p className="mt-2 max-w-2xl leading-7 text-slate-600">{t("deals.guided.flightDetails.pendingBody")}</p></div> : requiredStage === stage && stage === firstStage ? <div data-deals-guided-journey-foundation className="mt-6 rounded-2xl border border-blue-200 bg-white p-5 shadow-sm sm:p-8"><p className="text-lg font-extrabold text-slate-950">{t("deals.guided.foundationTitle")}</p><p className="mt-2 max-w-2xl leading-7 text-slate-600">{t("deals.guided.foundationBody")}</p></div> : null}
      </section>
    </div>
