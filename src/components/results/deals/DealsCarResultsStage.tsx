...
export function DealsCarResultsStage({ search }: { search: DealsSearch }) {
  ...
  return <CarsResultsExperience results={results} inventoryStatus="available" hasSearchContext resultHeadingId="guided-car-results-heading" embedded detailsHrefForCar={(car) => buildGuidedDealsCarActionHref(search, car.id)} actionLabel={t("deals.guided.carResults.actionLabel")} resultHeadingRef={resultsHeadingRef} actionAriaLabelForCar={(car) => t("deals.guided.carResults.actionAriaLabel").replace("{model}", car.modelName).replace("{company}", car.rentalCompanyName)} />;
}
