...
import { buildDealsCarDetailsSelection } from "@/lib/deals/dealsCarDetails";
import type { TravelSearchResponse } from "@/lib/travel/searchContract";
import type { DealsSearch } from "@/lib/deals/dealsSearchParams";
import {
  buildDealsCarRequestIdentity,
  buildDealsCarRequestPayload,
} from "@/lib/deals/dealsCarResults";
...
function isNormalizedCarResult(value: unknown): value is NormalizedCarResult {
  ...
  return (
    ...
    typeof car.modelName === "string" &&
    typeof car.pickupLocation === "string" &&
    Array.isArray(car.offers)
  );
}

export function DealsCarResultsStage({
  search,
  onSelectCar,
}: {
  search: DealsSearch;
  onSelectCar: (car: DealsTripPlanCar) => void;
}) {
  ...
  const requestIdentity = useMemo(
    () => buildDealsCarRequestIdentity(search),
    [search],
  );
  const payloadJson = useMemo(
    () => JSON.stringify(buildDealsCarRequestPayload(search)),
    [search],
  );
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      if (controller.signal.aborted || latestRequestRef.current !== lifecycleId)
        return;
      setState("loading");
      setResults([]);
    ...
    fetch("/api/cars/search", {
      ...
      headers: {
        ...
        "x-search-request-id": `guided-car-${retryGeneration}`,
      },
      body: payloadJson,
      ...
      .then(async (response) => {
        ...
        try {
          data =
            (await response.json()) as TravelSearchResponse<NormalizedCarResult>;
        } catch {
          data = null;
        }
        if (
  ...
  if (state === "loading")
    return (
      <div
        ...
        data-deals-guided-car-results-loading
        aria-label={t("deals.guided.carResults.loading")}
      >
        <p className="text-lg font-bold text-slate-950">
          {t("deals.guided.carResults.loading")}
        </p>
        {[0, 1, 2].map((item) => (
          <div
            key={item}
  ...
  if (state === "error")
    return (
      <div
        data-deals-guided-car-results-error
        className="mt-6 rounded-2xl border border-red-200 bg-white p-5 shadow-sm sm:p-8"
      >
        <h2
          ref={errorHeadingRef}
          tabIndex={-1}
          className="text-xl font-extrabold text-slate-950 outline-none"
        >
          {t("deals.guided.carResults.errorTitle")}
        </h2>
        <p className="mt-2 max-w-2xl leading-7 text-slate-600">
          {t("deals.guided.carResults.errorBody")}
        </p>
        <Button type="button" className="mt-4 min-h-11" onClick={retry}>
          {t("deals.guided.carResults.retry")}
        </Button>
  ...
  if (state === "empty")
    return (
      <div
        data-deals-guided-car-results-empty
      ...
      >
        <h2
          ...
          className="text-xl font-extrabold text-slate-950 outline-none"
        >
          {t("deals.guided.carResults.emptyTitle")}
        </h2>
        <p className="mt-2 max-w-2xl leading-7 text-slate-600">
          {t("deals.guided.carResults.emptyBody")}
        </p>
        <Button type="button" className="mt-4 min-h-11" onClick={retry}>
          {t("deals.guided.carResults.retry")}
        </Button>
  ...
  const buildSelection = (car: NormalizedCarResult) =>
    resultReceivedAt === null
      ? null
      : buildDealsCarDetailsSelection({
          car,
          primaryOffer: getPrimaryCarOffer(car),
  ...
  return (
    <CarsResultsExperience
      ...
      embedded
      presentation="guided-planning"
      detailsHrefForCar={() => null}
      isCarSelectable={(car) => buildSelection(car) !== null}
      onSelectCar={(car) => {
        const selection = buildSelection(car);
        if (selection) onSelectCar(selection);
      }}
      actionLabel={t("deals.guided.carResults.actionLabel")}
      resultHeadingRef={resultsHeadingRef}
      actionAriaLabelForCar={(car) =>
        t("deals.guided.carResults.actionAriaLabel").replace(
          "{model}",
          `${car.modelName}${car.orSimilar ? ` ${t("deals.guided.carResults.orSimilar")}` : ""}`,
        )
      }
    />
  );
