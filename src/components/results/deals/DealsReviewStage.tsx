...
import { isDealsTripPlanExpired, type DealsTripPlan } from "@/lib/deals/dealsTripPlan";
...
export function DealsReviewStage({ plan, search, now }: { plan: DealsTripPlan; search: DealsSearch; now: number }) {
  ...
  const planExpired = isDealsTripPlanExpired(plan, now);
  const reviewState = planExpired ? "plan-expired" : status.canContinue ? "ready" : "product-expired";
  ...
  return <div data-deals-guided-review-stage data-deals-guided-review-state={reviewState} className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px] xl:items-start">
