...
test("staged requests suppress future products and Modify Search preserves activation", () => {
  assert.match(client, /included\[product\] && !\(stagedHotelJourneyActive && product !== "hotel"\)/);
  assert.match(client, /stagedHotelJourneyActive \? \{ \.\.\.search, journey: "staged" \} : search/);
  assert.match(client, /body: JSON\.stringify\(requestBody\)/);
  assert.match(client, /journey=staged/);
  assert.match(client, /stagedHotelJourneyActive \|\| loading \|\| failed/);
});
