...
export function DealsProductSection({ id, title, summary, icon, href, viewAll, supportingText, priceNotice, status, loadingLabel, emptyLabel, errorLabel, retryLabel, onRetry, notice, countLabel, children }: { id: string; title: string; summary: string; icon: ReactNode; href: string; viewAll: string; supportingText: string; priceNotice: string; status: string; loadingLabel: string; emptyLabel: string; errorLabel?: string; retryLabel: string; onRetry: () => void; notice?: string; countLabel?: string; children?: ReactNode }) {
  return <section aria-labelledby={id} aria-busy={status === "loading"} className="rounded-3xl border border-[#D8E1EC] bg-white p-5 shadow-sm sm:p-6">
    <div className="flex flex-col items-start justify-between gap-4 sm:flex-row"><div><h2 id={id} tabIndex={-1} className="flex items-center gap-2 text-xl font-extrabold text-slate-950">{icon}{title}</h2><p className="mt-1 text-sm text-slate-600">{summary}</p>{countLabel && <p className="mt-2 text-sm font-semibold text-slate-700">{countLabel}</p>}<p className="mt-1 text-xs text-slate-500">{supportingText}</p></div><Link href={href} className="inline-flex min-h-11 shrink-0 items-center rounded-xl bg-[#004BB8] px-5 py-2 text-center font-bold text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4">{viewAll}</Link></div>
    ...
    {status === "success" && <>{children}<p className="mt-4 text-xs text-slate-500">{priceNotice}</p></>}
  </section>;
}
