...
type Props = { modeLabel: string; opened: number; total: number; totalLabel: string | null; progressLabel: string; allOpened: boolean; hasExpired: boolean; nextId: string | null; resultsPath: string; t: (key: string) => string };

export function DealsHandoffSummary({ modeLabel, opened, total, totalLabel, progressLabel, allOpened, hasExpired, nextId, resultsPath, t }: Props) {
  ...
  return <aside aria-labelledby="trip-summary-title" className="order-1 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6 xl:order-2 xl:sticky xl:top-24">
    ...
    <Link href={resultsPath} className="mt-3 inline-flex min-h-11 items-center gap-2 font-bold text-[#004BB8] underline decoration-2 underline-offset-4 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2"><ArrowLeft aria-hidden className="size-4" />{t("deals.handoff.returnResults")}</Link>
  </aside>;
}
