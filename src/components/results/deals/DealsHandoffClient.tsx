...
export function DealsHandoffClient() {
  ...
  const open = (product: Exclude<DealsTripPlanProduct, "car">) => {
  ...
  const steps: Array<{ product: Exclude<DealsTripPlanProduct, "car">; title: string; summary: string; provider: string; id: string; expired: boolean; priceLabel: string; displayPrice: ReturnType<typeof formatDisplayPrice> | null }> = [];
  ...
  return <>
    <section aria-labelledby="deals-next-provider" className="mt-6 rounded-2xl border border-blue-200 bg-blue-50 p-5">
      ...
      {next.product && next.product !== "car" && next.href ? <a href={next.href} target="_blank" rel="noopener noreferrer" onClick={() => open(next.product as Exclude<DealsTripPlanProduct, "car">)} className="mt-3 inline-flex min-h-11 items-center rounded-xl bg-[#004BB8] px-5 font-bold text-white">{t(next.product === "flight" ? "deals.handoff.openFlight" : "deals.handoff.openStay")} <span className="sr-only">({t("deals.handoff.newTab")})</span></a> : <a href="#provider-steps" className="mt-3 inline-flex min-h-11 items-center rounded-xl border border-[#004BB8] px-5 font-bold text-[#004BB8]">{t("deals.handoff.reviewSteps")}</a>}
