...
export function DealsReviewItemCard({ item, price, t }: { item: DealsReviewItem; price: DisplayPrice | null; t: (key: string) => string }) {
  ...
  return <article aria-labelledby={`review-${item.product}-title`} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
    ...
    <dl className="mt-5 grid gap-3 sm:grid-cols-2">
      ...
      {item.details.map(detail => <div key={`${detail.label}-${detail.value}`}><dt className="text-xs font-bold uppercase tracking-wide text-slate-500">{detail.label}</dt><dd dir={detail.dir} className="mt-1 break-words font-semibold text-slate-800">{detail.value}</dd></div>)}
    </dl>
    ...
    <Link href={item.changeHref} className="focus-ring mt-5 inline-flex min-h-11 items-center rounded-xl border border-[#004BB8] px-4 py-2.5 font-bold text-[#004BB8]">{t(item.changeLabelKey)}<span className="sr-only">: {item.title}</span></Link>
  </article>;
}
