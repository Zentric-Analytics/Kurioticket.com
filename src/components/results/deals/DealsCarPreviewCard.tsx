...
export type DealsCarPreviewCardProps = { car: NormalizedCarResult; badgeKey: string; reasonKey?: string; locale: string; search: CarSearchParams; t: (key: string) => string; selected: boolean; onSelect: () => void };
...
export function DealsCarPreviewCard({ car, badgeKey, reasonKey, search, t, selected, onSelect }: DealsCarPreviewCardProps) {
  ...
  return <article className={`flex h-full min-w-0 flex-col overflow-hidden rounded-2xl border bg-white shadow-sm ${selected ? "border-[#004BB8] ring-2 ring-blue-100" : "border-[#D8E1EC]"}`}>
    ...
    {reasonKey && <p className="mt-1 text-xs text-slate-600">{t(reasonKey)}</p>}
    ...
    <div className="mt-auto pt-4"><p className="text-xs text-slate-600">{offer.rentalCompanyName || offer.bookingProviderName}</p><p className="text-2xl font-extrabold text-[#004BB8]" dir="ltr" aria-label={total.ariaLabel}>{total.formatted}</p><p className="text-xs text-slate-600">{interpolate(t("deals.results.car.perDayAndDays"), { price: daily.formatted, count: days })} · {t(offer.taxesAndFeesIncluded ? "deals.results.car.taxesIncluded" : "deals.results.car.taxesUnknown")}</p><p className="mt-2 text-xs text-slate-600">{t("deals.results.car.selectionDisclosure")}</p><button type="button" aria-pressed={selected} disabled={!selectable} aria-label={t(selected ? "deals.results.car.selectedAccessible" : selectable ? "deals.results.car.chooseAccessible" : "deals.results.car.unsafeSelection")} onClick={onSelect} className="mt-3 min-h-11 w-full rounded-xl bg-[#004BB8] px-4 font-bold text-white disabled:cursor-not-allowed disabled:bg-slate-300 disabled:text-slate-700">{t(selected ? "deals.results.car.selected" : "deals.results.car.choose")}</button></div></div>
  </article>;
}
