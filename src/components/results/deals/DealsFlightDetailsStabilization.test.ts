...
test("guided Flight details uses a stable loaded callback and request identity", async () => {
  ...
  assert.match(
    client,
    /onFlightLoadedRef\.current\?\.\(candidate, receivedAt\)/,
  );
...
test("guided Flight details clears loaded state, aborts ID changes, and rejects stale responses", async () => {
  ...
  assert.match(
    lifecycle,
    /setLoading\(true\); setDetailsError\(""\); setFlight\(null\); setResultReceivedAt\(null\); onFlightLoadedRef\.current\?\.\(null, null\)/,
  );
  ...
  assert.match(
    lifecycle,
    /if \(candidate\.id\.trim\(\) !== id\.trim\(\)\) throw new Error/,
  );
  assert.match(
    client,
...
test("guided Flight details Retry enters loading before focus completion and uses translated copy", async () => {
  const [client, en] = await Promise.all([
    readFile(clientUrl, "utf8"),
    readFile(enUrl, "utf8"),
  ]);
  const retry = client.slice(
    client.indexOf("const retry = () =>"),
    client.indexOf("const itineraryLegs"),
  );
  assert.match(
    retry,
    /retryFocusRef\.current = true; setLoading\(true\); setDetailsError\(""\); setFlight\(null\); setResultReceivedAt\(null\); onFlightLoadedRef\.current\?\.\(null, null\); setRetryToken/,
  );
  assert.match(
    client,
  ...
  assert.match(
    client,
    /t\["deals\.guided\.flightDetails\.retry"\] \|\| enTranslations\["deals\.guided\.flightDetails\.retry"\]/,
  );
  assert.match(
    en,
...
test("Flight details heading semantics remain valid in guided and standalone modes", async () => {
  ...
  assert.match(
    client,
    /<Card className="overflow-hidden[\s\S]*\{titleTag === "h1" \? <h1 ref=\{headingRef\}[\s\S]*: <h2 ref=\{headingRef\}/,
  );
  assert.match(
    client,
    /<div className="mt-5"><SelectedFlightSummary[\s\S]*headingLevel=\{standalone \? "h2" : "h3"\}[\s\S]*routeHeadingLevel=\{standalone \? "h2" : "h3"\}/,
  );
});

test("guided Flight details provider and confirmation regressions stay source-suppressed", async () => {
  const [client, stage, shell] = await Promise.all([
    readFile(clientUrl, "utf8"),
    readFile(stageUrl, "utf8"),
  ...
  assert.match(
    client,
    /\{standalone \? <div className="min-w-0 lg:col-start-2 lg:row-start-1"><ProviderComparisonPanel/,
  );
  assert.doesNotMatch(
    stage,
    /api\/redirect|writeDealsStagedJourneyPlan|replaceDealsFlightSelection/,
  );
  assert.match(shell, /confirmGuidedFlightSelection/);
  assert.match(
    shell,
    /attemptGuidedConfirmation\([\s\S]*write: writeDealsStagedJourneyPlan[\s\S]*if \(!result\.ok\)[\s\S]*setPlanState[\s\S]*router\.push/,
  );
  assert.match(shell, /<DealsCarResultsStage/);
  assert.match(shell, /<DealsReviewStage/);
});

test("guided Flight details keeps Car confirmation copy and behavior scoped", async () => {
  const [stage, shell, en] = await Promise.all([
    readFile(stageUrl, "utf8"),
    readFile(shellUrl, "utf8"),
    readFile(enUrl, "utf8"),
  ]);
  ...
  assert.match(
    en,
    /"deals\.guided\.flightDetails\.confirmCars":\s*"Choose this flight"/,
  );
  assert.match(
    en,
    /"deals\.guided\.flightDetails\.confirmCarsA11y":\s*"Choose \{airline\} flight from \{origin\} to \{destination\}"/,
  );
  assert.match(
    stage,
    /const includesCar = getIncludedProducts\(search\.mode\)\.car;[\s\S]*const label = includesCar[\s\S]*t\("deals\.guided\.flightDetails\.confirmCars"\)/,
  );
  assert.match(
    stage,
    /const accessibleLabel = selection[\s\S]*includesCar[\s\S]*t\("deals\.guided\.flightDetails\.confirmCarsA11y"\)/,
  );
  assert.match(
    stage,
    /kind: "guided-flight"[\s\S]*if \(selection && !confirming\) onConfirm\(selection\)/,
  );
  assert.match(shell, /confirmGuidedFlightSelection/);
  assert.match(shell, /attemptGuidedConfirmation/);
  assert.match(shell, /getNextDealsJourneyStage/);
