...
type Props = {
  ...
  rates: ReturnType<typeof useCurrencyRates>;
  t: (key: string) => string;
  progressUnsaved: boolean;
  announcement: string;
  onOpen: (product: DealsTripPlanProduct) => boolean | void;
...
export function DealsHandoffExperience({
  ...
  const steps = useMemo(
    () => getDealsHandoffSteps(plan, now, locale, orderedProducts),
    [plan, now, locale, orderedProducts],
  );
  const actionable = steps.filter((step) => step.status !== "expired");
  const opened = actionable.filter((step) => step.status === "opened").length;
  const hasExpired = steps.some((step) => step.status === "expired");
  const hasUnavailableAction = steps.some(
    (step) => step.status !== "expired" && !step.href,
  );
  ...
  return (
    <>
      ...
      {guided && allOpened && (
        <div
          role="status"
          className="mt-5 rounded-xl bg-emerald-50 p-4 text-emerald-950"
        >
          <p className="font-bold">{t("deals.guided.handoff.allOpened")}</p>
          <p className="mt-1 text-sm leading-6">
            {t("deals.guided.handoff.allOpenedBody")}
          </p>
        </div>
      )}
      <div
        data-deals-handoff-ready-grid
        className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px] xl:items-start"
      >
        <DealsHandoffSummary
        ...
        <div className="order-2 min-w-0 xl:order-1">
          ...
          {progressUnsaved && (
            <div
              ...
              role="alert"
              tabIndex={-1}
              className="mb-4 rounded-xl bg-amber-50 p-4 text-sm font-semibold text-amber-900"
            >
              {t(
                guided
          ...
          <ol
            id="provider-steps"
            aria-label={t("deals.handoff.providerSteps")}
            className="space-y-4"
          >
            {steps.map((step) => (
              <li key={step.product}>
                <DealsHandoffStepCard
                  step={step}
                  displayCurrency={selectedCurrency}
                  resultsPath={plan.resultsPath}
                  t={t}
                  onOpen={() => onOpen(step.product)}
                  onRecoveryNavigation={guided ? start : undefined}
                  unavailableHref={
                    guided ? recoveryHrefs?.[step.product] : undefined
                  }
                  unavailableLabel={
                    guided
                      ? t(
                          `deals.guided.handoff.change${step.product === "hotel" ? "Stay" : step.product === "flight" ? "Flight" : "Car"}`,
                        )
                      : undefined
                  }
                  unavailableBody={
                    guided
                      ? t("deals.guided.handoff.actionUnavailable")
                      : undefined
                  }
                  price={
                    Number.isFinite(step.sourcePrice) &&
                    step.sourcePrice > 0 &&
                    step.sourceCurrency.trim()
                      ? formatDisplayPrice({
                          amount: step.sourcePrice,
          ...
          {!guided && plan.carsResultsPath && (
            <Link
              href={plan.carsResultsPath}
              className="mt-4 inline-flex min-h-11 items-center font-bold text-[#004BB8] underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2"
            >
              {t("deals.handoff.compareCars")}
            </Link>
          )}
        </div>
      </div>
