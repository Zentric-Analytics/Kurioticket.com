...
export function DealsHandoffExperience({ plan, now, locale, selectedCurrency, rates, t, progressUnsaved, announcement, onOpen, orderedProducts, guided = false, recoveryHrefs, combinedTotal }: Props) {
  ...
  const hasUnavailableAction = steps.some(step => step.status !== "expired" && !step.href);
  const legacyNext = getNextDealsProviderStep(plan, now);
  const allOpened = !hasExpired && !hasUnavailableAction && actionable.length > 0 && opened === actionable.length;
  ...
  const progress = t("deals.handoff.progress").replace("{{opened}}", String(opened)).replace("{{total}}", String(total));
  return <>
    ...
    <div data-deals-handoff-ready-grid className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px] xl:items-start">
      <DealsHandoffSummary modeLabel={t(modeKeys[plan.mode])} opened={opened} total={total} totalLabel={combined === null ? null : formatCurrency(combined, selectedCurrency)} progressLabel={progress} allOpened={guided ? allOpened : legacyNext.allOpened} hasExpired={hasExpired} hasUnavailableAction={guided && hasUnavailableAction} t={t} />
