...
export function DealsGuidedHandoffClient({ search }: { search: DealsSearch }) {
  ...
  const activate = (product: DealsTripPlanProduct): boolean => {
    ...
    if (!result.ok) {
      ...
      setActivationFailure(result.failure); window.setTimeout(() => errorRef.current?.focus(), 0); return false;
    }
  ...
  if (!readResult || now === null) content = <DealsHandoffSkeleton label={t("deals.guided.handoff.loading")} />;
    ...
    if (!validation.ok && validation.reason === "incomplete") { state = "incomplete"; content = <State kind="warning" title={t("deals.guided.handoff.incompleteTitle")} body={t("deals.guided.handoff.incompleteBody")} action={t("deals.guided.handoff.refresh")} href={buildDealsJourneyUrl(getEarliestIncompleteDealsJourneyStage(search.mode, plan), search)} start={start} />; }
      ...
      <div ref={errorRef} tabIndex={-1}>{activationFailure && <ActivationAlert failure={activationFailure} t={t} retry={() => { const product = "product" in activationFailure ? activationFailure.product : null; setActivationFailure(null); if (product) document.getElementById(`handoff-${product}-action`)?.focus(); }} />}</div>
...
function ActivationAlert({ failure, t, retry }: { failure: GuidedActivationFailure; t: (key: string) => string; retry: () => void }) {
  const body = failure.kind === "storage-unavailable" ? "activationFailedBody" : failure.kind === "selection-changed" ? "selectionChanged" : failure.kind === "plan-expired" ? "planExpiredAtClick" : failure.kind === "product-expired" ? "productExpiredAtClick" : failure.kind === "action-unavailable" ? "actionUnavailable" : "missingBody";
  return <div id="guided-handoff-activation-error" role="alert" className="mb-5 rounded-xl border border-amber-300 bg-amber-50 p-4 text-amber-950"><p className="font-bold">{t("deals.guided.handoff.activationFailedTitle")}</p><p className="mt-1 text-sm leading-6">{t(`deals.guided.handoff.${body}`)}</p>{failure.kind === "storage-unavailable" && <button type="button" onClick={retry} className="focus-ring mt-3 min-h-11 font-bold underline">{t("deals.guided.handoff.retryStep")}</button>}</div>;
}
