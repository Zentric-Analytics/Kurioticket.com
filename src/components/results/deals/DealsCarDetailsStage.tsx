...
import {
  ...
  isCurrentDealsCarDetailsResponse,
} from "@/lib/deals/dealsCarDetails";

type LoadState = "loading" | "available" | "unavailable" | "error";

export function DealsCarDetailsStage({
  search,
  carId,
  plan,
  confirming,
  ...
  useEffect(() => {
    ...
    fetch("/api/cars/search", {
      ...
      headers: {
        ...
        "x-search-request-id": `guided-car-details-${retryGeneration}`,
      },
      body: payloadJson,
      ...
      .then(async (response) => {
        ...
        const selected = data.results.find((item) =>
          isCurrentDealsCarDetailsResponse(effectiveCarId, item),
        );
        if (!selected) {
          setCar(null);
          setPrimaryOffer(null);
        ...
        setCar(selected);
        setPrimaryOffer(selectedPrimaryOffer ?? null);
        setResultReceivedAt(receivedAt);
        setState("available");
      })
  ...
  useEffect(() => {
    ...
    if (state === "unavailable")
      unavailableRef.current?.focus({ preventScroll: true });
    if (state === "error") errorRef.current?.focus({ preventScroll: true });
    retryFocusRef.current = false;
  }, [state]);

  const selection =
    car && primaryOffer && effectiveCarId && resultReceivedAt
      ? buildDealsCarDetailsSelection({
          car,
          primaryOffer,
          requestedCarId: effectiveCarId,
  ...
  if (state === "loading")
    return (
      <div
        ...
        tabIndex={-1}
        role="status"
        data-deals-guided-car-details-loading
        className="mt-6 space-y-4 outline-none"
        aria-label={t("deals.guided.carDetails.loading")}
      >
        <p className="text-lg font-bold text-slate-950">
          {t("deals.guided.carDetails.loading")}
        </p>
  ...
  if (state === "error")
    return (
      <div
        data-deals-guided-car-details-error
      ...
      >
        ...
        >
          {t("deals.guided.carDetails.errorTitle")}
        </h2>
        <p className="mt-2 max-w-2xl leading-7 text-slate-600">
          {t("deals.guided.carDetails.errorBody")}
        </p>
        <Button type="button" className="mt-4 min-h-11" onClick={retry}>
          {t("deals.guided.carDetails.retry")}
        </Button>
        <span className="sr-only">{error}</span>
      </div>
    );
  if (state === "unavailable" || !car)
    return (
      <div
        data-deals-guided-car-details-unavailable
      ...
      >
        <h2
          ...
          className="text-xl font-extrabold text-slate-950 outline-none"
        >
          {t("deals.guided.carDetails.unavailableTitle")}
        </h2>
        <p className="mt-2 max-w-2xl leading-7 text-slate-600">
          {t("deals.guided.carDetails.unavailableBody")}
        </p>
        <Button type="button" className="mt-4 min-h-11" onClick={retry}>
          {t("deals.guided.carDetails.retry")}
        </Button>
  ...
  return (
    <CarDetailsExperience
      ...
      itemHeadingLevel={4}
      modelHeadingRef={modelRef}
      primaryAction={{
        kind: "guided-car",
        enabled: Boolean(selection) && !confirming,
        pending: confirming,
        label: t("deals.guided.carDetails.choose"),
        accessibleLabel: t("deals.guided.carDetails.chooseA11y")
          .replace("{model}", car.modelName)
          .replace("{company}", car.rentalCompanyName),
        unavailableMessage: t("deals.guided.carDetails.unavailableConfirm"),
        error: confirmationError,
        onActivate: () => {
          if (selection && !confirming) onConfirm(selection);
        },
      }}
    />
  );
