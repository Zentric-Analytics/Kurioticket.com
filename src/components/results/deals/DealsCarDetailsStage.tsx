...
export function DealsCarDetailsStage({ search, carId, plan, confirming, confirmationError, onConfirm }: { search: DealsSearch; carId: string | null; plan: DealsTripPlan | null; confirming: boolean; confirmationError: string; onConfirm: (selection: DealsTripPlanCar) => void }) {
  ...
  const carRequestIdentity = buildDealsCarRequestIdentity(search);
  const carRequestPayload = buildDealsCarRequestPayload(search);
  const payloadJson = JSON.stringify(carRequestPayload);
  const requestIdentity = `${carRequestIdentity}::${effectiveCarId ?? ""}`;
  const [retryGeneration, setRetryGeneration] = useState(0);
  const [state, setState] = useState<LoadState>("loading");
  const [car, setCar] = useState<NormalizedCarResult | null>(null);
  const [primaryOffer, setPrimaryOffer] = useState<CarOffer | null>(null);
  const [resultReceivedAt, setResultReceivedAt] = useState<number | null>(null);
  const [error, setError] = useState("");
  const latestRequestRef = useRef("");
  const selectionSearchRef = useRef(search);
  ...
  useEffect(() => {
    selectionSearchRef.current = search;
  }, [search]);

  useEffect(() => {
    if (!effectiveCarId) { queueMicrotask(() => { setState("unavailable"); setCar(null); setPrimaryOffer(null); setResultReceivedAt(null); }); return; }
    const controller = new AbortController();
    const lifecycleId = `${requestIdentity}::${retryGeneration}`;
    latestRequestRef.current = lifecycleId;
    queueMicrotask(() => { if (controller.signal.aborted || latestRequestRef.current !== lifecycleId) return; setState("loading"); setCar(null); setPrimaryOffer(null); setResultReceivedAt(null); setError(""); if (retryFocusRef.current) loadingRef.current?.focus({ preventScroll: true }); });
    fetch("/api/cars/search", { method: "POST", headers: { "Content-Type": "application/json", "x-search-request-id": `guided-car-details-${retryGeneration}` }, body: payloadJson, signal: controller.signal }).then(async (response) => {
      ...
      if (!selected) { setCar(null); setPrimaryOffer(null); setResultReceivedAt(null); setState("unavailable"); return; }
      const selectedPrimaryOffer = getPrimaryCarOffer(selected);
      const receivedAt = Date.now();
      const selectionSearch = selectionSearchRef.current;
      if (!buildDealsCarDetailsSelection({ car: selected, primaryOffer: selectedPrimaryOffer, requestedCarId: effectiveCarId, search: selectionSearch, resultReceivedAt: receivedAt })) { setCar(null); setPrimaryOffer(null); setResultReceivedAt(null); setState("unavailable"); return; }
      setCar(selected); setPrimaryOffer(selectedPrimaryOffer ?? null); setResultReceivedAt(receivedAt); setState("available");
    }).catch((reason: unknown) => { if (controller.signal.aborted || latestRequestRef.current !== lifecycleId) return; setCar(null); setPrimaryOffer(null); setResultReceivedAt(null); setError(reason instanceof Error ? reason.message : "unavailable"); setState("error"); });
    return () => controller.abort();
  }, [effectiveCarId, payloadJson, requestIdentity, retryGeneration]);
  ...
  const selection = car && primaryOffer && effectiveCarId && resultReceivedAt ? buildDealsCarDetailsSelection({ car, primaryOffer, requestedCarId: effectiveCarId, search, resultReceivedAt }) : null;
  const retry = () => { retryFocusRef.current = true; setState("loading"); setCar(null); setPrimaryOffer(null); setResultReceivedAt(null); setError(""); setRetryGeneration((value) => value + 1); };
