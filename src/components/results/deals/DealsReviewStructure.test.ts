...
const handoff = fs.readFileSync("src/components/results/deals/DealsGuidedHandoffClient.tsx", "utf8");
...
test("guided handoff uses staged storage and safe activation", () => {
  assert.match(handoff, /data-deals-guided-handoff/);
  assert.match(handoff, /readDealsStagedJourneyPlan/);
  assert.match(handoff, /writeDealsStagedJourneyPlan/);
  assert.doesNotMatch(handoff, /readDealsTripPlan|writeDealsTripPlan|removeDealsTripPlan|bookingUrl|partnerRedirectUrl|window\.open/);
});
