...
export function DealsReviewSummary({ modeLabel, count, disclosure, canContinue, continueHref, expiredNames, t }: { modeLabel: string; count: number; disclosure: string; canContinue: boolean; continueHref: string; expiredNames: string[]; t: (key: string) => string }) {
  ...
  return <aside aria-labelledby="deals-review-summary-title" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm xl:sticky xl:top-24">
    <h2 id="deals-review-summary-title" className="text-xl font-extrabold text-slate-950">Review coordinated options</h2>
    <dl className="mt-4 space-y-3 text-sm"><div><dt className="font-bold text-slate-500">{t("deals.guided.review.package")}</dt><dd className="mt-1 font-semibold text-slate-900">{modeLabel}</dd></div><div><dt className="font-bold text-slate-500">{t("deals.guided.review.includedOptions")}</dt><dd className="mt-1 font-semibold text-slate-900">{count}</dd></div></dl>
