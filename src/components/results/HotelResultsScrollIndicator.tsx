...
import { calculateHotelResultsScrollIndicatorGeometry } from "@/lib/hotels/hotelResultsScrollIndicator";

const MOBILE_RESULTS_QUERY = "(max-width: 639px)";
const IDLE_FADE_DELAY_MS = 650;
const MIN_TRACK_HEIGHT_PX = 96;

export function HotelResultsScrollIndicator() {
  ...
  useEffect(() => {
    ...
    const previousRouteMarker = root.getAttribute(
      "data-hotel-results-scroll-indicator",
    );

    const setRouteMarker = () => {
      if (media.matches) root.setAttribute("data-hotel-results-scroll-indicator", "");
      else root.removeAttribute("data-hotel-results-scroll-indicator");
    };
    setRouteMarker();
    ...
    return () => {
      ...
      if (previousRouteMarker === null)
        root.removeAttribute("data-hotel-results-scroll-indicator");
      else
        root.setAttribute(
          "data-hotel-results-scroll-indicator",
          previousRouteMarker,
  ...
  return (
    ...
    >
      <div
        ref={thumbRef}
        data-hotel-results-scroll-thumb
