...
export function DealsResultsClient({ initialSearch: search, invalid }: { initialSearch: DealsSearch; invalid: boolean }) {
  ...
  return <main className={`flex-1 overflow-x-clip bg-[#f6f8fb] pb-8 ${plan?.flight || plan?.hotel || plan?.car ? "pb-96 sm:pb-56" : ""}`}><DealsResultsSearchSummary search={search} locale={locale} t={t} modeLabel={t(modeKeys[search.mode])} onModify={() => setEditorOpen(true)} modifyExpanded={editorOpen} modifyButtonRef={activeModifyTriggerRef} /><DealsResultsBreadcrumbs t={t} /><div className="page-shell max-w-6xl pt-5 sm:pt-6">{editorOpen && editor}<section id="package-options" tabIndex={-1} aria-label={t("deals.results.package.title")} aria-busy={loading} className="scroll-mt-20 outline-none">
