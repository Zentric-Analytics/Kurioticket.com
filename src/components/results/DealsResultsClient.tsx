...
import type { ContractResult, TravelSearchMode } from "@/lib/travel/searchContract";
...
type ProductState<T> = { status: Status; results: ContractResult<T>[]; receivedAt?: number; errorKey?: string; warnings: string[]; servedFromFallback: boolean; latencyMs?: number; warningCategory?: string; mode?: TravelSearchMode; sourceLabel?: string };
...
export function DealsResultsClient({ initialSearch: search, invalid }: { initialSearch: DealsSearch; invalid: boolean }) {
  ...
  const request = useCallback((product: "flight" | "hotel" | "car", signal?: AbortSignal) => {
    ...
    void fetch(isFlight ? "/api/flights/search" : isCar ? "/api/cars/search" : "/api/hotels/search", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(isFlight ? buildFlightApiPayload(search) : isCar ? buildCarApiPayload(search) : buildHotelApiPayload(search)), signal }).then(async response => {
      ...
      dispatch({ product, value: { status: results.length ? "success" : "empty", results, ...(results.length ? { receivedAt: Date.now() } : {}), ...metadata, ...(typeof record.mode === "string" ? { mode: record.mode as TravelSearchMode } : {}), ...(typeof record.sourceLabel === "string" ? { sourceLabel: record.sourceLabel } : {}) } } as Action);
  ...
  return <main className={`flex-1 overflow-x-clip bg-[#f6f8fb] pb-8 ${plan?.flight || plan?.hotel || plan?.car ? "pb-96 sm:pb-56" : ""}`}><DealsResultsSearchSummary search={search} locale={locale} t={t} modeLabel={t(modeKeys[search.mode])} onModify={() => setEditorOpen(true)} modifyExpanded={editorOpen} modifyButtonRef={activeModifyTriggerRef} /><DealsResultsBreadcrumbs t={t} /><div className="page-shell max-w-6xl pt-5 sm:pt-6">{editorOpen && editor}<section aria-labelledby="package-options" aria-busy={loading}>
    <div className="max-w-3xl"><h1 id="package-options" tabIndex={-1} className="scroll-mt-20 rounded-sm text-2xl font-extrabold text-slate-950 outline-none focus-visible:ring-2 focus-visible:ring-[#004BB8]/40">{t("deals.results.package.title")}</h1><p className="mt-2 text-slate-600">{t("deals.results.package.intro")}</p></div>
    ...
    {candidates.length > 0 && <><DealsPackageResultsToolbar count={candidates.length} mode={search.mode} value={packageSort} onChange={setPackageSort}/><ol aria-label={t("deals.results.package.title")} className="mt-4 space-y-4 sm:mt-6 sm:space-y-6">{sortedCandidates.map(candidate => <li key={candidate.id}><DealsPackageCard candidate={candidate} selected={[candidate.flight?.id, candidate.hotel?.id, candidate.car?.id].filter(Boolean).join("::") === [plan?.flight?.id, plan?.hotel?.id, plan?.car?.id].filter(Boolean).join("::")} t={t} onSelect={() => selectPackage(candidate)} /></li>)}</ol></>}
