...
import { buildDealsPlanContextKey, getVisibleDealsPlan, readDealsStagedJourneyPlan, readDealsTripPlan, removeDealsStagedJourneyPlan, removeDealsTripPlan, writeDealsStagedJourneyPlan, writeDealsTripPlan } from "@/lib/deals/dealsTripPlanStorage";
...
export function DealsResultsClient({ initialSearch: search, invalid, stagedRequested = false }: { initialSearch: DealsSearch; invalid: boolean; stagedRequested?: boolean }) {
  ...
  const [storedPlan, setStoredPlan] = useState<DealsTripPlan | null>(null); const [storedPlanContextKey, setStoredPlanContextKey] = useState<string | null>(null);
  const plan = getVisibleDealsPlan(storedPlan, storedPlanContextKey, planContextKey); const setPlan = useCallback((next: DealsTripPlan | null) => { setStoredPlan(next); setStoredPlanContextKey(next ? planContextKey : null); }, [planContextKey, setStoredPlan, setStoredPlanContextKey]); const [announcement, setAnnouncement] = useState("");
  ...
  useEffect(() => { if (invalid) return; const flight = new AbortController(); const hotel = new AbortController(); const car = new AbortController(); if (included.flight && !stagedHotelJourneyActive) request("flight", flight.signal); if (included.hotel) request("hotel", hotel.signal); if (included.car && !stagedHotelJourneyActive) request("car", car.signal); return () => { flight.abort(); hotel.abort(); car.abort(); }; }, [included.flight, included.hotel, included.car, invalid, request, stagedHotelJourneyActive]);
  useEffect(() => {
  ...
  }, [fingerprint, included.flight, pendingFingerprint, setPlan, stagedHotelJourneyActive, t]);
