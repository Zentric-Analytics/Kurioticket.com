...
export function DealsResultsClient({ initialSearch: search, invalid }: { initialSearch: DealsSearch; invalid: boolean }) {
  ...
  if (invalid) return <main className="page-shell flex-1 py-10"><div className="rounded-3xl border border-rose-200 bg-white p-8 text-center"><CircleAlert aria-hidden className="mx-auto h-10 w-10 text-rose-600" /><p className="mt-3 text-xs font-bold uppercase text-rose-700">{t("deals.results.tripOverview")}</p><h1 className="mt-1 text-2xl font-extrabold text-slate-950">{t("deals.results.invalidTitle")}</h1><p className="mt-2 text-slate-600">{t("deals.results.editor.correctHere")}</p><button ref={modifyButtonRef} type="button" aria-expanded={editorOpen} aria-controls="deals-modify-search-dialog" onClick={() => setEditorOpen(true)} className="mt-5 min-h-11 rounded-xl bg-[#004BB8] px-6 font-extrabold text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4">{t("deals.results.editor.correctSearch")}</button></div>{editor}<p className="sr-only" aria-live="polite">{announcement}</p></main>;
  ...
  return <main className={`flex-1 bg-slate-50 py-6 sm:py-8 ${plan?.flight || plan?.hotel ? "pb-64 sm:pb-44" : ""}`}><div className="page-shell"><DealsTripOverview search={search} locale={locale} t={t} modeLabel={t(modeKeys[search.mode])} onModify={() => editorOpen ? closeEditor() : setEditorOpen(true)} modifyExpanded={editorOpen} modifyButtonRef={modifyButtonRef} />{editorOpen && editor}<div className="mt-6 space-y-6">
    {included.flight && <DealsProductSection id="flight-options" title={t("deals.results.flightOptions")} summary={`${overview.flight.title} · ${overview.flight.dates}`} icon={<Plane aria-hidden className="text-[#004BB8]" />} href={buildFlightResultsUrl(search)} viewAll={state.flight.status === "success" ? viewAll("deals.results.viewFlightsCount", state.flight.results.length) : t("deals.results.viewFlights")} supportingText={t("deals.results.openCompleteResults")} priceNotice={t("deals.results.priceResponsibility")} status={state.flight.status} loadingLabel={t("searchingFlights")} emptyLabel={t("deals.results.flightEmpty")} errorLabel={t(state.flight.errorKey ?? "deals.results.providerUnavailable")} retryLabel={t("retry")} onRetry={() => request("flight")} notice={notice(state.flight)} countLabel={state.flight.status === "success" ? count(state.flight.results.length, flightPreviews.length) : undefined}><div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{flightPreviews.map(({ result, badgeKey, reasonKey }) => <DealsFlightPreviewCard key={result.id} flight={result} badgeKey={badgeKey} reasonKey={reasonKey} locale={locale} t={t} selected={plan?.flight?.id === result.id} onSelect={() => selectFlight(result)} />)}</div></DealsProductSection>}
