...
import { countHotelNights, dealsPreviewLimit, getOverviewData, normalizeMetadata } from "@/lib/deals/dealsResultsPresentation";
...
export function DealsResultsClient({ initialSearch: search, invalid }: { initialSearch: DealsSearch; invalid: boolean }) {
  ...
  const count = (length: number) => t("deals.results.showingOptions").replace("{{visible}}", String(Math.min(dealsPreviewLimit, length))).replace("{{total}}", String(length));
  return <main className="flex-1 bg-slate-50 py-6 sm:py-8"><div className="page-shell"><DealsTripOverview search={search} locale={locale} t={t} modeLabel={t(modeKeys[search.mode])} /><div className="mt-6 space-y-6">
    {included.flight && <DealsProductSection id="flight-options" title={t("deals.results.flightOptions")} summary={`${overview.flight.title} · ${overview.flight.dates}`} icon={<Plane aria-hidden className="text-[#004BB8]" />} href={buildFlightResultsUrl(search)} viewAll={t("deals.results.viewFlights")} status={state.flight.status} loadingLabel={t("searchingFlights")} emptyLabel={t("deals.results.flightEmpty")} errorLabel={t(state.flight.errorKey ?? "deals.results.providerUnavailable")} retryLabel={t("retry")} onRetry={() => request("flight")} notice={notice(state.flight)} countLabel={state.flight.status === "success" ? count(state.flight.results.length) : undefined}><div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{state.flight.results.slice(0, dealsPreviewLimit).map(flight => <DealsFlightPreviewCard key={flight.id} flight={flight} href={buildFlightResultsUrl(search)} locale={locale} t={t} />)}</div></DealsProductSection>}
    {included.hotel && <DealsProductSection id="stay-options" title={t("deals.results.stayOptions")} summary={`${overview.hotel.title} · ${overview.hotel.dates}`} icon={<BedDouble aria-hidden className="text-[#004BB8]" />} href={buildHotelResultsUrl(search)} viewAll={t("deals.results.viewHotels")} status={state.hotel.status} loadingLabel={t("searchingHotels")} emptyLabel={t("deals.results.hotelEmpty")} errorLabel={t(state.hotel.errorKey ?? "deals.results.providerUnavailable")} retryLabel={t("retry")} onRetry={() => request("hotel")} notice={notice(state.hotel)} countLabel={state.hotel.status === "success" ? count(state.hotel.results.length) : undefined}><div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{state.hotel.results.slice(0, dealsPreviewLimit).map(hotel => <DealsHotelPreviewCard key={hotel.id} hotel={hotel} href={buildHotelResultsUrl(search)} locale={locale} nights={countHotelNights(search.hotelCheckIn, search.hotelCheckOut)} rooms={search.hotelRooms} t={t} />)}</div></DealsProductSection>}
