...
import { countHotelNights, getOverviewData, normalizeMetadata, selectDealsCarPreviews, selectDealsFlightPreviews, selectDealsHotelPreviews } from "@/lib/deals/dealsResultsPresentation";
...
import { DealsCarPreviewCard } from "./deals/DealsCarPreviewCard";
import { DealsPreviewRail } from "./deals/DealsPreviewRail";
...
export function DealsResultsClient({ initialSearch: search, invalid }: { initialSearch: DealsSearch; invalid: boolean }) {
  ...
  useEffect(() => { const timer = window.setTimeout(() => { const result = readDealsTripPlan(fingerprint); if (result.status === "valid") { const cleaned = { ...result.plan, opened: { ...result.plan.opened } }; delete cleaned.car; delete cleaned.opened.car; if (cleaned.flight || cleaned.hotel) { setPlan(cleaned); setPersistence(writeDealsTripPlan(cleaned) ? "saved" : "unavailable"); } else { removeDealsTripPlan(); setPlan(null); setPersistence("idle"); } } else if (result.status === "storage_unavailable") setPersistence("unavailable"); }, 0); return () => window.clearTimeout(timer); }, [fingerprint]);
  ...
  const warningKey = [plan?.flight, plan?.hotel].filter(Boolean).length > 1 ? "deals.results.editor.clearMultiple" : plan?.flight ? "deals.results.editor.clearFlight" : plan?.hotel ? "deals.results.editor.clearStay" : null;
  ...
  useEffect(() => {
    ...
    if (fingerprint === pendingFingerprint) {
      const applied = window.setTimeout(() => {
        ...
        requestAnimationFrame(() => { const heading = document.getElementById(included.flight ? "flight-options" : "stay-options"); heading?.scrollIntoView({ behavior: window.matchMedia("(prefers-reduced-motion: reduce)").matches ? "auto" : "smooth", block: "start" }); heading?.focus({ preventScroll: true }); });
      }, 0);
  ...
  const notice = (item: ProductState<unknown>) => item.servedFromFallback ? t("deals.results.fallbackNotice") : undefined;
  const flightPreviews = selectDealsFlightPreviews(state.flight.results); const hotelPreviews = selectDealsHotelPreviews(state.hotel.results); const carPreviews = selectDealsCarPreviews(state.car.results); const carSearch = buildCarApiPayload(search);
  const viewAll = (key: string, total: number) => t(key).replace("{{count}}", String(total));
  return <main className={`flex-1 overflow-x-clip bg-[#f6f8fb] pb-8 ${plan?.flight || plan?.hotel ? "pb-80 sm:pb-52" : ""}`}><DealsResultsSearchSummary search={search} locale={locale} t={t} modeLabel={t(modeKeys[search.mode])} onModify={() => setEditorOpen(true)} modifyExpanded={editorOpen} modifyButtonRef={modifyButtonRef} /><DealsResultsBreadcrumbs t={t} /><div className="page-shell pt-5 sm:pt-6">{editorOpen && editor}<div className="space-y-6 lg:space-y-0 lg:divide-y lg:divide-slate-200 lg:[&>section]:py-8 lg:[&>section:first-child]:pt-0 lg:[&>section:last-child]:pb-0">
    {included.flight && <DealsProductSection id="flight-options" headingLevel={1} title={t("deals.results.flightOptions")} summary={`${overview.flight.title} · ${overview.flight.dates}`} icon={<Plane aria-hidden className="text-[#004BB8]" />} href={buildFlightResultsUrl(search)} viewAll={state.flight.status === "success" ? viewAll("deals.results.viewFlightsCount", state.flight.results.length) : t("deals.results.viewFlights")} status={state.flight.status} loadingLabel={t("searchingFlights")} emptyLabel={t("deals.results.flightEmpty")} errorLabel={t(state.flight.errorKey ?? "deals.results.providerUnavailable")} retryLabel={t("retry")} onRetry={() => request("flight")} notice={notice(state.flight)}><DealsPreviewRail ariaLabel={t("deals.results.flightOptions")}>{flightPreviews.map(({ result, badgeKey, reasonKey }) => <DealsFlightPreviewCard key={result.id} flight={result} badgeKey={badgeKey} reasonKey={reasonKey} locale={locale} t={t} selected={plan?.flight?.id === result.id} onSelect={() => selectFlight(result)} />)}</DealsPreviewRail></DealsProductSection>}
    {included.hotel && <DealsProductSection id="stay-options" headingLevel={included.flight ? 2 : 1} title={t("deals.results.stayOptions")} summary={`${overview.hotel.title} · ${overview.hotel.dates}`} icon={<BedDouble aria-hidden className="text-[#004BB8]" />} href={buildHotelResultsUrl(search)} viewAll={state.hotel.status === "success" ? viewAll("deals.results.viewHotelsCount", state.hotel.results.length) : t("deals.results.viewHotels")} status={state.hotel.status} loadingLabel={t("searchingHotels")} emptyLabel={t("deals.results.hotelEmpty")} errorLabel={t(state.hotel.errorKey ?? "deals.results.providerUnavailable")} retryLabel={t("retry")} onRetry={() => request("hotel")} notice={notice(state.hotel)}><DealsPreviewRail ariaLabel={t("deals.results.stayOptions")}>{hotelPreviews.map(({ result, badgeKey, reasonKey }) => <DealsHotelPreviewCard key={result.id} hotel={result} badgeKey={badgeKey} reasonKey={reasonKey} locale={locale} nights={countHotelNights(search.hotelCheckIn, search.hotelCheckOut)} rooms={search.hotelRooms} t={t} selected={plan?.hotel?.id === result.id} onSelect={() => selectHotel(result)} />)}</DealsPreviewRail></DealsProductSection>}
    {included.car && <DealsProductSection id="car-options" headingLevel={2} title={t("deals.results.carOptions")} summary={`${overview.car.title} · ${overview.car.dates} · ${overview.car.pickupTime} – ${overview.car.returnTime}`} icon={<Car aria-hidden className="text-[#004BB8]" />} href={buildCarResultsUrl(search)} viewAll={state.car.status === "success" ? viewAll("deals.results.viewCarsCount", state.car.results.length) : t("deals.results.viewCars")} status={state.car.status} loadingLabel={t("searchingCars")} emptyLabel={t("deals.results.carEmpty")} errorLabel={t(state.car.errorKey ?? "deals.results.carUnavailable")} retryLabel={t("retry")} onRetry={() => request("car")}><DealsPreviewRail ariaLabel={t("deals.results.carOptions")}>{carPreviews.map(({ result, badgeKey }) => <DealsCarPreviewCard key={result.id} car={result} badgeKey={badgeKey} locale={locale} search={carSearch} t={t} />)}</DealsPreviewRail></DealsProductSection>}
