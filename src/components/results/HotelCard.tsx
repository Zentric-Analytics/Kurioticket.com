...
export function HotelCard({ hotel, index }: { hotel: PublicHotelResult; index: number }) {
  ...
  return (
    <Card className="hotel-result-card group overflow-hidden rounded-[1.4rem] border-indigo-100/80 bg-white shadow-[0_22px_70px_-42px_rgba(49,46,129,0.7)] transition duration-200 hover:-translate-y-0.5 hover:border-violet-200 hover:shadow-[0_28px_80px_-42px_rgba(67,56,202,0.75)]">
      <div className="hotel-result-card-grid grid">
        <div className="hotel-result-card-image-wrapper p-3 pb-0 lg:pb-3 lg:pr-0">
          <div className="hotel-result-card-image relative aspect-[16/10] min-h-[230px] overflow-hidden rounded-[1.15rem] bg-indigo-50 lg:h-[216px] lg:min-h-0">
            <Image
              src={imageUrl}
              alt={`${hotel.name} stay option${hotel.location ? ` near ${hotel.location}` : ""}`}
              fill
              className="object-cover transition duration-500 group-hover:scale-[1.035]"
              sizes="(min-width: 1120px) 250px, (min-width: 1024px) 230px, 100vw"
            />
        ...
        <div className="hotel-result-card-price flex flex-col border-t border-indigo-100 p-4 pt-4 sm:p-5 lg:border-l lg:border-t-0 lg:pl-5 lg:text-right">
          ...
          <div className="hotel-result-card-scores mt-3 flex flex-wrap gap-x-3 gap-y-1 text-[11px] font-bold uppercase tracking-wide text-slate-500 lg:justify-end">
          ...
          <p className="hotel-result-card-provider-note mt-3 flex items-start gap-2 text-left text-[12px] leading-5 text-slate-600 lg:justify-end lg:text-right">
