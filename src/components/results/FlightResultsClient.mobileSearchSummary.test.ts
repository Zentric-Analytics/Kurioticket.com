...
test("mobile search summary remains the compact whole-card Edit Search launcher", () => {
  ...
  assert.match(summary, /\{mobileTravelerSummary\}/);
...
test("mobile search summary uses the refined compact neutral card system", () => {
  ...
  assert.match(summary, /truncate text-\[16px\] font-bold/);
  assert.match(summary, /truncate text-\[12\.5px\] font-semibold/);
  assert.match(
    summary,
    /h-9 w-9[\s\S]*rounded-\[10px\][\s\S]*bg-slate-50 text-slate-700/,
  );
