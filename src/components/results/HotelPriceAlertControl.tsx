...
export function HotelPriceAlertControl({ search, results }: { search: HotelSearchParams; results: PublicHotelResult[] }) {
  ...
  return <section aria-label={t("travel.account.hotelAlert.title")} className="mb-5 rounded-2xl border border-blue-100 bg-white p-4 shadow-sm">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-bold text-slate-950">{t("travel.account.hotelAlert.title")}</h2><p className="mt-1 text-sm text-slate-600">{t("travel.account.hotelAlert.body")}</p></div><button type="button" className="min-h-11 rounded-xl bg-[#004BB8] px-4 font-bold text-white" onClick={() => { setStatus("idle"); setOpen(true); }}>{t("travel.account.hotelAlert.create")}</button></div>
    {open ? <div className="mt-4 flex flex-wrap items-end gap-3"><label className="text-sm font-semibold text-slate-700">{t("travel.account.hotelAlert.target")} ({currency})<input autoFocus inputMode="decimal" value={target} onChange={(event) => { setTarget(event.target.value); setStatus("idle"); }} className="mt-1 block min-h-11 rounded-lg border border-slate-300 px-3" /></label><button disabled={status === "saving"} type="button" onClick={() => void create()} className="min-h-11 rounded-lg bg-[#004BB8] px-4 font-bold text-white">{status === "saving" ? t("loading") : t("travel.account.hotelAlert.save")}</button><button type="button" onClick={() => setOpen(false)} className="min-h-11 px-3 font-semibold text-slate-700">{t("cancel")}</button></div> : null}
