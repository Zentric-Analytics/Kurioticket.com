...
function SecuritySettingRow({ title, body, action, danger = false }: SecuritySettingRowProps) {
  return (
    <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-x-4 gap-y-3 px-1 py-7 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:gap-6 sm:px-2 sm:py-6">
      <div className="col-span-2 min-w-0 sm:col-span-1">
        ...
        <p className="mt-4 max-w-[15rem] text-[1.05rem] leading-8 text-slate-600 sm:mt-1.5 sm:max-w-2xl sm:text-sm sm:leading-6 sm:text-muted">{body}</p>
      </div>
      <button
        type="button"
        className={cn(
          "focus-ring self-center justify-self-end rounded-none border-0 bg-transparent p-0 text-right text-[1.05rem] font-medium leading-6 transition hover:underline sm:inline-flex sm:min-h-10 sm:w-auto sm:items-center sm:justify-center sm:rounded-xl sm:border sm:px-4 sm:text-sm sm:font-semibold sm:no-underline",
          danger
  ...
  return (
    <section aria-labelledby="security-title" className="mx-auto min-w-0 max-w-[62rem] space-y-7 lg:space-y-5 xl:max-w-[64rem]">
      <div className="px-1 text-left sm:px-2">
        ...
        <p className="mt-7 max-w-[19rem] text-[1.15rem] leading-9 text-slate-600 sm:mt-3 sm:max-w-2xl sm:text-sm sm:leading-6 sm:text-muted">
