    ...
    <>
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white">
        <div className="page-shell flex min-h-[88px] items-center justify-between gap-4 py-4">
          ...
          <div className="hidden items-center gap-3 md:flex">
            ...
            {isSignedIn ? (
              ...
              <>
                <LinkButton href="/auth/login" variant="ghost" size="sm" className="h-11 rounded-full px-4 text-base font-semibold">
                  {t.login}
                </LinkButton>
                <LinkButton href="/auth/register" size="sm" className="h-11 rounded-full px-5 text-base font-semibold">
                  {t.signUp}
                </LinkButton>
              </>
            )}
        ...
        <div className="hidden border-t border-slate-100 lg:block">
          <nav className="page-shell flex items-center gap-2 py-3">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="rounded-lg px-4 py-2 text-base font-semibold text-slate-900 hover:bg-violet-50 hover:text-[#6d28d9]"
              >
                {item.label}
              </Link>
            ))}
      ...
      {open && (
        <div className="fixed inset-x-0 top-[88px] z-30 border-b border-slate-200 bg-white p-4 shadow-lg md:hidden">
