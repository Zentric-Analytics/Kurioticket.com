        ...
        <nav className="hidden items-center gap-1.5 lg:flex">
          {navItems.map((item) => (
            <Link key={item.href} href={item.href} className="rounded-md px-3 py-2 text-base font-black text-slate-900 hover:bg-violet-50 hover:text-[#6d28d9]">
              {navTranslations[language][item.label as keyof typeof navTranslations["en"]] ?? item.label}
            </Link>
          ))}
        </nav>
            ...
            {languageOpen ? (
              <div className="absolute right-0 top-12 z-50 min-w-40 rounded-md border border-slate-200 bg-white p-1 shadow-lg" role="menu" aria-label="Language options">
                {languageOptions.map((item) => (
                  <button
                    key={`${item.flag} ${navTranslations[language][item.label as keyof typeof navTranslations["en"]] ?? item.label}`}
                    type="button"
                    role="menuitemradio"
                  ...
                  >
                    {`${item.flag} ${navTranslations[language][item.label as keyof typeof navTranslations["en"]] ?? item.label}`}
                  </button>
                ))}
              </div>
            ) : null}
          ...
          {isSignedIn ? (
            ...
            <>
              <Link href="/auth/signin" className="focus-ring inline-flex h-9 items-center justify-center rounded-md px-3 text-sm font-semibold text-navy transition hover:bg-surface-muted">
                {navTranslations[language].Login}
              </Link>
              <LinkButton href="/auth/signup" variant="ghost" size="sm" className="bg-[#5b21d6] text-white hover:bg-[#4c1d95] hover:text-white">
                {navTranslations[language].SignUp}
              </LinkButton>
            </>
          )}
        </div>
