...
export function AppHeader() {
  ...
  const renderFlag = (flag: string | undefined, code: string) =>
    flag ? (
          ...
          <div className="relative" ref={languageRef}>
            ...
            >
              <span className="text-sm font-semibold text-slate-900">
                {renderFlag(selectedLanguage.flag, selectedLanguage.code)} {selectedLanguage.label} ({selectedLanguage.code})
              </span>
        ...
        <div className="page-shell pb-4 md:hidden">
          <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
            ...
            <Button
              ...
              size="sm"
              onClick={() => setLanguageOpen((value) => !value)}
              className="h-12 justify-start gap-2 rounded-full border border-slate-200 bg-white px-4 shadow-sm"
            >
              {renderFlag(selectedLanguage.flag, selectedLanguage.code)}
              <span className="font-semibold text-slate-900">{selectedLanguage.label}</span>
              <span className="text-xs text-slate-500">({selectedLanguage.code})</span>
            </Button>
          </div>
        </div>
      ) : null}
