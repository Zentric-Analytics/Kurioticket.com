...
import { LANGUAGE_CHANGE_EVENT, getLanguageFromStorage, getUiTranslations, type LanguageCode } from "@/lib/language";
    ...
    <footer className="border-t border-border bg-navy text-white">
      <div className="page-shell grid gap-8 py-10 md:grid-cols-[1.2fr_1fr_1fr]">
        <div>
          ...
          <p className="mt-3 max-w-md text-sm leading-6 text-slate-300">{t.footerAbout}</p>
          <p className="mt-4 text-xs leading-5 text-slate-400">{t.footerMeta}</p>
        </div>
        <div>
          <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-300">{t.platform}</h2>
        ...
        <div>
          <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-300">{t.legal}</h2>
          <div className="mt-3 grid gap-2 text-sm text-slate-300">
