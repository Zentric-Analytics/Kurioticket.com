...
test("tester role grants preview access without developer build notifications", () => {
  ...
  assert.equal(capabilities.includes("ANDROID_BUILD_NOTIFICATIONS"), false);
  assert.equal(capabilities.includes("IOS_BUILD_NOTIFICATIONS"), false);
});
