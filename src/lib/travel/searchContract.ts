...
export type TravelInventorySource="duffel"|"hotelbeds"|"kurioticket-static-cars";
...
export function classifyHotels(results:PublicHotelResult[],warnings:string[],requestId:string):TravelSearchResponse<PublicHotelResult>{const classified=results.map(result=>({...result,searchPolicy:{source:"hotelbeds" as const,bookable:true,action:{kind:"internal-detail" as const,href:`/hotels/details/${encodeURIComponent(result.id)}`,enabled:true as const}}}));return{results:classified,status:status(classified.length,warnings),source:"hotelbeds",warnings,partial:Boolean(warnings.length),requestId};}
