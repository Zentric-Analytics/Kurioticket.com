...
import { getEmailVerificationRedirect, sendEmailVerificationCode } from "@/services/emailVerificationService";
  ...
  CredentialsProvider({
    ...
    async authorize(credentials) {
      ...
      if (user.status !== "ACTIVE") {
        console.error("[auth:credentials-account-unavailable]", { email, status: user.status });
        logSafeAuthDiagnostics("[auth:credentials-account-unavailable-diagnostics]", { email, role: user.role, status: user.status });
      ...
      if (!valid) {
        ...
        logSafeAuthDiagnostics("[auth:credentials-invalid-password-diagnostics]", { email });
        return null;
      }
  ...
  callbacks: {
    async signIn({ user }) {
      ...
      const dbUser = await getPrisma().user.findUnique({
        where: { email },
        select: { id: true, status: true, role: true, emailVerified: true, name: true },
      });
      if (dbUser?.status && dbUser.status !== "ACTIVE") {
        console.error("[auth:signin-account-unavailable]", { email, status: dbUser.status });
        logSafeAuthDiagnostics("[auth:signin-account-unavailable-diagnostics]", { email, role: dbUser.role, status: dbUser.status });
    ...
    async jwt({ token, user }) {
      if (user) {
        ...
        token.emailVerified = Boolean((user as { emailVerified?: Date | string | null }).emailVerified);
      }
      ...
      if (token.email && isDatabaseConfigured()) {
        ...
        const dbUser = await getPrisma().user.findUnique({
          where: { email },
          select: { id: true, role: true, isPremium: true, status: true, emailVerified: true },
        });
        if (dbUser) {
          const isAdminEmail = adminEmails.includes(email);
          const role = isAdminEmail ? "ADMIN" : dbUser.role === "ADMIN" ? "USER" : dbUser.role;
