...
test("permissive cars remain explicitly unverified and never imply inventory", () => {
  ...
  assert.deepEqual(result, { kind: "unverified", canSubmit: true, canSubmitQuery: true, coverage: "unverified", message: "You can continue with this typed location. It is not provider-verified and static results may be unavailable." });
});

test("permissive hotel text is explicitly unverified while flights retain external discovery", () => {
  const hotel = resolveStaticSearch({ product: "hotels", typedValue: "Unknown", allowUnverifiedText: true });
  assert.equal(hotel.kind, "unverified");
  assert.equal(hotel.canSubmit, true);
