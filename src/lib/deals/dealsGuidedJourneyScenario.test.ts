...
const confirmationOrder: Record<string, DealsTripPlanProduct[]> = {
...
for (const mode of ["hotel-flight", "hotel-car", "flight-car", "hotel-flight-car"] as const) test(`${mode} completes confirmations, Review, handoff, reload, and real storage isolation`, () => {
  ...
  assert.deepEqual(getGuidedDealsProductOrder(mode), confirmationOrder[mode]);
  for (const product of confirmationOrder[mode]) {
  ...
  assert.ok(rendered); assert.deepEqual(getDealsReviewItems(rendered, search, 300, "en").map(item => item.product), confirmationOrder[mode]);
