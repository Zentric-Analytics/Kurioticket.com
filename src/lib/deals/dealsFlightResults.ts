...
export function buildDealsFlightResultsSearchInput(
  search: DealsSearch,
...
export function buildDealsFlightInventoryIdentity(
  search: DealsSearch,
  displayCurrency: string,
) {
...
export function buildDealsFlightDetailsJourneyUrl(
  search: DealsSearch,
  flightId: unknown,
): string | null {
  const normalizedFlightId = normalizeDealsJourneyFlightId(flightId);
  if (!normalizedFlightId || !isStageInDealsMode("flight-details", search.mode))
    return null;
