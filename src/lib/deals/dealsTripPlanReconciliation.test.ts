...
const policy = (href: string, overrides: Partial<TravelResultPolicy> = {}): TravelResultPolicy => ({ mode: "live", bookable: true, sourceLabel: "P", action: { kind: "internal-detail", href, enabled: true }, ...overrides });
...
const hotel = { id: "h", provider: "P", totalPrice: 2, currency: "USD", inventoryKind: "bookable", searchPolicy: policy("/hotels/details/h") } as ContractResult<PublicHotelResult>;
...
test("hotel reconciliation rejects discovery and demo policy", () => { assert.equal(reconcileDealsHotelSelection(plan, [hotel]), plan); for (const results of [[], [{ ...hotel, searchPolicy: policy("/hotels/details/h", { mode: "discovery", bookable: false }) }], [{ ...hotel, searchPolicy: policy("/hotels/details/h", { mode: "demo", bookable: false }) }]]) { const removed = reconcileDealsHotelSelection(plan, results); assert.equal(removed.hotel, undefined); assert.equal(removed.flight, plan.flight); assert.equal(removed.opened.hotel, undefined); } });
