...
test("hotel reconciliation rejects unavailable server policy", () => { assert.equal(reconcileDealsHotelSelection(plan, [hotel]), plan); for (const results of [[], [{ ...hotel, searchPolicy: policy("/hotels/details/h", { source: "kurioticket-static-cars", bookable: false }) }], [{ ...hotel, searchPolicy: policy("/hotels/details/wrong", { source: "kurioticket-static-hotels", bookable: false }) }]]) { const removed = reconcileDealsHotelSelection(plan, results); assert.equal(removed.hotel, undefined); assert.equal(removed.flight, plan.flight); assert.equal(removed.opened.hotel, undefined); } });
