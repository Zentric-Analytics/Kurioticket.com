...
const plan: DealsTripPlan = {
  ...
  hotel: { id: "hotel /?&=✓", provider: "Hotelbeds", name: "Parkwest Bicycle Casino", location: "Los Angeles - CA", checkIn: "2026-08-01", checkOut: "2026-08-03", roomType: "DELUXE KING ROOM", sourcePrice: 500, sourceCurrency: "USD", resultReceivedAt: now, detailsPath: "/hotels/details/example" },
...
test("builds structured localized flight, hotel, and car steps without mutating the plan", () => {
  ...
  assert.equal(flight.routeLabel, "LOS → LAX"); assert.equal(flight.flightNumber, "SN7261"); assert.match(flight.departureLabel, /Aug 1, 2026/); assert.match(flight.departureLabel, /11:35 PM/); assert.doesNotMatch(flight.departureLabel, /T23:35/); assert.equal(flight.status, "next"); assert.equal(flight.actionKind, "provider-handoff"); assert.equal(flight.href, "/redirect?id=flight+%2F%3F%26%3D%E2%9C%93&type=flight"); assert.notEqual(flight.href, plan.flight?.detailsPath);
  const hotel = steps[1]; assert.equal(hotel.product, "hotel"); if (hotel.product !== "hotel") return;
  assert.match(hotel.checkInLabel, /Aug 1, 2026/); assert.match(hotel.checkOutLabel, /Aug 3, 2026/); assert.equal(hotel.nights, 2); assert.equal(hotel.roomType, "Deluxe King Room"); assert.equal(hotel.actionKind, "provider-handoff"); assert.equal(hotel.href, "/redirect?id=hotel+%2F%3F%26%3D%E2%9C%93&type=hotel"); assert.notEqual(hotel.href, plan.hotel?.detailsPath);
  const car = steps[2]; assert.equal(car.product, "car"); if (car.product !== "car") return;
  assert.equal(car.pickupLocation, "LAX"); assert.equal(car.returnLocation, "Downtown"); assert.match(car.pickupLabel, /9:30 AM/); assert.equal(car.rentalDays, 2); assert.equal(car.category, "Compact Car"); assert.equal(car.actionKind, "internal-details"); assert.equal(car.href, plan.car?.detailsPath); assert.doesNotMatch(car.href ?? "", /\/redirect|type=car/);
  assert.deepEqual(plan, before); assert.doesNotMatch(JSON.stringify(steps), /booked|confirmed|completed/i);
});

test("cars without an internal details path remain unavailable", () => {
  const steps = getDealsHandoffSteps({ ...plan, car: { ...plan.car!, detailsPath: undefined } }, now, "en-US");
  const car = steps.find(step => step.product === "car");
