...
const plusDays = (days: number) => { const date = new Date(); date.setDate(date.getDate() + days); return iso(date); };
...
export const createDefaultDealsSearch = (): DealsSearch => ({
  ...
  flightDepartureDate: plusDays(14), flightReturnDate: plusDays(21), flightAdults: 1, flightChildren: 0, flightInfants: 0, flightCabinClass: "economy",
  hotelDestination: "", hotelCheckIn: plusDays(14), hotelCheckOut: plusDays(21), hotelAdults: 2, hotelChildren: 0, hotelRooms: 1, hotelPetFriendly: false,
  carPickupLocation: "", carReturnToDifferentLocation: false, carReturnLocation: "", carPickupDate: plusDays(14), carReturnDate: plusDays(21), carPickupTime: "10:00", carReturnTime: "10:00", carDriverAge: defaultDriverAge,
});
...
const date = (value: string, fallback: string) => isValidIsoDate(value) ? value : fallback;
...
export function parseDealsSearchParams(input: QueryInput): DealsSearch {
  ...
  return {
    ...
    flightDepartureDate: date(get(input, "flightDepartureDate"), defaults.flightDepartureDate), flightReturnDate: date(get(input, "flightReturnDate"), defaults.flightReturnDate),
    ...
    hotelDestination: get(input, "hotelDestination"), hotelCheckIn: date(get(input, "hotelCheckIn"), defaults.hotelCheckIn), hotelCheckOut: date(get(input, "hotelCheckOut"), defaults.hotelCheckOut),
    ...
    carPickupDate: date(get(input, "carPickupDate"), defaults.carPickupDate), carReturnDate: date(get(input, "carReturnDate"), defaults.carReturnDate),
...
export function validateFlightSearch(search: DealsSearch, today = todayIso()) {
  ...
  if (!isValidIsoDate(search.flightReturnDate) || search.flightReturnDate < search.flightDepartureDate) errors.flightReturnDate = "deals.error.flightDates";
  return errors;
}
...
export const getDealsSummaries = (search: DealsSearch) => ({
  flight: `${search.flightOriginCode}–${search.flightDestinationCode} · ${search.flightDepartureDate}–${search.flightReturnDate}`,
