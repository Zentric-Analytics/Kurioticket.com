...
export const dealsProductOrder = [
  "hotel",
  "flight",
  "car",
] as const satisfies readonly DealsProduct[];
...
export type DealsSearch = {
  ...
  flightOriginText: string;
  flightOriginCode: string;
  flightDestinationText: string;
  flightDestinationCode: string;
  flightDepartureDate: string;
  flightReturnDate: string;
  sharedDestination: string;
  sharedTravelStartDate: string;
  sharedTravelEndDate: string;
  flightAdults: number;
  ...
  carReturnToDifferentLocation: boolean;
  ...
  carDriverAge: string;
  stayDestinationLinked: boolean;
  stayDatesLinked: boolean;
  carPickupLinked: boolean;
  carDatesLinked: boolean;
};

export type DealsValidationErrors = Partial<
  Record<DealsProduct, Record<string, string>>
>;
type QueryInput =
  | URLSearchParams
...
export const isDealsPackageMode = (value: string): value is DealsPackageMode =>
  dealsPackageModes.includes(value as DealsPackageMode);
export const getIncludedProducts = (
  mode: DealsPackageMode,
): Record<DealsProduct, boolean> => ({
  flight: mode !== "hotel-car",
  hotel: mode !== "flight-car",
  car: mode !== "hotel-flight",
});
export const getIncludedProductList = (
  mode: DealsPackageMode,
): DealsProduct[] =>
  dealsProductOrder.filter((product) => getIncludedProducts(mode)[product]);

export function getDealsPackageModeForProducts(
  products: Iterable<DealsProduct>,
): DealsPackageMode | null {
  const selected = new Set(products);
  const key = dealsProductOrder
    .filter((product) => selected.has(product))
    .join("-");
  return isDealsPackageMode(key) ? key : null;
}

export function tryToggleDealsProduct(
  mode: DealsPackageMode,
  product: DealsProduct,
): DealsProductToggleResult {
  ...
  return nextMode
    ? { changed: true, mode: nextMode }
    : { changed: false, mode, reason: "minimum-products" };
}
...
export const createDefaultDealsSearch = (): DealsSearch => ({
  ...
  carPickupTime: "10:00",
  carReturnTime: "10:00",
  carDriverAge: defaultDriverAge,
  stayDestinationLinked: true,
  stayDatesLinked: true,
  carPickupLinked: true,
  carDatesLinked: true,
});
...
export const normalizeIataCode = (value: string) =>
...
export function parseDealsSearchParams(input: QueryInput): DealsSearch {
  ...
  const flightTripType =
    tripTypeValue === "one-way" ? "one-way" : defaults.flightTripType;
  ...
  const sharedAdults = integer(
    get(input, `${partyPrimary}Adults`) || get(input, `${partyFallback}Adults`),
    defaults.flightAdults,
  );
  const sharedChildren = integer(
    get(input, `${partyPrimary}Children`) ||
      get(input, `${partyFallback}Children`),
    defaults.flightChildren,
  );
  const explicitLink = (key: string, inferred: boolean) => {
    const value = get(input, key);
    return value === "true" ? true : value === "false" ? false : inferred;
  };
  const stayDestinationLinked = explicitLink(
    "stayDestinationLinked",
    !hotelDestination || hotelDestination === flightDestinationText,
  );
  const stayDatesLinked = explicitLink(
    "stayDatesLinked",
    (!hotelCheckIn && !hotelCheckOut) ||
      (hotelCheckIn === flightDepartureDate &&
        hotelCheckOut === parsedFlightReturnDate),
  );
  const carPickupLinked = explicitLink(
    "carPickupLinked",
    !carPickupLocation ||
      carPickupLocation === hotelDestination ||
      carPickupLocation === flightDestinationText,
  );
  const carDatesLinked = explicitLink(
    "carDatesLinked",
    (!carPickupDate && !carReturnDate) ||
      (carPickupDate === hotelCheckIn && carReturnDate === hotelCheckOut) ||
      (carPickupDate === flightDepartureDate &&
        carReturnDate === parsedFlightReturnDate),
  );
  const sharedDestination =
    get(input, "sharedDestination") ||
    (included.flight ? flightDestinationText : hotelDestination) ||
    (carPickupLinked ? carPickupLocation : "");
  const sharedTravelStartDate =
    date(get(input, "sharedTravelStartDate")) ||
    (included.flight ? flightDepartureDate : "") ||
    (included.hotel ? hotelCheckIn : "") ||
    (included.car ? carPickupDate : "");
  const sharedTravelEndDate =
    date(get(input, "sharedTravelEndDate")) ||
    (included.flight && flightTripType === "round-trip"
      ? parsedFlightReturnDate
      : "") ||
    (included.hotel && (stayDatesLinked || !included.flight)
      ? hotelCheckOut
      : "") ||
  ...
  return {
    mode,
    flightTripType,
    flightOriginText: get(input, "flightOriginText"),
    flightOriginCode: normalizeIataCode(get(input, "flightOriginCode")),
    flightDestinationText,
    flightDestinationCode: normalizeIataCode(
      get(input, "flightDestinationCode"),
    ),
    flightDepartureDate:
      included.flight && sharedTravelStartDate
        ? sharedTravelStartDate
        : flightDepartureDate,
    flightReturnDate:
      flightTripType === "one-way"
        ? ""
        : sharedTravelEndDate || parsedFlightReturnDate,
    sharedDestination,
    sharedTravelStartDate,
    sharedTravelEndDate,
    ...
    carReturnTime: timeOptions.includes(get(input, "carReturnTime"))
      ? get(input, "carReturnTime")
      : defaults.carReturnTime,
...
export const buildHotelResultsUrl = (search: DealsSearch) =>
  `/hotels/results?${new URLSearchParams({ destination: search.hotelDestination.trim(), checkIn: search.hotelCheckIn, checkOut: search.hotelCheckOut, guests: String(search.hotelAdults + search.hotelChildren), rooms: String(search.hotelRooms) })}`;
export const buildCarApiPayload = (search: DealsSearch): CarSearchParams => ({
  pickupLocation: search.carPickupLocation.trim(),
  dropoffLocation: search.carReturnToDifferentLocation
    ? search.carReturnLocation.trim()
    : search.carPickupLocation.trim(),
  ...
  driverAge: search.carDriverAge,
});
export const buildCarResultsUrl = (search: DealsSearch) =>
  `/cars/results?${new URLSearchParams(buildCarApiPayload(search))}`;
export const buildFlightApiPayload = (search: DealsSearch) => ({
  tripType: search.flightTripType,
  origin: normalizeIataCode(search.flightOriginCode),
  destination: normalizeIataCode(search.flightDestinationCode),
  departureDate: search.flightDepartureDate,
  ...(search.flightTripType === "round-trip"
    ? { returnDate: search.flightReturnDate }
    : {}),
  travelers: search.flightAdults + search.flightChildren + search.flightInfants,
  adults: search.flightAdults,
  children: search.flightChildren,
  infants: search.flightInfants,
  cabinClass: search.flightCabinClass,
});
export const buildHotelApiPayload = (search: DealsSearch) => ({
  destination: normalizeHotelDestinationSearchValue(search.hotelDestination),
  checkIn: search.hotelCheckIn,
  checkOut: search.hotelCheckOut,
  guests: search.hotelAdults + search.hotelChildren,
  rooms: search.hotelRooms,
});

export function validateFlightSearch(search: DealsSearch, today = todayIso()) {
  const errors: Record<string, string> = {};
  if (!normalizeIataCode(search.flightOriginCode))
    errors.flightOriginCode = "deals.error.flightOriginCode";
  if (!normalizeIataCode(search.flightDestinationCode))
    errors.flightDestinationCode = "deals.error.flightDestinationCode";
  const counts = [
    search.flightAdults,
  ...
  if (
    ...
    counts.reduce((a, b) => a + b, 0) > 9
  )
    errors.flightPassengers = "deals.error.flightPassengers";
  if (search.flightInfants > search.flightAdults)
    errors.flightInfants = "deals.error.flightInfants";
  if (
    !isValidIsoDate(search.flightDepartureDate) ||
    search.flightDepartureDate < today
  )
    errors.flightDepartureDate = "deals.error.flightDates";
  if (
    search.flightTripType === "round-trip" &&
    (!isValidIsoDate(search.flightReturnDate) ||
      search.flightReturnDate < search.flightDepartureDate)
  )
    errors.flightReturnDate = "deals.error.flightDates";
  return errors;
}
export function validateHotelSearch(search: DealsSearch, today = todayIso()) {
  const errors: Record<string, string> = {};
  if (search.hotelDestination.trim().length < 2)
    errors.hotelDestination = "deals.error.hotelDestination";
  const guests = search.hotelAdults + search.hotelChildren;
  if (
    ![search.hotelAdults, search.hotelChildren, search.hotelRooms].every(
      Number.isInteger,
    ) ||
    search.hotelAdults < 1 ||
    search.hotelChildren < 0 ||
    guests > 12
  )
    errors.hotelGuests = "deals.error.hotelGuests";
  if (
    !Number.isInteger(search.hotelRooms) ||
    search.hotelRooms < 1 ||
    search.hotelRooms > 6
  )
    errors.hotelRooms = "deals.error.hotelRooms";
  if (
    !isValidIsoDate(search.hotelCheckIn) ||
    !isValidIsoDate(search.hotelCheckOut) ||
    search.hotelCheckIn < today ||
    search.hotelCheckOut <= search.hotelCheckIn
  )
    errors.hotelDates = "deals.error.hotelDates";
  return errors;
}
export function toCarsFormValues(search: DealsSearch): CarsFormValues {
  return {
...
export const validateCarSearch = (search: DealsSearch, today = todayIso()) =>
  validateCarsForm(toCarsFormValues(search), today);
export function validateDealsSearch(
  search: DealsSearch,
  today = todayIso(),
): DealsValidationErrors {
  const included = getIncludedProducts(search.mode);
  const errors: DealsValidationErrors = {};
  if (included.flight) {
    const value = validateFlightSearch(search, today);
    if (Object.keys(value).length) errors.flight = value;
  }
  if (included.hotel) {
    const value = validateHotelSearch(search, today);
    if (Object.keys(value).length) errors.hotel = value;
  }
  if (included.car) {
    const value = validateCarSearch(search, today);
    if (Object.keys(value).length) errors.car = value;
  }
  return errors;
}
export const getDealsSummaries = (search: DealsSearch) => ({
  flight:
    ...
    (search.flightTripType === "one-way" || search.flightReturnDate)
      ? `${search.flightOriginCode}–${search.flightDestinationCode} · ${search.flightDepartureDate}${search.flightReturnDate ? `–${search.flightReturnDate}` : ""}`
      : "Travel dates",
  hotel:
    search.hotelCheckIn && search.hotelCheckOut
      ? `${search.hotelDestination} · ${search.hotelCheckIn}–${search.hotelCheckOut}`
      : "Check-in — Check-out",
  car:
