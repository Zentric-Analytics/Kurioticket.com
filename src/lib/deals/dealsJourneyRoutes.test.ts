...
test("expiry-aware correction chooses earliest included expired product but keeps Review", async () => {
  ...
  const plan = { ...base, expiresAt: DEALS_TRIP_PLAN_TTL_MS * 2, hotel: { id: "h", provider: "p", name: "h", location: "l", checkIn: "i", checkOut: "o", sourcePrice: 1, sourceCurrency: "USD", resultReceivedAt: 1 }, flight: { id: "f", provider: "p", airline: "a", origin: "o", destination: "d", departure: "d", arrival: "a", duration: "1h", sourcePrice: 1, sourceCurrency: "USD", resultReceivedAt: 2 }, car: { id: "c", provider: "p", rentalCompany: "r", modelName: "m", categoryLabel: "c", pickupLocation: "p", returnLocation: "r", pickupDate: "d", pickupTime: "t", dropoffDate: "d", dropoffTime: "t", sourcePrice: 1, sourceCurrency: "USD", resultReceivedAt: 3, detailsPath: "/cars/details/c?pickupLocation=p&dropoffLocation=r&pickupDate=d&pickupTime=t&dropoffDate=d&dropoffTime=t&driverAge=30" } };
...
test("plan expiry wins before the Review product-expiry exception in every mode", async () => {
  const { getRequiredDealsJourneyStageAt, getFirstDealsJourneyStage } = await import("./dealsJourneyRoutes");
  const { createDealsTripPlan } = await import("./dealsTripPlan");
  const ids = { hotelId: null, flightId: null, carId: null };
  for (const mode of ["hotel-flight", "hotel-car", "flight-car", "hotel-flight-car"] as const) {
    const base = createDealsTripPlan({ mode, searchFingerprint: "x", resultsPath: "/deals/results" }, 0);
    const hotel = { id: "h", provider: "p", name: "h", location: "l", checkIn: "i", checkOut: "o", sourcePrice: 1, sourceCurrency: "USD", resultReceivedAt: 1 };
    const flight = { id: "f", provider: "p", airline: "a", origin: "o", destination: "d", departure: "d", arrival: "a", duration: "1h", sourcePrice: 1, sourceCurrency: "USD", resultReceivedAt: 1 };
    const car = { id: "c", provider: "p", rentalCompany: "r", modelName: "m", categoryLabel: "c", pickupLocation: "p", returnLocation: "r", pickupDate: "d", pickupTime: "t", dropoffDate: "d", dropoffTime: "t", sourcePrice: 1, sourceCurrency: "USD", resultReceivedAt: 1, detailsPath: "/cars/details/c?pickupLocation=p&dropoffLocation=r&pickupDate=d&pickupTime=t&dropoffDate=d&dropoffTime=t&driverAge=30" };
    const plan = { ...base, ...(mode !== "flight-car" ? { hotel } : {}), ...(mode !== "hotel-car" ? { flight } : {}), ...(mode !== "hotel-flight" ? { car } : {}) };
    assert.equal(getRequiredDealsJourneyStageAt("review", mode, plan, ids, plan.expiresAt), getFirstDealsJourneyStage(mode));
  }
});
