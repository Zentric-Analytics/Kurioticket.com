...
import {
  ...
  getDealsReviewChangeHref,
  getDealsReviewItems,
  getDealsReviewStatus,
  getDealsReviewTotalPlan,
} from "./dealsReviewPresentation";
import {
...
const plan: DealsTripPlan = {
  ...
  flight: {
    ...
    origin: "LOS",
    destination: "LAX",
    departure: "2026-08-01T09:30",
    arrival: "2026-08-01T11:00",
    duration: "1h 30m",
    sourcePrice: 200,
    sourceCurrency: "EUR",
    resultReceivedAt: now,
  ...
  car: {
    ...
    resultReceivedAt: now,
    detailsPath:
      "/cars/details/c1?pickupLocation=LAX&dropoffLocation=LAX&pickupDate=2026-08-03&pickupTime=09%3A30&dropoffDate=2026-08-05&dropoffTime=09%3A30&driverAge=30",
  },
};
...
test("maps review products in canonical included order and omits excluded products", () => {
  assert.deepEqual(
    getDealsReviewItems(
      { ...plan, mode: "hotel-flight", car: undefined },
  ...
  assert.deepEqual(
    getDealsReviewItems(
      { ...plan, mode: "hotel-car", flight: undefined },
      { ...search, mode: "hotel-car" },
      now,
      "en-US",
    ).map((i) => i.product),
    ["hotel", "car"],
  ...
  assert.deepEqual(
    getDealsReviewItems(
      { ...plan, mode: "flight-car", hotel: undefined },
...
test("preserves stored product facts, prices, currencies, durations, and providers", () => {
  ...
  assert.equal(hotel.provenance, undefined);
  assert.equal(hotel.sourcePrice, 300);
  assert.equal(hotel.sourceCurrency, "USD");
  assert.equal(
    hotel.details.some((d) => d.value === "2"),
    true,
  );
  assert.equal(
    hotel.details.some((d) => d.value === "Deluxe King"),
    true,
  );
  assert.equal(flight.title, "Kuri Air");
  assert.equal(flight.provenance?.value, "Flight Provider");
  assert.equal(flight.subtitle, "LOS → LAX");
  assert.equal(
    flight.details.some((d) => d.value === "KT123"),
    true,
  );
  assert.equal(
    flight.details.some((d) => d.value === "1h 30m"),
    true,
  );
  assert.equal(car.title, "Corolla or similar");
  assert.match(car.subtitle, /Compact/);
  assert.equal(car.provenance, undefined);
  assert.equal(
    car.details.some((d) => d.value === "2"),
    true,
  );
});
...
test("computes completeness, freshness, change URLs, and handoff URL safely", () => {
  ...
  assert.deepEqual(
    getDealsReviewStatus({ ...plan, car: undefined }, now).missing,
    ["car"],
  );
  assert.match(
  ...
  assert.equal((href.match(/journey=guided/g) ?? []).length, 1);
  assert.doesNotMatch(href, /hotelId=|flightId=|carId=|redirect|bookingUrl/);
});
...
test("filters totals and freshness to the current package without mutating the plan", () => {
  ...
  const staleExtraCar = {
    ...
    car: { ...plan.car!, resultReceivedAt: now - DEALS_TRIP_PLAN_TTL_MS },
  };
  assert.equal(getDealsReviewStatus(staleExtraCar, now).canContinue, true);
  assert.notEqual(
    getDealsTripPlanEstimatedTotal(
      getDealsReviewTotalPlan(staleExtraCar),
      "USD",
      { EUR: 1.1, USD: 1 },
    ),
    null,
  ...
  const staleIncluded = {
    ...plan,
    flight: { ...plan.flight!, resultReceivedAt: now - DEALS_TRIP_PLAN_TTL_MS },
  };
...
test("conversion availability never changes continuation eligibility", () => {
  ...
  const available = getDealsTripPlanEstimatedTotal(
    getDealsReviewTotalPlan(included),
    "GBP",
    { USD: 1, EUR: 1.1, GBP: 0.8 },
  );
  ...
  assert.equal(getDealsReviewStatus(included, now).canContinue, true);
  const excludedRateMissing = {
    ...plan,
    mode: "hotel-flight" as const,
    car: { ...plan.car!, sourceCurrency: "XYZ" },
  };
  assert.notEqual(
    getDealsTripPlanEstimatedTotal(
      getDealsReviewTotalPlan(excludedRateMissing),
      "USD",
      { USD: 1, EUR: 1.1 },
    ),
    null,
  );
  const includedRateMissing = {
    ...included,
    flight: { ...plan.flight!, sourceCurrency: "XYZ" },
  };
  assert.equal(
    getDealsTripPlanEstimatedTotal(
      getDealsReviewTotalPlan(includedRateMissing),
      "USD",
      { USD: 1 },
    ),
    null,
