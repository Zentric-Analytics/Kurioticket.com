...
export type DealsTripPlanProduct = "flight" | "hotel";
...
export type DealsTripPlan = { version: 1; mode: DealsPackageMode; searchFingerprint: string; resultsPath: string; carsResultsPath?: string; createdAt: number; updatedAt: number; expiresAt: number; flight?: DealsTripPlanFlight; hotel?: DealsTripPlanHotel; opened: { flight?: number; hotel?: number; car?: number } };
...
export function getDealsTripPlanReadiness(mode: DealsPackageMode, plan: Pick<DealsTripPlan, "flight" | "hotel">) {
  ...
  return { ready: missing.length === 0, missing, guidanceKey: missing.length === 2 ? "deals.tripPlan.chooseBoth" : missing[0] === "flight" ? "deals.tripPlan.chooseFlight" : missing[0] === "hotel" ? "deals.tripPlan.chooseStay" : "deals.tripPlan.continue" };
}
