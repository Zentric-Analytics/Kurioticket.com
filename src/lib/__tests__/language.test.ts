...
test("Hindi account customization and booking preferences do not fall back to English", () => {
  const hi = getTranslations("hi");

  const expectedHindiPreferences: Array<[keyof typeof hi, string]> = [
  ...
  for (const [key, value] of expectedHindiPreferences) {
  ...
  for (const [key] of expectedHindiPreferences) {
    if (String(key).includes("customization") || String(key).includes("cancel") || String(key).includes("savePreferences")) {
      assert.ok(customizationSource.includes(String(key)), `Customization preferences render path should resolve ${String(key)} through i18n.`);
    }
    if (String(key).includes("booking") || String(key).includes("cancel") || String(key).includes("savePreferences")) {
      assert.ok(bookingSource.includes(String(key)), `Booking preferences render path should resolve ${String(key)} through i18n.`);
    }
  }

  assert.ok(
    customizationSource.includes('value: "English"') &&
      customizationSource.includes('value: "USD"') &&
      customizationSource.includes('value: "United States"') &&
      customizationSource.includes('name={field.id}') &&
      customizationSource.includes('action="#"'),
    "Customization preferences should keep selected values, field names, and form action unchanged.",
  );
  assert.ok(
    bookingSource.includes('name={field.id}') && bookingSource.includes('action="#"'),
    "Booking preferences should keep field names and form action unchanged.",
  );
});
