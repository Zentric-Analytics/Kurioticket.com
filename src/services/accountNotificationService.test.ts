...
test("material security events use transactional email and safe metadata", async () => {
  ...
  await recordAccountEvent({ userId: "user-1", email: "user@example.com", eventKey: "security:passkey-added:passkey-1", type: "SECURITY_UPDATE", title: "Passkey added", body: "A passkey was added.", actionPath: "/security", metadata: { passkeyId: "passkey-1" } });
...
test("follow-up notification failure does not throw after a successful mutation", async () => {
  ...
  assert.equal(await recordAccountEventSafely({ userId: "user-1", email: "user@example.com", eventKey: "security:2fa-enabled:transition-1", type: "SECURITY_UPDATE", title: "2FA enabled", body: "2FA was enabled.", actionPath: "/security" }), null);
});
