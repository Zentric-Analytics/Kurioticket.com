...
export const createSavedItemInputSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("car"), resultId: text, provider: text, modelName: text, categoryLabel: text, pickupLocation: text, dropoffLocation: text, pickupDate: calendarDate, pickupTime: clockTime, dropoffDate: calendarDate, dropoffTime: clockTime, driverAge: z.coerce.number().int().min(18).max(70), totalPrice: money, currency: z.string().length(3).transform(v => v.toUpperCase()), payload: json.default({}) }),
