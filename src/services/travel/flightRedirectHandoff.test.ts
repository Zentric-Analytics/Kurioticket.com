...
afterEach(() => {
  if (originalPartners === undefined)
    delete process.env.FLIGHT_HANDOFF_PARTNERS_JSON;
...
test("final handoff requires a server-allowlisted destination name", async () => {
  ...
  assert.deepEqual(
    await revalidateFlightRedirectHandoff({
  ...
  process.env.FLIGHT_HANDOFF_PARTNERS_JSON = JSON.stringify({
    "book.partner.test": "Example Partner",
  });
