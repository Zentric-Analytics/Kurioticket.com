...
export function getStaticHotelById(id: string) {
  ...
  return staticHotelCatalogue.find((hotel) => hotel.id === normalized || hotel.slug === normalized) ?? null;
}
...
export function buildStaticHotelResult(record: StaticHotelRecord, search: HotelSearchParams): NormalizedHotelResult {
  const stayNights = calculateHotelStayNights(search.checkIn, search.checkOut);
  const totalPrice = record.indicativeNightlyPrice * stayNights;
  const scores = scoreHotel({ totalPrice, rating: record.classificationStars, amenities: [...record.amenities], arrivalFriendly: record.searchTags.some((tag) => /central|transit/i.test(tag)) });
  return {
    id: record.id,
    ...
    cancellationInfo: "Live availability and booking terms are not currently offered.",
    taxesAndFeesIncluded: undefined,
    bookingUrl: "",
    partnerRedirectUrl: "",
    ...scores,
    recommendationReasons: ["Destination-relevant property for trip planning.", "Price is an indicative estimate for the selected stay."],
    badges: [],
    rawProviderReference: { catalogueId: record.id, latitude: record.latitude, longitude: record.longitude, lastReviewed: record.lastReviewed, pricingKind: "indicative", realTimeAvailability: false },
  };
}

export function buildStaticHotelResults(search: HotelSearchParams) {
  return searchStaticHotelCatalogue(search.destination).map((record) => buildStaticHotelResult(record, search));
}
