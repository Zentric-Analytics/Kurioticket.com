...
export async function searchHotels(search: HotelSearchParams): Promise<AggregatedResult<NormalizedHotelResult>> {
  ...
  return {results,providerStatuses:[provider],warnings:provider.status==="failed"?["Hotel availability is temporarily unavailable. Please try again."]:[],servedFromFallback:false,latencyMs:Date.now()-startedAt,...(provider.status!=="success"?{unavailableMessage:"Hotel availability is temporarily unavailable. Please try again."}:{})};
}
