...
const planningRooms = (
  ...
  {
    id: `${id}-classic-flex`,
    name: "Classic room — flexible planning",
    bedConfiguration: "One queen bed or two single beds",
    features: ["Standard room layout", "Wi-Fi"],
    mealPlan: "Room only",
    cancellationInfo:
      "Flexible planning terms; final terms are not yet confirmed.",
    taxesAndFeesIncluded: undefined,
    indicativeNightlyPrice: base,
  },
  {
    id: `${id}-classic-breakfast`,
    ...
    taxesAndFeesIncluded: undefined,
    indicativeNightlyPrice: Math.round(base * 1.12),
  },
  {
    id: `${id}-spacious-flex`,
    name: "Larger room — flexible planning",
    bedConfiguration: "One king bed",
    features: ["Additional room space", "Seating area"],
    mealPlan: "Room only",
    cancellationInfo:
      "Flexible planning terms; final terms are not yet confirmed.",
    taxesAndFeesIncluded: undefined,
    indicativeNightlyPrice: Math.round(base * 1.28),
  },
];

const staticHotelProperties: readonly Omit<StaticHotelRecord, "roomOptions">[] =
  [
    {
      ...common,
      id: "the-savoy-london",
...
// Options are declared deterministically from each catalogue property's stable identity and base estimate.
export const staticHotelCatalogue: readonly StaticHotelRecord[] =
  staticHotelProperties.map((hotel) => ({
    ...hotel,
    roomOptions: planningRooms(hotel.id, hotel.indicativeNightlyPrice),
  }));
