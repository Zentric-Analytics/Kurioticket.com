...
const EXPECTED = Object.freeze({ projectId: '89f6fd88-c0d7-495a-9e2b-8301b09f407d', package: 'com.kurioticket.app', profile: 'production', runtime: 'production-0.3.2', channel: 'production', api: 'https://kurioticket.com' });
...
export function validateProductionVersionEvidence(versionEvidence) {
  if (!versionEvidence || Array.isArray(versionEvidence) || typeof versionEvidence !== 'object') {
    throw new Error('Production version evidence is missing or malformed.');
  }
  const { remoteVersionStatus, currentRemoteVersionCode, proposedVersionCode, playRecordStatus, uploadedVersionCodes, highestUploadedVersionCode } = versionEvidence;
  if (!['absent', 'present'].includes(playRecordStatus) || !Array.isArray(uploadedVersionCodes)) {
    throw new Error('Production Play version evidence is internally inconsistent.');
  }
  if (uploadedVersionCodes.some((value) => !isPositiveSafeInteger(value)) || new Set(uploadedVersionCodes).size !== uploadedVersionCodes.length) {
    throw new Error('Production Play uploaded versionCodes are malformed.');
  }
  const reviewedMaximum = uploadedVersionCodes.length === 0 ? null : Math.max(...uploadedVersionCodes);
  if (highestUploadedVersionCode !== reviewedMaximum || (uploadedVersionCodes.length > 0 && playRecordStatus !== 'present')) {
    throw new Error('Production Play version evidence is internally inconsistent.');
  }
  if (remoteVersionStatus === 'uninitialized') {
    if (currentRemoteVersionCode !== null || proposedVersionCode !== 1 || uploadedVersionCodes.length !== 0 || highestUploadedVersionCode !== null) {
      throw new Error('Production uninitialized version evidence is internally inconsistent.');
    }
    return proposedVersionCode;
  }
  if (remoteVersionStatus !== 'configured') {
    throw new Error('Production version evidence has an unsupported remote state.');
  }
  if (!isPositiveSafeInteger(currentRemoteVersionCode) || !isPositiveSafeInteger(proposedVersionCode)) {
    throw new Error('Production configured EAS version evidence is malformed.');
  }
  if (proposedVersionCode !== currentRemoteVersionCode + 1) {
    throw new Error('Production proposed versionCode is not the next EAS version.');
  }
  if (highestUploadedVersionCode !== null && proposedVersionCode <= highestUploadedVersionCode) {
    throw new Error('Production proposed versionCode does not exceed reviewed Play history.');
  }
  return proposedVersionCode;
}
...
export function validateProductionDryRun({ approvedSha, headSha, mainContainsSha, versionEvidence, credential, workflow, policy, eas }) {
  const proposedVersionCode = validateProductionVersionEvidence(versionEvidence);
  const checks = [
  ...
  return { kind: 'dry-run', status: 'READY_TO_SUBMIT_PRODUCTION_BUILD', approvedSha, package: EXPECTED.package, projectId: EXPECTED.projectId, profile: EXPECTED.profile, runtime: EXPECTED.runtime, channel: EXPECTED.channel, proposedVersionCode, credentialResolution: 'reviewed-complete-and-frozen', submissionPerformed: false };
}
