...
export function verifyProductionIpa({ plist, publicConfig, expoConfig, provisioningProfile, certificateSerials }) {
  ...
  if (publicConfig?.ios?.bundleIdentifier !== releasePolicy.production.bundleIdentifier || publicConfig?.name !== releasePolicy.production.displayName) throw new Error("Resolved Production app identity mismatch.");
  if (publicConfig?.extra?.environment?.apiBaseUrl !== releasePolicy.production.apiBaseUrl || publicConfig?.extra?.environment?.isPreview !== false) throw new Error("Resolved Production environment mismatch.");
  if (publicConfig?.runtimeVersion !== releasePolicy.production.runtimeVersion || expoConfig?.EXUpdatesRuntimeVersion !== releasePolicy.production.runtimeVersion) throw new Error("IPA Production runtime mismatch.");
...
if (process.argv[1] && pathToFileURL(resolve(process.argv[1])).href === import.meta.url) {
  ...
  const evidence = verifyProductionIpa({
    plist,
    publicConfig: JSON.parse(readFileSync(args["public-config"], "utf8")),
