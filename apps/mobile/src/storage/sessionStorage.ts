...
import { publishSession } from "./sessionEvents";
...
export async function readSession(): Promise<StoredSession | null> {
  ...
  try {
    ...
    if (!session.token || new Date(session.expires) <= new Date()) { await removeInvalidSession(); return null; }
    return session;
  } catch { await removeInvalidSession(); return null; }
}
async function removeInvalidSession() { if (Platform.OS === "web") webStorage()?.removeItem(SESSION_KEY); else await SecureStore.deleteItemAsync(SESSION_KEY); publishSession(null); }
