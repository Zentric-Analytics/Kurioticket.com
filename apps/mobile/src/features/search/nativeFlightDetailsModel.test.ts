...
test("fare cards preserve accessible selection and authoritative content",()=>{const rail=fareRail();assert.match(rail,/accessibilityRole="radiogroup"/);assert.match(rail,/accessibilityLabel="Available fares"/);assert.match(rail,/details\.fareChoices\.map\(\(choice\)=>/);assert.match(rail,/<Pressable accessibilityRole="radio" accessibilityState=\{\{selected:isSelected\}\} onPress=\{\(\)=>setSelectedKey\(choice\.key\)\}/);assert.match(rail,/\{choice\.label\}/);assert.match(rail,/displayPrices\[choice\.key\]\?\.formatted/);assert.match(rail,/accessibilityLabel=\{displayPrices\[choice\.key\]\?\.accessibilityLabel\}/);assert.match(rail,/style=\{s\.fareSelectionControl\}>\s*<View style=\{s\.fareIdentity\}>/);assert.match(rail,/<Luggage size=\{15\}[^>]*>[\s\S]*?\{choice\.label\}|<Luggage size=\{15\}[^>]*\/>[\s\S]*?\{choice\.label\}/);assert.doesNotMatch(rail,/<Pressable[^>]*style=\{s\.fareSelectionControl\}>[\s\S]*?s\.farePrice/);assert.match(rail,/compactFareTerms\(choice\.distinguishingTerms,details\.search\.tripType,5\)/);assert.doesNotMatch(rail,/slice\(0,\s*3\)/);});
...
test("fare hierarchy centers identity, left-aligns benefits, then trails price in normal flow",()=>{const rail=fareRail();const styles=between("fareSectionTitle:","fareInfoDeck:");assert.match(styles,/fareCard:\{borderRadius:15,paddingHorizontal:12,paddingVertical:10,gap:5\}/);assert.doesNotMatch(styles,/fareCard:\{[^}]*(?:minHeight|height|marginHorizontal|flexGrow)/);assert.match(styles,/fareSelectionControl:\{alignSelf:"stretch",alignItems:"center"\}/);assert.match(styles,/fareBenefits:\{alignSelf:"stretch",alignItems:"flex-start",gap:4\}/);assert.doesNotMatch(styles,/fareBenefits:\{[^}]*(?:marginLeft|marginHorizontal|alignSelf:"center")/);assert.match(styles,/fareBenefitRow:\{maxWidth:"100%",flexDirection:"row",alignItems:"flex-start",gap:7\}/);assert.match(styles,/fareBenefitCopy:\{flexShrink:1,minWidth:0,gap:1\}/);assert.doesNotMatch(styles,/fareBenefitCopy:\{[^}]*flex:1/);assert.match(styles,/fareIdentity:\{flexDirection:"row",alignItems:"center",justifyContent:"center",gap:6\}/);assert.match(styles,/farePriceBlock:\{alignSelf:"flex-end",alignItems:"flex-end",gap:1,maxWidth:"100%"\}/);assert.doesNotMatch(styles,/farePriceBlock:\{[^}]*position:/);assert.match(styles,/farePriceLabel:\{[^}]*textAlign:"right"/);assert.match(styles,/farePrice:\{maxWidth:"100%"[^}]*textAlign:"right"\}/);assert.match(styles,/fareBenefitHeading:\{alignSelf:"flex-start"[^}]*gap:4\}/);assert.doesNotMatch(styles,/fareBenefitRow:\{[^}]*justifyContent:"space-between"/);assert.match(styles,/fareBenefitTitle:\{flexShrink:1/);assert.match(styles,/fareBenefitChevronExpanded:\{transform:\[\{rotate:"180deg"\}\]\}/);assert.match(rail,/\{fareTerms\.length\?<View style=\{s\.fareBenefits\}>[\s\S]*?<View style=\{s\.farePriceBlock\}><Text[^>]*>Total price<\/Text><Text[^>]*accessibilityLabel=\{displayPrices\[choice\.key\]\?\.accessibilityLabel\}[^>]*>\{displayPrices\[choice\.key\]\?\.formatted/);assert.match(rail,/width:loadedFareCardWidth/);});
