...
const cardWidth=(windowWidth:number,fareCount:number)=>{
  const availableWidth=windowWidth-36;
  if(fareCount<=1)return Math.min(394,Math.max(236,availableWidth));
  return Math.min(330,Math.max(236,availableWidth-54));
};
...
test("single fares use the available content width without carousel discovery space",()=>{
  assert.match(source,/nativeLoadedFareCardWidth = \(windowWidth: number, fareCount: number\)/);
  assert.match(source,/if \(fareCount <= 1\)/);
  ...
  for(const width of [320,360,375,390,430]){
    ...
    assert.equal(cardWidth(width,1),available,`${width}px single fare should fill its content width`);
  }
});

test("multiple fares retain a readable card and an intentional next-card reveal",()=>{
  assert.match(source,/availableWidth - nextFareReveal/);
  assert.match(source,/const nextFareReveal = 54/);
  assert.match(source,/faresMultiple:\{paddingRight:38\}/);
  for(const width of [320,360,375,390,430]){
    const available=width-36;
    const multiple=cardWidth(width,2);
    assert.ok(multiple>=236,`${width}px card remains usable`);
    assert.ok(multiple<available,`${width}px rail reveals another fare`);
    assert.ok(available-multiple>=48,`${width}px has a discoverable next-card clue`);
  }
});

test("fare hierarchy is restrained, content-driven, and keeps alternatives readable",()=>{const rail=fareRail();const styles=between("fareSectionTitle:","fareInfoDeck:");assert.match(styles,/fareSectionTitle:\{fontSize:18,lineHeight:23,fontWeight:"700"/);assert.match(styles,/fareCard:\{borderRadius:15,padding:15,gap:14\}/);assert.doesNotMatch(styles,/fareCard:\{[^}]*minHeight/);assert.match(styles,/fareCardSelected:\{borderWidth:1\.5\}/);assert.match(styles,/fareCardUnselected:\{borderWidth:1\}/);assert.match(styles,/farePrice:\{fontSize:22,lineHeight:27,fontWeight:"800"\}/);assert.match(styles,/fareLabel:\{fontSize:15,lineHeight:20,fontWeight:"700"\}/);assert.match(styles,/fareBenefitTitle:\{[^}]*fontSize:13[^}]*fontWeight:"600"/);assert.match(styles,/fareBenefitDetail:\{fontSize:12,lineHeight:17,fontWeight:"400"\}/);assert.match(rail,/backgroundColor:theme\.surface/);assert.doesNotMatch(rail,/backgroundColor:isSelected\?ui\.blue|opacity:/);assert.match(rail,/color:isSelected\?ui\.blue:theme\.textPrimary/);});
...
test("initial selected fare is positioned once without changing selection",()=>{const positioning=between("export const nativeInitialFareRailOffset", "export function nativeFlightEditSearchParams");assert.match(positioning,/selectedIndex <= 0 \|\| fareCount <= 1/);assert.match(positioning,/contentWidth - viewportWidth/);const rail=fareRail();assert.match(rail,/positionedFareSetRef\.current===fareSet/);assert.match(rail,/findIndex\(\(\{key\}\)=>key===selected\.key\)/);assert.match(rail,/scrollTo\(\{x:nativeInitialFareRailOffset/);assert.match(rail,/animated:false/);assert.equal((rail.match(/scrollTo\(/g)??[]).length,1);});
