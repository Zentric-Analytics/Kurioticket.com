...
test("Flight Result structure and SectionList controls remain intact", () => {
  ...
  assert.match(screen, /<Animated\.SectionList[\s\S]*?ListHeaderComponent=\{animatedFlightDateStrip\}[\s\S]*?renderSectionHeader[\s\S]*?renderItem[\s\S]*?<FlightResultsSummaryRow[\s\S]*?<FlightCard/);
