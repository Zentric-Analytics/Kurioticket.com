...
test("Flight Result structure and SectionList controls remain intact", () => {
  ...
  assert.match(screen, /<Animated\.SectionList[\s\S]*?ListHeaderComponent=\{status === "loading" \? \([\s\S]*?<FlightLoadingExperience[\s\S]*?\) : animatedFlightDateStrip\}[\s\S]*?renderSectionHeader[\s\S]*?<FlightResultsSummaryRow[\s\S]*?renderItem[\s\S]*?<FlightCard/);
