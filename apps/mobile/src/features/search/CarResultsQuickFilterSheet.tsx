...
type Props = { kind: Kind; results: CarResult[]; filters: SelectedCarFilters; sort: CarSort; displayCurrency: string; exchangeRates: ExchangeRates; onApplyFilters: (filters: SelectedCarFilters) => void; onApplySort: (sort: CarSort) => void; onClose: () => void };

export function CarResultsQuickFilterSheet({kind,results,filters,sort,displayCurrency,exchangeRates,onApplyFilters,onApplySort,onClose}:Props){
 const {theme}=useAppTheme(); const {locale,direction}=useMobileLocalization(); const copy=useMemo(()=>carFilterCopy(locale,displayCurrency,exchangeRates),[locale,displayCurrency,exchangeRates]);
 const group=useMemo(()=>visibleCarFilterGroups(results,displayCurrency,exchangeRates).find(item=>item.id===kind),[results,kind,displayCurrency,exchangeRates]);
 const sortOptions=useMemo<Array<{value:CarSort;label:string;description:string}>>(()=>[
  {value:"recommended",label:carText(locale,"carsResults.recommended","Recommended"),description:carText(locale,"carsResults.recommendedDescription","Best overall value first")},
  {value:"lowestTotal",label:carText(locale,"carsResults.totalPrice","Total price"),description:carText(locale,"carsResults.lowestTotalDescription","Lowest rental total first")},
  {value:"topRated",label:carText(locale,"carsResults.topRated","Top rated"),description:carText(locale,"carsResults.topRatedDescription","Highest supplier rating first")},
 ],[locale]);
 ...
 const title=kind==="sort"?carText(locale,"carsResults.sort","Sort"):group?carFilterGroupLabel(copy,group):copy.filters; const reset=()=>{mark();if(kind==="sort")setDraftSort("recommended");else setDraft([]);};
