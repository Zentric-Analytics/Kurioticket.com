...
test("quick Flight filters use a dark soft blur without a separate scrim",()=>{assert.match(shell,/transparent=\{!fullScreen\}/);assert.match(shell,/animationType=\{fullScreen \? "slide" : "none"\}/);assert.match(shell,/"overFullScreen"/);assert.match(shell,/BlurView/);assert.match(shell,/intensity=\{12\}/);assert.match(shell,/tint="dark"/);assert.match(shell,/Animated\.View/);assert.match(shell,/Animated\.timing\(quickBackdropOpacity/);assert.match(shell,/Animated\.timing\(quickSheetTranslateY/);assert.doesNotMatch(shell,/rgba\(15, 23, 42, 0\.35\)|styles\.scrim|backgroundColor:\s*"rgba\(/);assert.doesNotMatch(shell,/tint=\{theme\.dark \? "dark" : "light"\}|tint="light"/);assert.match(shell,/KeyboardAvoidingView/);assert.match(shell,/onRequestClose=\{onClose\}/);assert.match(shell,/onAccessibilityEscape=\{onClose\}/);assert.match(shell,/borderTopLeftRadius: 24/);assert.doesNotMatch(shell,/animationType="fade"|compactMenu|measureInWindow/);});
