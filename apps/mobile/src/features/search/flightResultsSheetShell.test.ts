...
test("quick Flight filters use a mild blur plus restrained dark scrim",()=>{assert.match(shell,/transparent=\{!fullScreen\}/);assert.match(shell,/animationType=\{fullScreen \? "slide" : "none"\}/);assert.match(shell,/"overFullScreen"/);assert.match(shell,/BlurView/);assert.match(shell,/intensity=\{4\}/);assert.match(shell,/tint="default"/);assert.match(shell,/quickBackdropScrim/);assert.match(shell,/rgba\(15, 23, 42, 0\.18\)/);assert.match(shell,/Animated\.View/);assert.match(shell,/Animated\.timing\(quickBackdropOpacity/);assert.match(shell,/Animated\.timing\(quickSheetTranslateY/);assert.doesNotMatch(shell,/intensity=\{12\}|tint="dark"|rgba\(15, 23, 42, 0\.35\)/);assert.doesNotMatch(shell,/tint=\{theme\.dark \? "dark" : "light"\}|tint="light"/);assert.match(shell,/KeyboardAvoidingView/);assert.match(shell,/onRequestClose=\{onClose\}/);assert.match(shell,/onAccessibilityEscape=\{onClose\}/);assert.match(shell,/borderTopLeftRadius: 24/);assert.doesNotMatch(shell,/animationType="fade"|compactMenu|measureInWindow/);});
