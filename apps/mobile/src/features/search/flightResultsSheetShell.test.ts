...
test("Flight quick filters use a mild blur plus restrained dark scrim while shared consumers can retain legacy backdrop",()=>{assert.match(shell,/transparent=\{!fullScreen\}/);assert.match(shell,/animationType=\{fullScreen \? "slide" : "none"\}/);assert.match(shell,/"overFullScreen"/);assert.match(shell,/type QuickBackdropVariant = "flight" \| "legacy"/);assert.match(shell,/quickBackdropVariant = "flight"/);assert.match(shell,/const flightQuickBackdrop = quickBackdropVariant === "flight"/);assert.match(shell,/const quickBackdropIntensity = flightQuickBackdrop \? 4 : 12/);assert.match(shell,/const quickBackdropTint = flightQuickBackdrop \? "default" : "dark"/);assert.match(shell,/intensity=\{quickBackdropIntensity\}/);assert.match(shell,/tint=\{quickBackdropTint\}/);assert.match(shell,/flightQuickBackdrop \? <View style=\{\[StyleSheet\.absoluteFill, styles\.quickBackdropScrim\]\}\/\> : null/);assert.match(shell,/rgba\(15, 23, 42, 0\.18\)/);assert.match(shell,/Animated\.View/);assert.match(shell,/Animated\.timing\(quickBackdropOpacity/);assert.match(shell,/Animated\.timing\(quickSheetTranslateY/);assert.match(shell,/KeyboardAvoidingView/);assert.match(shell,/onRequestClose=\{onClose\}/);assert.match(shell,/onAccessibilityEscape=\{onClose\}/);assert.match(shell,/borderTopLeftRadius: 24/);assert.doesNotMatch(shell,/animationType="fade"|compactMenu|measureInWindow/);});
