...
export function ApprovedCarResultsScreen() {
  ...
  const load = useCallback(async () => {
    if (!plan.plan) {
      setStatus("error");
      setMessage(plan.error || "Invalid car search");
      return;
    }
    ...
    setMessage("");
    try {
      const response = await travelApi.searchCars(plan.plan.payload, {
        signal: controller.signal,
        requestId,
      });
      if (controller.signal.aborted || sequence !== searchSequence.current) return;
      const acceptance = acceptCanonicalResults(response.results, safeCanonicalCarResult);
      if (acceptance.rejectedIds.length)
        console.warn("[travel-search] canonical car results failed client safety checks", {
          requestId: response.requestId,
          canonicalCount: acceptance.canonicalCount,
          acceptedCount: acceptance.accepted.length,
          rejectedIds: acceptance.rejectedIds,
        });
      setResults(acceptance.accepted);
      if (canonicalResultsWereSilentlyLost(acceptance)) {
        setStatus("error");
        setMessage("The canonical search returned inventory that this app could not render safely.");
      } else {
        setStatus(acceptance.accepted.length ? "ready" : "empty");
        setMessage(response.warnings?.[0] || "");
        void recordRecentSearchBestEffort(buildRecentSearch("car", plan.plan.payload));
      }
    } catch (error) {
      if (controller.signal.aborted || sequence !== searchSequence.current) return;
      setStatus("error");
      setMessage(error instanceof Error ? error.message : "Car search failed");
    }
  }, [plan.plan?.key, retry]);
  useEffect(() => {
    const executionKey = `${plan.plan?.key ?? "invalid"}:${retry}`;
  ...
  }, [load, plan.plan?.key, retry]);
  useEffect(() => {
    if (carResultsApplyingTimer.current) clearTimeout(carResultsApplyingTimer.current);
    carResultsApplyingTimer.current = undefined;
    setCarResultsApplying(false);
  }, [plan.plan?.key]);
  useEffect(
    () => () => {
      if (carResultsApplyingTimer.current) clearTimeout(carResultsApplyingTimer.current);
  ...
  const startCarResultsTransition = () => {
    if (carResultsApplyingTimer.current) clearTimeout(carResultsApplyingTimer.current);
    setCarResultsApplying(true);
    requestAnimationFrame(() => carScrollRef.current?.scrollTo({ y: 0, animated: true }));
    carResultsApplyingTimer.current = setTimeout(() => {
      carResultsApplyingTimer.current = undefined;
      setCarResultsApplying(false);
  ...
  const quickGroups = useMemo(
    () =>
      carQuickFilterGroupIds.flatMap((id) => {
        const group = filterGroups.find((candidate) => candidate.id === id);
        return group ? [group] : [];
      }),
    [filterGroups],
  );
  ...
  const pickup = String(payload.pickupDate || "");
  const dropoff = String(payload.dropoffDate || "");
  ...
  const openAllFilters = () => {
    carFilterSessionDirtyRef.current = false;
    setFilterSheetGroupId(null);
    setFilterSheetVisible(true);
  };
  const openQuickFilter = (groupId: string) => {
    carFilterSessionDirtyRef.current = false;
    setFilterSheetGroupId(groupId);
    setFilterSheetVisible(true);
  };
  const changeCarFilters = (next: SelectedCarFilters) => {
    carFilterSessionDirtyRef.current = true;
    setFilters(next);
  };
  const closeFilterSheet = () => {
    setFilterSheetVisible(false);
    if (carFilterSessionDirtyRef.current) {
      carFilterSessionDirtyRef.current = false;
      startCarResultsTransition();
    }
  };
  const openDeal = (result: CarResult) =>
    router.push({
  ...
  return (
    <SafeAreaView style={[r.safe, { backgroundColor: theme.background }]} edges={["top"]}>
      ...
      <ScrollView horizontal style={r.filterRail} showsHorizontalScrollIndicator={false} contentContainerStyle={r.filters}>
        ...
        {quickGroups.map((group) => (
          <Pill key={group.id} label={carFilterGroupLabel(copy, group)} active={(filters[group.id]?.length ?? 0) > 0} onPress={() => openQuickFilter(group.id)} />
        ))}
      </ScrollView>
      <ScrollView ref={carScrollRef} alwaysBounceVertical={false} bounces={false} overScrollMode="never" scrollEventThrottle={16} onScroll={handleCarScroll} contentContainerStyle={[r.body, { paddingBottom: Math.max(insets.bottom + 16, 16) }]}>
        {message ? (
          <Text accessibilityRole="alert" style={r.notice}>
            {message}
          </Text>
        ) : null}
        ...
        {status === "ready" ? (
          <>
            <NativeCarPriceAlert plan={plan.plan} results={results} available={availability.priceAlerts} />
            {carResultsApplying ? (
              <CarSkeletons />
            ) : (
              <>
                <View style={r.found}>
                  <Text style={[r.foundTitle, { color: theme.textPrimary }]}>{filtered.length} results found</Text>
                  <Pressable accessibilityRole="button" accessibilityLabel={`Sort by ${sort === "lowestTotal" ? "Total price" : "Recommended"}`} onPress={() => setSort((value) => (value === "lowestTotal" ? "recommended" : "lowestTotal"))} style={r.sort}>
                    <Text style={[r.sortPrefix, { color: theme.textSecondary }]}>Sort by:</Text>
                    <Text style={[r.sortValue, { color: theme.textPrimary }]}>{sort === "lowestTotal" ? "Total price" : "Recommended"}</Text>
                    <FlowIcon name="chevronDown" size={14} color={theme.icon} />
                  </Pressable>
                </View>
                {filtered.length ? (
                  <>
                    {filtered.map((result, index) => (
                      <CarResultCard key={result.id} result={result} rank={index} imageUri={image(result.imageUrl)} searchParams={payload} onViewDeal={() => openDeal(result)} />
                    ))}
                  </>
                ) : (
                  <Empty title="No cars match these filters" body="Clear filters to see the available rental cars." retry={clearFilters} retryLabel="Clear filters" edit={edit} />
                )}
              </>
            )}
          </>
      ...
      {carBackToTop ? (
        <Pressable
          ...
          onPress={() => carScrollRef.current?.scrollTo({ y: 0, animated: true })}
          style={[
            r.carBackToTop,
            {
      ...
      <CarFilterSheet visible={filterSheetVisible} groupId={filterSheetGroupId} results={results} filters={filters} onChange={changeCarFilters} onClose={closeFilterSheet} />
...
function CarResultsHeader({ destination, secondaryLine, onEdit }: { destination: string; secondaryLine: string; onEdit: () => void }) {
  const { theme } = useAppTheme();
  return (
    <View accessibilityLabel="Car search summary" style={[r.carHeader, { backgroundColor: theme.background }]}>
      <View style={r.carHeaderMainRow}>
        ...
        <Pressable
          accessibilityRole="button"
          accessibilityLabel={`Edit car search. ${destination}. ${secondaryLine}`}
          onPress={onEdit}
          style={({ pressed }) => [
            r.carSummaryCard,
            {
            ...
            pressed && r.carSummaryCardPressed,
          ]}
        >
          <View style={r.carSummaryText}>
            <Text numberOfLines={1} ellipsizeMode="tail" style={[r.carSummaryDestination, { color: theme.textPrimary }]}>
              {destination}
            </Text>
            <Text numberOfLines={1} ellipsizeMode="tail" style={[r.carSummarySecondary, { color: theme.textSecondary }]}>
              {secondaryLine}
            </Text>
          </View>
...
function CarSkeletons() {
  return (
    <View accessibilityRole="progressbar" accessibilityState={{ busy: true }} accessibilityLabel="Updating car results" style={r.skeletonGroup}>
      <View style={r.foundSkeleton} />
      {[0, 1, 2].map((key) => (
        <View key={key} style={r.skeleton}>
          <View style={r.skeletonImage} />
          <View style={r.skeletonLines}>
            <View style={r.skeletonLine} />
            <View style={[r.skeletonLine, { width: "65%" }]} />
            <View style={[r.skeletonLine, { width: "82%" }]} />
          </View>
        </View>
      ))}
    </View>
...
const r = StyleSheet.create({
  ...
  carHeaderControlPressed: { opacity: 0.55 },
  carSummaryCard: {
    flex: 1,
    minWidth: 0,
    minHeight: 64,
  ...
  carSummaryCardPressed: { opacity: 0.76 },
  carSummaryText: { flex: 1, minWidth: 0, justifyContent: "center" },
  carSummaryDestination: {
    fontSize: 16,
    lineHeight: 20,
    fontWeight: "700",
  ...
  filterRail: { height: 64, flexGrow: 0, flexShrink: 0 },
  filters: { paddingHorizontal: 16, paddingVertical: 10, gap: 8 },
  body: { paddingHorizontal: 10, gap: 14 },
  notice: {
    color: ui.navy,
  ...
  sortPrefix: { fontSize: 11, color: ui.muted },
  sortValue: { fontSize: 14, color: ui.navy },
  carPriceAlert: {
    width: "100%",
    minHeight: 52,
    borderRadius: 12,
  ...
  carPriceAlertCopy: { flex: 1, minWidth: 0 },
  carPriceAlertTitle: {
    fontSize: 12.5,
    lineHeight: 16,
    fontWeight: "700",
  ...
  carPriceAlertSwitchIos: { transform: [{ translateY: 8 }] },
  carBackToTop: {
    position: "absolute",
    right: 16,
    width: 44,
  ...
  foundSkeleton: { height: 44, borderRadius: 12, backgroundColor: "#EEF1F6" },
  skeleton: {
    height: 232,
    borderRadius: 13,
    borderWidth: 1,
  ...
  skeletonImage: { width: "40%", backgroundColor: "#EEF1F6" },
  skeletonLines: { flex: 1, padding: 16, gap: 15 },
  skeletonLine: {
    height: 13,
    borderRadius: 6,
