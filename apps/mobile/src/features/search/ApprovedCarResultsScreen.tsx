...
import { ActivityIndicator, Alert, KeyboardAvoidingView, Modal, Platform, Pressable, ScrollView, StyleSheet, Switch, Text, TextInput, View, type NativeScrollEvent, type NativeSyntheticEvent } from "react-native";
import { SafeAreaView, useSafeAreaInsets } from "react-native-safe-area-context";
import { router, useFocusEffect, useLocalSearchParams } from "expo-router";
import { ArrowLeft, ArrowUp, Bell, SquarePen } from "lucide-react-native";
...
function CarPriceAlert({plan,results}:{plan:NonNullable<ReturnType<typeof buildSearchPlan>["plan"]>;results:CarResult[]}) {
  const {theme}=useAppTheme(); const {availability}=useFeatureAvailability(); const presentation=useMemo(()=>carAlertPresentation("car",plan,results),[plan.key,results]);
  const [match,setMatch]=useState<MobilePriceAlert>(); const [busy,setBusy]=useState(false); const [open,setOpen]=useState(false); const [target,setTarget]=useState(""); const [error,setError]=useState(""); const currency=presentation.currencies[0]??"";
  const reconcile=useCallback(async()=>{if(!await readSession().catch(()=>null)){setMatch(undefined);return;}try{setMatch(matchingCarPriceAlert((await travelApi.priceAlerts()).alerts,plan));}catch{}},[plan.key]); useFocusEffect(useCallback(()=>{void reconcile();},[reconcile]));
  const toggle=async(next:boolean)=>{if(busy)return;if(!await readSession().catch(()=>null)){Alert.alert("Sign in required","Sign in to track prices for this search.");return;}if(next&&!match){if(availability.priceAlerts)setOpen(true);return;}setBusy(true);try{if(match)await travelApi.updatePriceAlertStatus(match.id,next?"ACTIVE":"PAUSED");await reconcile();}catch(e){setError(e instanceof TravelApiError?e.message:"Unable to update price alert.");}finally{setBusy(false);}};
  const create=async()=>{const value=Number(target);if(!/^\d+(?:\.\d{1,2})?$/.test(target.trim())||!Number.isFinite(value)||value<=0){setError("Enter a valid target rental total and try again.");return;}setBusy(true);setError("");try{await travelApi.createPriceAlert(buildCarPriceAlertPayload(plan,value,currency));await reconcile();setOpen(false);}catch(e){setError(e instanceof TravelApiError&&e.status===409?"This rental car alert already exists. Manage it in Price alerts.":e instanceof TravelApiError?e.message:"Unable to create price alert.");await reconcile();}finally{setBusy(false);}};
  if(!presentation.enabled)return null; const tracking=match?.status==="ACTIVE"; const disabled=busy||(!availability.priceAlerts&&!tracking);
  return <View accessibilityLabel="Rental car price alert" style={[r.carPriceAlert,{backgroundColor:theme.surface,borderColor:theme.priceAlertBorder}]}><Bell accessible={false} size={17} strokeWidth={2} color={theme.priceAlertAccent}/><View style={r.carPriceAlertCopy}><Text numberOfLines={1} style={[r.carPriceAlertTitle,{color:theme.textPrimary}]}>Track rental car prices</Text></View><View style={r.carPriceAlertSwitchSlot}>{busy?<ActivityIndicator size="small" color={theme.priceAlertAccent}/>:null}<Switch style={Platform.OS==="ios"?r.carPriceAlertSwitchIos:undefined} hitSlop={6} accessibilityRole="switch" accessibilityLabel="Track rental car prices" accessibilityState={{checked:tracking,disabled,busy}} disabled={disabled} value={tracking} onValueChange={(next)=>void toggle(next)} trackColor={{false:theme.dark?"#465269":"#CBD5E1",true:theme.switchTrackActive}} thumbColor="#FFFFFF" ios_backgroundColor={theme.dark?"#465269":"#CBD5E1"}/></View><Modal visible={open} transparent animationType="slide" onRequestClose={()=>!busy&&setOpen(false)}><KeyboardAvoidingView style={r.alertBackdrop} behavior={Platform.OS==="ios"?"padding":"height"}><View style={[r.alertSheet,{backgroundColor:theme.surface,borderColor:theme.border}]}><Text accessibilityRole="header" style={[r.alertTitle,{color:theme.textPrimary}]}>Track rental car prices</Text><Text style={{color:theme.textSecondary}}>Get notified when the lowest total for this complete rental search reaches your target.</Text><TextInput autoFocus accessibilityLabel={`Target rental total in ${currency}`} value={target} onChangeText={(value)=>{setTarget(value);setError("");}} keyboardType="decimal-pad" editable={!busy} placeholder={`Target rental total (${currency})`} style={[r.alertInput,{color:theme.textPrimary,borderColor:theme.border}]}/>{error?<Text accessibilityRole="alert" style={r.alertError}>{error}</Text>:null}<Pressable disabled={busy} onPress={()=>void create()} style={r.alertPrimary}><Text style={r.alertPrimaryText}>{busy?"Creating…":"Create alert"}</Text></Pressable><Pressable disabled={busy} onPress={()=>setOpen(false)} style={r.alertCancel}><Text style={{color:theme.textPrimary}}>Cancel</Text></Pressable></View></KeyboardAvoidingView></Modal></View>;
}
const r=StyleSheet.create({alertBackdrop:{flex:1,justifyContent:"flex-end",backgroundColor:"rgba(0,0,0,.35)"},alertSheet:{padding:20,gap:12,borderTopWidth:1,borderTopLeftRadius:20,borderTopRightRadius:20},alertTitle:{fontSize:20,fontWeight:"800"},alertInput:{minHeight:50,borderWidth:1,borderRadius:10,paddingHorizontal:12},alertError:{color:"#B4232C"},alertPrimary:{minHeight:48,borderRadius:10,backgroundColor:ui.blue,alignItems:"center",justifyContent:"center"},alertPrimaryText:{color:"white",fontWeight:"800"},alertCancel:{minHeight:44,alignItems:"center",justifyContent:"center"},safe:{flex:1,backgroundColor:"white"},carHeader:{paddingTop:12,paddingHorizontal:12,paddingBottom:12},carHeaderMainRow:{width:"100%",flexDirection:"row",alignItems:"center"},carHeaderSide:{width:52,flexShrink:0},carHeaderBack:{width:44,height:44,alignItems:"center",justifyContent:"center"},carHeaderControlPressed:{opacity:0.55},carSummaryCard:{flex:1,minWidth:0,minHeight:64,borderWidth:1,borderRadius:13,paddingLeft:16,flexDirection:"row",alignItems:"center",overflow:"hidden"},carSummaryCardPressed:{opacity:0.76},carSummaryText:{flex:1,minWidth:0,justifyContent:"center"},carSummaryDestination:{fontSize:16,lineHeight:20,fontWeight:"700",fontFamily:appFonts.bold},carSummarySecondary:{marginTop:3,fontSize:12.5,lineHeight:17,fontWeight:"600",fontFamily:appFonts.semibold},carSummaryEditSlot:{width:44,height:44,flexShrink:0,alignItems:"center",justifyContent:"center"},filterRail:{height:64,flexGrow:0,flexShrink:0},filters:{paddingHorizontal:16,paddingVertical:10,gap:8},body:{paddingHorizontal:10,gap:14},notice:{color:ui.navy,backgroundColor:"#F2F6FF",borderRadius:10,padding:12},found:{minHeight:44,flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:8,paddingHorizontal:6},foundTitle:{fontSize:16,fontWeight:"800",color:ui.navy},sort:{minHeight:44,flexDirection:"row",alignItems:"center",gap:5,paddingHorizontal:2},sortPrefix:{fontSize:11,color:ui.muted},sortValue:{fontSize:14,color:ui.navy},carPriceAlert:{width:"100%",minHeight:52,borderRadius:12,borderWidth:1,paddingHorizontal:12,paddingVertical:4,flexDirection:"row",alignItems:"center",gap:8},carPriceAlertCopy:{flex:1,minWidth:0},carPriceAlertTitle:{fontSize:12.5,lineHeight:16,fontWeight:"700",fontFamily:appFonts.bold},carPriceAlertSwitchSlot:{minWidth:51,minHeight:44,flexShrink:0,flexDirection:"row",alignItems:"center",justifyContent:"flex-end",gap:4},carPriceAlertSwitchIos:{transform:[{translateY:8}]},carBackToTop:{position:"absolute",right:16,width:44,height:44,borderRadius:22,borderWidth:1,alignItems:"center",justifyContent:"center",zIndex:19,elevation:4},skeletonGroup:{gap:14},foundSkeleton:{height:44,borderRadius:12,backgroundColor:"#EEF1F6"},skeleton:{height:232,borderRadius:13,borderWidth:1,borderColor:ui.border,overflow:"hidden",flexDirection:"row"},skeletonImage:{width:"40%",backgroundColor:"#EEF1F6"},skeletonLines:{flex:1,padding:16,gap:15},skeletonLine:{height:13,borderRadius:6,backgroundColor:"#EEF1F6",width:"90%"}});
