...
export function ApprovedCarResultsScreen() {
  ...
  return <SafeAreaView style={[r.safe,{backgroundColor:theme.background}]} edges={["top"]}>
    ...
    <ScrollView horizontal style={[r.filterRail,{backgroundColor:theme.dark?theme.surface:"#FFFFFF"}]} showsHorizontalScrollIndicator={false} alwaysBounceHorizontal={false} contentContainerStyle={r.filters}>
    ...
    <ScrollView ref={carScrollRef} alwaysBounceVertical={false} bounces={false} overScrollMode="never" scrollEventThrottle={16} onScroll={handleCarScroll} contentContainerStyle={[r.body,{paddingBottom:Math.max(insets.bottom + 16,16)}]}>
      ...
      {status==="ready"?<><NativeCarPriceAlert plan={plan.plan} results={results} available={availability.priceAlerts}/>{carResultsApplying?<CarSkeletons/>:<><View style={r.found}><Text style={[r.foundTitle,{color:theme.textPrimary}]}>{filtered.length} results found</Text></View>{filtered.length?<>{filtered.map((result,index)=><CarResultCard key={result.id} result={result} rank={index} imageUri={image(result.imageUrl)} searchParams={payload} onViewDeal={()=>openDeal(result)}/>)}</>:<Empty title="No cars match these filters" body="Clear filters to see the available rental cars." retry={clearFilters} retryLabel="Clear filters" edit={edit}/>}</>}</>:null}
    </ScrollView>
