...
export function FlightFilterSheet({visible,section,filters,options,results,priceValue,currency,priceFilteringReady,onChange,onClose,onComplete}:Props){
 ...
 const countLabel=(value:number)=>`${value} ${value===1?"flight":"flights"}`;
 const row=(field:"fromAirports"|"toAirports",v:string)=>{const insight=insights[field].get(v);return <Check key={field+v} label={v} trailing={insight?String(insight.count):undefined} selected={working[field].includes(v)} compact={!full} onPress={()=>toggle(field,v)}/>};
 ...
 {full&&(options.baggage||options.refundable)&&<FlightFilterSection title="Fare preferences">{options.baggage&&<Check label="Baggage included" selected={working.baggageIncluded} onPress={()=>update({...working,baggageIncluded:!working.baggageIncluded})}/>} {options.refundable&&<Check label="Flexible / refundable" selected={working.refundable} onPress={()=>update({...working,refundable:!working.refundable})}/>}</FlightFilterSection>}
 </ScrollView>
 </FlightResultsSheetShell>;
}

function Check({label,secondary,trailing,selected,logo,compact=false,onPress}:{label:string;secondary?:string;trailing?:string;selected:boolean;logo?:React.ReactNode;compact?:boolean;onPress:()=>void}){
 const {theme}=useAppTheme();
 const detail=[secondary,trailing].filter(Boolean).join(", ");
