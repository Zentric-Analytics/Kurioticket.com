...
export function FlightFilterSection({title,actions,children,compact=false,showTitle=true}:{title:string;actions?:React.ReactNode;children:React.ReactNode;compact?:boolean;showTitle?:boolean}){
 const {theme}=useAppTheme();
 return <View style={[s.section,compact&&s.compactSection]}>{showTitle?<View style={[s.sectionHead,compact&&s.compactSectionHead]}><Text style={[s.sectionTitle,compact&&s.compactSectionTitle,{color:theme.textPrimary}]}>{title}</Text>{actions}</View>:null}{children}</View>;
}
...
export function FlightFilterSheet({visible,section,filters,options,results,priceValue,currency,priceFilteringReady,onChange,onClose,onComplete}:Props){
 ...
 const priceLabel=(insight:FlightFilterInsight|undefined)=>insight?.lowestPrice==null?undefined:formatCurrency(insight.lowestPrice,insightCurrency);
 const insightDetail=(insight:FlightFilterInsight|undefined,price:string|undefined)=>insight?[countLabel(insight.count),price].filter(Boolean).join(", "):undefined;
 const row=(field:"fromAirports"|"toAirports",v:string)=>{const insight=insights[field].get(v),price=priceLabel(insight);return <Check key={field+v} label={v} trailing={full&&insight?String(insight.count):price} accessibilityDetail={insightDetail(insight,full?undefined:price)} selected={working[field].includes(v)} compact={!full} onPress={()=>toggle(field,v)}/>};
 ...
 {(full||section==="stops")&&<FlightFilterSection title={copy.stops} compact={!full} showTitle={full}>{([{v:"nonstop",l:copy.nonstop},{v:"one",l:copy.oneStop},{v:"twoPlus",l:copy.twoStops}] as const).map(x=>{const insight=insights.stops.get(x.v),price=priceLabel(insight);return <Check key={x.v} label={x.l} secondary={full&&insight?countLabel(insight.count):undefined} trailing={full&&price?copy.fromPrice(price):price} accessibilityDetail={full?undefined:insightDetail(insight,price)} selected={working.stops?.includes(x.v)??false} compact={!full} onPress={()=>toggleStop(x.v)}/>})}</FlightFilterSection>}
 {(full||section==="airlines")&&<FlightFilterSection title={copy.airlines} compact={!full} showTitle={full}><TextInput accessibilityLabel={copy.searchAirlines} placeholder={copy.searchAirlines} placeholderTextColor={theme.textSecondary} value={airlineSearch} onChangeText={setAirlineSearch} style={[s.search,{color:theme.textPrimary,backgroundColor:theme.surface,borderColor:theme.border}]}/>{visibleAirlines.map(name=>{const insight=insights.airlines.get(name),price=priceLabel(insight);return <Check key={name} label={name} trailing={full&&insight?String(insight.count):price} accessibilityDetail={insightDetail(insight,full?undefined:price)} selected={working.airlines.includes(name)} compact={!full} onPress={()=>toggle("airlines",name)}/>})}{!airlineSearch.trim()&&options.airlines.length>5?<Pressable accessibilityRole="button" onPress={()=>setShowAllAirlines(x=>!x)} style={s.showMore}><Text style={s.footerClearText}>{showAllAirlines?copy.showLess:copy.showMore}</Text></Pressable>:null}</FlightFilterSection>}
 {(full||section==="airports")&&(options.fromAirports.length>0||options.toAirports.length>0)&&<FlightFilterSection title={copy.airports} compact={!full} showTitle={full}>{options.fromAirports.length>0&&<><Text style={[s.subhead,!full&&s.compactSubhead,{color:theme.textPrimary}]}>{copy.from}</Text>{options.fromAirports.map(v=>row("fromAirports",v))}</>}{options.toAirports.length>0&&<><Text style={[s.subhead,!full&&s.compactSubhead,{color:theme.textPrimary}]}>{copy.to}</Text>{options.toAirports.map(v=>row("toAirports",v))}</>}</FlightFilterSection>}
...
const s=StyleSheet.create({
 ...
 compactRow:{minHeight:48,paddingHorizontal:10,paddingVertical:5,gap:10},
