...
export function NativeKayakCarDetailScreen() {
  ...
  useEffect(() => {
    ...
    if (!plan.plan || !resultId) { setStatus("unavailable"); return; }
    let active = true;
    void travelApi.searchCars(plan.plan.payload).then((response) => {
      if (!active) return;
...
function KayakCarDetailContent({ result, params }: { result: CarResult; params: Params }) {
  ...
  const search = {
    ...
    pickupDate: String(one(params.pickupDate) || ""),
    pickupTime: String(one(params.pickupTime) || ""),
    dropoffDate: String(one(params.dropoffDate) || ""),
    dropoffTime: String(one(params.dropoffTime) || ""),
    driverAge: String(one(params.driverAge) || ""),
  };
  const returnToResults = () => {
    if (carResultsStack) {
      const dismissCount = carResultsDismissCount(navigation.getState());
      if (dismissCount) { router.dismiss(dismissCount); return; }
    }
  ...
  return <SafeAreaView style={[s.safe, { backgroundColor: canvas }]} edges={["top"]}>
    <View style={[s.backHeader, { backgroundColor: canvas }]}>
      <Pressable accessibilityRole="button" accessibilityLabel="Back to Cars results" onPress={returnToResults} style={s.backLink}>
        <ArrowLeft size={18} color={accent} />
        <Text style={[s.backText, { color: accent }]}>Back to Cars results</Text>
      </Pressable>
    </View>
    <ScrollView style={{ backgroundColor: canvas }} contentContainerStyle={{ paddingBottom: Math.max(insets.bottom + 24, 24) }}>
      <View style={[s.card, { borderColor: theme.border, backgroundColor: informationSurface }]}>
        <View style={[s.imagePanel, { backgroundColor: theme.surface }]}>
          {image ? <Image source={{ uri: image }} resizeMode="cover" style={s.image} accessibilityLabel={result.imageAlt || result.modelName} /> : <View style={s.imageUnavailable}><CarFront size={48} color={theme.textSecondary} /><Text style={{ color: theme.textSecondary }}>Vehicle image unavailable</Text></View>}
        </View>
        <View style={s.information}>
          ...
          <View style={s.specGrid}>
            <Spec icon={<Users size={15} color="#64748B" />} label={specs.passengers} theme={theme} />
            <Spec icon={<DoorOpen size={15} color="#64748B" />} label={specs.doors} theme={theme} />
            <Spec icon={<FlowIcon name={result.transmission === "automatic" ? "transmissionAutomatic" : "transmissionManual"} size={15} color="#64748B" />} label={specs.transmission} theme={theme} />
            <Spec icon={<BriefcaseBusiness size={15} color="#64748B" />} label={specs.bags} theme={theme} />
          </View>
        </View>
      </View>
...
function KayakCarUnavailable() {
  const { theme } = useAppTheme();
  return <SafeAreaView style={[s.safe, { backgroundColor: theme.background }]}><View style={s.unavailable}><CarFront size={44} color={colors.blue} /><Text style={[s.dealHeading, { color: theme.textPrimary }]}>This KAYAK sandbox car is no longer available</Text><Text style={[s.disclosure, { color: theme.textSecondary }]}>Return to Cars results and refresh your search.</Text><Pressable accessibilityRole="button" onPress={() => router.back()} style={s.testAction}><Text style={s.testActionText}>Back to Cars results</Text></Pressable></View></SafeAreaView>;
}

const s = StyleSheet.create({
  safe: { flex: 1 },
  backHeader: { minHeight: 48, paddingHorizontal: 16, justifyContent: "center" },
  backLink: { minHeight: 44, alignSelf: "flex-start", flexDirection: "row", alignItems: "center", gap: 7 },
  backText: { fontSize: 14, lineHeight: 19, fontWeight: "700", fontFamily: appFonts.bold },
  card: { marginHorizontal: 14, borderWidth: 1, borderRadius: 13, overflow: "hidden" },
  imagePanel: { width: "100%", aspectRatio: 16 / 9 },
  image: { width: "100%", height: "100%" },
  imageUnavailable: { flex: 1, alignItems: "center", justifyContent: "center", gap: 8 },
  information: { paddingHorizontal: 16, paddingVertical: 14 },
  sandboxLabel: { fontSize: 10, lineHeight: 14, fontWeight: "800", fontFamily: appFonts.bold, letterSpacing: 0.7, textTransform: "uppercase", color: "#004BB8" },
  title: { marginTop: 5, fontSize: 20, lineHeight: 25, fontWeight: "800", fontFamily: appFonts.extraBold, letterSpacing: -0.4 },
  category: { marginTop: 2, fontSize: 10, lineHeight: 15, fontWeight: "800", fontFamily: appFonts.bold, letterSpacing: 1.2, textTransform: "uppercase", color: "#004BB8" },
  location: { marginTop: 10, flexDirection: "row", alignItems: "flex-start", gap: 6 },
  locationText: { flex: 1, fontSize: 12, lineHeight: 17, fontWeight: "500", fontFamily: appFonts.medium },
  specGrid: { marginTop: 14, flexDirection: "row", flexWrap: "wrap", rowGap: 10, columnGap: 8 },
  spec: { width: "48%", minWidth: 0, flexDirection: "row", alignItems: "flex-start", gap: 6 },
  specText: { flex: 1, minWidth: 0, fontSize: 12, lineHeight: 16, fontWeight: "500", fontFamily: appFonts.medium },
  dealCard: { marginHorizontal: 14, marginTop: 14, borderWidth: 1, borderRadius: 13, padding: 16 },
  dealHeading: { fontSize: 18, lineHeight: 24, fontWeight: "700", fontFamily: appFonts.bold },
  disclosure: { marginTop: 5, fontSize: 12, lineHeight: 18, fontWeight: "400", fontFamily: appFonts.regular },
  provider: { marginTop: 14, fontSize: 13, lineHeight: 18, fontWeight: "600", fontFamily: appFonts.semibold },
  priceRow: { marginTop: 8, alignItems: "flex-start" },
  price: { maxWidth: "100%", fontSize: 20, lineHeight: 24, fontWeight: "600", fontFamily: appFonts.semibold, fontVariant: ["tabular-nums"] },
  perDay: { fontSize: 11, lineHeight: 15, fontWeight: "400", fontFamily: appFonts.regular },
  testAction: { marginTop: 16, minHeight: 48, borderRadius: 8, backgroundColor: colors.blue, paddingHorizontal: 14, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7 },
  testActionText: { fontSize: 13, lineHeight: 18, fontWeight: "700", fontFamily: appFonts.bold, color: "white", textAlign: "center" },
  disabledAction: { opacity: 0.55 },
  pressed: { opacity: 0.82 },
  loading: { padding: 16, gap: 12 },
  loadingLine: { height: 36, borderRadius: 8 },
  loadingHero: { height: 360, borderRadius: 11 },
