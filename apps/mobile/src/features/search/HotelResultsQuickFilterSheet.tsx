...
type Props = {
  kind: HotelResultsQuickFilterKind;
...
export function HotelResultsQuickFilterSheet({ kind, filters, options, displayCurrency, rates, stayNights, onChange, onClose }: Props) {
  ...
  const initialMin = filters.minimumPrice ?? price?.minimum ?? 0,
    initialMax = filters.maximumPrice ?? price?.maximum ?? 0;
  ...
  const syncText = (next: { min: number; max: number }) => {
    markUpdating();
    setRange(next);
    setMinimumText(String(Math.round(toDisplay(next.min))));
    setMaximumText(String(Math.round(toDisplay(next.max))));
  };
  const commitText = (edge: "min" | "max", text: string) => {
    if (!price || !text.trim()) return syncText(range);
    const parsed = Number(text);
    if (!Number.isFinite(parsed)) return syncText(range);
    const usd = fromDisplay(parsed);
    if (usd === null || !Number.isFinite(usd)) return syncText(range);
    syncText(
      edge === "min"
        ? {
            min: Math.min(Math.max(price.minimum, usd), range.max),
            max: range.max,
          }
        : {
            min: range.min,
            max: Math.max(Math.min(price.maximum, usd), range.min),
          },
    );
  };
  const reset = () => {
    ...
    switch (kind) {
      case "price":
        if (price) syncText({ min: price.minimum, max: price.maximum });
        break;
      case "stars":
        setStars([]);
        break;
  ...
  const toggleStar = (star: HotelStarRating) => {
    markUpdating();
    setStars((current) => (current.includes(star) ? current.filter((value) => value !== star) : [...current, star]).sort((a, b) => b - a) as HotelStarRating[]);
  };
  const toggleFacility = (value: string) => {
    markUpdating();
    setFacilities((current) => (current.includes(value) ? current.filter((item) => item !== value) : [...current, value]));
  };
  const toggleRoomType = (value: string) => {
    markUpdating();
    setRoomTypes((current) => (current.includes(value) ? current.filter((item) => item !== value) : [...current, value]));
  };
  return (
    <Modal visible transparent animationType="slide" presentationStyle="overFullScreen" onRequestClose={onClose} accessibilityViewIsModal>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.bottom} pointerEvents="box-none">
          <View
            style={[
              styles.sheet,
              {
                maxHeight: Math.min(height * 0.76, 620),
          ...
          >
              ...
              {kind === "price" && price ? (
                <View style={styles.priceCard}>
                  ...
                  <View style={styles.inputs}>
                    <PriceInput label="Minimum" value={minimumText} onChange={setMinimumText} onEnd={() => commitText("min", minimumText)} />
                    <PriceInput label="Maximum" value={maximumText} onChange={setMaximumText} onEnd={() => commitText("max", maximumText)} />
                  </View>
                  <FlightRangeSlider available={{ min: price.minimum, max: price.maximum }} selected={range} step={25} accessibilityLabel="Estimated stay total range" formatValue={(value) => formatMarketCurrency(toDisplay(value), visibleCurrency)} onChange={syncText} />
                  <View style={styles.endpoints}>
                    <Text style={{ color: theme.textSecondary }}>{formatMarketCurrency(toDisplay(range.min), visibleCurrency)}</Text>
                    <Text style={{ color: theme.textSecondary }}>{formatMarketCurrency(toDisplay(range.max), visibleCurrency)}</Text>
                  </View>
                </View>
              ) : null}
            ...
            >
              <Pressable accessibilityRole="button" onPress={reset} style={[styles.reset, { borderColor: theme.border }]}>
              ...
              <Pressable
                ...
                style={styles.apply}
              >
                <View accessibilityLiveRegion="polite" style={styles.applyContent}>
                  {filterUpdating ? (
                    <>
                      <ActivityIndicator size="small" color="white" />
                      <Text style={[styles.buttonText, { color: "white" }]}>Updating filters…</Text>
                    </>
                  ) : (
...
function PriceInput({ label, value, onChange, onEnd }: { label: string; value: string; onChange: (v: string) => void; onEnd: () => void }) {
  const { theme } = useAppTheme();
  return (
    <View style={styles.inputColumn}>
      <Text style={[styles.inputLabel, { color: theme.textSecondary }]}>{label}</Text>
      <TextInput
        accessibilityLabel={`${label} estimated stay total`}
...
function Choice({ label, count, selected, onPress }: { label: string; count: number; selected: boolean; onPress: () => void }) {
  const { theme } = useAppTheme();
  return (
    <Pressable
      ...
      style={[
        styles.choice,
        {
          backgroundColor: selected ? "rgba(37, 99, 235, 0.05)" : "transparent",
          borderColor: selected ? ui.blue : theme.border,
          borderWidth: selected ? 1 : StyleSheet.hairlineWidth,
        },
      ]}
    >
      <Text style={[styles.choiceLabel, { color: selected ? ui.blue : theme.textPrimary }]}>{label}</Text>
      <Text style={[styles.count, { color: theme.textSecondary }]}>{count}</Text>
      {selected ? <Check size={18} color={ui.blue} /> : null}
    </Pressable>
  );
}
const styles = StyleSheet.create({
  ...
  copy: { flex: 1 },
  title: { fontSize: 18, fontFamily: appFonts.bold, fontWeight: "700" },
  subtitle: { fontSize: 12, lineHeight: 18, fontFamily: appFonts.medium },
  close: {
    width: 44,
    height: 44,
    alignItems: "center",
  ...
  priceCard: { paddingVertical: 4 },
  help: { fontSize: 13, marginBottom: 14, fontFamily: appFonts.medium },
  inputs: { flexDirection: "row", gap: 10, marginBottom: 12 },
  inputColumn: { flex: 1 },
  inputLabel: { fontSize: 12, fontFamily: appFonts.semibold, marginBottom: 5 },
  input: {
    height: 44,
    borderWidth: 1,
    borderRadius: 8,
  ...
  count: { fontSize: 13, fontFamily: appFonts.medium },
  empty: {
    paddingVertical: 20,
    textAlign: "center",
    fontFamily: appFonts.medium,
  ...
  buttonText: { fontSize: 15, fontFamily: appFonts.bold, fontWeight: "700" },
});
