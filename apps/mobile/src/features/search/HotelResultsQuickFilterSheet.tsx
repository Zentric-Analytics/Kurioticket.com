 ...
 return <Modal visible transparent animationType="slide" presentationStyle="overFullScreen" onRequestClose={onClose} accessibilityViewIsModal>
  <View style={styles.overlay} onAccessibilityEscape={onClose}>
   ...
   <KeyboardAvoidingView behavior={Platform.OS==="ios"?"padding":"height"} style={styles.bottom} pointerEvents="box-none">
    <View style={[styles.sheet,{maxHeight:Math.min(height*.76,620),backgroundColor:theme.surface}]}>
     ...
     <ScrollView style={styles.body} contentContainerStyle={[styles.content,kind!=="price"&&styles.chips]} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={kind==="facilities"||kind==="roomTypes"}>
      ...
      {kind==="stars"?([5,4,3,2,1] as HotelStarRating[]).filter(star=>options.starCounts[star]>0).map(star=><Choice key={star} label={`${star} ★`} count={options.starCounts[star]} selected={stars.includes(star)} onPress={()=>toggleStar(star)}/>):null}
...
const styles=StyleSheet.create({overlay:{flex:1,justifyContent:"flex-end"},backdropScrim:{backgroundColor:"rgba(15, 23, 42, 0.35)"},bottom:{justifyContent:"flex-end"},sheet:{width:"100%",borderTopLeftRadius:24,borderTopRightRadius:24,overflow:"hidden",shadowColor:"#0F172A",shadowOpacity:.2,shadowRadius:18,elevation:16},header:{minHeight:56,paddingLeft:54,paddingRight:54,flexDirection:"row",alignItems:"center",borderBottomWidth:StyleSheet.hairlineWidth},copy:{flex:1},title:{textAlign:"center",fontSize:18,fontFamily:appFonts.bold,fontWeight:"700"},subtitle:{fontSize:12,lineHeight:18,fontFamily:appFonts.medium},close:{position:"absolute",right:8,width:44,height:44,alignItems:"center",justifyContent:"center"},body:{flexShrink:1},content:{padding:16,gap:8},chips:{flexDirection:"column"},priceCard:{padding:0},help:{fontSize:13,marginBottom:14,fontFamily:appFonts.medium},inputs:{flexDirection:"row",gap:10,marginBottom:12},inputColumn:{flex:1},inputLabel:{fontSize:12,fontFamily:appFonts.semibold,marginBottom:5},input:{height:44,borderWidth:1,borderRadius:8,paddingHorizontal:10,fontFamily:appFonts.medium},endpoints:{flexDirection:"row",justifyContent:"space-between"},choice:{minHeight:44,width:"100%",borderWidth:1,borderRadius:9,paddingHorizontal:12,flexDirection:"row",alignItems:"center",gap:10},choiceLabel:{flex:1,fontSize:14,fontFamily:appFonts.semibold},count:{fontSize:13,fontFamily:appFonts.medium},empty:{paddingVertical:20,textAlign:"center",fontFamily:appFonts.medium},footer:{flexDirection:"row",gap:10,paddingHorizontal:16,paddingTop:12,borderTopWidth:StyleSheet.hairlineWidth},reset:{minWidth:90,height:49,borderWidth:1,borderRadius:12,alignItems:"center",justifyContent:"center"},apply:{flex:1,height:49,borderRadius:12,backgroundColor:ui.blue,alignItems:"center",justifyContent:"center"},buttonText:{fontSize:15,fontFamily:appFonts.bold,fontWeight:"700"}});
