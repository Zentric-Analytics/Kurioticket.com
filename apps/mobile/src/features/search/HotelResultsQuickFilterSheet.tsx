...
export function HotelResultsQuickFilterSheet({kind,sort,filters,options,displayCurrency,rates,stayNights,onSortChange,onChange,onClose}:Props){
 ...
 const reset=()=>{switch(kind){case "sort":setSortMode(defaultHotelSort);break;case "price":if(price)syncText({min:price.minimum,max:price.maximum});break;case "stars":setStars([]);break;case "facilities":setFacilities([]);break;case "roomTypes":setRoomTypes([]);break;}};
