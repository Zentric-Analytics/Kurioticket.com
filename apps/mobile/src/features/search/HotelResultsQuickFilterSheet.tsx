...
export function HotelResultsQuickFilterSheet({kind,sort,filters,options,displayCurrency,rates,stayNights,onSortChange,onChange,onClose}:Props){
 ...
 const reset=()=>{if(kind!=="sort")markUpdating();switch(kind){case "sort":setSortMode(defaultHotelSort);break;case "price":if(price)syncText({min:price.minimum,max:price.maximum});break;case "stars":setStars([]);break;case "facilities":setFacilities([]);break;case "roomTypes":setRoomTypes([]);break;}};
   ...
   <KeyboardAvoidingView behavior={Platform.OS==="ios"?"padding":"height"} style={styles.bottom} pointerEvents="box-none">
    <View style={[styles.sheet,{maxHeight:Math.min(height*.76,620),backgroundColor:theme.background}]}>
     ...
     <ScrollView style={styles.body} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={kind==="facilities"||kind==="roomTypes"}>
      {kind==="sort"?<View accessibilityRole="radiogroup" style={styles.sortChoices}>{hotelSortOptions.map(option=><Choice key={option.value} label={option.label} selected={sortMode===option.value} accessibilityRole="radio" onPress={()=>setSortMode(option.value)}/>)}</View>:null}
