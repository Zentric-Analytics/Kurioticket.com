...
test("Flight dates scroll naturally before the platform-specific pinned rail",()=>{assert.match(s,/ListHeaderComponent=\{hasFlightDateStrip \?[\s\S]*\{flightDateStrip\}[\s\S]*renderSectionHeader/);assert.match(s,/stickySectionHeadersEnabled=\{Platform\.OS !== "android"\}/);});
