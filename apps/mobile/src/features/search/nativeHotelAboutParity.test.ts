...
test("native About owns web-aligned geometry, typography, rhythm, and dark-mode colors", () => {
  ...
  for (const rule of [/fontSize: 20/, /lineHeight: 28/, /fontWeight: "800"/, /appFonts\.extraBold/]) assert.match(heading, rule);
  for (const rule of [/marginTop: 12/, /fontSize: 14/, /lineHeight: 24/, /fontWeight: "400"/, /appFonts\.regular/]) assert.match(description, rule);
  for (const rule of [/marginTop: 28/, /fontSize: 16/, /lineHeight: 24/, /fontWeight: "600"/, /appFonts\.semibold/]) assert.match(subheading, rule);
  for (const rule of [/minHeight: 56/, /borderRadius: 12/, /paddingHorizontal: 12/, /paddingVertical: 10/, /gap: 8/]) assert.match(highlight, rule);
  for (const rule of [/fontSize: 14/, /lineHeight: 20/, /fontWeight: "500"/, /appFonts\.medium/]) assert.match(highlightText, rule);
  for (const rule of [/fontWeight: "400"/, /appFonts\.regular/]) assert.match(infoText, rule);
  for (const rule of [/fontWeight: "400"/, /appFonts\.regular/]) assert.match(accessibilityText, rule);
