...
test("web retains its inset dialog and flex-stage gallery viewer contract", () => {
  ...
  assert.match(webDialog, /fixed inset-0[^\"]*h-\[100dvh\][^\"]*bg-slate-950\/90/);
  assert.match(webDialog, /p-\[max\(0\.75rem,env\(safe-area-inset-top\)\)_max\(0\.75rem,env\(safe-area-inset-right\)\)_max\(0\.75rem,env\(safe-area-inset-bottom\)\)_max\(0\.75rem,env\(safe-area-inset-left\)\)\]/);
  assert.match(webDialog, /relative flex min-h-0 w-full[^\"]*flex-col[^\"]*overflow-hidden[^\"]*rounded-2xl bg-slate-950/);
  assert.match(webDialog, /flex shrink-0 items-center justify-between gap-3 px-2 py-2/);
...
test("NativeHotelGallery owns a full-screen viewer modal and shared selection", () => {
  ...
  assert.match(gallery, /<Modal[\s\S]*?visible=\{viewerOpen\}[\s\S]*?animationType="fade"[\s\S]*?transparent[\s\S]*?presentationStyle="overFullScreen"[\s\S]*?onRequestClose=\{closeViewer\}/);
  ...
  assert.doesNotMatch(gallery.match(/const closeViewer[\s\S]*?;/)?.[0] ?? "", /setActiveUrl|setActiveIndex/);
});

test("native viewer owns a dim backdrop and rounded inner dialog", () => {
  assert.match(gallery, /style=\{\[s\.viewerBackdrop,/);
  assert.match(gallery, /<View style=\{s\.viewerDialog\}>/);
  assert.doesNotMatch(gallery, /s\.viewer(?:,|\])/);
  assert.match(styleRule("viewerBackdrop", "viewerDialog"), /flex: 1[\s\S]*backgroundColor: "rgba\(2,6,23,\.90\)"[\s\S]*alignItems: "stretch"[\s\S]*justifyContent: "center"/);
  assert.match(styleRule("viewerDialog", "viewerHeader"), /flex: 1[\s\S]*minHeight: 0[\s\S]*width: "100%"[\s\S]*borderRadius: 16[\s\S]*overflow: "hidden"[\s\S]*backgroundColor: "#020617"/);
});
...
test("native viewer applies safe minimum outer insets and pages at dialog width", () => {
  ...
  assert.match(gallery, /const viewerInsetRight = Math\.max\(insets\.right, 12\)/);
  assert.match(gallery, /const viewerInsetBottom = Math\.max\(insets\.bottom, 12\)/);
  assert.match(gallery, /const viewerInsetLeft = Math\.max\(insets\.left, 12\)/);
  assert.match(gallery, /const viewerWidth = viewportWidth - viewerInsetLeft - viewerInsetRight/);
  assert.match(gallery, /paddingTop: viewerInsetTop[\s\S]*paddingRight: viewerInsetRight[\s\S]*paddingBottom: viewerInsetBottom[\s\S]*paddingLeft: viewerInsetLeft/);
...
test("header and flex-driven stage match web geometry", () => {
  assert.match(styleRule("viewerHeader", "viewerTitle"), /flexShrink: 0[\s\S]*gap: 12[\s\S]*paddingHorizontal: 8[\s\S]*paddingVertical: 8/);
  assert.doesNotMatch(styleRule("viewerHeader", "viewerTitle"), /minHeight: 56/);
  assert.match(styleRule("viewerStage", "viewerPager"), /flex: 1[\s\S]*minHeight: 0[\s\S]*position: "relative"/);
  assert.match(styleRule("viewerPager", "viewerPage"), /flex: 1/);
  assert.match(styleRule("viewerPage", "viewerImage"), /flex: 1[\s\S]*height: "100%"/);
  assert.match(styleRule("viewerImage", "viewerArrow"), /width: "100%"[\s\S]*height: "100%"/);
  const viewerStyles = native.slice(native.indexOf("  viewerBackdrop:"), native.indexOf("  modalBackdrop:"));
  assert.doesNotMatch(viewerStyles, /aspectRatio|maxHeight|height: (?:400|450)|viewportHeight/);
});

test("counter belongs to the stage and thumbnails anchor at the dialog bottom", () => {
  const stageMarkup = gallery.slice(gallery.indexOf("<View style={s.viewerStage}>"), gallery.indexOf("style={s.viewerThumbnailScroller}"));
  assert.match(stageMarkup, /style=\{s\.viewerCounter\}/);
  assert.match(styleRule("viewerCounter", "viewerThumbnailScroller"), /position: "absolute"[\s\S]*bottom: 8/);
  assert.match(styleRule("viewerThumbnailScroller", "viewerThumbnailStrip"), /flexGrow: 0[\s\S]*flexShrink: 0/);
  assert.match(styleRule("viewerThumbnailStrip", "viewerThumbnailFrame"), /gap: 8[\s\S]*paddingHorizontal: 8[\s\S]*paddingVertical: 12/);
...
test("viewer pages are virtualized over every filtered image and use contain", () => {
  assert.match(gallery, /<FlatList[\s\S]*?ref=\{viewerScroll\}[\s\S]*?horizontal[\s\S]*?pagingEnabled[\s\S]*?data=\{images\}/);
  ...
  assert.match(gallery, /removeClippedSubviews=\{Platform\.OS === "android"\}/);
