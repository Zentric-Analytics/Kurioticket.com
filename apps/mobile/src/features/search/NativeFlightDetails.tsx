...
import { AccessibilityInfo, Alert, Animated, Easing, ImageBackground, Linking, Pressable, ScrollView, Share, StyleSheet, Text, useWindowDimensions, View } from "react-native";
...
export function NativeFlightDetails({ params }: { params: Params }) {
  ...
  return <SafeAreaView edges={[]} style={[s.safe,{backgroundColor:contentCanvasColor}]}><StatusBar style="light" translucent backgroundColor="transparent"/><View testID="flight-details-floating-controls" style={[s.heroControls,s.floatingControls,{top:inset.top+8}]}><Pressable accessibilityRole="button" accessibilityLabel="Back to results" onPress={()=>router.back()} style={s.heroIconButton}><ArrowLeft size={20} color="#0F172A"/></Pressable><View style={s.heroActions}><IconButton label={saved?"Remove saved flight":"Save flight"} onPress={()=>savedFlights.toggle(savedOffer,nativeFlightEditSearchParams(details,one(params.currency)))} iconStyle={s.heroHeartIcon}><Heart size={18} color={saved ? androidFavoriteColors.savedStroke : androidFavoriteColors.unsavedStroke} fill={saved?androidFavoriteColors.savedFill:androidFavoriteColors.unsavedFill}/></IconButton><IconButton label="Share flight" onPress={()=>void share()} iconStyle={s.heroShareIcon}><FlowIcon name="share" size={18} color="#0F172A"/></IconButton></View></View><ScrollView testID="flight-details-scroll-content" style={{backgroundColor:contentCanvasColor}} contentContainerStyle={[s.content,{paddingBottom:120+inset.bottom}]}>
    ...
    <View testID="flight-details-itinerary-overlap" style={s.itineraryStack}>{(offer.legs?.length?offer.legs:[]).map((leg,index)=><Itinerary key={`${leg.departureTime}-${index}`} leg={leg} index={index} heroOverlap={index===0} offerAirlineName={offer.airlineName} offerAirlineLogo={offer.airlineLogo} theme={theme} intlLocale={intlLocale}/>)}</View>
    ...
    <ScrollView testID="flight-details-loading-scroll" style={{backgroundColor:contentCanvasColor}} contentContainerStyle={[s.content,{paddingBottom:120+bottomInset}]}>
      <View testID="flight-details-loading-hero" style={[s.hero,{paddingTop:topInset+64,backgroundColor:theme.dark?"#27272A":"#E2E8F0"}]}>
        <Animated.View testID="flight-details-loading-copy" pointerEvents="none" accessibilityElementsHidden importantForAccessibility="no-hide-descendants" style={[s.heroCopy,{opacity}]}>
          ...
          <View testID="flight-details-loading-city-route" style={[s.loadingLine,s.loadingCityRouteLine,placeholder]}/>
        </Animated.View>
      ...
      <Animated.View testID="flight-details-loading-body" pointerEvents="none" accessibilityElementsHidden importantForAccessibility="no-hide-descendants" style={[s.contentBody,{opacity}]}>
        <View testID="flight-details-loading-itinerary-overlap" style={s.itineraryStack}>
          <View testID="flight-details-loading-itinerary" style={[s.itineraryCard,s.loadingItineraryCard,{backgroundColor:"transparent",borderColor:theme.border}]}>
            <ItineraryHeroSurface color={theme.surface}/>
...
function ItineraryHeroSurface({color}:{color:string}) { return <><Svg pointerEvents="none" style={s.itineraryHeroCurveSurface} viewBox="0 0 100 64" preserveAspectRatio="none"><Path d="M0 12 Q50 64 100 12 L100 64 L0 64 Z" fill={color}/></Svg><View pointerEvents="none" style={[s.itineraryHeroLowerSurface,{backgroundColor:color}]}/></>; }
...
function Itinerary({leg,index,heroOverlap,offerAirlineName,offerAirlineLogo,theme,intlLocale}:{leg:FlightLeg;index:number;heroOverlap:boolean;offerAirlineName:string;offerAirlineLogo?:string|null;theme:ReturnType<typeof useAppTheme>["theme"];intlLocale:string}) {
  ...
  return <View style={[s.itineraryCard,{backgroundColor:heroOverlap?"transparent":theme.surface,borderColor:theme.border}]}>
    {heroOverlap?<ItineraryHeroSurface color={theme.surface}/>:null}
