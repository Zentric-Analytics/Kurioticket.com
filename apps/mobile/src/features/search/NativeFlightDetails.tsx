    ...
    {displayPricesReady?<ScrollView accessibilityRole="radiogroup" accessibilityLabel="Available fares" horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[s.fares,details.fareChoices.length>1?s.faresMultiple:s.faresSingle]}>
      {details.fareChoices.map((choice)=>{
        ...
        return <View key={choice.key} style={[s.fareCard,{width:loadedFareCardWidth,backgroundColor:isSelected?(theme.dark?FLIGHT_DETAILS_DARK_SELECTED:FLIGHT_DETAILS_LIGHT_SELECTED):theme.surface,borderColor:isSelected?ui.blue:surfaceBorderColor},isSelected?s.fareCardSelected:s.fareCardUnselected,isSelected?(theme.dark?s.fareCardSelectedDark:s.fareCardSelectedLight):(theme.dark?s.fareCardUnselectedDark:s.fareCardUnselectedLight)]}>
          ...
          <View accessible={false} accessibilityElementsHidden importantForAccessibility="no-hide-descendants" pointerEvents="none" style={[s.farePriceBlock,{borderTopColor:isSelected?(theme.dark?"#315A8F":"#BDD5F3"):surfaceBorderColor}]}><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8} style={[s.farePrice,{color:fareHeadingTextColor}]}>{displayPrices[choice.key]?.formatted??"Price unavailable"}</Text></View>
        </View>;
      })}
...
function FlightDetailsLoadingSkeleton({theme,topInset,bottomInset,fareCardWidth,viewportWidth}:{theme:ReturnType<typeof useAppTheme>["theme"];topInset:number;bottomInset:number;fareCardWidth:number;viewportWidth:number}) {
  ...
  return <SafeAreaView edges={[]} style={[s.safe,{backgroundColor:contentCanvasColor}]}>
    ...
    <View testID="flight-details-loading-controls" style={[s.heroControls,s.floatingControls,Platform.OS==="android"&&s.androidFlatControlLayer,{top:topInset+8}]}>
...
function ItineraryGlossSurface({dark,testID}:{dark:boolean;testID:string}) {
  ...
  return <Svg testID={testID} pointerEvents="none" accessible={false} style={s.itineraryGlossSurface} viewBox="0 0 100 100" preserveAspectRatio="none">
    <Defs><LinearGradient id={`${testID}-gradient`} x1="0" y1="0" x2="0" y2="1"><Stop offset="0" stopColor="#FFFFFF"/><Stop offset="0.42" stopColor="#FFFFFF"/><Stop offset="1" stopColor={FLIGHT_DETAILS_LIGHT_ITINERARY_SURFACE}/></LinearGradient></Defs>
    <Rect width="100" height="100" rx="14" fill={`url(#${testID}-gradient)`}/>
  </Svg>;
}
