...
export function NativeFlightDetails({ params }: { params: Params }) {
  ...
  const offer=selected.offer; const provider=selectedDeal?.providerName??(selected.handoff.available ? selected.handoff.providerName : "provider");
  ...
  return <SafeAreaView edges={["top"]} style={[s.safe,{backgroundColor:theme.background}]}><TopBar backgroundColor={theme.background} hasScrolled={hasScrolled}/><ScrollView testID="flight-details-scroll-content" scrollEventThrottle={16} onScroll={({nativeEvent})=>{const next=nativeEvent.contentOffset.y>1;if(next!==hasScrolledRef.current){hasScrolledRef.current=next;setHasScrolled(next);}}} contentContainerStyle={[s.content,{paddingBottom:120+inset.bottom}]}>
    ...
    <View testID="fare-information-deck" style={s.fareInfoDeck}>
      ...
      <View testID="fare-information-active-content" style={s.fareInfoBody}><FareSurface tab={tab} choice={selected} dealPrices={displayPrices} selectedDealOfferId={selectedDeal?.offerId??null} onSelectDeal={setSelectedDealOfferId} theme={theme}/></View>
    </View>
...
function FareSurface({tab,choice,dealPrices,selectedDealOfferId,onSelectDeal,theme}:{tab:"deals"|"details"|"conditions"|"extras";choice:FlightDetailsFareChoice;dealPrices:Record<string,DisplayPrice>;selectedDealOfferId:string|null;onSelectDeal:(id:string)=>void;theme:ReturnType<typeof useAppTheme>["theme"]}) {
