...
export function NativeFlightDetails({ params }: { params: Params }) {
  ...
  return <SafeAreaView edges={["top"]} style={[s.safe,{backgroundColor:theme.background}]}><TopBar backgroundColor={theme.background} hasScrolled={hasScrolled}/><ScrollView testID="flight-details-scroll-content" scrollEventThrottle={16} onScroll={({nativeEvent})=>{const next=nativeEvent.contentOffset.y>1;if(next!==hasScrolledRef.current){hasScrolledRef.current=next;setHasScrolled(next);}}} contentContainerStyle={[s.content,{paddingBottom:120+inset.bottom}]}>
    ...
    <View testID="flight-details-route-summary" style={s.routeSummary}><View style={s.routeContent}><Text accessibilityRole="header" style={[s.route,{color:theme.textPrimary}]}>{flightDetailsRouteLabel(details.search.tripType,offer.legs??[],offer.originAirport,offer.destinationAirport)}</Text><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.85} style={[s.routeMetadata,{color:theme.textSecondary}]}>{tripMetadata}</Text></View><View style={s.routeActions}><IconButton label={saved?"Remove saved flight":"Save flight"} onPress={()=>savedFlights.toggle(savedOffer,nativeFlightEditSearchParams(details,one(params.currency)))} glyphStyle={s.routeActionLeadingGlyph}><Heart size={18} color={saved ? androidFavoriteColors.savedStroke : androidFavoriteColors.unsavedStroke} fill={saved?androidFavoriteColors.savedFill:androidFavoriteColors.unsavedFill}/></IconButton><IconButton label="Share flight" onPress={()=>void share()} glyphStyle={s.routeActionTrailingGlyph}><FlowIcon name="share" size={18} color={theme.icon}/></IconButton></View></View>
    ...
    {displayPricesReady?<ScrollView accessibilityRole="radiogroup" accessibilityLabel="Available fares" horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[s.fares,details.fareChoices.length>1?s.faresMultiple:s.faresSingle]}>
      {details.fareChoices.map((choice)=>{
        ...
        return <View key={choice.key} style={[s.fareCard,{width:loadedFareCardWidth,backgroundColor:theme.surface,borderColor:isSelected?ui.blue:theme.border},isSelected?s.fareCardSelected:s.fareCardUnselected]}>
          ...
          <View accessible={false} accessibilityElementsHidden importantForAccessibility="no-hide-descendants" pointerEvents="none" style={s.farePriceBlock}><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8} style={[s.farePrice,{color:isSelected?ui.blue:theme.textPrimary}]}>{displayPrices[choice.key]?.formatted??"Price unavailable"}</Text></View>
        </View>;
      })}
    ...
    <View testID="fare-information-deck" style={s.fareInfoDeck}>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={[s.fareTabRail,{borderBottomColor:theme.border}]} contentContainerStyle={s.fareTabRailContent}>
        <View accessibilityRole="tablist" accessibilityLabel="Fare information" style={s.fareTabList}>{([['deals','Compare deals'],['details','Fare details'],['conditions','Fare conditions'],['extras','Optional extras']] as const).map(([key,label])=><Pressable accessibilityRole="tab" accessibilityState={{selected:tab===key}} key={key} onPress={()=>setTab(key)} style={s.fareInfoTab}><Text numberOfLines={1} style={[s.fareInfoTabText,{color:tab===key?ui.blue:(theme.dark?"#C2CCE0":"#435477")},tab===key&&s.fareInfoTabTextActive]}>{label}</Text>{tab===key?<View style={s.fareTabIndicator}/>:null}</Pressable>)}</View>
      </ScrollView>
      <View testID="fare-information-active-content" style={s.fareInfoBody}><FareSurface tab={tab} choice={selected} dealPrices={displayPrices} onDeal={handoff} booking={booking} fareReady={fareReady} theme={theme}/></View>
    </View>
  ...
  return <SafeAreaView edges={["top"]} style={[s.safe,{backgroundColor:theme.background}]}>
    ...
    <ScrollView testID="flight-details-loading-skeleton" accessibilityRole="progressbar" accessibilityState={{busy:true}} accessibilityLabel="Loading flight details" contentContainerStyle={[s.loadingContent,{paddingBottom:32+bottomInset}]}>
      <Animated.View pointerEvents="none" style={[s.loadingBody,{opacity}]}>
        ...
        <View style={s.loadingInfoDeck}><View style={[s.loadingTabs,{borderBottomColor:theme.border}]}>{(["27%","22%","25%"] as const).map((width,index)=><View key={index} style={[s.loadingLine,{width},placeholder]}/>)}</View><View style={s.loadingInfoBody}>{(["92%","68%","84%"] as const).map((width,index)=><View key={index} style={[s.loadingLine,{width},placeholder]}/>)}</View></View>
      </Animated.View>
    </ScrollView>
  </SafeAreaView>;
}
...
function IconButton({label,onPress,children,glyphStyle}:{label:string;onPress:()=>void;children:React.ReactNode;glyphStyle?:React.ComponentProps<typeof View>["style"]}){return <Pressable accessibilityRole="button" accessibilityLabel={label} onPress={onPress} style={s.iconButton}><View style={s.routeActionGlyph}><View style={glyphStyle}>{children}</View></View></Pressable>}
...
function FareSurface({tab,choice,dealPrices,onDeal,booking,fareReady,theme}:{tab:"deals"|"details"|"conditions"|"extras";choice:FlightDetailsFareChoice;dealPrices:Record<string,DisplayPrice>;onDeal:(id:string)=>Promise<void>;booking:boolean;fareReady:boolean;theme:ReturnType<typeof useAppTheme>["theme"]}) {
  ...
  // Restrained local semantic pairs keep fare information legible in both themes without making status copy compete with the active blue tab.
  const categoryColor=theme.dark?"#94A3B8":"#64748B";
  const conditionColors={positive:theme.dark?"#6FCF97":"#237A4B",negative:theme.dark?"#E6A0A8":"#A64550",informational:theme.dark?"#AAB5CD":"#56658E",penalty:theme.dark?"#D6B36A":"#8A651F"};
