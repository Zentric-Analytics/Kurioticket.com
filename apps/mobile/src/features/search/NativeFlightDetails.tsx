...
export function NativeFlightDetails({ params }: { params: Params }) {
  ...
  </ScrollView><View style={[s.sticky,{paddingBottom:12+inset.bottom,backgroundColor:theme.surface,borderTopColor:theme.border}]}><View style={s.checkoutTotal}><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.65} style={[s.total,{color:theme.textPrimary}]}>{displayPricesReady?(activePrice?.formatted??"Price unavailable") : "Loading price…"}</Text><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.82} style={[s.checkoutLabel,{color:theme.textSecondary}]}>Total for {details.search.travelers} traveler{details.search.travelers===1?"":"s"}</Text></View><View style={[s.checkoutAction,Platform.OS==="android"&&s.checkoutActionAndroid]}><Pressable accessibilityRole="button" accessibilityState={{disabled:booking||!fareReady||(!selectedDeal&&!selected.handoff.available)}} disabled={booking||!fareReady||(!selectedDeal&&!selected.handoff.available)} onPress={()=>void handoff(selectedDeal?.offerId??offer.id)} style={s.continue}><Text style={s.continueText}>{booking?"Checking offer…":"Continue deal"}</Text></Pressable></View></View></SafeAreaView>;
}
