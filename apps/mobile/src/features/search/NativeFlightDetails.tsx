...
export const nativeLoadedFareCardWidth = (windowWidth: number) =>
  Math.min(330, Math.max(236, (windowWidth - 36) * 0.84));
...
export function NativeFlightDetails({ params }: { params: Params }) {
  ...
  const loadedFareCardWidth = nativeLoadedFareCardWidth(windowWidth);
  ...
  const searchDates=(details.search.tripType==="multi-city"?details.search.legs.map(({departureDate})=>departureDate):[details.search.departureDate,details.search.returnDate]).filter((value):value is string=>Boolean(value)).map((value)=>providerLocalFlightDateLong(value)).filter((value):value is string=>Boolean(value));
  const tripMetadata=[`${FLIGHT_TRIP_TYPE_LABELS[details.search.tripType]}`,`${details.search.travelers} traveler${details.search.travelers===1?"":"s"}`,...searchDates].join(" • ");
  ...
  return <SafeAreaView edges={["top"]} style={[s.safe,{backgroundColor:theme.background}]}><TopBar backgroundColor={theme.background} hasScrolled={hasScrolled}><View style={s.topActions}><IconButton label={saved?"Remove saved flight":"Save flight"} onPress={()=>savedFlights.toggle(savedOffer,nativeFlightEditSearchParams(details,one(params.currency)))}><Heart size={20} color={saved ? androidFavoriteColors.savedStroke : androidFavoriteColors.unsavedStroke} fill={saved?androidFavoriteColors.savedFill:androidFavoriteColors.unsavedFill}/></IconButton><IconButton label="Share flight" onPress={()=>void share()}><FlowIcon name="share" size={20} color={theme.icon}/></IconButton></View></TopBar><ScrollView testID="flight-details-scroll-content" scrollEventThrottle={16} onScroll={({nativeEvent})=>{const next=nativeEvent.contentOffset.y>1;if(next!==hasScrolledRef.current){hasScrolledRef.current=next;setHasScrolled(next);}}} contentContainerStyle={[s.content,{paddingBottom:120+inset.bottom}]}>
    ...
    <Text style={[s.itinerarySectionLabel,{color:theme.textSecondary}]}>Flight itinerary</Text>
    ...
    <ScrollView ref={fareRailRef} accessibilityRole="radiogroup" horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.fares} onContentSizeChange={()=>{const fareSet=details.fareChoices.map(({key})=>key).join("|");if(positionedFareSetRef.current===fareSet)return;positionedFareSetRef.current=fareSet;const selectedIndex=details.fareChoices.findIndex(({key})=>key===selected.key);fareRailRef.current?.scrollTo({x:nativeInitialFareRailOffset(selectedIndex,loadedFareCardWidth,windowWidth-36,details.fareChoices.length),animated:false});}}>
      {details.fareChoices.map((choice)=>{
        ...
        return <Pressable accessibilityRole="radio" accessibilityState={{selected:isSelected}} key={choice.key} onPress={()=>setSelectedKey(choice.key)} style={[s.fareCard,{width:loadedFareCardWidth,backgroundColor:theme.surface,borderColor:isSelected?ui.blue:theme.border},isSelected?s.fareCardSelected:s.fareCardUnselected]}>
          ...
          <View style={s.fareBenefits}>{fareTerms.map((row,i)=><FareBenefitRow key={`${row.index}-${row.rowIndex}-${i}`} category={row.term.category} semantic={row.term.semantic} text={row.text} titleColor={theme.textPrimary} detailColor={theme.textSecondary}/>)}</View>
        </Pressable>;
      })}
    </ScrollView>
