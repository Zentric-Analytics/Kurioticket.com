    ...
    <ScrollView testID="flight-details-loading-skeleton" accessibilityRole="progressbar" accessibilityState={{busy:true}} accessibilityLabel="Loading flight details" contentContainerStyle={[s.loadingContent,{paddingBottom:32+bottomInset}]}>
      <Animated.View pointerEvents="none" style={[s.loadingBody,{opacity}]}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.loadingFares}>
          {[0,1].map(key=><View key={key} style={[s.loadingFareCard,{width:fareCardWidth,backgroundColor:theme.surface,borderColor:theme.border}]}><View style={s.loadingFareIdentity}><View style={s.loadingFareNameRow}><View style={[s.loadingFareIcon,placeholder]}/><View style={[s.loadingLine,s.loadingFareName,placeholder]}/></View><View style={[s.loadingLine,s.loadingFarePrice,placeholder]}/></View>{(["88%","72%","80%"] as const).map((width,index)=><View key={index} style={s.loadingBenefitRow}><View style={[s.loadingBenefitDot,placeholder]}/><View style={[s.loadingLine,{width},placeholder]}/></View>)}</View>)}
        </ScrollView>
        <View style={s.loadingInfoDeck}><View style={s.loadingTabs}>{(["27%","22%","25%"] as const).map((width,index)=><View key={index} style={[s.loadingLine,{width},placeholder]}/>)}</View><View style={[s.loadingInfoBody,{backgroundColor:theme.surface,borderColor:theme.border}]}>{(["92%","68%","84%"] as const).map((width,index)=><View key={index} style={[s.loadingLine,{width},placeholder]}/>)}</View></View>
      </Animated.View>
    </ScrollView>
  </SafeAreaView>;
}
...
function FareBenefitRow({category,semantic,text,titleColor,detailColor,expanded,onToggle}:{category:FlightFareTerm["category"];semantic:FlightFareTerm["semantic"];text:string;titleColor:string;detailColor:string;expanded:boolean;onToggle:()=>void}){const {title,detail}=nativeFareBenefitPresentation(category,text);return <Pressable accessibilityRole="button" accessibilityLabel={`${title}${expanded?`, ${detail}`:""}, ${expanded?"collapse details":"expand details"}`} accessibilityState={{expanded}} onPress={onToggle} style={s.fareBenefitRow}><View style={s.fareStatusColumn}><FareStatusIcon semantic={semantic}/></View><View style={s.fareBenefitCopy}><View style={s.fareBenefitHeading}><Text style={[s.fareBenefitTitle,{color:titleColor}]}>{title}</Text><ChevronDown size={13} color={detailColor} style={expanded?s.fareBenefitChevronExpanded:undefined}/></View>{expanded?<Text style={[s.fareBenefitDetail,{color:detailColor}]}>{detail}</Text>:null}</View></Pressable>}
