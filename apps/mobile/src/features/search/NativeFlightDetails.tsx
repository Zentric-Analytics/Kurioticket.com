...
export function NativeFlightDetails({ params }: { params: Params }) {
  ...
  const heroIconColor=theme.dark?theme.icon:"#0F172A";
  ...
  </ScrollView><View style={[s.sticky,theme.dark?s.stickyDark:s.stickyLight,{paddingBottom:Math.max(inset.bottom,10),backgroundColor:theme.surface,borderTopColor:surfaceBorderColor}]}><View style={s.checkoutTotal}><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.85} style={[s.checkoutLabel,{color:theme.textSecondary}]}>Total for {details.search.travelers} traveler{details.search.travelers===1?"":"s"}</Text><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.72} style={[s.total,{color:theme.textPrimary}]}>{displayPricesReady?(activePrice?.formatted??"Price unavailable") : "Loading price…"}</Text></View><View style={s.checkoutAction}><Button label={booking?"Checking offer…":`Continue to ${provider}`} disabled={booking||!fareReady||(!selectedDeal&&!selected.handoff.available)} onPress={()=>void handoff(selectedDeal?.offerId??offer.id)}/></View></View></SafeAreaView>;
}
...
function FlightDetailsLoadingSkeleton({theme,topInset,bottomInset,fareCardWidth,viewportWidth}:{theme:ReturnType<typeof useAppTheme>["theme"];topInset:number;bottomInset:number;fareCardWidth:number;viewportWidth:number}) {
  ...
  return <SafeAreaView edges={[]} style={[s.safe,{backgroundColor:contentCanvasColor}]}>
    ...
    <View testID="flight-details-loading-controls" style={[s.heroControls,s.floatingControls,{top:topInset+8}]}>
      <Pressable accessibilityRole="button" accessibilityLabel="Back to results" onPress={()=>router.back()} style={s.heroIconButton}><DetailGlassSurface dark={theme.dark} variant="carsOptical" style={s.heroIconGlass}/><ArrowLeft size={20} color={heroIconColor}/></Pressable>
    ...
    <ScrollView testID="flight-details-loading-scroll" contentInsetAdjustmentBehavior="never" bounces={false} alwaysBounceVertical={false} overScrollMode="never" style={{backgroundColor:contentCanvasColor}} contentContainerStyle={[s.content,{width:viewportWidth,maxWidth:viewportWidth,paddingBottom:120+bottomInset}]}>
      ...
      <Animated.View testID="flight-details-loading-body" pointerEvents="none" accessibilityElementsHidden importantForAccessibility="no-hide-descendants" style={[s.contentBody,{opacity}]}>
        <View testID="flight-details-loading-itinerary-overlap" style={s.itineraryStack}>
          <View testID="flight-details-loading-itinerary" style={[s.itineraryCard,s.loadingItineraryCard,theme.dark?s.itineraryCardDark:s.itineraryCardLight,{backgroundColor:theme.surface,borderColor:surfaceBorderColor}]}>
...
function Itinerary({leg,index,offerAirlineName,offerAirlineLogo,theme,intlLocale}:{leg:FlightLeg;index:number;offerAirlineName:string;offerAirlineLogo?:string|null;theme:ReturnType<typeof useAppTheme>["theme"];intlLocale:string}) {
  ...
  return <View style={[s.itineraryCard,theme.dark?s.itineraryCardDark:s.itineraryCardLight,{backgroundColor:theme.surface,borderColor:surfaceBorderColor}]}>
    ...
    <View style={s.segmentList}>{leg.segments.map((segment,i)=>{
      ...
      return <View key={`${segment.originAirport}-${segment.destinationAirport}-${segment.departureTime}-${i}`} style={s.segmentItem}>
        {layover?<View style={[s.segmentConnection,{backgroundColor:theme.background,borderColor:theme.border}]}><Text style={[s.segmentConnectionText,{color:theme.textSecondary}]}>Connection at {layoverLabel(layover.airport)} · {layover.duration}</Text></View>:null}
