...
export function NativeFlightDetails({ params }: { params: Params }) {
  ...
  if (state !== "available" || !details || !selected) return <SafeAreaView edges={["top"]} style={[s.safe,{backgroundColor:theme.background}]}><TopBar backgroundColor={theme.surface}/><View style={s.center}><Text accessibilityRole="header" style={[s.title,{color:theme.textPrimary}]}>{state === "loading" ? "Checking current flight details…" : state === "unavailable" ? "This flight is no longer available" : "We couldn’t load this flight"}</Text>{state!=="loading"?<><Text style={[s.bodyText,{color:theme.textSecondary}]}>{message}</Text><Button label="Retry" onPress={reload}/><Button label="Back to results" onPress={()=>router.back()}/></>:null}</View></SafeAreaView>;
  ...
  return <SafeAreaView edges={["top"]} style={[s.safe,{backgroundColor:theme.background}]}><TopBar backgroundColor={theme.surface} hasScrolled={hasScrolled}><View style={s.topActions}><IconButton label={saved?"Remove saved flight":"Save flight"} onPress={()=>savedFlights.toggle(savedOffer,nativeFlightEditSearchParams(details,one(params.currency)))}><Heart size={20} color={androidFavoriteColors.stroke} fill={saved?androidFavoriteColors.savedFill:androidFavoriteColors.unsavedFill}/></IconButton><IconButton label="Share flight" onPress={()=>void share()}><FlowIcon name="share" size={20} color={theme.icon}/></IconButton></View></TopBar><ScrollView testID="flight-details-scroll-content" scrollEventThrottle={16} onScroll={({nativeEvent})=>{const next=nativeEvent.contentOffset.y>1;if(next!==hasScrolledRef.current){hasScrolledRef.current=next;setHasScrolled(next);}}} contentContainerStyle={[s.content,{paddingBottom:120+inset.bottom}]}>
    ...
    <Text style={[s.sectionTitle,{color:theme.textPrimary}]}>Flight itinerary</Text>{(offer.legs?.length?offer.legs:[]).map((leg,index)=><Itinerary key={`${leg.departureTime}-${index}`} leg={leg} index={index} offerAirlineName={offer.airlineName} offerAirlineLogo={offer.airlineLogo} theme={theme}/>) }
  ...
  return <View style={[s.itineraryCard,{backgroundColor:theme.surface,borderColor:theme.border}]}>
    ...
    <View style={s.journeySummary}>
      <View style={s.journeyEndpoint}>
        <Text style={[s.journeyTime,{color:theme.textPrimary}]}>{clock(leg.departureTime)}</Text>
      ...
      <View style={[s.journeyEndpoint,s.arrivalEndpoint]}>
        <Text style={[s.journeyTime,{color:theme.textPrimary}]}>{clock(leg.arrivalTime)}</Text>
...
const s=StyleSheet.create({safe:{flex:1},topBar:{minHeight:52,flexDirection:"row",alignItems:"center",justifyContent:"space-between",paddingHorizontal:16,paddingVertical:4,zIndex:1},topBarScrolled:{borderBottomWidth:StyleSheet.hairlineWidth,borderBottomColor:"rgba(15, 23, 42, 0.12)",shadowColor:"#0F172A",shadowOffset:{width:0,height:1},shadowOpacity:0.06,shadowRadius:4,elevation:1},topActions:{flexDirection:"row",alignItems:"center",gap:2,flexShrink:0},back:{minHeight:44,flexShrink:1,minWidth:0,flexDirection:"row",alignItems:"center",gap:7},backText:{color:ui.blue,fontWeight:"800",flexShrink:1},center:{flex:1,justifyContent:"center",padding:24,gap:16},content:{paddingHorizontal:18,paddingTop:5,gap:14},routeSummary:{flexDirection:"row",alignItems:"flex-start",gap:10,marginBottom:4},routeContent:{flex:1,minWidth:0,gap:2},title:{fontSize:23,fontWeight:"900"},route:{fontSize:27,fontWeight:"900"},bodyText:{fontSize:14,lineHeight:20},iconButton:{width:44,height:44,alignItems:"center",justifyContent:"center"},edit:{minHeight:44,flexShrink:0,flexDirection:"row",alignItems:"center",gap:6,paddingHorizontal:10,borderWidth:1,borderColor:ui.blue,borderRadius:10},editText:{color:ui.blue,fontWeight:"800"},sectionTitle:{fontSize:20,fontWeight:"900",marginTop:8},card:{borderWidth:1,borderRadius:14,padding:14,gap:7},itineraryCard:{borderWidth:1,borderRadius:15,padding:17,gap:0},itineraryHeader:{flexDirection:"row",alignItems:"flex-start",justifyContent:"space-between",gap:12},direction:{color:ui.blue,fontSize:12,lineHeight:17,fontWeight:"900",letterSpacing:.4,textTransform:"uppercase",flexShrink:1},itineraryDate:{fontSize:13,lineHeight:18,fontWeight:"600",textAlign:"right",flexShrink:0},journeySummary:{flexDirection:"row",alignItems:"center",gap:8,marginTop:20},journeyEndpoint:{flex:.9,minWidth:0,gap:3},arrivalEndpoint:{alignItems:"flex-end"},journeyTime:{fontSize:22,lineHeight:28,fontWeight:"900"},airportCode:{fontSize:17,lineHeight:22,fontWeight:"800"},journeyCenter:{flex:1.2,minWidth:72,alignItems:"center",gap:6},journeyDuration:{fontSize:13,lineHeight:18,fontWeight:"700",textAlign:"center"},flightPath:{alignSelf:"stretch",flexDirection:"row",alignItems:"center"},pathDot:{width:6,height:6,borderRadius:3,backgroundColor:ui.blue,flexShrink:0},pathLine:{height:StyleSheet.hairlineWidth,flex:1,minWidth:4},stopStatus:{fontSize:12,lineHeight:17,fontWeight:"600",textAlign:"center"},airportDetails:{flexDirection:"row",alignItems:"flex-start",gap:20,marginTop:22},airportColumn:{flex:1,minWidth:0,gap:8},arrivalAirportColumn:{alignItems:"flex-end"},airportName:{fontSize:13,lineHeight:18,fontWeight:"600"},terminal:{fontSize:12,lineHeight:17,fontWeight:"500"},arrivalText:{textAlign:"right"},itineraryDivider:{height:StyleSheet.hairlineWidth,marginVertical:17},airlineRows:{gap:14},airlineRow:{flexDirection:"row",alignItems:"flex-start",gap:10},airlineCopy:{flex:1,minWidth:0,gap:2},airlineName:{fontSize:14,lineHeight:19,fontWeight:"700"},flightNumber:{fontSize:13,lineHeight:18,fontWeight:"600"},operatedBy:{fontSize:11,lineHeight:16,fontWeight:"500",marginTop:1},small:{fontSize:12,lineHeight:17},fares:{gap:10},fareCard:{width:230,minHeight:145,borderWidth:2,borderRadius:14,padding:14,gap:6},fareLabel:{fontSize:17,fontWeight:"900"},farePrice:{color:ui.blue,fontSize:18,fontWeight:"900"},tabs:{flexDirection:"row",flexWrap:"wrap",gap:4},tab:{minHeight:44,justifyContent:"center",paddingHorizontal:9,borderBottomWidth:2,borderBottomColor:"transparent"},activeTab:{borderBottomColor:ui.blue},tabText:{fontSize:12,fontWeight:"800"},deal:{flexDirection:"row",justifyContent:"space-between",alignItems:"center",gap:8},extra:{paddingBottom:8},link:{minHeight:44,justifyContent:"center"},notice:{padding:12,borderRadius:10,borderWidth:1,borderColor:ui.blue},noticeText:{color:ui.blue,fontWeight:"700"},sticky:{position:"absolute",left:0,right:0,bottom:0,minHeight:88,borderTopWidth:1,paddingHorizontal:18,paddingTop:10,flexDirection:"row",justifyContent:"space-between",alignItems:"center",gap:12},total:{fontSize:22,fontWeight:"900"}});
