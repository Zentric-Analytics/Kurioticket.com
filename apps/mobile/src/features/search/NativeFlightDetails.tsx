...
function FareBenefitRow({category,semantic,text,titleColor,detailColor,expanded,onToggle}:{category:FlightFareTerm["category"];semantic:FlightFareTerm["semantic"];text:string;titleColor:string;detailColor:string;expanded:boolean;onToggle:()=>void}){const presentation=nativeFareBenefitPresentation(category,text);const detail=presentation.value??presentation.detail;const content=<><View style={s.fareStatusColumn}><FareStatusIcon semantic={semantic}/></View><View style={s.fareBenefitCopy}><View style={s.fareBenefitHeading}><Text style={[s.fareBenefitTitle,{color:titleColor}]}>{presentation.title}</Text>{detail?<ChevronDown size={13} color={detailColor} style={expanded?s.fareBenefitChevronExpanded:undefined}/>:null}</View>{expanded&&detail?<Text style={[s.fareBenefitDetail,{color:detailColor}]}>{detail}</Text>:null}</View></>;return detail?<Pressable accessibilityRole="button" accessibilityLabel={`${presentation.title}${expanded?`, ${detail}`:""}, ${expanded?"collapse details":"expand details"}`} accessibilityState={{expanded}} onPress={onToggle} style={s.fareBenefitRow}>{content}</Pressable>:<View style={s.fareBenefitRow}>{content}</View>}
