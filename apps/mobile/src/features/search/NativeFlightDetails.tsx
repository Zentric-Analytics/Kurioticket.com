...
export function NativeFlightDetails({ params }: { params: Params }) {
  ...
  const [fareState, setFareState] = useState<{ key: string; value: DisplayPrice | null } | null>(null);
  ...
  const fareReady = Boolean(selected && fareState?.key === selected.key);
  const fare = fareReady ? fareState?.value ?? null : null;
  useEffect(() => {
    if (!selected) { setFareState(null); return; }
    let active=true;
    const selectedFareKey = selected.key;
    Promise.all([
      ...
      setFareState({ key: selectedFareKey, value: createFlightDetailFare(selected.offer.price, selected.offer.currency, currency, exchange) });
    }); return()=>{active=false;};
  ...
  return <SafeAreaView edges={["top"]} style={[s.safe,{backgroundColor:theme.background}]}><Back/><ScrollView contentContainerStyle={[s.content,{paddingBottom:120+inset.bottom}]}>
    ...
    <FareSurface tab={tab} choice={selected} displayFare={fare} onDeal={handoff} booking={booking} fareReady={fareReady} theme={theme}/>
