...
export function NativeFlightDetails({ params }: { params: Params }) {
  ...
  return <SafeAreaView edges={[]} style={[s.safe,{backgroundColor:theme.background}]}><StatusBar style="light" translucent backgroundColor="transparent"/><ScrollView testID="flight-details-scroll-content" contentContainerStyle={[s.content,{paddingBottom:120+inset.bottom}]}>
...
function IconButton({label,onPress,children}:{label:string;onPress:()=>void;children:React.ReactNode}){return <Pressable accessibilityRole="button" accessibilityLabel={label} onPress={onPress} style={s.heroIconButton}>{children}</Pressable>}
