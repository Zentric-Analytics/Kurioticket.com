...
import { AccessibilityInfo, Alert, Animated, Easing, Image, ImageBackground, Linking, Platform, Pressable, ScrollView, Share, StyleSheet, Text, useWindowDimensions, View } from "react-native";
...
const FLIGHT_DETAILS_ITINERARY_RADIUS = 15;
...
export function NativeFlightDetails({ params }: { params: Params }) {
  ...
  if (state === "loading") return <FlightDetailsLoadingSkeleton theme={theme} topInset={inset.top} bottomInset={inset.bottom} fareCardWidth={fareCardWidth} viewportWidth={windowWidth}/>;
  if (state !== "available" || !details || !selected) return <SafeAreaView edges={[]} style={[s.safe,{backgroundColor:theme.background}]}><FlightDetailsBrandHeader topInset={inset.top} loading/><View style={s.center}><Text accessibilityRole="header" style={[s.title,{color:theme.textPrimary}]}>{state === "unavailable" ? "This flight is no longer available" : "We couldn’t load this flight"}</Text><Text style={[s.bodyText,{color:theme.textSecondary}]}>{message}</Text><Button label="Retry" onPress={reload}/><Button label="Back to results" onPress={()=>router.back()}/></View></SafeAreaView>;

  const offer=selected.offer; const activeOffer=selectedDeal?.offer??offer;
  const checkoutDisabled=booking||!fareReady||(!selectedDeal&&!selected.handoff.available);
  ...
  return <SafeAreaView edges={[]} style={[s.safe,{backgroundColor:contentCanvasColor}]}><FlightDetailsBrandHeader topInset={inset.top} saved={saved} onToggleSaved={()=>savedFlights.toggle(savedOffer,nativeFlightEditSearchParams(details,one(params.currency)))} onShare={()=>void share()}/><ScrollView testID="flight-details-scroll-content" contentInsetAdjustmentBehavior="never" bounces={false} alwaysBounceVertical={false} overScrollMode="never" style={{backgroundColor:contentCanvasColor}} contentContainerStyle={[s.content,{width:windowWidth,maxWidth:windowWidth,paddingBottom:120+inset.bottom}]}>
    <ImageBackground testID="flight-details-hero" source={require("../../../assets/heroes/flight-details-hero.webp")} resizeMode="cover" style={s.hero} imageStyle={s.heroImage}>
    ...
    {displayPricesReady?<ScrollView accessibilityRole="radiogroup" accessibilityLabel="Available fares" horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[s.fares,details.fareChoices.length>1?s.faresMultiple:s.faresSingle]}>
      {details.fareChoices.map((choice)=>{
        ...
        return <View key={choice.key} style={[s.fareCard,{width:loadedFareCardWidth,backgroundColor:isSelected?(theme.dark?FLIGHT_DETAILS_DARK_SELECTED:FLIGHT_DETAILS_LIGHT_SELECTED):theme.surface,borderColor:isSelected?ui.blue:surfaceBorderColor},isSelected?s.fareCardSelected:s.fareCardUnselected,isSelected?(theme.dark?s.fareCardSelectedDark:s.fareCardSelectedLight):(theme.dark?s.fareCardUnselectedDark:s.fareCardUnselectedLight)]}>
          ...
          <View pointerEvents="box-none" style={s.fareContent}>
            <View accessible={false} accessibilityElementsHidden importantForAccessibility="no-hide-descendants" pointerEvents="none" style={s.fareSelectionControl}>
              <View style={s.fareIdentity}><View style={[s.fareIconContainer,{backgroundColor:theme.dark?"#17365D":"#EAF3FF",borderColor:theme.dark?"#315A8F":"#CFE3FA"}]}><Luggage size={14} color={ui.blue}/></View><Text numberOfLines={2} ellipsizeMode="tail" style={[s.fareLabel,{color:theme.textPrimary}]}>{choice.label}</Text></View>
            </View>
      ...
      {details.fareChoices.slice(0,2).map((choice)=><View key={choice.key} style={[s.loadingFareCard,theme.dark?s.fareCardUnselectedDark:s.fareCardUnselectedLight,{width:loadedFareCardWidth,backgroundColor:theme.surface,borderColor:surfaceBorderColor}]}>
    ...
    <View testID="fare-information-deck" style={s.fareInfoDeck}>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={[s.fareTabRail,{borderBottomColor:surfaceBorderColor}]} contentContainerStyle={s.fareTabRailContent}>
        <View accessibilityRole="tablist" accessibilityLabel="Fare information" style={s.fareTabList}>{([['deals','Compare deals'],['details','Fare details'],['conditions','Fare conditions'],['extras','Optional extras']] as const).map(([key,label])=><Pressable accessibilityRole="tab" accessibilityState={{selected:tab===key}} key={key} onPress={()=>setTab(key)} style={s.fareInfoTab}><Text numberOfLines={1} style={[s.fareInfoTabText,{color:tab===key?fareHeadingTextColor:theme.textSecondary},tab===key&&s.fareInfoTabTextActive]}>{label}</Text>{tab===key?<View style={s.fareTabIndicator}/>:null}</Pressable>)}</View>
      </ScrollView>
  ...
  </ScrollView><View style={[s.sticky,theme.dark?s.stickyDark:s.stickyLight,{paddingBottom:Math.max(inset.bottom,10),backgroundColor:theme.surface,borderTopColor:surfaceBorderColor}]}><View style={s.checkoutTotal}><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.72} style={[s.total,{color:theme.textPrimary}]}>{displayPricesReady?(activePrice?.formatted??"Price unavailable") : "Loading price…"}</Text><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.85} style={[s.checkoutLabel,{color:theme.textSecondary}]}>Total for {details.search.travelers} traveler{details.search.travelers===1?"":"s"}</Text></View><View style={[s.checkoutAction,Platform.OS==="android"&&s.checkoutActionAndroid]}><Pressable accessibilityRole="button" accessibilityState={{disabled:checkoutDisabled}} disabled={checkoutDisabled} onPress={()=>void handoff(selectedDeal?.offerId??offer.id)} style={[s.continue,checkoutDisabled&&s.continueDisabled]}><Text style={s.continueText}>{booking?"Checking offer…":"Continue deal"}</Text></Pressable></View></View></SafeAreaView>;
}

function FlightDetailsLoadingSkeleton({theme,topInset,bottomInset,fareCardWidth,viewportWidth}:{theme:ReturnType<typeof useAppTheme>["theme"];topInset:number;bottomInset:number;fareCardWidth:number;viewportWidth:number}) {
  ...
  return <SafeAreaView edges={[]} style={[s.safe,{backgroundColor:contentCanvasColor}]}>
    <FlightDetailsBrandHeader topInset={topInset} loading/>
    ...
    <ScrollView testID="flight-details-loading-scroll" contentInsetAdjustmentBehavior="never" bounces={false} alwaysBounceVertical={false} overScrollMode="never" style={{backgroundColor:contentCanvasColor}} contentContainerStyle={[s.content,{width:viewportWidth,maxWidth:viewportWidth,paddingBottom:120+bottomInset}]}>
      <View testID="flight-details-loading-hero" style={[s.hero,{backgroundColor:theme.dark?"#27272A":"#E2E8F0"}]}>
      ...
      <Animated.View testID="flight-details-loading-body" pointerEvents="none" accessibilityElementsHidden importantForAccessibility="no-hide-descendants" style={[s.contentBody,{opacity}]}>
        ...
        <ScrollView testID="flight-details-loading-fares" horizontal showsHorizontalScrollIndicator={false} scrollEnabled={false} contentContainerStyle={s.loadingFares}>
          {[0,1].map(key=><View key={key} style={[s.loadingFareCard,theme.dark?s.fareCardUnselectedDark:s.fareCardUnselectedLight,{width:fareCardWidth,backgroundColor:theme.surface,borderColor:surfaceBorderColor}]}>
        ...
        <View testID="flight-details-loading-info" style={s.loadingInfoDeck}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} scrollEnabled={false} style={s.loadingTabRail}>
            <View testID="flight-details-loading-tabs" style={[s.loadingTabs,{borderBottomColor:surfaceBorderColor}]}>{[110,90,120,110].map((width,index)=><View key={index} style={[s.loadingLine,{width},placeholder]}/>)}</View>
          </ScrollView>
    ...
    <View testID="flight-details-loading-checkout" pointerEvents="none" accessibilityElementsHidden importantForAccessibility="no-hide-descendants" style={[s.sticky,theme.dark?s.stickyDark:s.stickyLight,{paddingBottom:Math.max(bottomInset,10),backgroundColor:theme.surface,borderTopColor:surfaceBorderColor}]}>
...
function FlightDetailsBrandHeader({topInset,saved=false,onToggleSaved,onShare,loading=false}:{topInset:number;saved?:boolean;onToggleSaved?:()=>void;onShare?:()=>void;loading?:boolean}) {
  const saveDisabled=loading||!onToggleSaved;
  const shareDisabled=loading||!onShare;
  const saveColor=saveDisabled?"#94A3B8":saved?androidFavoriteColors.savedStroke:androidFavoriteColors.unsavedStroke;
  const shareColor=shareDisabled?"#94A3B8":androidFavoriteColors.shareStroke;
  return <><StatusBar style="dark" translucent backgroundColor="#FFFFFF"/><View testID="flight-details-brand-header" style={[s.brandHeader,{paddingTop:topInset}]}>
    <View style={s.brandHeaderRow}>
      <View style={s.brandHeaderLeading}>
        <Pressable accessibilityRole="button" accessibilityLabel="Back to results" onPress={()=>router.back()} style={s.brandHeaderAction}><ArrowLeft size={25} strokeWidth={2.2} color="#0F172A"/></Pressable>
        <Image accessibilityLabel="Kurioticket" accessibilityIgnoresInvertColors source={require("../../../assets/kurioticket-logo-primary-light-bg.png")} resizeMode="contain" style={s.brandHeaderLogo}/>
      </View>
      <View style={s.brandHeaderTrailing}>
        <Pressable accessibilityRole="button" accessibilityLabel={saved?"Remove saved flight":"Save flight"} accessibilityState={{disabled:saveDisabled,selected:saved}} disabled={saveDisabled} onPress={onToggleSaved} style={s.brandHeaderAction}><Heart size={21} strokeWidth={androidFavoriteColors.strokeWidth} color={saveColor} fill={saved?androidFavoriteColors.savedFill:androidFavoriteColors.unsavedFill}/></Pressable>
        <Pressable accessibilityRole="button" accessibilityLabel="Share flight" accessibilityState={{disabled:shareDisabled}} disabled={shareDisabled} onPress={onShare} style={s.brandHeaderAction}><FlowIcon name="share" size={20} strokeWidth={androidFavoriteColors.strokeWidth} color={shareColor}/></Pressable>
      </View>
    </View>
  </View></>;
}
    ...
    <View style={s.journeySummary}>
      ...
      <View style={s.journeyCenter}>
        ...
        <View style={s.flightPath} accessibilityElementsHidden importantForAccessibility="no-hide-descendants">
          <View style={s.pathDot}/><View style={[s.pathLine,{backgroundColor:pathColor}]}/><FlowIcon name="flight" size={16} color={ui.blue}/><View style={[s.pathLine,{backgroundColor:pathColor}]}/><View style={s.pathDot}/>
        </View>
      ...
      return <View key={`${segment.originAirport}-${segment.destinationAirport}-${segment.departureTime}-${i}`} style={s.segmentItem}>
        ...
        <View style={s.segmentRow}>
          ...
          <View style={s.segmentCopy}>
            <View style={s.segmentHeading}><Text style={[s.segmentRoute,{color:theme.textPrimary}]}>{segment.originAirport} → {segment.destinationAirport}</Text><Text style={[s.segmentTimes,{color:theme.textPrimary}]}>{clock(segment.departureTime)} – {clock(segment.arrivalTime)}</Text></View>
...
const s=StyleSheet.create({safe:{flex:1},brandHeader:{backgroundColor:"#FFFFFF"},brandHeaderRow:{height:64,paddingHorizontal:16,flexDirection:"row",alignItems:"center",justifyContent:"space-between"},brandHeaderLeading:{flexDirection:"row",alignItems:"center",gap:6,flexShrink:1,minWidth:0},brandHeaderTrailing:{flexDirection:"row",alignItems:"center",flexShrink:0},brandHeaderAction:{width:44,height:44,alignItems:"center",justifyContent:"center"},brandHeaderLogo:{width:128,height:32,flexShrink:1},loadingLine:{height:10,borderRadius:5},loadingRouteLine:{width:"62%",height:32,borderRadius:8},loadingMetadataLine:{width:"58%",height:16},loadingItineraryCard:{height:226},loadingDateLine:{width:"34%",height:12},loadingDirectionLine:{width:"24%",height:9},loadingJourneyRow:{flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:18,marginTop:25},loadingTimeLine:{width:"22%",height:16},loadingPathLine:{flex:1,height:4},loadingAirportRow:{flexDirection:"row",justifyContent:"space-between",marginTop:11},loadingAirportLine:{width:"18%",height:12},loadingDivider:{height:StyleSheet.hairlineWidth,marginVertical:28},loadingAirlineLine:{width:"48%",height:12},loadingFareHeading:{width:136,height:23,marginTop:12},loadingFares:{gap:10,paddingTop:12,paddingBottom:18,paddingRight:38},loadingFareCard:{height:142,position:"relative",borderWidth:1.5,borderRadius:15,paddingHorizontal:12,paddingTop:6,paddingBottom:8,gap:4},loadingFareContent:{alignSelf:"stretch",gap:5,paddingBottom:31},loadingFareIdentity:{alignSelf:"stretch",alignItems:"center",gap:1},loadingFareNameRow:{maxWidth:"100%",flexDirection:"row",alignItems:"flex-start",gap:6},loadingFareIcon:{width:24,height:24,borderRadius:8,flexShrink:0},loadingFareName:{width:72,height:12,marginTop:6},loadingFarePriceBlock:{position:"absolute",bottom:6,left:12,right:12,alignItems:"center"},loadingFarePrice:{width:82,height:16},loadingBenefitRow:{flexDirection:"row",alignItems:"center",gap:7},loadingBenefitDot:{width:16,height:16,borderRadius:8},loadingCheckoutTotal:{gap:5,flex:1,minWidth:0,maxWidth:"46%"},loadingCheckoutLabel:{width:"78%",maxWidth:124,height:10},loadingCheckoutPrice:{width:"88%",maxWidth:142,height:27},loadingCheckoutButton:{flex:1,maxWidth:210,minWidth:132,height:45,borderRadius:8},loadingInfoDeck:{height:177,marginTop:12},loadingTabRail:{flexGrow:0,marginHorizontal:-10},loadingTabs:{height:48,borderBottomWidth:1,flexDirection:"row",alignItems:"center",gap:22},loadingInfoBody:{flex:1,paddingHorizontal:4,paddingVertical:18,gap:17},center:{flex:1,justifyContent:"center",padding:24,gap:16},content:{paddingTop:0},contentBody:{paddingHorizontal:18,gap:14},hero:{minHeight:318,justifyContent:"flex-end",paddingHorizontal:18,paddingBottom:122},heroImage:{},heroOverlay:{...StyleSheet.absoluteFillObject,backgroundColor:"rgba(5, 13, 26, 0.30)"},heroCurve:{position:"absolute",left:0,right:0,bottom:-1,width:"100%",height:65},heroTextFade:{position:"absolute",left:0,right:0,bottom:66,height:150},heroCopy:{gap:3},title:{fontSize:23,fontWeight:"900"},route:{color:"#FFFFFF",fontSize:27,lineHeight:32,fontWeight:"800",textShadowColor:"rgba(0, 0, 0, 0.35)",textShadowOffset:{width:0,height:1},textShadowRadius:4},routeMetadata:{color:"#FFFFFF",fontSize:11,lineHeight:16,fontWeight:"700",letterSpacing:.55,textTransform:"uppercase",textShadowColor:"rgba(0, 0, 0, 0.4)",textShadowOffset:{width:0,height:1},textShadowRadius:3},bodyText:{fontSize:14,lineHeight:20},sectionTitle:{fontSize:20,fontWeight:"900",marginTop:8},fareSectionTitle:{fontSize:18,lineHeight:23,fontWeight:"800",letterSpacing:-.15,marginTop:12},card:{borderWidth:1,borderRadius:14,padding:14,gap:7},itineraryStack:{gap:14,marginHorizontal:-10,marginTop:-104,zIndex:1},itineraryCard:{position:"relative",borderWidth:1,borderRadius:FLIGHT_DETAILS_ITINERARY_RADIUS,padding:15,gap:0},itineraryGlossSurface:{...StyleSheet.absoluteFillObject,borderRadius:FLIGHT_DETAILS_ITINERARY_RADIUS,overflow:"hidden"},itineraryCardLight:{shadowColor:ui.navy,shadowOffset:{width:0,height:6},shadowOpacity:.14,shadowRadius:18,elevation:4},itineraryCardDark:{shadowColor:"#000000",shadowOffset:{width:0,height:5},shadowOpacity:.32,shadowRadius:18,elevation:4},itineraryHeader:{flexDirection:"row",alignItems:"flex-start",justifyContent:"space-between",gap:12},direction:{color:ui.blue,flexShrink:1,fontSize:11,lineHeight:15,fontWeight:"800",letterSpacing:.5,textTransform:"uppercase"},itineraryDate:{flexShrink:0,fontSize:11,lineHeight:15,fontWeight:"500",textAlign:"right"},journeySummary:{flexDirection:"row",alignItems:"center",gap:8,marginTop:14},journeyEndpoint:{flex:1.1,minWidth:0,gap:3},arrivalEndpoint:{alignItems:"flex-end"},journeyTime:{fontSize:19,lineHeight:24,fontWeight:"800",fontVariant:["tabular-nums"]},airportCode:{fontSize:13,lineHeight:17,fontWeight:"700"},airportDate:{marginTop:1,fontSize:9.5,lineHeight:12,fontWeight:"500"},journeyCenter:{flex:.8,minWidth:72,alignItems:"center",gap:5},journeyDuration:{fontSize:11,lineHeight:16,fontWeight:"600",textAlign:"center"},flightPath:{alignSelf:"stretch",flexDirection:"row",alignItems:"center"},pathDot:{width:6,height:6,borderRadius:3,backgroundColor:ui.blue,flexShrink:0},pathLine:{height:1,flex:1,minWidth:4,opacity:.42},stopStatus:{fontSize:10,lineHeight:13,fontWeight:"500",textAlign:"center"},airportDetails:{flexDirection:"row",alignItems:"flex-start",gap:20,marginTop:13},airportColumn:{flex:1,minWidth:0,gap:5},arrivalAirportColumn:{alignItems:"flex-end"},airportName:{fontSize:12,lineHeight:17,fontWeight:"500"},terminal:{fontSize:11,lineHeight:16,fontWeight:"400"},arrivalText:{textAlign:"right"},itineraryDivider:{height:StyleSheet.hairlineWidth,marginVertical:13,opacity:.72},segmentList:{gap:0},segmentItem:{gap:0},segmentConnection:{borderWidth:StyleSheet.hairlineWidth,borderRadius:9,paddingHorizontal:11,paddingVertical:8,marginBottom:2,flexDirection:"row",alignItems:"center",gap:7},segmentConnectionText:{flex:1,minWidth:0,fontSize:11,lineHeight:16,fontWeight:"600"},segmentConnectionDuration:{fontWeight:"500"},segmentRow:{flexDirection:"row",alignItems:"flex-start",gap:10,paddingVertical:9},segmentCopy:{flex:1,minWidth:0,gap:2},segmentHeading:{flexDirection:"row",alignItems:"flex-start",justifyContent:"space-between",gap:10},segmentRoute:{flexShrink:1,fontSize:13,lineHeight:18,fontWeight:"700"},segmentTimes:{flexShrink:0,maxWidth:"48%",fontSize:11,lineHeight:16,fontWeight:"600",textAlign:"right",fontVariant:["tabular-nums"]},segmentMeta:{fontSize:11,lineHeight:16,fontWeight:"500"},segmentDetail:{fontSize:11,lineHeight:16,fontWeight:"400"},technicalInformation:{gap:7},technicalHeading:{fontSize:12,lineHeight:16,fontWeight:"700",letterSpacing:.65,textTransform:"uppercase",marginBottom:2},technicalRow:{flexDirection:"row",alignItems:"flex-start",justifyContent:"space-between",gap:12},technicalLabel:{flex:1,minWidth:0,fontSize:11,lineHeight:16,fontWeight:"600"},technicalValue:{flexShrink:1,maxWidth:"52%",fontSize:11,lineHeight:16,fontWeight:"400",textAlign:"right"},small:{fontSize:12,lineHeight:17},checkoutTotal:{flex:1,minWidth:0,gap:1},total:{maxWidth:"100%",fontSize:19,lineHeight:22,fontWeight:"600",fontFamily:appFonts.semibold,letterSpacing:-0.25,textAlign:"left",fontVariant:["tabular-nums"]},checkoutLabel:{flexShrink:1,minWidth:0,fontSize:11,lineHeight:16,fontWeight:"600",fontFamily:appFonts.semibold},checkoutAction:{flex:.78,minWidth:140,maxWidth:180},checkoutActionAndroid:{flex:.76,minWidth:132,maxWidth:176},continue:{width:"100%",minHeight:48,borderRadius:8,backgroundColor:ui.blue,paddingHorizontal:12,alignItems:"center",justifyContent:"center"},continueDisabled:{opacity:.65},continueText:{fontSize:12,lineHeight:16,fontWeight:"700",fontFamily:appFonts.bold,color:"white",textAlign:"center"},fares:{alignItems:"flex-start",gap:10,paddingTop:12,paddingBottom:18},faresMultiple:{paddingRight:38},faresSingle:{paddingRight:0},fareCard:{borderWidth:1.5,borderRadius:15,minHeight:142,position:"relative",paddingHorizontal:12,paddingTop:6,paddingBottom:8,gap:4},fareCardSelected:{zIndex:1},fareCardUnselected:{zIndex:0},fareCardSelectedLight:{shadowColor:ui.navy,shadowOffset:{width:0,height:6},shadowOpacity:.18,shadowRadius:14,elevation:5},fareCardSelectedDark:{shadowColor:"#000000",shadowOffset:{width:0,height:6},shadowOpacity:.38,shadowRadius:14,elevation:5},fareCardUnselectedLight:{shadowColor:ui.navy,shadowOffset:{width:0,height:2},shadowOpacity:.06,shadowRadius:6,elevation:1},fareCardUnselectedDark:{shadowColor:"#000000",shadowOffset:{width:0,height:2},shadowOpacity:.2,shadowRadius:6,elevation:1},fareContent:{alignSelf:"stretch",gap:5,paddingBottom:31},fareSelectionControl:{alignSelf:"stretch",alignItems:"flex-start"},fareIdentity:{alignSelf:"center",maxWidth:"100%",flexDirection:"row",alignItems:"center",gap:7,marginBottom:1},fareIconContainer:{width:24,height:24,borderRadius:8,borderWidth:1,alignItems:"center",justifyContent:"center",flexShrink:0},fareLabel:{flexShrink:1,minWidth:0,fontSize:13,lineHeight:17,fontWeight:"800",letterSpacing:.1},farePriceBlock:{position:"absolute",bottom:6,left:12,right:12,alignItems:"center",maxWidth:"100%"},farePrice:{maxWidth:"100%",fontSize:19,lineHeight:23,fontWeight:"800",textAlign:"center",fontVariant:["tabular-nums"]},fareBenefits:{alignSelf:"stretch",alignItems:"flex-start",gap:5},fareBenefitRow:{alignSelf:"stretch",maxWidth:"100%",paddingRight:8,flexDirection:"row",alignItems:"flex-start",gap:7},fareStatusColumn:{width:15,alignItems:"center"},fareStatus:{width:14,height:14,borderRadius:7,borderWidth:1,alignItems:"center",justifyContent:"center"},fareStatusPositive:{borderColor:ui.green},fareStatusNeutral:{borderColor:"#94A3B8"},fareStatusMinus:{width:7,height:1.5,borderRadius:1,backgroundColor:"#64748B"},fareStatusDot:{width:3,height:3,borderRadius:2,backgroundColor:"#64748B"},fareBenefitCopy:{flex:1,minWidth:0,gap:1},fareBenefitTitle:{fontSize:12,lineHeight:16,fontWeight:"600"},fareBenefitChevron:{width:15,flexShrink:0,alignItems:"center",paddingTop:2},fareBenefitChevronExpanded:{transform:[{rotate:"180deg"}]},fareBenefitDetail:{fontSize:11,lineHeight:16,fontWeight:"400"},fareInfoDeck:{gap:0,marginTop:12},fareTabRail:{flexGrow:0,borderBottomWidth:1,marginHorizontal:-10},fareTabRailContent:{paddingHorizontal:0},fareTabList:{flexDirection:"row",gap:22},fareInfoTab:{minHeight:48,justifyContent:"center",position:"relative",paddingHorizontal:0},fareInfoTabText:{fontSize:14,lineHeight:20,fontWeight:"600"},fareInfoTabTextActive:{fontWeight:"800"},fareTabIndicator:{position:"absolute",height:3,borderRadius:2,backgroundColor:ui.blue,left:2,right:2,bottom:-1},fareInfoBody:{paddingHorizontal:4,paddingTop:7,paddingBottom:4},infoSections:{gap:0},infoSection:{paddingVertical:12},sectionDivider:{borderTopWidth:StyleSheet.hairlineWidth},fareGroupLabel:{fontSize:11,lineHeight:15,fontWeight:"700",letterSpacing:.85,textTransform:"uppercase",marginBottom:9},segmentContext:{fontSize:14,lineHeight:19,fontWeight:"600",marginBottom:11},cabinGroup:{paddingVertical:10},fareFactGroup:{gap:8},fareGroupDivider:{height:StyleSheet.hairlineWidth,marginVertical:11},detailRow:{flexDirection:"row",justifyContent:"space-between",alignItems:"flex-start",gap:16},detailLabel:{flexShrink:0,fontSize:13,lineHeight:19,fontWeight:"400"},detailValue:{flex:1,fontSize:13,lineHeight:19,fontWeight:"500",textAlign:"right"},detailTotal:{fontWeight:"700"},quietText:{fontSize:13,lineHeight:19,fontWeight:"400"},emissionsCard:{marginVertical:12,borderRadius:8,paddingHorizontal:11,paddingVertical:9,flexDirection:"row",flexWrap:"wrap",alignItems:"center",justifyContent:"space-between",gap:8},emissionsIdentity:{flexDirection:"row",alignItems:"center",flexGrow:1,flexShrink:1,minWidth:180,gap:7},emissionsLabel:{flexShrink:1,fontSize:13,lineHeight:18,fontWeight:"600"},emissionsValueGroup:{marginLeft:"auto",alignItems:"flex-end",flexShrink:1},emissionsValue:{fontSize:14,lineHeight:18,fontWeight:"600",textAlign:"right"},emissionsContext:{fontSize:11,lineHeight:15,fontWeight:"400",textAlign:"right"},secondaryFacts:{borderTopWidth:StyleSheet.hairlineWidth,paddingVertical:11},providerFreshness:{gap:1},providerFreshnessLabel:{fontSize:11,lineHeight:16,fontWeight:"500"},providerFreshnessValue:{fontSize:11,lineHeight:16,fontWeight:"400"},emptyState:{paddingVertical:18,paddingHorizontal:10,alignItems:"center",gap:5},emptyTitle:{fontSize:14,lineHeight:19,fontWeight:"600",textAlign:"center"},emptyDescription:{fontSize:13,lineHeight:19,fontWeight:"400",textAlign:"center"},dealList:{gap:10,paddingVertical:12},dealCard:{minHeight:96,borderWidth:1,borderRadius:14,paddingHorizontal:15,paddingVertical:13,justifyContent:"space-between",gap:14},dealCardSelectedLight:{shadowColor:ui.navy,shadowOffset:{width:0,height:4},shadowOpacity:.12,shadowRadius:10,elevation:3},dealCardSelectedDark:{shadowColor:"#000000",shadowOffset:{width:0,height:4},shadowOpacity:.3,shadowRadius:10,elevation:3},dealCardPressed:{opacity:.88},dealTop:{flexDirection:"row",alignItems:"flex-start",justifyContent:"space-between",gap:12},dealProvider:{flex:1,minWidth:0,fontSize:15,lineHeight:20,fontWeight:"700"},dealRadio:{width:20,height:20,borderRadius:10,borderWidth:1.5,alignItems:"center",justifyContent:"center"},dealRadioDot:{width:8,height:8,borderRadius:4,backgroundColor:ui.blue},dealBottom:{flexDirection:"row",alignItems:"flex-end",justifyContent:"space-between",gap:12},dealFareLabel:{flex:1,minWidth:0,fontSize:12,lineHeight:17,fontWeight:"500"},dealPrice:{flexShrink:0,maxWidth:"60%",fontSize:18,lineHeight:22,fontWeight:"800",textAlign:"right",fontVariant:["tabular-nums"]},conditionGroup:{paddingVertical:12},conditionRow:{paddingVertical:9,flexDirection:"row",alignItems:"flex-start",gap:11},conditionCopy:{flex:1,minWidth:0,gap:2},conditionState:{fontSize:13,lineHeight:18,fontWeight:"600"},conditionScope:{fontSize:11,lineHeight:16,fontWeight:"400"},conditionPenalty:{fontSize:11,lineHeight:16,fontWeight:"500"},factDescription:{fontSize:13,lineHeight:19,fontWeight:"400",marginBottom:7},airlineIdentity:{fontSize:13,lineHeight:19,fontWeight:"500",marginBottom:3},linkRow:{minHeight:44,flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:12},linkText:{flex:1,fontSize:13,lineHeight:18,fontWeight:"600"},serviceRow:{paddingVertical:12},serviceCopy:{minWidth:0,gap:4},serviceHeading:{flexDirection:"row",alignItems:"flex-start",justifyContent:"space-between",gap:12},serviceDescription:{flex:1,fontSize:13,lineHeight:19,fontWeight:"600"},servicePrice:{flexShrink:0,maxWidth:"42%",fontSize:13,lineHeight:19,fontWeight:"700",textAlign:"right",fontVariant:["tabular-nums"]},serviceMeta:{fontSize:12,lineHeight:17,fontWeight:"400"},optionalServicesEmpty:{paddingBottom:2},loyaltyProgrammes:{fontSize:13,lineHeight:19,fontWeight:"500",flexWrap:"wrap"},notice:{padding:12,borderRadius:10,borderWidth:1,borderColor:ui.blue},noticeText:{color:ui.blue,fontWeight:"700"},sticky:{position:"absolute",left:0,right:0,bottom:0,minHeight:88,borderTopWidth:StyleSheet.hairlineWidth,paddingHorizontal:18,paddingTop:11,flexDirection:"row",justifyContent:"space-between",alignItems:"center",gap:14,elevation:7},stickyLight:{shadowColor:ui.navy,shadowOffset:{width:0,height:-4},shadowOpacity:.1,shadowRadius:12},stickyDark:{shadowColor:"#000000",shadowOffset:{width:0,height:-4},shadowOpacity:.28,shadowRadius:12}});
