...
export function NativeFlightDetails({ params }: { params: Params }) {
  ...
  return <SafeAreaView edges={[]} style={[s.safe,{backgroundColor:contentCanvasColor}]}><StatusBar style="light" translucent backgroundColor="transparent"/><View testID="flight-details-floating-controls" style={[s.heroControls,s.floatingControls,{top:inset.top+8}]}><Pressable accessibilityRole="button" accessibilityLabel="Back to results" onPress={()=>router.back()} style={s.heroIconButton}><DetailGlassSurface dark={theme.dark} variant="carsOptical" style={s.heroIconGlass}/><ArrowLeft size={20} color={heroIconColor}/></Pressable><View style={s.heroActions}><DetailGlassSurface dark={theme.dark} variant="carsOptical" style={s.heroActionsGlass}/><IconButton label={saved?"Remove saved flight":"Save flight"} onPress={()=>savedFlights.toggle(savedOffer,nativeFlightEditSearchParams(details,one(params.currency)))} iconStyle={s.heroHeartIcon}><Heart size={17} strokeWidth={androidFavoriteColors.strokeWidth} color={saved ? androidFavoriteColors.savedStroke : theme.dark ? theme.icon : androidFavoriteColors.unsavedStroke} fill={saved?androidFavoriteColors.savedFill:androidFavoriteColors.unsavedFill}/></IconButton><IconButton label="Share flight" onPress={()=>void share()} iconStyle={s.heroShareIcon}><FlowIcon name="share" size={17} strokeWidth={androidFavoriteColors.strokeWidth} color={theme.dark ? theme.icon : androidFavoriteColors.shareStroke}/></IconButton></View></View><ScrollView testID="flight-details-scroll-content" style={{backgroundColor:contentCanvasColor}} contentContainerStyle={[s.content,{width:windowWidth,maxWidth:windowWidth,paddingBottom:120+inset.bottom}]}>
    ...
    {displayPricesReady?<ScrollView accessibilityRole="radiogroup" accessibilityLabel="Available fares" horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[s.fares,details.fareChoices.length>1?s.faresMultiple:s.faresSingle]}>
      {details.fareChoices.map((choice)=>{
        ...
        return <View key={choice.key} style={[s.fareCard,{width:loadedFareCardWidth,backgroundColor:isSelected?(theme.dark?"#14243E":"#F8FBFF"):theme.surface,borderColor:isSelected?ui.blue:theme.border},isSelected?s.fareCardSelected:s.fareCardUnselected,isSelected?(theme.dark?s.fareCardSelectedDark:s.fareCardSelectedLight):(theme.dark?s.fareCardUnselectedDark:s.fareCardUnselectedLight)]}>
          ...
          <View accessible={false} accessibilityElementsHidden importantForAccessibility="no-hide-descendants" pointerEvents="none" style={[s.farePriceBlock,{borderTopColor:theme.border}]}><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8} style={[s.farePrice,{color:fareHeadingTextColor}]}>{displayPrices[choice.key]?.formatted??"Price unavailable"}</Text></View>
        </View>;
      })}
      ...
      {details.fareChoices.slice(0,2).map((choice)=><View key={choice.key} style={[s.loadingFareCard,theme.dark?s.fareCardUnselectedDark:s.fareCardUnselectedLight,{width:loadedFareCardWidth,backgroundColor:theme.surface,borderColor:theme.border}]}>
  ...
  </ScrollView><View style={[s.sticky,theme.dark?s.stickyDark:s.stickyLight,{paddingBottom:Math.max(inset.bottom,10),backgroundColor:theme.surface,borderTopColor:theme.border}]}><View style={s.checkoutTotal}><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.85} style={[s.checkoutLabel,{color:theme.textSecondary}]}>Total for {details.search.travelers} traveler{details.search.travelers===1?"":"s"}</Text><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.72} style={[s.total,{color:theme.textPrimary}]}>{displayPricesReady?(activePrice?.formatted??"Price unavailable") : "Loading price…"}</Text></View><View style={s.checkoutAction}><Button label={booking?"Checking offer…":`Continue to ${provider}`} disabled={booking||!fareReady||(!selectedDeal&&!selected.handoff.available)} onPress={()=>void handoff(selectedDeal?.offerId??offer.id)}/></View></View></SafeAreaView>;
}
...
function FlightDetailsLoadingSkeleton({theme,topInset,bottomInset,fareCardWidth,viewportWidth}:{theme:ReturnType<typeof useAppTheme>["theme"];topInset:number;bottomInset:number;fareCardWidth:number;viewportWidth:number}) {
  ...
  return <SafeAreaView edges={[]} style={[s.safe,{backgroundColor:contentCanvasColor}]}>
    ...
    <ScrollView testID="flight-details-loading-scroll" style={{backgroundColor:contentCanvasColor}} contentContainerStyle={[s.content,{width:viewportWidth,maxWidth:viewportWidth,paddingBottom:120+bottomInset}]}>
      ...
      <Animated.View testID="flight-details-loading-body" pointerEvents="none" accessibilityElementsHidden importantForAccessibility="no-hide-descendants" style={[s.contentBody,{opacity}]}>
        <View testID="flight-details-loading-itinerary-overlap" style={s.itineraryStack}>
          <View testID="flight-details-loading-itinerary" style={[s.itineraryCard,s.loadingItineraryCard,theme.dark?s.itineraryCardDark:s.itineraryCardLight,{backgroundColor:theme.surface,borderColor:theme.dark?theme.border:ui.border}]}>
        ...
        <ScrollView testID="flight-details-loading-fares" horizontal showsHorizontalScrollIndicator={false} scrollEnabled={false} contentContainerStyle={s.loadingFares}>
          {[0,1].map(key=><View key={key} style={[s.loadingFareCard,theme.dark?s.fareCardUnselectedDark:s.fareCardUnselectedLight,{width:fareCardWidth,backgroundColor:theme.surface,borderColor:theme.border}]}>
    ...
    <View testID="flight-details-loading-checkout" pointerEvents="none" accessibilityElementsHidden importantForAccessibility="no-hide-descendants" style={[s.sticky,theme.dark?s.stickyDark:s.stickyLight,{paddingBottom:Math.max(bottomInset,10),backgroundColor:theme.surface,borderTopColor:theme.border}]}>
...
function Itinerary({leg,index,offerAirlineName,offerAirlineLogo,theme,intlLocale}:{leg:FlightLeg;index:number;offerAirlineName:string;offerAirlineLogo?:string|null;theme:ReturnType<typeof useAppTheme>["theme"];intlLocale:string}) {
  ...
  return <View style={[s.itineraryCard,theme.dark?s.itineraryCardDark:s.itineraryCardLight,{backgroundColor:theme.surface,borderColor:theme.dark?theme.border:ui.border}]}>
