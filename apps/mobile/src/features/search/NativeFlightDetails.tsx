...
export function NativeFlightDetails({ params }: { params: Params }) {
  ...
  return <SafeAreaView edges={["top"]} style={[s.safe,{backgroundColor:theme.background}]}><TopBar backgroundColor={theme.background} hasScrolled={hasScrolled}><View style={s.topActions}><IconButton label={saved?"Remove saved flight":"Save flight"} onPress={()=>savedFlights.toggle(savedOffer,nativeFlightEditSearchParams(details,one(params.currency)))}><Heart size={20} color={androidFavoriteColors.stroke} fill={saved?androidFavoriteColors.savedFill:androidFavoriteColors.unsavedFill}/></IconButton><IconButton label="Share flight" onPress={()=>void share()}><FlowIcon name="share" size={20} color={theme.icon}/></IconButton></View></TopBar><ScrollView testID="flight-details-scroll-content" scrollEventThrottle={16} onScroll={({nativeEvent})=>{const next=nativeEvent.contentOffset.y>1;if(next!==hasScrolledRef.current){hasScrolledRef.current=next;setHasScrolled(next);}}} contentContainerStyle={[s.content,{paddingBottom:120+inset.bottom}]}>
    ...
    <ScrollView accessibilityRole="radiogroup" horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.fares}>
      {details.fareChoices.map((choice)=>{
        ...
        return <Pressable accessibilityRole="radio" accessibilityState={{selected:isSelected}} key={choice.key} onPress={()=>setSelectedKey(choice.key)} style={[s.fareCard,{width:fareCardWidth,backgroundColor:theme.surface,borderColor:isSelected?ui.blue:theme.border},isSelected?s.fareCardSelected:s.fareCardUnselected]}>
          <View style={s.fareIdentity}>
            <View style={s.fareIconContainer}><FlowIcon name="briefcase" size={19} color={ui.blue}/></View>
            <View style={s.fareIdentityCopy}>
              <Text style={[s.fareLabel,{color:theme.textPrimary}]}>{choice.label}</Text>
              <Text style={[s.farePrice,{color:isSelected?ui.blue:theme.textPrimary}]}>{displayPrices[choice.key]?.formatted??"—"}</Text>
            </View>
          </View>
          <View style={s.fareBenefits}>{fareTerms.map((row,i)=><FareBenefitRow key={`${row.index}-${row.rowIndex}-${i}`} semantic={row.term.semantic} text={row.text} textColor={theme.textSecondary}/>)}</View>
        </Pressable>;
      })}
    </ScrollView>
...
function FareBenefitRow({semantic,text,textColor}:{semantic:"positive"|"negative"|"informational";text:string;textColor:string}){return <View style={s.fareBenefitRow}><View style={s.fareStatusColumn}><FareStatusIcon semantic={semantic}/></View><Text style={[s.fareBenefitText,{color:textColor}]}>{text}</Text></View>}
    ...
    <View style={s.airlineRows}>{leg.segments.map((segment,i)=>{
      ...
      return <View key={`${segment.originAirport}-${segment.destinationAirport}-${segment.departureTime}-${i}`} style={s.airlineRow}>
        ...
        <View style={s.airlineCopy}>
          ...
          {segment.distanceKm!==undefined?<Text style={[s.flightDistance,{color:theme.textSecondary}]}>Flight distance · {Math.round(segment.distanceKm).toLocaleString()} km</Text>:null}
...
function FareSurface({tab,choice,dealPrices,onDeal,booking,fareReady,theme}:{tab:"deals"|"details"|"conditions"|"extras";choice:FlightDetailsFareChoice;dealPrices:Record<string,DisplayPrice>;onDeal:(id:string)=>Promise<void>;booking:boolean;fareReady:boolean;theme:ReturnType<typeof useAppTheme>["theme"]}) {
  ...
  if(tab==="details") return <View style={s.infoSections}>
    ...
    {(p?.totalEmissionsKg!==undefined||p?.updatedAt)?<View style={[s.secondaryFacts,{borderTopColor:theme.border}]}>{detailRow("Estimated CO₂",p?.totalEmissionsKg!==undefined?`${p.totalEmissionsKg.toLocaleString()} kg for this offer`:undefined)}{detailRow("Provider offer last updated",p?.updatedAt&&providerTimestamp(p.updatedAt))}</View>:null}
  </View>;
...
const s=StyleSheet.create({safe:{flex:1},topBar:{minHeight:52,flexDirection:"row",alignItems:"center",justifyContent:"space-between",paddingHorizontal:16,paddingVertical:4,zIndex:1},topBarScrolled:{borderBottomWidth:StyleSheet.hairlineWidth,borderBottomColor:"rgba(15, 23, 42, 0.12)",shadowColor:"#0F172A",shadowOffset:{width:0,height:1},shadowOpacity:0.06,shadowRadius:4},topActions:{flexDirection:"row",alignItems:"center",gap:2,flexShrink:0},back:{minHeight:44,flexShrink:1,minWidth:0,flexDirection:"row",alignItems:"center",gap:7},backText:{color:ui.blue,fontWeight:"800",flexShrink:1},center:{flex:1,justifyContent:"center",padding:24,gap:16},content:{paddingHorizontal:18,paddingTop:5,gap:14},routeSummary:{flexDirection:"row",alignItems:"flex-start",gap:10,marginBottom:4},routeContent:{flex:1,minWidth:0,gap:2},title:{fontSize:23,fontWeight:"900"},route:{fontSize:27,fontWeight:"900"},bodyText:{fontSize:14,lineHeight:20},iconButton:{width:44,height:44,alignItems:"center",justifyContent:"center"},edit:{minHeight:44,flexShrink:0,flexDirection:"row",alignItems:"center",gap:6,paddingHorizontal:10,borderWidth:1,borderColor:ui.blue,borderRadius:10},editText:{color:ui.blue,fontWeight:"800"},sectionTitle:{fontSize:20,fontWeight:"900",marginTop:8},card:{borderWidth:1,borderRadius:14,padding:14,gap:7},itineraryStack:{gap:14,marginTop:-8},itineraryCard:{borderWidth:1,borderRadius:15,padding:15,gap:0,shadowColor:"#0F172A",shadowOffset:{width:0,height:3},shadowOpacity:.07,shadowRadius:12,elevation:1},itineraryHeader:{flexDirection:"row",alignItems:"flex-start",justifyContent:"space-between",gap:12},direction:{color:ui.blue,fontSize:12,lineHeight:17,fontWeight:"700",letterSpacing:.4,textTransform:"uppercase",flexShrink:1},itineraryDate:{fontSize:12,lineHeight:17,fontWeight:"500",textAlign:"right",flexShrink:0},journeySummary:{flexDirection:"row",alignItems:"center",gap:8,marginTop:16},journeyEndpoint:{flex:1.1,minWidth:0,gap:3},arrivalEndpoint:{alignItems:"flex-end"},journeyTime:{fontSize:22,lineHeight:28,fontWeight:"700"},airportCode:{fontSize:17,lineHeight:22,fontWeight:"700"},journeyCenter:{flex:.8,minWidth:72,alignItems:"center",gap:5},journeyDuration:{fontSize:12,lineHeight:17,fontWeight:"600",textAlign:"center"},flightPath:{alignSelf:"stretch",flexDirection:"row",alignItems:"center"},pathDot:{width:6,height:6,borderRadius:3,backgroundColor:ui.blue,flexShrink:0},pathLine:{height:StyleSheet.hairlineWidth,flex:1,minWidth:4},stopStatus:{fontSize:12,lineHeight:17,fontWeight:"500",textAlign:"center"},connectionList:{marginTop:7,gap:2,alignItems:"center"},connectionText:{fontSize:11,lineHeight:16,fontWeight:"500",textAlign:"center"},airportDetails:{flexDirection:"row",alignItems:"flex-start",gap:20,marginTop:16},airportColumn:{flex:1,minWidth:0,gap:6},arrivalAirportColumn:{alignItems:"flex-end"},airportName:{fontSize:13,lineHeight:18,fontWeight:"500"},terminal:{fontSize:12,lineHeight:17,fontWeight:"400"},arrivalText:{textAlign:"right"},itineraryDivider:{height:StyleSheet.hairlineWidth,marginVertical:12},airlineRows:{gap:12},airlineRow:{flexDirection:"row",alignItems:"flex-start",gap:10},airlineCopy:{flex:1,minWidth:0,gap:1},airlineName:{fontSize:14,lineHeight:19,fontWeight:"600"},flightNumber:{fontSize:12,lineHeight:17,fontWeight:"500"},flightDistance:{fontSize:11,lineHeight:16,fontWeight:"400",marginTop:1},operatedBy:{fontSize:11,lineHeight:16,fontWeight:"400",marginTop:1},small:{fontSize:12,lineHeight:17},fares:{gap:10,paddingRight:38},fareCard:{minHeight:176,borderRadius:15,padding:15,gap:16},fareCardSelected:{borderWidth:2},fareCardUnselected:{borderWidth:1},fareIdentity:{flexDirection:"row",alignItems:"center",gap:11},fareIconContainer:{width:40,height:40,borderRadius:10,alignItems:"center",justifyContent:"center",backgroundColor:"rgba(7, 94, 232, 0.08)"},fareIdentityCopy:{flex:1,minWidth:0,gap:2},fareLabel:{fontSize:16,lineHeight:20,fontWeight:"700"},farePrice:{fontSize:21,lineHeight:26,fontWeight:"800"},fareBenefits:{gap:9},fareBenefitRow:{flexDirection:"row",alignItems:"flex-start",gap:9},fareStatusColumn:{width:20,alignItems:"center",paddingTop:1},fareStatus:{width:19,height:19,borderRadius:10,borderWidth:1,alignItems:"center",justifyContent:"center"},fareStatusPositive:{borderColor:ui.green},fareStatusNeutral:{borderColor:"#94A3B8"},fareStatusMinus:{width:8,height:1.5,borderRadius:1,backgroundColor:"#64748B"},fareStatusDot:{width:3,height:3,borderRadius:2,backgroundColor:"#64748B"},fareBenefitText:{flex:1,minWidth:0,fontSize:13,lineHeight:18,fontWeight:"500"},fareInfoDeck:{borderWidth:1,borderRadius:15,overflow:"hidden"},fareTabRail:{flexGrow:0},fareTabRailContent:{paddingHorizontal:14},fareTabList:{flexDirection:"row",gap:22},fareInfoTab:{minHeight:48,justifyContent:"center",position:"relative",paddingHorizontal:2},fareInfoTabText:{fontSize:13,lineHeight:18,fontWeight:"600"},fareInfoTabTextActive:{fontWeight:"700"},fareTabIndicator:{position:"absolute",height:3,borderRadius:2,backgroundColor:ui.blue,left:2,right:2,bottom:0},fareInfoDivider:{height:StyleSheet.hairlineWidth},fareInfoBody:{paddingHorizontal:14,paddingVertical:4},infoSections:{gap:0},infoSection:{paddingVertical:12},sectionDivider:{borderTopWidth:StyleSheet.hairlineWidth},groupLabel:{fontSize:14,lineHeight:19,fontWeight:"600",marginBottom:7},cabinGroup:{paddingVertical:8,gap:7},detailRow:{flexDirection:"row",justifyContent:"space-between",alignItems:"flex-start",gap:16},detailLabel:{flexShrink:0,fontSize:13,lineHeight:18,fontWeight:"500"},detailValue:{flex:1,fontSize:13,lineHeight:18,fontWeight:"600",textAlign:"right"},detailTotal:{fontWeight:"800"},quietText:{fontSize:13,lineHeight:19,fontWeight:"400"},secondaryFacts:{borderTopWidth:StyleSheet.hairlineWidth,paddingVertical:12,gap:7},emptyState:{paddingVertical:24,paddingHorizontal:10,alignItems:"center",gap:5},emptyTitle:{fontSize:14,lineHeight:19,fontWeight:"700",textAlign:"center"},emptyDescription:{fontSize:13,lineHeight:19,fontWeight:"400",textAlign:"center"},dealRow:{minHeight:68,paddingVertical:13,flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:12},dealIdentity:{flex:1,minWidth:0,flexDirection:"row",alignItems:"center",gap:10},providerMark:{width:32,height:32,borderRadius:8,borderWidth:1,alignItems:"center",justifyContent:"center"},providerMonogram:{fontSize:13,fontWeight:"700"},dealProvider:{flex:1,fontSize:13,lineHeight:18,fontWeight:"600"},dealAction:{alignItems:"flex-end",gap:3},dealPrice:{fontSize:13,lineHeight:18,fontWeight:"700"},viewDeal:{color:ui.blue,fontSize:12,lineHeight:17,fontWeight:"700"},conditionRow:{paddingVertical:13,flexDirection:"row",alignItems:"flex-start",gap:11},conditionCopy:{flex:1,minWidth:0,gap:2},conditionTitle:{fontSize:14,lineHeight:19,fontWeight:"600"},conditionState:{fontSize:13,lineHeight:18,fontWeight:"500"},conditionScope:{fontSize:11,lineHeight:16,fontWeight:"400"},infoRow:{paddingVertical:12,flexDirection:"row",alignItems:"flex-start",gap:11},infoIcon:{width:20,height:20,borderRadius:10,borderWidth:1,alignItems:"center",justifyContent:"center"},infoGlyph:{fontSize:12,lineHeight:15,fontWeight:"800"},infoCopy:{flex:1,minWidth:0,gap:2},infoTitle:{fontSize:13,lineHeight:18,fontWeight:"600"},infoDescription:{fontSize:13,lineHeight:19,fontWeight:"400"},linkRow:{minHeight:44,flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:12},linkText:{flex:1,fontSize:13,lineHeight:18,fontWeight:"700"},serviceRow:{paddingVertical:13,flexDirection:"row",alignItems:"flex-start",gap:11},serviceIcon:{width:32,height:32,borderRadius:8,borderWidth:1,alignItems:"center",justifyContent:"center"},serviceCopy:{flex:1,minWidth:0,gap:3},serviceHeading:{flexDirection:"row",alignItems:"flex-start",justifyContent:"space-between",gap:12},serviceDescription:{flex:1,fontSize:13,lineHeight:18,fontWeight:"600"},servicePrice:{flexShrink:0,maxWidth:"42%",fontSize:13,lineHeight:18,fontWeight:"700",textAlign:"right"},serviceMeta:{fontSize:12,lineHeight:17,fontWeight:"400"},notice:{padding:12,borderRadius:10,borderWidth:1,borderColor:ui.blue},noticeText:{color:ui.blue,fontWeight:"700"},sticky:{position:"absolute",left:0,right:0,bottom:0,minHeight:88,borderTopWidth:1,paddingHorizontal:18,paddingTop:10,flexDirection:"row",justifyContent:"space-between",alignItems:"center",gap:12},total:{fontSize:22,fontWeight:"900"}});
