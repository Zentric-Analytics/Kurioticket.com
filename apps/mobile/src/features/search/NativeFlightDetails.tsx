...
export function NativeFlightDetails({ params }: { params: Params }) {
  ...
  const tripMetadata=[`${FLIGHT_TRIP_TYPE_LABELS[details.search.tripType]}`,`${details.search.travelers} traveler${details.search.travelers===1?"":"s"}`].join(" • ");
      ...
      {details.fareChoices.map((choice)=>{
        ...
        return <Pressable accessibilityRole="radio" accessibilityState={{selected:isSelected}} key={choice.key} onPress={()=>setSelectedKey(choice.key)} style={[s.fareCard,{width:loadedFareCardWidth,backgroundColor:theme.surface,borderColor:isSelected?ui.blue:theme.border},isSelected?s.fareCardSelected:s.fareCardUnselected]}>
          <View style={s.fareIdentity}>
            <View style={[s.fareIconContainer,{backgroundColor:isSelected?"rgba(7, 94, 232, 0.08)":theme.background}]}><Luggage size={15} color={ui.blue}/></View>
            <Text style={[s.fareLabel,{color:theme.textPrimary}]}>{choice.label}</Text>
            <Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8} accessibilityLabel={displayPrices[choice.key]?.accessibilityLabel} style={[s.farePrice,{color:isSelected?ui.blue:theme.textPrimary}]}>{displayPrices[choice.key]?.formatted??"—"}</Text>
          </View>
    ...
    <ScrollView testID="flight-details-loading-skeleton" accessibilityRole="progressbar" accessibilityState={{busy:true}} accessibilityLabel="Loading flight details" contentContainerStyle={[s.loadingContent,{paddingBottom:32+bottomInset}]}>
      <Animated.View pointerEvents="none" style={[s.loadingBody,{opacity}]}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.loadingFares}>
          {[0,1].map(key=><View key={key} style={[s.loadingFareCard,{width:fareCardWidth,backgroundColor:theme.surface,borderColor:theme.border}]}><View style={s.loadingFareIdentity}><View style={[s.loadingFareIcon,placeholder]}/><View style={[s.loadingLine,s.loadingFareName,placeholder]}/><View style={[s.loadingLine,s.loadingFarePrice,placeholder]}/></View>{(["88%","72%","80%"] as const).map((width,index)=><View key={index} style={s.loadingBenefitRow}><View style={[s.loadingBenefitDot,placeholder]}/><View style={[s.loadingLine,{width},placeholder]}/></View>)}</View>)}
        </ScrollView>
  ...
  return <View style={[s.itineraryCard,{backgroundColor:theme.surface,borderColor:theme.border}]}>
    ...
    {hasTechnicalInformation?<>
      ...
      <View style={s.technicalInformation}>
        ...
        {distanceSegments.map((segment,i)=><View key={`${segment.originAirport}-${segment.destinationAirport}-distance-${i}`} style={s.technicalRow}><Text style={[s.technicalLabel,{color:theme.textPrimary}]}>{leg.segments.length===1?"Distance":`${segment.originAirport} → ${segment.destinationAirport} distance`}</Text><Text style={[s.technicalValue,{color:theme.textSecondary}]}>{Math.round(segment.distanceKm!).toLocaleString()} km</Text></View>)}
        {aircraftSegments.map(({segment,aircraft},i)=><View key={`${segment.originAirport}-${segment.destinationAirport}-aircraft-${i}`} style={s.technicalRow}><Text style={[s.technicalLabel,{color:theme.textPrimary}]}>{leg.segments.length===1?"Aircraft":`${segment.originAirport} → ${segment.destinationAirport} aircraft`}</Text><Text style={[s.technicalValue,{color:theme.textSecondary}]}>{aircraft}</Text></View>)}
...
function FareSurface({tab,choice,dealPrices,onDeal,booking,fareReady,theme}:{tab:"deals"|"details"|"conditions"|"extras";choice:FlightDetailsFareChoice;dealPrices:Record<string,DisplayPrice>;onDeal:(id:string)=>Promise<void>;booking:boolean;fareReady:boolean;theme:ReturnType<typeof useAppTheme>["theme"]}) {
  ...
  const groupLabel=(label:string)=><Text style={[s.fareGroupLabel,{color:theme.textSecondary}]}>{label}</Text>;
  const divider=(key:string)=><View key={key} style={[s.fareGroupDivider,{backgroundColor:theme.border}]}/>;
  const freshness=(updatedAt:string)=><View style={s.providerFreshness}><Text style={[s.providerFreshnessLabel,{color:theme.textSecondary}]}>Provider offer last updated</Text><Text style={[s.providerFreshnessValue,{color:theme.textSecondary}]}>{providerTimestamp(updatedAt)}</Text></View>;
  ...
  if(tab==="extras") return <View style={s.infoSections}><View style={s.infoSection}>{groupLabel("Optional services")}{p?.optionalServices?.length?p.optionalServices.map((service,index)=><View key={`${service.type}-${service.description}-${service.journeyContext||"trip"}-${service.price}-${service.currency}-${service.maximumQuantity??"na"}-${service.travelerCount??"na"}-${index}`} style={[s.serviceRow,index>0&&{borderTopColor:theme.border,borderTopWidth:StyleSheet.hairlineWidth}]}><View style={s.serviceCopy}><View style={s.serviceHeading}><Text style={[s.serviceDescription,{color:theme.textPrimary}]}>{service.description}</Text><Text style={[s.servicePrice,{color:theme.textPrimary}]}>{sourceMoney(service.price,service.currency)}{service.pricedPerTraveler?" each":""}</Text></View>{service.travelerCount?<Text style={[s.serviceMeta,{color:theme.textSecondary}]}>Available for {service.travelerCount} traveler{service.travelerCount===1?"":"s"}</Text>:null}{service.maximumQuantity!==undefined?<Text style={[s.serviceMeta,{color:theme.textSecondary}]}>{service.pricedPerTraveler?"Maximum quantity per traveler":"Maximum quantity"}: {service.maximumQuantity}</Text>:null}{service.journeyContext?<Text style={[s.serviceMeta,{color:theme.textSecondary}]}>{service.journeyContext}</Text>:null}</View></View>):<Text style={[s.quietText,s.optionalServicesEmpty,{color:theme.textSecondary}]}>No optional services were supplied by this provider.</Text>}</View>{p?.supportedLoyaltyProgrammes?.length?<View style={[s.infoSection,s.sectionDivider,{borderTopColor:theme.border}]}>{groupLabel("Loyalty programmes")}<Text style={[s.loyaltyProgrammes,{color:theme.textPrimary}]}>{p.supportedLoyaltyProgrammes.join(", ")}</Text></View>:null}</View>;
}
const s=StyleSheet.create({safe:{flex:1},loadingContent:{paddingHorizontal:18,paddingTop:5},loadingBody:{gap:14},loadingRouteSummary:{gap:6,marginBottom:0},loadingLine:{height:10,borderRadius:5},loadingRouteLine:{width:"62%",height:18,borderRadius:8},loadingMetadataLine:{width:"38%",height:10},loadingItineraryCard:{height:226,borderWidth:1,borderRadius:15,padding:15,marginHorizontal:-6},loadingDirectionLine:{width:"24%",height:9},loadingJourneyRow:{flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:18,marginTop:25},loadingTimeLine:{width:"22%",height:16},loadingPathLine:{flex:1,height:4},loadingAirportRow:{flexDirection:"row",justifyContent:"space-between",marginTop:11},loadingAirportLine:{width:"18%",height:12},loadingDivider:{height:StyleSheet.hairlineWidth,marginVertical:28},loadingAirlineLine:{width:"48%",height:12},loadingFareHeading:{width:132,height:18,marginTop:8},loadingFares:{gap:10,paddingRight:38},loadingFareCard:{height:132,borderWidth:1,borderRadius:15,padding:12,gap:8},loadingFareIdentity:{flexDirection:"row",alignItems:"center",gap:8},loadingFareIcon:{width:30,height:30,borderRadius:8},loadingFareName:{flex:1,height:12},loadingFarePrice:{width:"36%",height:16},loadingBenefitRow:{flexDirection:"row",alignItems:"center",gap:7},loadingBenefitDot:{width:16,height:16,borderRadius:8},loadingInfoDeck:{height:174,borderWidth:1,borderRadius:15,overflow:"hidden",marginHorizontal:-6},loadingTabs:{height:48,flexDirection:"row",alignItems:"center",gap:22,paddingHorizontal:14},loadingInfoBody:{padding:18,gap:17},topBar:{minHeight:52,flexDirection:"row",alignItems:"center",justifyContent:"space-between",paddingHorizontal:16,paddingVertical:4,zIndex:1},topBarScrolled:{borderBottomWidth:StyleSheet.hairlineWidth,borderBottomColor:"rgba(15, 23, 42, 0.12)"},back:{minHeight:44,flexShrink:1,minWidth:0,flexDirection:"row",alignItems:"center",gap:7},backText:{color:ui.blue,fontWeight:"800",flexShrink:1},center:{flex:1,justifyContent:"center",padding:24,gap:16},content:{paddingHorizontal:18,paddingTop:5,gap:14},routeSummary:{flexDirection:"row",alignItems:"center",gap:4,marginBottom:-4},routeContent:{flex:1,minWidth:0,gap:1},routeActions:{flexDirection:"row",alignItems:"center",gap:0,flexShrink:0},title:{fontSize:23,fontWeight:"900"},route:{fontSize:18,lineHeight:22,fontWeight:"800"},routeMetadata:{fontSize:11,lineHeight:15,fontWeight:"500"},bodyText:{fontSize:14,lineHeight:20},iconButton:{width:44,height:44,alignItems:"center",justifyContent:"center"},sectionTitle:{fontSize:20,fontWeight:"900",marginTop:8},fareSectionTitle:{fontSize:17,lineHeight:22,fontWeight:"700",marginTop:8},card:{borderWidth:1,borderRadius:14,padding:14,gap:7},itineraryStack:{gap:14,marginHorizontal:-6},itineraryCard:{borderWidth:1,borderRadius:15,padding:15,gap:0,shadowColor:"#0F172A",shadowOffset:{width:0,height:3},shadowOpacity:.07,shadowRadius:12,elevation:1},itineraryHeader:{flexDirection:"row",alignItems:"flex-start",justifyContent:"space-between",gap:12},direction:{color:ui.blue,flexShrink:1,fontSize:10,lineHeight:14,fontWeight:"700",letterSpacing:.4,textTransform:"uppercase"},itineraryDate:{flexShrink:0,fontSize:11,lineHeight:15,fontWeight:"500",textAlign:"right"},journeySummary:{flexDirection:"row",alignItems:"center",gap:8,marginTop:14},journeyEndpoint:{flex:1.1,minWidth:0,gap:3},arrivalEndpoint:{alignItems:"flex-end"},journeyTime:{fontSize:16,lineHeight:21,fontWeight:"700"},airportCode:{fontSize:12,lineHeight:16,fontWeight:"700"},airportDate:{marginTop:1,fontSize:9.5,lineHeight:12,fontWeight:"500"},journeyCenter:{flex:.8,minWidth:72,alignItems:"center",gap:5},journeyDuration:{fontSize:11,lineHeight:16,fontWeight:"600",textAlign:"center"},flightPath:{alignSelf:"stretch",flexDirection:"row",alignItems:"center"},pathDot:{width:6,height:6,borderRadius:3,backgroundColor:ui.blue,flexShrink:0},pathLine:{height:StyleSheet.hairlineWidth,flex:1,minWidth:4},stopStatus:{fontSize:10,lineHeight:13,fontWeight:"500",textAlign:"center"},connectionList:{marginTop:12,borderRadius:9,paddingHorizontal:11,paddingVertical:9,gap:7},connectionRow:{gap:1},connectionDuration:{fontSize:11,lineHeight:16,fontWeight:"600"},connectionAirport:{fontSize:10,lineHeight:15,fontWeight:"400"},airportDetails:{flexDirection:"row",alignItems:"flex-start",gap:20,marginTop:14},airportColumn:{flex:1,minWidth:0,gap:6},arrivalAirportColumn:{alignItems:"flex-end"},airportName:{fontSize:12,lineHeight:17,fontWeight:"600"},terminal:{fontSize:11,lineHeight:16,fontWeight:"400"},arrivalText:{textAlign:"right"},itineraryDivider:{height:StyleSheet.hairlineWidth,marginVertical:12},airlineRows:{gap:8,marginTop:10},airlineRow:{flexDirection:"row",alignItems:"flex-start",gap:10},airlineCopy:{flex:1,minWidth:0,gap:1},airlineName:{fontSize:13,lineHeight:18,fontWeight:"600"},flightNumber:{fontSize:11,lineHeight:16,fontWeight:"500"},technicalInformation:{gap:6},technicalHeading:{fontSize:11,lineHeight:15,fontWeight:"700",letterSpacing:.7,textTransform:"uppercase",marginBottom:2},technicalRow:{flexDirection:"row",alignItems:"flex-start",justifyContent:"space-between",gap:12},technicalLabel:{flex:1,minWidth:0,fontSize:11,lineHeight:16,fontWeight:"600"},technicalValue:{flexShrink:1,maxWidth:"52%",fontSize:11,lineHeight:16,fontWeight:"400",textAlign:"right"},operatedBy:{fontSize:11,lineHeight:16,fontWeight:"400",marginTop:1},small:{fontSize:12,lineHeight:17},fares:{gap:10},faresMultiple:{paddingRight:38},faresSingle:{paddingRight:0},fareCard:{borderRadius:15,padding:12,gap:8},fareCardSelected:{borderWidth:1.5},fareCardUnselected:{borderWidth:1},fareIdentity:{flexDirection:"row",alignItems:"center",gap:8},fareIconContainer:{width:30,height:30,borderRadius:8,alignItems:"center",justifyContent:"center",flexShrink:0},fareLabel:{flex:1,minWidth:0,fontSize:13,lineHeight:17,fontWeight:"700"},farePrice:{flexShrink:1,maxWidth:"55%",fontSize:18,lineHeight:22,fontWeight:"800",textAlign:"right"},fareBenefits:{gap:6},fareBenefitRow:{flexDirection:"row",alignItems:"flex-start",gap:7},fareStatusColumn:{width:17,alignItems:"center"},fareStatus:{width:16,height:16,borderRadius:8,borderWidth:1,alignItems:"center",justifyContent:"center"},fareStatusPositive:{borderColor:ui.green},fareStatusNeutral:{borderColor:"#94A3B8"},fareStatusMinus:{width:8,height:1.5,borderRadius:1,backgroundColor:"#64748B"},fareStatusDot:{width:3,height:3,borderRadius:2,backgroundColor:"#64748B"},fareBenefitCopy:{flex:1,minWidth:0,gap:2},fareBenefitHeading:{flexDirection:"row",alignItems:"flex-start",justifyContent:"space-between",gap:10},fareBenefitTitle:{flex:1,minWidth:0,fontSize:12,lineHeight:17,fontWeight:"600"},fareBenefitValue:{flexShrink:0,maxWidth:"44%",fontSize:11,lineHeight:17,fontWeight:"600",textAlign:"right"},fareBenefitDetail:{fontSize:11,lineHeight:16,fontWeight:"400"},fareInfoDeck:{borderWidth:1,borderRadius:15,overflow:"hidden",marginHorizontal:-6},fareTabRail:{flexGrow:0},fareTabRailContent:{paddingHorizontal:14},fareTabList:{flexDirection:"row",gap:22},fareInfoTab:{minHeight:48,justifyContent:"center",position:"relative",paddingHorizontal:2},fareInfoTabText:{fontSize:13,lineHeight:18,fontWeight:"600"},fareInfoTabTextActive:{fontWeight:"700"},fareTabIndicator:{position:"absolute",height:2,borderRadius:1,backgroundColor:ui.blue,left:2,right:2,bottom:0},fareInfoDivider:{height:StyleSheet.hairlineWidth},fareInfoBody:{paddingHorizontal:14,paddingVertical:4},infoSections:{gap:0},infoSection:{paddingVertical:12},sectionDivider:{borderTopWidth:StyleSheet.hairlineWidth},fareGroupLabel:{fontSize:11,lineHeight:15,fontWeight:"700",letterSpacing:.7,textTransform:"uppercase",marginBottom:8},segmentContext:{fontSize:14,lineHeight:19,fontWeight:"700",marginBottom:10},cabinGroup:{paddingVertical:10},fareFactGroup:{gap:7},fareGroupDivider:{height:StyleSheet.hairlineWidth,marginVertical:11},detailRow:{flexDirection:"row",justifyContent:"space-between",alignItems:"flex-start",gap:16},detailLabel:{flexShrink:0,fontSize:13,lineHeight:18,fontWeight:"500"},detailValue:{flex:1,fontSize:13,lineHeight:18,fontWeight:"600",textAlign:"right"},detailTotal:{fontWeight:"800"},quietText:{fontSize:13,lineHeight:19,fontWeight:"400"},emissionsCard:{marginVertical:12,borderRadius:8,paddingHorizontal:11,paddingVertical:9,flexDirection:"row",flexWrap:"wrap",alignItems:"center",justifyContent:"space-between",gap:8},emissionsIdentity:{flexDirection:"row",alignItems:"center",flexGrow:1,flexShrink:1,minWidth:180,gap:7},emissionsLabel:{flexShrink:1,fontSize:13,lineHeight:18,fontWeight:"600"},emissionsValueGroup:{marginLeft:"auto",alignItems:"flex-end",flexShrink:1},emissionsValue:{fontSize:14,lineHeight:18,fontWeight:"700",textAlign:"right"},emissionsContext:{fontSize:11,lineHeight:15,fontWeight:"500",textAlign:"right"},secondaryFacts:{borderTopWidth:StyleSheet.hairlineWidth,paddingVertical:11},providerFreshness:{gap:1},providerFreshnessLabel:{fontSize:11,lineHeight:16,fontWeight:"500"},providerFreshnessValue:{fontSize:11,lineHeight:16,fontWeight:"400"},emptyState:{paddingVertical:18,paddingHorizontal:10,alignItems:"center",gap:5},emptyTitle:{fontSize:14,lineHeight:19,fontWeight:"700",textAlign:"center"},emptyDescription:{fontSize:13,lineHeight:19,fontWeight:"400",textAlign:"center"},dealRow:{minHeight:68,paddingVertical:13,flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:12},dealProvider:{flex:1,minWidth:0,fontSize:13,lineHeight:18,fontWeight:"600"},dealAction:{alignItems:"flex-end",gap:3},dealPrice:{fontSize:13,lineHeight:18,fontWeight:"700"},viewDealAction:{minHeight:44,justifyContent:"center"},viewDeal:{color:ui.blue,fontSize:12,lineHeight:17,fontWeight:"700"},conditionGroup:{paddingVertical:12},conditionRow:{paddingVertical:7,flexDirection:"row",alignItems:"flex-start",gap:11},conditionCopy:{flex:1,minWidth:0,gap:1},conditionState:{fontSize:13,lineHeight:18,fontWeight:"600"},conditionScope:{fontSize:11,lineHeight:16,fontWeight:"400"},conditionPenalty:{fontSize:11,lineHeight:16,fontWeight:"500"},factDescription:{fontSize:13,lineHeight:19,fontWeight:"400",marginBottom:7},airlineIdentity:{fontSize:13,lineHeight:19,fontWeight:"600",marginBottom:3},linkRow:{minHeight:44,flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:12},linkText:{flex:1,fontSize:13,lineHeight:18,fontWeight:"700"},serviceRow:{paddingVertical:12},serviceCopy:{minWidth:0,gap:3},serviceHeading:{flexDirection:"row",alignItems:"flex-start",justifyContent:"space-between",gap:12},serviceDescription:{flex:1,fontSize:13,lineHeight:18,fontWeight:"600"},servicePrice:{flexShrink:0,maxWidth:"42%",fontSize:13,lineHeight:18,fontWeight:"700",textAlign:"right"},serviceMeta:{fontSize:12,lineHeight:17,fontWeight:"400"},optionalServicesEmpty:{paddingBottom:2},loyaltyProgrammes:{fontSize:13,lineHeight:19,fontWeight:"500",flexWrap:"wrap"},notice:{padding:12,borderRadius:10,borderWidth:1,borderColor:ui.blue},noticeText:{color:ui.blue,fontWeight:"700"},sticky:{position:"absolute",left:0,right:0,bottom:0,minHeight:88,borderTopWidth:1,paddingHorizontal:18,paddingTop:10,flexDirection:"row",justifyContent:"space-between",alignItems:"center",gap:12},total:{fontSize:22,fontWeight:"900"}});
