...
export function HotelResultsBrandLegalFooter() {
  ...
  const sellerNotice = `${legalProfile.company.legalName} — ${getCaliforniaSellerOfTravelNotice()}`;
  ...
  return (
    ...
    >
      ...
      <View accessibilityLabel="Hotel results legal links" style={[styles.links, { direction }]}>
        <LegalLink label={copy.privacy} onPress={() => router.push(privacyRoute)} />
        <LegalLink label={copy.terms} onPress={() => router.push(termsRoute)} />
        <LegalLink label={copy.cookies} onPress={() => void Linking.openURL(COOKIE_POLICY_URL)} />
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  logo: { width: 150, height: 30, alignSelf: "flex-start", marginBottom: 8 },
  ...
  linkText: { color: colors.blue, fontFamily: appFonts.semibold, fontSize: 12, lineHeight: 18 },
});
