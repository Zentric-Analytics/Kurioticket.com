...
export function CarFilterSheet({ visible, groupId, results, filters, onChange, onClose }: Props) {
  ...
  const update = (next: SelectedCarFilters) => {
    onChange(next);
  ...
  return (
    <FlightResultsSheetShell
      visible={visible}
      fullScreen={full}
      quickBackdropVariant="legacy"
      title={title}
      subtitle={full ? (active ? `${active} ${copy.applied}` : copy.allCars) : undefined}
      ...
      footer={
        <View style={styles.footer}>
          {full && active ? (
            <Pressable accessibilityRole="button" onPress={clear} style={styles.footerClear}>
              <Text style={styles.clearText}>{copy.clearAll}</Text>
            </Pressable>
          ) : null}
          <View style={styles.primary}>
            <View accessibilityLiveRegion="polite">
              <Button label={filterUpdating ? copy.updatingFilters : `${copy.show} ${matching} ${matching === 1 ? copy.car : copy.cars}`} loading={filterUpdating} onPress={onClose} />
            </View>
          </View>
        </View>
      }
    >
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        {shownGroups.map((group) => {
          const open = selectedGroup ? true : expanded[group.id] === true;
          const selectedCount = filters[group.id]?.length ?? 0;
          return (
            <View key={group.id} style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border }]}>
              <Pressable accessibilityRole="button" accessibilityState={{ expanded: open }} onPress={() => !selectedGroup && setExpanded((value) => ({ ...value, [group.id]: !open }))} style={[styles.sectionHeader, direction === "rtl" && styles.rtlRow]}>
                <Text
                  style={[
                    styles.sectionTitle,
                    {
                ...
                >
                  {carFilterGroupLabel(copy, group)}
                </Text>
                <View style={[styles.sectionMeta, direction === "rtl" && styles.rtlRow]}>
                  {selectedCount ? <Text style={styles.countBadge}>{selectedCount}</Text> : null}
                  {!selectedGroup ? (
                    <ChevronDown
                      size={19}
                      color={theme.icon}
              ...
              {open ? (
                <View style={[styles.options, { borderTopColor: theme.border }]}>
                  {group.options.map((option) => {
                    const selected = filters[group.id]?.includes(option.id) ?? false;
                    return (
                      <Pressable key={option.id} accessibilityRole="checkbox" accessibilityState={{ checked: selected }} accessibilityLabel={`${carFilterOptionLabel(copy, option)}, ${option.count}`} onPress={() => toggle(group.id, option.id)} style={[styles.row, direction === "rtl" && styles.rtlRow]}>
                        <View
                          style={[
                            styles.box,
                            {
                        ...
                        >
                          {carFilterOptionLabel(copy, option)}
                        </Text>
                        <Text
                          style={[
                            styles.optionCount,
...
const styles = StyleSheet.create({
  ...
  row: { minHeight: 48, flexDirection: "row", alignItems: "center", gap: 11 },
  box: {
    width: 21,
    height: 21,
    borderWidth: 1.5,
