...
export function CarFilterSheet({ visible, groupId, results, filters, onChange, onClose }: Props) {
  ...
  const toggle = (group: string, option: string) => { const selected = filters[group] ?? []; const next = selected.includes(option) ? selected.filter((value) => value !== option) : [...selected, option]; markUpdating(); onChange({ ...filters, [group]: next }); };
  const title = selectedGroup ? carFilterGroupLabel(copy, selectedGroup) : copy.filters;
  return <FlightResultsSheetShell visible={visible} fullScreen={full} quickBackdropVariant="legacy" title={title} subtitle={full ? (active ? `${active} ${copy.applied}` : copy.allCars) : undefined} closeLabel={copy.close} onClose={onClose} headerAction={full && active ? <Pressable accessibilityRole="button" onPress={()=>{markUpdating();clear();}} style={styles.clear}><Text style={styles.clearText}>{copy.clearAll}</Text></Pressable> : undefined} footer={<View style={styles.footer}>{full && active ? <Pressable accessibilityRole="button" onPress={()=>{markUpdating();clear();}} style={styles.footerClear}><Text style={styles.clearText}>{copy.clearAll}</Text></Pressable> : null}<View style={styles.primary}>{filterUpdating?<View accessibilityRole="progressbar" accessibilityLiveRegion="polite" accessibilityState={{busy:true,disabled:true}} style={styles.updating}><ActivityIndicator size="small" color="white"/><Text style={styles.updatingText}>{copy.updatingFilters}</Text></View>:<Button label={`${copy.show} ${matching} ${matching === 1 ? copy.car : copy.cars}`} onPress={onClose} />}</View></View>}>
...
const styles = StyleSheet.create({ scroll: { flexShrink: 1 }, content: { paddingHorizontal: 16, paddingVertical: 16, gap: 10 }, card: { borderWidth: StyleSheet.hairlineWidth, borderRadius: 12, overflow: "hidden" }, sectionHeader: { minHeight: 56, paddingHorizontal: 16, flexDirection: "row", alignItems: "center", gap: 12 }, rtlRow: { flexDirection: "row-reverse" }, sectionTitle: { flex: 1, fontSize: 15, lineHeight: 20, fontWeight: "700", fontFamily: appFonts.bold }, sectionMeta: { flexDirection: "row", alignItems: "center", gap: 8 }, countBadge: { minWidth: 22, borderRadius: 11, paddingHorizontal: 7, paddingVertical: 2, textAlign: "center", color: "#235A9F", backgroundColor: "#E2EAF3", fontSize: 11, fontWeight: "700" }, options: { borderTopWidth: StyleSheet.hairlineWidth, paddingHorizontal: 14, paddingVertical: 6 }, row: { minHeight: 48, flexDirection: "row", alignItems: "center", gap: 11 }, box: { width: 21, height: 21, borderWidth: 1.5, borderRadius: 4, alignItems: "center", justifyContent: "center" }, rowLabel: { flex: 1, fontSize: 14, lineHeight: 19, fontFamily: appFonts.medium }, optionCount: { minWidth: 26, textAlign: "right", fontSize: 12, fontFamily: appFonts.medium }, clear: { minHeight: 44, justifyContent: "center", paddingHorizontal: 8 }, clearText: { color: ui.blue, fontSize: 13, fontWeight: "700", fontFamily: appFonts.bold }, footer: { flexDirection: "row", alignItems: "center", gap: 12 }, footerClear: { minWidth: 82, minHeight: 50, alignItems: "center", justifyContent: "center" }, primary: { flex: 1 }, updating: { height: 50, borderRadius: 10, backgroundColor: ui.blue, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8 }, updatingText: { color: "white", fontSize: 16, fontWeight: "700", fontFamily: appFonts.bold } });
