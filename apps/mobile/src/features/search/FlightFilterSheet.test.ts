...
test("main Filter retains full-screen live editing and a live result action",()=>{assert.match(sheet,/fullScreen=\{full\}/);assert.match(sheet,/fullScreenFooterMinimumBottomPadding=\{12\}/);assert.match(sheet,/const working=full\?filters:draft/);assert.match(sheet,/matchingFlightCount\(results,working/);assert.match(sheet,/View \$\{count\}/);assert.match(sheet,/No matching flights/);assert.match(sheet,/emptyFlightFilters\(\)/);});
