...
test("Car edit modal follows the retained native Hotel sheet architecture",()=>{for(const contract of ["Modal","KeyboardAvoidingView","SafeAreaView","onRequestClose","useSearchPickerMotion","useRetainedPickerContext","Edit search","<CarSearchPanel","editAppearance","submitNavigation=\"replace\"","onBeforeNavigate={onClose}","submitLabel=\"Search\""])assert.ok(modal.includes(contract),contract);assert.doesNotMatch(modal,/>\s*Edit car search\s*</);});
