...
test("one opaque safe-area shell explicitly owns the three child editor safe-area insets", () => {
  ...
  assert.match(shell, /import \{ SafeAreaProvider, SafeAreaView \} from "react-native-safe-area-context"/);
  assert.match(shell, /<Modal[\s\S]*?<SafeAreaProvider>[\s\S]*?<SafeAreaView/);
  assert.match(shell, /edges=\{\["top", "bottom", "left", "right"\]\}/);
  assert.doesNotMatch(shell, /useSafeAreaInsets/);
  assert.doesNotMatch(shell, /paddingTop: insets\.top/);
  assert.doesNotMatch(shell, /paddingBottom: insets\.bottom/);
  assert.doesNotMatch(shell, /initialWindowMetrics/);
