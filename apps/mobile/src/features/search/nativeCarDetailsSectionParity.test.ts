...
test("the web tab reference remains the mobile parity contract", () => {
  for (const contract of ["flex", "min-h-12", "whitespace-nowrap", "font-sans", "font-semibold", "leading-[normal]", "inset-x-2", "h-0.5", "text-[#475569]", "text-[#075EE8]"])
...
test("content below the three tabs shares one section-heading hierarchy", () => {
  ...
  for (const heading of ["compareHeading", "pickupHeading", "locationHeading"]) {
    for (const contract of ["fontSize:12", "lineHeight:18", 'fontWeight:"700"', "fontFamily:appFonts.bold", "letterSpacing:-.2"])
  ...
  for (const contract of ["marginTop:4", "fontSize:11", "lineHeight:16", 'fontWeight:"500"'])
