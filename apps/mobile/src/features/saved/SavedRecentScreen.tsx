...
type SavedCardLayout = ReturnType<typeof regionPreviewCardLayout>;
...
function SavedCard({ model, remove, layout }: { model: SavedCardModel; remove: (item: MobileSavedItem) => void; layout: SavedCardLayout }) {
  const { theme } = useAppTheme();
  const content = <Fragment><Image source={FALLBACK_SOURCE} accessibilityLabel="Travel image" resizeMode="cover" style={[styles.image, { width: layout.imageHeight, height: layout.imageHeight }]} /><View style={styles.copy}><Text numberOfLines={2} style={[styles.name, { color: theme.text }]}>{model.title}</Text><Text numberOfLines={1} style={[styles.secondary, { color: theme.muted }]}>{model.secondary}</Text></View><Pressable accessibilityRole="button" accessibilityLabel={`Remove ${model.title} from saved`} hitSlop={8} onPress={(event) => { event.stopPropagation(); remove(model.item); }} style={({ pressed }) => [styles.removeTouchTarget, pressed && styles.removePressed]}><View style={[styles.remove, { backgroundColor: theme.surface, borderColor: theme.border }]}><FlowIcon name="close" color={theme.icon} size={16} /></View></Pressable></Fragment>;
  const cardStyle = [styles.card, { width: layout.cardWidth, height: layout.cardHeight, backgroundColor: theme.surface, borderColor: theme.border }] as const;
  return model.open ? <Pressable testID="saved-card" accessibilityRole="button" accessibilityLabel={`Open ${model.title}, ${model.secondary}`} onPress={model.open} style={({ pressed }) => [cardStyle, pressed && styles.pressed]}>{content}</Pressable> : <View testID="saved-card" accessibilityLabel={`${model.title}, ${model.secondary}`} style={cardStyle}>{content}</View>;
}
...
export function SavedRecentScreen() {
  ...
  const savedCardLayout = regionPreviewCardLayout(windowWidth);
