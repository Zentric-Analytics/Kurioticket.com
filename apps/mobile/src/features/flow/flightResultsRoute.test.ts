...
test("flight results route uses the approved screen without a legacy alert surface", () => {
  ...
  assert.match(screen, /ApprovedResultsScreen product=\{product\}/);
  assert.doesNotMatch(screen, /LegacyTravelResultsScreen|Track this search/);
});
