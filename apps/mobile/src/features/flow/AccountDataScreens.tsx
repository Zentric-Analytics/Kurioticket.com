...
import { formatLastChecked, formatPriceAlertAmount, priceAlertTripSummary, priceDifferencePresentation, statusLabel } from "./priceAlertPresentation";
...
export function PriceAlertsScreen() {
  ...
  const lastSeen = formatPriceAlertAmount(alert.currency, alert.lastSeenPrice);
  ...
  return <View key={alert.id} style={[styles.card, flowStyles.shadow, { backgroundColor: theme.surface, borderColor: theme.border }]}>
    <View style={styles.cardTopRow}>
      <View style={[styles.alertIconTile, { backgroundColor: theme.priceAlertSurface, borderColor: theme.priceAlertBorder }]}><FlowIcon name={alert.type === "FLIGHT" ? "flight" : alert.type === "CAR" ? "car" : "hotel"} size={21} color={theme.priceAlertAccent} /></View>
      <View style={styles.alertHeading}>
        <Text numberOfLines={1} style={[styles.alertRoute, { color: theme.text }]}>{route}</Text>
        {trip.primary ? <Text style={[styles.alertMeta, { color: theme.muted }]}>{trip.primary}</Text> : null}
        {trip.secondary ? <Text style={[styles.alertMeta, { color: theme.muted }]}>{trip.secondary}</Text> : null}
      </View>
      <View testID={`price-alert-status-${alert.id}`} accessibilityLabel={`Status: ${status}`} style={[styles.statusPill, { backgroundColor: theme.priceAlertSurface, borderColor: theme.priceAlertBorder }]}><Text style={[styles.statusText, { color: theme.priceAlertAccent }]}>{status}</Text></View>
    </View>
    {target ? <View style={styles.targetBlock}><Text style={[styles.targetLabel, { color: theme.muted }]}>Target price</Text><Text style={[styles.targetValue, { color: theme.text }]}>{target}</Text></View> : null}
    {lastSeen ? <Text style={[styles.monitoringText, { color: theme.textSecondary }]}>Last seen {lastSeen}{difference ? ` · ${difference}` : ""}</Text> : null}
    {lastChecked ? <Text style={[styles.checkedText, { color: theme.muted }]}>Last checked {lastChecked}</Text> : null}
    <View style={[styles.actions, { borderTopColor: theme.border }]}>
      <View>{alert.status === "ACTIVE" ? <Pressable accessibilityRole="button" accessibilityLabel={`Pause alert for ${route}`} disabled={pending[alert.id]} onPress={() => void mutateStatus(alert, "PAUSED")} style={({ pressed }) => [styles.action, pressed && styles.pressed]}><Text style={[styles.actionText, { color: theme.priceAlertAccent }]}>Pause alert</Text></Pressable> : null}{alert.status === "PAUSED" && availability.priceAlerts ? <Pressable accessibilityRole="button" accessibilityLabel={`Reactivate alert for ${route}`} disabled={pending[alert.id]} onPress={() => void mutateStatus(alert, "ACTIVE")} style={({ pressed }) => [styles.action, pressed && styles.pressed]}><Text style={[styles.actionText, { color: theme.priceAlertAccent }]}>Reactivate</Text></Pressable> : null}</View>
      <Pressable accessibilityRole="button" accessibilityLabel={`Delete alert for ${route}`} disabled={pending[alert.id]} onPress={() => confirmDelete(alert)} style={({ pressed }) => [styles.delete, pressed && styles.pressed]}><Text style={styles.deleteText}>Delete</Text></Pressable>
    </View>
  </View>;
}) : <><View style={styles.emptyTopSpacer} /><View testID="price-alert-illustration" style={[styles.illustration, { width: Math.min(220, windowWidth - 72), height: Math.min(198, (windowWidth - 72) * .9) }]}><PriceAlertIllustration border={theme.border} surface={theme.surface} muted={theme.muted} /></View><View style={[styles.illustrationGap, windowHeight < 760 && styles.illustrationGapShort]} /><Text style={[styles.emptyTitle, { color: theme.text }]}>{availability.priceAlerts ? "Track prices for your trip" : "Price alerts are temporarily unavailable"}</Text><Text style={[styles.emptyText, { color: theme.muted }]}>{availability.priceAlerts ? "Create a price alert from a Flight, Hotel, or Cars search and we’ll help you keep an eye on fare changes." : "New price alerts can’t be created right now. Please try again later."}</Text>{availability.priceAlerts ? <Pressable accessibilityRole="button" accessibilityLabel="Search flights" onPress={() => router.dismissTo("/(tabs)")} style={({ pressed }) => [styles.emptyPrimary, pressed && styles.pressed]}><Text style={styles.emptyPrimaryText}>Search flights</Text></Pressable> : null}<View style={styles.emptyBottomSpacer} /></>}</ScrollView> : null}</Shell>;
}
function routeLabel(alert: MobilePriceAlert) { return alert.origin && alert.origin.trim().toLowerCase() !== alert.destination.trim().toLowerCase() ? `${alert.origin} → ${alert.destination}` : alert.destination; }
const styles = StyleSheet.create({
  ...
  alertIconTile: { width: 40, height: 40, borderRadius: 11, borderWidth: 1, alignItems: "center", justifyContent: "center" },
  alertHeading: { flex: 1, minWidth: 0, gap: 2 },
  alertRoute: { fontSize: 17, lineHeight: 22, fontWeight: "800" },
  alertMeta: { fontSize: 13, lineHeight: 18 },
  ...
  targetBlock: { marginTop: 12, gap: 1 },
  targetLabel: { fontSize: 12, lineHeight: 16 },
  targetValue: { fontSize: 20, lineHeight: 25, fontWeight: "800" },
  monitoringText: { marginTop: 8, fontSize: 13, lineHeight: 18 },
  checkedText: { fontSize: 12, lineHeight: 17 },
  ...
  actions: { minHeight: 45, flexDirection: "row", alignItems: "flex-end", justifyContent: "space-between", borderTopWidth: 1, marginTop: 10, paddingTop: 4 }, action: { minHeight: 44, justifyContent: "center", paddingHorizontal: 4, paddingRight: 16 }, actionText: { fontSize: 14, fontWeight: "700" }, delete: { minHeight: 44, justifyContent: "center", paddingHorizontal: 4, paddingLeft: 16 }, deleteText: { color: "#B4232C", fontSize: 14, fontWeight: "700" },
