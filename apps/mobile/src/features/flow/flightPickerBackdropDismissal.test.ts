...
test("From and To share a backdrop that closes without choosing an airport", () => {
  ...
  assert.match(airportSheet, /<SafeAreaView[^>]*>[\s\S]*?<Pressable[^>]*onPress=\{dismissAirportSheet\}[^>]*accessibilityLabel="Close airport picker"\/><Animated\.View accessibilityViewIsModal/);
  assert.match(airportSheet, /onRequestClose=\{dismissAirportSheet\}/);
...
test("Flight calendars opt into the backwards-compatible calendar backdrop", () => {
  ...
  assert.match(calendar, /context\.dismissOnBackdropPress \? <Pressable[^>]*onPress=\{onClose\}[^>]*accessibilityLabel="Close calendar"\/> : <View pointerEvents="none"/);
...
test("Travelers & Cabin backdrop and Android Back cancel rather than commit draft values", () => {
  ...
  assert.match(travelerSheet, /<PrimaryButton label="Done" icon=\{null\} size="compact" onPress=\{\(\)=>onDone\(draft\)\}\/>/);
  assert.match(travelerSheet, /accessibilityLabel="Close Travelers & Cabin"[\s\S]*?onPress=\{onCancel\}/);
  assert.doesNotMatch(travelerSheet, /<Cancel onPress=\{onCancel\}/);
