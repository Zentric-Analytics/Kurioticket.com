...
import { buildAutomaticCarPriceAlertPayload, carAlertPresentation, matchingCarPriceAlert } from "./carPriceAlertModel";
...
const alert = (status: MobilePriceAlert["status"], query = plan.payload, mode: MobilePriceAlert["mode"] = "AUTOMATIC"): MobilePriceAlert => ({ id: status, type: "CAR", origin: "Paris", destination: "Paris", targetPrice: mode === "TARGET" ? "100" : null, mode, currency: "USD", status, createdAt: "", updatedAt: "", lastSeenPrice: null, lastCheckedAt: null, query });

test("builds a complete automatic car alert payload and normalizes currency", () => { assert.deepEqual(buildAutomaticCarPriceAlertPayload(plan, 90, " usd "), { type: "CAR", origin: "Paris", destination: "Paris", baselinePrice: 90, mode: "AUTOMATIC", currency: "USD", query: plan.payload }); });
...
test("target alerts do not control the automatic toggle", () => { assert.equal(matchingCarPriceAlert([alert("ACTIVE", plan.payload, "TARGET")], plan), undefined); });
