...
import { buildSearchPlan, validBookableCar, validBookableHotel, validDiscoveryHotel, validFlight } from "./travelSearchModel";
...
test("shared result policy accepts website inventory and still rejects malformed records", () => {
  ...
  const discovery = { id: "h", provider: "Places", name: "Hotel", inventoryKind: "discovery", searchPolicy: { mode: "discovery", bookable: false, sourceLabel: "Places", action: { kind: "internal-detail", href: "/hotels/details/h", enabled: true } } };
  assert.equal(validBookableHotel(discovery as never), false);
  assert.equal(validDiscoveryHotel(discovery as never), true);
  const demoCar = { id: "c", rentalCompanyName: "Demo", isDemo: true, offers: [{ bookingProviderName: "Demo", totalPrice: 50, currency: "USD" }], searchPolicy: { mode: "demo", bookable: false, sourceLabel: "Kurioticket Demo Catalogue", action: { kind: "internal-detail", href: "/cars/details/c", enabled: true } } };
  assert.equal(validBookableCar(demoCar as never), true);
  assert.equal(validBookableCar({ ...demoCar, searchPolicy: { ...demoCar.searchPolicy, bookable: true } } as never), false);
});
