...
export function CarLocationSheet({ mode, selectedValue, onChoose, onClose }: { mode: CarLocationPickerMode; selectedValue: string; onChoose: (value: string) => void; onClose: () => void }) {
  ...
  const retainedContext = useRef<{ mode: Exclude<CarLocationPickerMode, undefined>; selectedValue: string }>({ mode: "pickup", selectedValue: "" });
  if (mode) retainedContext.current = { mode, selectedValue };
  const presented = mode ? { mode, selectedValue } : retainedContext.current;
