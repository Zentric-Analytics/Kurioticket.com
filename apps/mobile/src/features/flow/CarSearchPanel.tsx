...
export function CarSearchPanel({ params, embedded = false, showSubmit = true, submitLabel = "Search cars", requireManualDetails = false, startWithEmptyRentalDates = false }: Props) {
  ...
  return <View style={[!embedded && ft.styles.card, !embedded && ft.styles.shadow]}>
    ...
    <FieldError errors={[errors.pickupDate,errors.dropoffDate]}><CompactSearchField label="Rental dates" value={rentalDatesSummary(form.pickupDate,form.dropoffDate)} muted={!form.pickupDate || !form.dropoffDate} icon="calendar" onPress={() => setDatesOpen(true)}/></FieldError>
    <FieldError errors={[errors.pickupTime,errors.dropoffTime]}><CompactSearchField label="Pick-up / Return time" value={rentalTimesSummary(form.pickupTime,form.dropoffTime)} muted={!form.pickupTime || !form.dropoffTime} icon="clock" onPress={() => setTimesOpen(true)}/></FieldError>
