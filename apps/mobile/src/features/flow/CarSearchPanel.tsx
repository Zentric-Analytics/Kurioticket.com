...
export function CarSearchPanel({ params, embedded = false, showSubmit = true, submitLabel = "Search cars", requireManualDetails = false }: Props) {
  ...
  return <View style={[!embedded && ft.styles.card, !embedded && ft.styles.shadow]}>
    ...
    <View style={styles.pairedRow}><FieldError error={errors.pickupDate} style={styles.pairedHalf}><Field label="Pick-up date" value={form.pickupDate ? displayDate(form.pickupDate) : "Select pick-up date"} icon="calendar" onPress={() => setCalendar("pickupDate")}/></FieldError><FieldError error={errors.dropoffDate} style={styles.pairedHalf}><Field label="Drop-off date" value={form.dropoffDate ? displayDate(form.dropoffDate) : "Select drop-off date"} icon="calendar" onPress={() => setCalendar("dropoffDate")}/></FieldError></View>
    <View style={styles.pairedRow}><FieldError error={errors.pickupTime} style={styles.pairedHalf}><Field label="Pick-up time" value={form.pickupTime ? formatTime(form.pickupTime) : "Select pick-up time"} icon="clock" onPress={() => setTimeSheet("pickupTime")}/></FieldError><FieldError error={errors.dropoffTime} style={styles.pairedHalf}><Field label="Drop-off time" value={form.dropoffTime ? formatTime(form.dropoffTime) : "Select drop-off time"} icon="clock" onPress={() => setTimeSheet("dropoffTime")}/></FieldError></View>
