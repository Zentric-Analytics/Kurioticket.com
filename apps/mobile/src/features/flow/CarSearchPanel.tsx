...
export function CarLocationSheet({ mode, selectedValue, onChoose, onClose }: { mode: CarLocationPickerMode; selectedValue: string; onChoose: (value: string) => void; onClose: () => void }) {
  ...
  const changeQuery=(value:string)=>{filledQuery.current=undefined;setQuery(value);if(draft&&value!==draft.value)setDraft(undefined);}; const clearSearch=()=>{requestSequence.current+=1;filledQuery.current=undefined;setQuery("");setDraft(undefined);setSuggestions([]);setLoading(false);setError(false);inputRef.current?.focus();};
