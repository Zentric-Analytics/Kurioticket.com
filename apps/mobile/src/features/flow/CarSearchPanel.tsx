...
export function CarLocationSheet({ mode, selectedValue, onChoose, onClose }: { mode: CarLocationPickerMode; selectedValue: string; onChoose: (value: string) => void; onClose: () => void }) {
  const ft=useFlowTheme(); const active=Boolean(mode); const motion=useSearchPickerMotion(active); const context=useRetainedPickerContext(active,{mode,selectedValue}); const inputRef=useRef<TextInput>(null);
  const [query,setQuery]=useState(""); const [draft,setDraft]=useState<CarLocationSuggestion>(); const [suggestions,setSuggestions]=useState<CarLocationSuggestion[]>([]); const [loading,setLoading]=useState(false); const [error,setError]=useState(false); const selectionFilled=useRef(false);
  useEffect(()=>{if(!active)return;setQuery("");setDraft(undefined);setSuggestions([]);setLoading(false);setError(false);const frame=requestAnimationFrame(()=>inputRef.current?.focus());return()=>cancelAnimationFrame(frame);},[active,mode]);
  const eligible=hasMinimumLocationSearchLetters(query);
  useEffect(()=>{if(!mode||!eligible||selectionFilled.current){selectionFilled.current=false;setSuggestions([]);setLoading(false);setError(false);return;}let current=true;setLoading(true);setError(false);searchCarLocationSuggestions(query,{limit:8}).then(items=>{if(current)setSuggestions(items);}).catch(()=>{if(current){setSuggestions([]);setError(true);}}).finally(()=>{if(current)setLoading(false);});return()=>{current=false;};},[mode,query,eligible]);
  const changeQuery=(value:string)=>{setQuery(value);if(draft&&value!==draft.value)setDraft(undefined);}; const clearSearch=()=>{setQuery("");setDraft(undefined);setSuggestions([]);setError(false);inputRef.current?.focus();};
  const renderLocation=({item}:{item:CarLocationSuggestion})=>{const selected=item.id===draft?.id;const detail=item.airportCode?`${item.secondaryText} · ${item.airportCode}`:item.secondaryText;return <Pressable accessibilityRole="button" accessibilityLabel={`${item.primaryText}, ${detail}`} accessibilityState={{selected}} onPress={()=>{selectionFilled.current=true;setDraft(item);setQuery(item.value);}} style={[styles.locationChoice,{borderBottomColor:ft.colors.border},selected&&{backgroundColor:ft.colors.selected,borderLeftColor:ft.colors.selectedBorder}]}><View style={[styles.resultIcon,{backgroundColor:ft.colors.input}]}><FlowIcon name={item.kind==="airport"?"flight":item.kind==="city"?"city":"location"} size={21} color={ft.colors.icon}/></View><View style={styles.resultCopy}><Text numberOfLines={1} style={[ft.styles.value,selected&&{color:ft.colors.selectedPrimaryText}]}>{item.primaryText}</Text><Text numberOfLines={1} style={[ft.styles.meta,selected&&{color:ft.colors.selectedSecondaryText}]}>{detail}</Text></View></Pressable>;};
  const status=locationSearchLetterCount(query)===0?"Start typing to find a location.":!eligible?"Type at least 2 letters to see suggestions.":"No matching locations.";
  return <Modal transparent animationType="none" visible={motion.rendered} onRequestClose={onClose}><KeyboardAvoidingView pointerEvents={motion.pointerEvents} style={styles.keyboardViewport} behavior={Platform.OS === "ios" ? "padding" : "height"}><SafeAreaView edges={["top"]} style={styles.locationOverlay}><Animated.View pointerEvents="none" accessible={false} style={[StyleSheet.absoluteFill,styles.scrim,motion.backdropStyle]}/><Pressable style={StyleSheet.absoluteFill} accessibilityRole="button" accessibilityLabel="Close car location picker" onPress={onClose}/><Animated.View accessibilityViewIsModal onLayout={motion.onSheetLayout} style={[styles.locationSheet,{backgroundColor:ft.colors.surface,paddingBottom:20+motion.bottomSafeAreaInset},motion.sheetStyle]}><Text accessibilityRole="header" style={ft.styles.title}>{context.mode==="return"?"Choose return location":"Choose pick-up location"}</Text><View style={[styles.searchRow,{backgroundColor:ft.colors.input,borderColor:ft.colors.border}]}><FlowIcon name="location" size={20} color={ft.colors.icon}/><TextInput ref={inputRef} autoFocus accessibilityLabel="Search car locations" placeholder="Airport, city, or address" placeholderTextColor={ft.colors.placeholder} value={query} onChangeText={changeQuery} returnKeyType="search" style={[styles.searchInput,{color:ft.colors.text}]}/>{query?<Pressable accessibilityRole="button" accessibilityLabel="Clear car location search" onPress={clearSearch}><Text style={[styles.link,{color:ft.colors.selectedBorder}]}>Clear</Text></Pressable>:null}</View>{loading?<Text style={[styles.locationStatus,{color:ft.colors.secondaryText}]}>Finding locations…</Text>:error?<Text accessibilityRole="alert" style={[styles.locationStatus,{color:ft.colors.secondaryText}]}>Couldn’t load locations. Please try again.</Text>:<FlatList keyboardShouldPersistTaps="handled" data={suggestions} keyExtractor={item=>item.id} renderItem={renderLocation} contentContainerStyle={styles.locationList} ListEmptyComponent={<Text style={[styles.locationStatus,{color:ft.colors.secondaryText}]}>{status}</Text>}/>}<PrimaryButton label="Done" icon={null} size="compact" disabled={!draft} onPress={()=>{if(draft)onChoose(draft.value);}}/><Pressable accessibilityRole="button" accessibilityLabel="Cancel car location picker" onPress={onClose} style={styles.cancel}><Text style={[styles.link,{color:ft.colors.selectedBorder}]}>Cancel</Text></Pressable></Animated.View></SafeAreaView></KeyboardAvoidingView></Modal>;
}
