...
export function CarSearchPanel({ params, embedded = false, showSubmit = true, submitLabel = "Search cars", requireManualDetails = false, startWithEmptyRentalDates = false }: Props) {
  ...
  return <View style={[!embedded && ft.styles.card, !embedded && ft.styles.shadow]}>
    <LocationLauncher label="Pick-up location" value={form.pickupLocation} error={errors.pickupLocation} onPress={() => setLocationPicker("pickup")}/>
    ...
    {form.separateDropoff ? <LocationLauncher label="Drop-off location" value={form.dropoffLocation} error={errors.dropoffLocation} onPress={() => setLocationPicker("return")}/> : null}
...
function LocationLauncher({ label, value, error, onPress }: { label: string; value: string; error?: string; onPress: () => void }) { const ft = useFlowTheme(); return <View style={styles.inputField}><Pressable accessibilityRole="button" accessibilityLabel={label} onPress={onPress} style={styles.locationFieldRow}><FlowIcon name="location" size={22} color={ft.colors.icon}/><View style={styles.locationFieldContent}><Text style={ft.styles.label}>{label}</Text><Text style={[styles.locationValue,{color:value ? ft.colors.text : ft.colors.placeholder}]}>{value || "Enter city or airport"}</Text></View><FlowIcon name="chevron" size={18} color={ft.colors.icon}/></Pressable>{error ? <Text accessibilityRole="alert" style={styles.error}>{error}</Text> : null}</View>; }
