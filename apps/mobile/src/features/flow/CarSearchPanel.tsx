...
export function CarSearchPanel({ params, embedded = false, showSubmit = true, submitLabel = "Search cars" }: Props) {
  ...
  useEffect(() => {
    if (routeIntent !== previousIntent.current && Object.values(params).some(Boolean)) {
      const next = initializeCarForm(params); setForm(next.form); setErrors({}); setNotice(next.notice);
    }
...
function LocationInput({ inputRef, label, value, error, onChange }: { inputRef?: React.RefObject<TextInput | null>; label: string; value: string; error?: string; onChange: (value: string) => void }) { return <View style={styles.inputField}><Text style={flowStyles.label}>{label}</Text><View style={styles.inputRow}><TextInput ref={inputRef} accessibilityLabel={label} value={value} onChangeText={onChange} placeholder="Enter city or airport" placeholderTextColor={flowColors.muted} style={styles.input} returnKeyType="done"/><FlowIcon name="location" size={20}/></View>{error ? <Text accessibilityRole="alert" style={styles.error}>{error}</Text> : null}</View>; }
