...
import { searchCarLocations } from "../../api/locationSuggestions";
...
export function CarLocationSheet({ mode, selectedValue, onChoose, onClose }: { mode: CarLocationPickerMode; selectedValue: string; onChoose: (value: string) => void; onClose: () => void }) {
  ...
  const [query,setQuery]=useState(""); const [draft,setDraft]=useState<CarLocationSuggestion>(); const [suggestions,setSuggestions]=useState<CarLocationSuggestion[]>([]); const [loading,setLoading]=useState(false); const [error,setError]=useState(false); const filledQuery=useRef<string | undefined>(undefined); const generation=useRef(0);
  ...
  useEffect(()=>{const requestGeneration=++generation.current;if(!mode||!eligible){setSuggestions([]);setLoading(false);setError(false);return;}if(filledQuery.current===query){filledQuery.current=undefined;setSuggestions([]);setLoading(false);return;}filledQuery.current=undefined;const controller=new AbortController();const timer=setTimeout(()=>{setLoading(true);setError(false);void searchCarLocations(query,{limit:8,signal:controller.signal}).then(items=>{if(generation.current===requestGeneration)setSuggestions(items);}).catch(()=>{if(!controller.signal.aborted&&generation.current===requestGeneration){setSuggestions([]);setError(true);}}).finally(()=>{if(generation.current===requestGeneration)setLoading(false);});},220);return()=>{clearTimeout(timer);controller.abort();};},[mode,query,eligible]);
  const changeQuery=(value:string)=>{if(filledQuery.current&&filledQuery.current!==value)filledQuery.current=undefined;setQuery(value);if(draft&&value!==draft.value)setDraft(undefined);}; const clearSearch=()=>{filledQuery.current=undefined;setQuery("");setDraft(undefined);setSuggestions([]);setError(false);inputRef.current?.focus();};
  const renderLocation=({item}:{item:CarLocationSuggestion})=>{const selected=item.id===draft?.id;const detail=item.airportCode?`${item.secondaryText} · ${item.airportCode}`:item.secondaryText;return <Pressable accessibilityRole="button" accessibilityLabel={`${item.primaryText}, ${detail}`} accessibilityState={{selected}} onPress={()=>{filledQuery.current=item.value;setDraft(item);setQuery(item.value);}} style={[styles.locationChoice,{borderBottomColor:ft.colors.border},selected&&{backgroundColor:ft.colors.selected,borderLeftColor:ft.colors.selectedBorder}]}><View style={[styles.resultIcon,{backgroundColor:ft.colors.input}]}><FlowIcon name={item.kind==="airport"?"flight":item.kind==="city"?"city":"location"} size={21} color={ft.colors.icon}/></View><View style={styles.resultCopy}><Text numberOfLines={1} style={[ft.styles.value,selected&&{color:ft.colors.selectedPrimaryText}]}>{item.primaryText}</Text><Text numberOfLines={1} style={[ft.styles.meta,selected&&{color:ft.colors.selectedSecondaryText}]}>{detail}</Text></View></Pressable>;};
