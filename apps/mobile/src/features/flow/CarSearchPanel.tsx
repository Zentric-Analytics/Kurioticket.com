...
export function CarLocationSheet({ mode, selectedValue, onChoose, onClose }: { mode: CarLocationPickerMode; selectedValue: string; onChoose: (value: string) => void; onClose: () => void }) {
  ...
  const [query,setQuery]=useState(""); const [draft,setDraft]=useState<CarLocationSuggestion>(); const [suggestions,setSuggestions]=useState<CarLocationSuggestion[]>([]); const [loading,setLoading]=useState(false); const [error,setError]=useState(false); const programmaticFilledQuery=useRef(""); const requestSequence=useRef(0);
  useEffect(()=>{if(!active)return;setQuery("");setDraft(undefined);setSuggestions([]);setLoading(false);setError(false);},[active,mode]);
  useEffect(()=>{if(!active||!motion.openSettled)return;inputRef.current?.focus();},[active,motion.openSettled,mode]);
  const eligible=hasMinimumLocationSearchLetters(query);
  useEffect(()=>{const sequence=++requestSequence.current;if(!mode||!eligible){setSuggestions([]);setLoading(false);setError(false);return;}if(query===programmaticFilledQuery.current){programmaticFilledQuery.current="";setLoading(false);setError(false);return;}const controller=new AbortController();const timer=setTimeout(()=>{setLoading(true);setError(false);searchCarLocations(query,{signal:controller.signal}).then(items=>{if(sequence===requestSequence.current)setSuggestions(items);}).catch(error=>{if(sequence===requestSequence.current&&error?.name!=="AbortError"){setSuggestions([]);setError(true);}}).finally(()=>{if(sequence===requestSequence.current)setLoading(false);});},220);return()=>{clearTimeout(timer);controller.abort();};},[mode,query,eligible]);
  const changeQuery=(value:string)=>{if(value!==programmaticFilledQuery.current)programmaticFilledQuery.current="";setQuery(value);if(draft&&value!==draft.value)setDraft(undefined);}; const clearSearch=()=>{programmaticFilledQuery.current="";setQuery("");setDraft(undefined);setSuggestions([]);setError(false);inputRef.current?.focus();};
  const renderLocation=({item}:{item:CarLocationSuggestion})=>{const selected=item.id===draft?.id;const detail=item.airportCode?`${item.secondaryText} · ${item.airportCode}`:item.secondaryText;return <Pressable accessibilityRole="button" accessibilityLabel={`${item.primaryText}, ${detail}`} accessibilityState={{selected}} onPress={()=>{programmaticFilledQuery.current=item.value;setDraft(item);setQuery(item.value);}} style={[styles.locationChoice,{borderBottomColor:ft.colors.border},selected&&{backgroundColor:ft.colors.selected,borderLeftColor:ft.colors.selectedBorder}]}><View style={[styles.resultIcon,{backgroundColor:ft.colors.input}]}><FlowIcon name={item.kind==="airport"?"flight":item.kind==="city"?"city":"location"} size={21} color={ft.colors.icon}/></View><View style={styles.resultCopy}><Text numberOfLines={1} style={[ft.styles.value,selected&&{color:ft.colors.selectedPrimaryText}]}>{item.primaryText}</Text><Text numberOfLines={1} style={[ft.styles.meta,selected&&{color:ft.colors.selectedSecondaryText}]}>{detail}</Text></View></Pressable>;};
