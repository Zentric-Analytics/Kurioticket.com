...
export function CarSearchPanel({ params, editAppearance = false, showSubmit = true, submitLabel = "Search cars", requireManualDetails = false, startWithEmptyRentalDates = false, submitNavigation = "push", onBeforeNavigate }: Props) {
  ...
  const mainRows = <View style={styles.resultsEditStack}>
    <FieldError errors={[errors.pickupLocation]}><View style={editCardStyle}><ResultsEditRow label="PICKUP LOCATION" value={pickupLocationDisplay.primary || form.pickupLocation.trim() || "Airport, city, or address"} secondary={pickupLocationDisplay.secondary} muted={!form.pickupLocation} icon="location" onPress={() => setLocationPicker("pickup")} /></View></FieldError>
    {form.separateDropoff ? <FieldError errors={[errors.dropoffLocation]}><View style={editCardStyle}><ResultsEditRow label="DROP-OFF LOCATION" value={dropoffLocationDisplay.primary || form.dropoffLocation.trim() || "Enter city or airport"} secondary={dropoffLocationDisplay.secondary} muted={!form.dropoffLocation} icon="location" onPress={() => setLocationPicker("return")} /></View></FieldError> : null}
    <FieldError errors={[errors.pickupDate,errors.dropoffDate]}><View style={editCardStyle}><ResultsEditRow label="RENTAL DATES" value={rentalDatesSummary(form.pickupDate,form.dropoffDate)} muted={!form.pickupDate || !form.dropoffDate} icon="calendar" disclosure onPress={() => setDatesOpen(true)} /></View></FieldError>
    <FieldError errors={[errors.pickupTime,errors.dropoffTime]}><View style={editCardStyle}><ResultsEditRow label="PICK-UP / RETURN TIME" value={rentalTimesSummary(form.pickupTime,form.dropoffTime)} muted={!form.pickupTime || !form.dropoffTime} icon="clock" disclosure onPress={() => setTimesOpen(true)} /></View></FieldError>
    <FieldError errors={[errors.driverAge]}><View style={editCardStyle}><ResultsEditRow label="DRIVER AGE" value={form.driverAge === undefined ? "Select driver age" : `${form.driverAge} years old`} muted={form.driverAge === undefined} icon="person" disclosure onPress={() => setAgeOpen(true)} /></View></FieldError>
  </View>;
...
function ResultsEditRow({ label, value, secondary, muted = false, icon, disclosure = false, onPress, actionLabel, onAction }: { label: string; value: string; secondary?: string; muted?: boolean; icon: FlowIconName; disclosure?: boolean; onPress: () => void; actionLabel?: string; onAction?: () => void }) {
  const ft = useFlowTheme();
  ...
  return <View style={styles.resultsEditRow}>
