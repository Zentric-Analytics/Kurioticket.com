...
import { CompactSearchField, PrimaryButton, UnavailableNotice } from "./FlowPrimitives";
...
export const HotelSearchPanel = forwardRef<HotelSearchHandle, Props>(function HotelSearchPanel({ params, embedded = false, showSubmit = true, submitLabel = "Search hotels" }, ref) {
  ...
  return <View style={[!embedded && ft.styles.card, !embedded && ft.styles.shadow]}>
    ...
    <CompactSearchField label="Guests" value={`${countLabel(form.guests, "guest")}, ${countLabel(form.rooms, "room")}`} icon="person" onPress={() => setCountsOpen(true)}/>
    ...
    <HotelDestinationSheet visible={destinationOpen} value={form.destination} onDone={(destination) => { update({ ...form, destination }); if (destination.trim()) setErrors((current) => ({ ...current, destination: undefined })); setDestinationOpen(false); }} onCancel={() => setDestinationOpen(false)}/>
      ...
      <KeyboardAvoidingView style={styles.keyboardViewport} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        <SafeAreaView edges={["top", "bottom"]} style={styles.destinationOverlay}>
          ...
          <View accessibilityViewIsModal style={[styles.destinationSheet,{backgroundColor:ft.colors.surface}]}>
            ...
            <Pressable accessibilityRole="button" accessibilityLabel="Cancel destination changes" onPress={onCancel} style={styles.destinationCancel}><Text style={[styles.link,{color:ft.colors.selectedBorder}]}>Cancel</Text></Pressable>
          </View>
        </SafeAreaView>
...
function PickerIcon({ icon: Icon }: { icon: LucideIcon }) { const ft = useFlowTheme(); return <View accessible={false} accessibilityElementsHidden importantForAccessibility="no-hide-descendants" style={[styles.pickerIcon, { backgroundColor: ft.colors.selected }]}><Icon size={24} color={ft.colors.selectedBorder}/></View>; }
...
function CounterButton({ label, disabled, icon: Icon, onPress }: { label: string; disabled: boolean; icon: LucideIcon; onPress: () => void }) { const ft = useFlowTheme(); const color = disabled ? ft.colors.secondaryText : ft.colors.selectedBorder; return <Pressable accessibilityRole="button" accessibilityLabel={label} accessibilityState={{ disabled }} disabled={disabled} onPress={onPress} hitSlop={4} style={[styles.control, { borderColor: disabled ? ft.colors.border : ft.colors.selectedBorder }, disabled && styles.disabled]}><Icon accessible={false} accessibilityElementsHidden importantForAccessibility="no-hide-descendants" size={22} color={color}/></Pressable>; }
const styles = StyleSheet.create({ pad:{padding:8},error:{color:"#A21D25",fontSize:12,lineHeight:18,paddingHorizontal:12,paddingVertical:5},modalRoot:{flex:1,justifyContent:"flex-end",backgroundColor:SEARCH_PICKER_BACKDROP_COLOR},keyboardViewport:{flex:1},destinationOverlay:{flex:1,justifyContent:"flex-end",backgroundColor:SEARCH_PICKER_BACKDROP_COLOR},destinationSheet:{borderTopLeftRadius:24,borderTopRightRadius:24,padding:20,gap:14,maxHeight:"90%"},destinationHeader:{flexDirection:"row",alignItems:"center",justifyContent:"space-between",gap:12},destinationSearch:{minHeight:52,borderWidth:1,borderRadius:10,paddingHorizontal:14,flexDirection:"row",alignItems:"center",gap:10},destinationSearchInput:{flex:1,minWidth:0,fontSize:15},destinationResults:{flexShrink:1,minHeight:0},destinationList:{flexGrow:1},destinationChoice:{minHeight:76,flexDirection:"row",alignItems:"center",gap:12,borderBottomWidth:1,paddingVertical:10,paddingHorizontal:8},destinationIcon:{width:46,height:46,borderRadius:12,alignItems:"center",justifyContent:"center"},destinationStatus:{minHeight:120,textAlign:"center",paddingVertical:36,fontSize:14},destinationCancel:{alignSelf:"center",paddingHorizontal:12},link:{fontSize:14,fontWeight:"700",paddingVertical:10},sheetPosition:{justifyContent:"flex-end",maxHeight:"90%"},sheet:{borderTopLeftRadius:24,borderTopRightRadius:24,padding:20,gap:12,maxHeight:"100%"},sheetContent:{paddingBottom:4,gap:16},pickerSection:{gap:8},sectionLabel:{fontSize:11,lineHeight:16,fontWeight:"700",letterSpacing:1.5},pickerCard:{borderWidth:1,borderRadius:14,overflow:"hidden"},pickerDivider:{height:1,marginHorizontal:14},pickerRow:{minHeight:88,flexDirection:"row",alignItems:"center",gap:12,paddingHorizontal:14,paddingVertical:12},pickerIcon:{width:48,height:48,borderRadius:24,flexShrink:0,alignItems:"center",justifyContent:"center"},rowCopy:{flex:1,minWidth:0,gap:2},counterActions:{flexShrink:0,flexDirection:"row",alignItems:"center",gap:6},control:{width:40,height:40,borderRadius:20,borderWidth:1,alignItems:"center",justifyContent:"center"},disabled:{opacity:.4},count:{minWidth:22,textAlign:"center",fontSize:17,fontWeight:"800"},petRow:{minHeight:88,flexDirection:"row",alignItems:"center",gap:12,paddingHorizontal:14,paddingVertical:12},petCopy:{flex:1,minWidth:0,gap:2},petSwitchSlot:{width:52,flexShrink:0,alignItems:"flex-end",justifyContent:"center"} });
