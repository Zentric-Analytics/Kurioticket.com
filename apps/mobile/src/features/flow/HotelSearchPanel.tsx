...
export function HotelDestinationSheet({ visible, value, onDone, onCancel }: { visible: boolean; value: string; onDone: (destination: string) => void; onCancel: () => void }) {
  ...
  const programmaticFilledQuery = useRef("");
  ...
  useEffect(() => {
    if (!visible) return;
    setQuery(""); setDraft(undefined); setSuggestions([]); setLoading(false); setError(false); programmaticFilledQuery.current="";
  }, [visible, value]);

  useEffect(() => { if (!visible || !motion.openSettled) return; inputRef.current?.focus(); }, [visible, motion.openSettled, value]);

  useEffect(() => {
    if (!visible || !hasMinimumLocationSearchLetters(trimmedQuery)) { setSuggestions([]); setLoading(false); setError(false); return; }
    if (trimmedQuery === programmaticFilledQuery.current) { programmaticFilledQuery.current=""; setLoading(false); setError(false); return; }
  ...
  const clear = () => { setQuery(""); setDraft(undefined); programmaticFilledQuery.current=""; setSuggestions([]); setError(false); inputRef.current?.focus(); };
  const changeQuery = (next:string) => { if(next.trim()!==programmaticFilledQuery.current)programmaticFilledQuery.current=""; setQuery(next); if (draft && next !== draft.searchValue) setDraft(undefined); };
