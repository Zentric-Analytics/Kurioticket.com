...
export function HotelDestinationSheet({ visible, value, onDone, onCancel }: { visible: boolean; value: string; onDone: (destination: string) => void; onCancel: () => void }) {
  ...
  useEffect(() => {
    if (!visible || !hasMinimumLocationSearchLetters(trimmedQuery)) { programmaticFilledQuery.current=undefined; setSuggestions([]); setLoading(false); setError(false); return; }
    if (programmaticFilledQuery.current === trimmedQuery) { programmaticFilledQuery.current=undefined; setSuggestions([]); setLoading(false); return; }
    programmaticFilledQuery.current=undefined;
