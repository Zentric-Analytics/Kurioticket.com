...
export const HotelSearchPanel = forwardRef<HotelSearchHandle, Props>(function HotelSearchPanel({ params, embedded = false, showSubmit = true, submitLabel = "Search hotels" }, ref) {
  ...
  return <View style={[!embedded && ft.styles.card, !embedded && ft.styles.shadow]}>
    <View style={styles.inputField}><View style={styles.locationFieldRow}><FlowIcon name="location" size={22} color={ft.colors.icon}/><View style={styles.locationFieldContent}><Text style={ft.styles.label}>Destination</Text><TextInput ref={destinationRef} accessibilityLabel="Hotel destination" value={form.destination} onChangeText={(destination) => { update({ ...form, destination }); if (destination.trim()) setErrors((value) => ({ ...value, destination: undefined })); }} placeholder="City, area, or hotel" placeholderTextColor={ft.colors.placeholder} style={[styles.input, { color: ft.colors.text }]} returnKeyType="done"/></View></View>{errors.destination ? <Text accessibilityRole="alert" style={styles.error}>{errors.destination}</Text> : null}</View>
