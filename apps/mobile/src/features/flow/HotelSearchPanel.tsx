...
export function HotelDestinationSheet({ visible, value, onDone, onCancel }: { visible: boolean; value: string; onDone: (destination: string) => void; onCancel: () => void }) {
  ...
  useEffect(() => {
    if (!visible || !hasMinimumLocationSearchLetters(trimmedQuery) || trimmedQuery === programmaticFilledQuery.current) { setSuggestions([]); setLoading(false); setError(false); return; }
