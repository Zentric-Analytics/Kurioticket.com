...
function AirportSheet({ visible, title, onChoose, onClose }: { visible: boolean; title: string; onChoose: (airport: Airport) => void; onClose: () => void }) {
  const ft=useFlowTheme();const motion=useSearchPickerMotion(visible);const context=useRetainedPickerContext(visible,{title});const inputRef=useRef<TextInput>(null);const requestSequence=useRef(0);const filledQuery=useRef<string | undefined>(undefined);
  ...
  useEffect(()=>{if(!visible||!motion.openSettled)return;inputRef.current?.focus();},[visible,motion.openSettled,title]);
