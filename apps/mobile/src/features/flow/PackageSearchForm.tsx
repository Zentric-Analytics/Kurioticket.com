...
import { searchFlightPlaces } from "../../api/locationSuggestions";
...
function AirportSheet({ visible, title, onChoose, onClose }: { visible: boolean; title: string; onChoose: (airport: Airport) => void; onClose: () => void }) {
  const ft=useFlowTheme();const motion=useSearchPickerMotion(visible);const context=useRetainedPickerContext(visible,{title});const inputRef=useRef<TextInput>(null);const [query,setQuery]=useState("");const [draft,setDraft]=useState<Airport>();const [choices,setChoices]=useState<Airport[]>([]);const [loading,setLoading]=useState(false);const [error,setError]=useState(false);const filledQuery=useRef<string | undefined>(undefined);const generation=useRef(0);
  useEffect(()=>{if(!visible)return;setQuery("");setDraft(undefined);},[visible,title]);
  useEffect(()=>{if(!visible||!motion.openSettled)return;inputRef.current?.focus();},[visible,motion.openSettled,title]);
  const eligible=hasMinimumLocationSearchLetters(query);useEffect(()=>{const requestGeneration=++generation.current;if(!visible||!eligible){setChoices([]);setLoading(false);setError(false);return;}if(filledQuery.current===query){filledQuery.current=undefined;setChoices([]);setLoading(false);return;}filledQuery.current=undefined;const controller=new AbortController();const timer=setTimeout(()=>{setLoading(true);setError(false);void searchFlightPlaces(query,{context:title==="Choose origin"?"origin":"destination",signal:controller.signal}).then(items=>{if(generation.current!==requestGeneration)return;const byCode=new Map(airports.map(airport=>[airport.code,airport]));setChoices(items.map(item=>byCode.get(item.code)).filter((airport):airport is Airport=>Boolean(airport)));}).catch(()=>{if(!controller.signal.aborted&&generation.current===requestGeneration){setChoices([]);setError(true);}}).finally(()=>{if(generation.current===requestGeneration)setLoading(false);});},220);return()=>{clearTimeout(timer);controller.abort();};},[visible,eligible,query,title]);const changeQuery=(value:string)=>{if(filledQuery.current&&filledQuery.current!==value)filledQuery.current=undefined;setQuery(value);if(draft&&value!==`${draft.city} (${draft.code})`)setDraft(undefined);};const clear=()=>{filledQuery.current=undefined;setQuery("");setDraft(undefined);setChoices([]);setError(false);inputRef.current?.focus();};
  return <Modal transparent animationType="none" visible={motion.rendered} onRequestClose={onClose}><KeyboardAvoidingView pointerEvents={motion.pointerEvents} style={styles.modalRoot} behavior={Platform.OS === "ios" ? "padding" : "height"}><Animated.View pointerEvents="none" accessible={false} style={[StyleSheet.absoluteFill,styles.scrim,motion.backdropStyle]}/><Pressable style={StyleSheet.absoluteFill} accessibilityRole="button" accessibilityLabel="Close airport search" onPress={onClose}/><Animated.View accessibilityViewIsModal onLayout={motion.onSheetLayout} style={[styles.sheet,{backgroundColor:ft.colors.surface,paddingBottom:18+motion.bottomSafeAreaInset},motion.sheetStyle]}><PickerSheetHeader title={context.title} onClose={onClose} closeLabel={context.title === "Choose origin" ? "Close origin picker" : "Close destination picker"}/><View style={[styles.airportSearchShell,{backgroundColor:ft.colors.input,borderColor:ft.colors.border}]}><FlowIcon name="location" size={20} color={ft.colors.icon}/><TextInput ref={inputRef} accessibilityLabel="Search airports" value={query} onChangeText={changeQuery} placeholder="Search code, airport, city, or country" placeholderTextColor={ft.colors.placeholder} style={[styles.airportSearchInput,{color:ft.colors.text}]}/>{query?<Pressable accessibilityRole="button" accessibilityLabel="Clear airport search" onPress={clear}><Text style={{color:ft.colors.selectedBorder,fontWeight:"800"}}>Clear</Text></Pressable>:null}</View><ScrollView keyboardShouldPersistTaps="handled">{choices.map(airport=>{const selected=airport.code===draft?.code;return <Pressable key={airport.code} accessibilityRole="button" accessibilityState={{selected}} onPress={()=>{filledQuery.current=`${airport.city} (${airport.code})`;setDraft(airport);setQuery(filledQuery.current);}} style={[styles.choice,{borderBottomColor:ft.colors.border},selected&&{backgroundColor:ft.colors.selected}]}><Text style={[ft.styles.value,selected&&{color:ft.colors.selectedPrimaryText}]}>{airport.city} ({airport.code})</Text><Text style={ft.styles.meta}>{airport.name}</Text></Pressable>;})}{loading?<Text style={[styles.airportStatus,{color:ft.colors.secondaryText}]}>Finding airports…</Text>:error?<Text accessibilityRole="alert" style={[styles.airportStatus,{color:ft.colors.secondaryText}]}>Couldn’t load airports. Please try again.</Text>:choices.length===0?<Text style={[styles.airportStatus,{color:ft.colors.secondaryText}]}>{locationSearchLetterCount(query)===0?"Start typing to find an airport.":!eligible?"Type at least 2 letters to see suggestions.":"No airports match this search."}</Text>:null}</ScrollView><PrimaryButton label="Done" icon={null} size="compact" disabled={!draft} onPress={()=>{if(draft)onChoose(draft);}}/></Animated.View></KeyboardAvoidingView></Modal>;
}
