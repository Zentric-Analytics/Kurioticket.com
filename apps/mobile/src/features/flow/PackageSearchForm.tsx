...
function AirportSheet({ visible, title, mode, onChoose, onClose }: { visible: boolean; title: string; mode: PackageMode; onChoose: (airport: Airport) => void; onClose: () => void }) {
  const ft=useFlowTheme();const motion=useSearchPickerMotion(visible, { controlledOpening: true });const context=useRetainedPickerContext(visible,{title,mode});const inputRef=useRef<TextInput>(null);const requestSequence=useRef(0);const filledQuery=useRef<string | undefined>(undefined);
  const [query,setQuery]=useState("");const [draft,setDraft]=useState<Airport>();const [choices,setChoices]=useState<FlightPlaceSuggestion[]>([]);const [loading,setLoading]=useState(false);const [error,setError]=useState(false);
  useEffect(()=>{if(!visible)return;requestSequence.current+=1;filledQuery.current=undefined;setQuery("");setDraft(undefined);setChoices([]);setLoading(false);setError(false);},[visible,title]);
  const keyboardPresentation = useSearchPickerKeyboardPresentation(visible, motion.rendered, title, inputRef, motion);
