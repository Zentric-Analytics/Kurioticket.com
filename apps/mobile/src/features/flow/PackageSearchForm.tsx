...
import { airportByCode } from "./flightSearchModel";
...
function AirportSheet({ visible, title, onChoose, onClose }: { visible: boolean; title: string; onChoose: (airport: Airport) => void; onClose: () => void }) {
  const ft=useFlowTheme();const motion=useSearchPickerMotion(visible);const context=useRetainedPickerContext(visible,{title});const inputRef=useRef<TextInput>(null);const [query,setQuery]=useState("");const [draft,setDraft]=useState<Airport>();
