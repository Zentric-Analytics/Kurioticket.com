...
function AirportSheet({ visible, title, onChoose, onClose }: { visible: boolean; title: string; onChoose: (airport: Airport) => void; onClose: () => void }) {
  const ft=useFlowTheme();const motion=useSearchPickerMotion(visible, { controlledOpening: true });const context=useRetainedPickerContext(visible,{title});const inputRef=useRef<TextInput>(null);const requestSequence=useRef(0);const filledQuery=useRef<string | undefined>(undefined);
