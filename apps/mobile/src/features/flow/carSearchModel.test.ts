...
test("Cars page renders the manual-selection placeholders", () => {
  ...
  const model = readFileSync(`${process.cwd()}/src/features/flow/carSearchModel.ts`, "utf8");
  for (const placeholder of ["Enter city or airport", "Pickup date", "Return date", "Select pick-up time", "Select return time", "Select driver age"]) assert.match(panel + model, new RegExp(placeholder));
});
