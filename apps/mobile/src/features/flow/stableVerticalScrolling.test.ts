...
test("major vertical screens disable iOS bounce and Android overscroll", () => {
  const screens: [string, RegExp][] = [
    ...
    ["src/features/recent/RecentSearchesScreen.tsx", /<ScrollView[^>]*contentContainerStyle=\{recentLoaded && !recent\.length \? styles\.emptyContent : styles\.content\}[^>]*>/],
...
test("flight results keep fixed chrome, native sticky quick controls, and stable outer scrolling", () => {
  ...
  assert.match(owner, /ListHeaderComponent=\{<View>\{dateStrip\}<\/View>\}/);
  assert.match(owner, /renderSectionHeader=\{\(\) => filterRail\}/);
