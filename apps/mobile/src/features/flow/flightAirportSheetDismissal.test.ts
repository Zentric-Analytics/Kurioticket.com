...
test("the airport sheet has a dedicated accessible backdrop dismissal target", () => {
  assert.match(airportSheet, /<View style=\{styles\.modalRoot\}><Pressable style=\{\[StyleSheet\.absoluteFill,\{backgroundColor:ft\.colors\.overlay\}\]\} onPress=\{onClose\} accessibilityRole="button" accessibilityLabel="Close airport picker"\/>/);
...
test("the interactive sheet is a sibling above the backdrop, not its child", () => {
  ...
  const keyboardViewport = airportSheet.indexOf("<KeyboardAvoidingView");
  const sheet = airportSheet.indexOf("<View accessibilityViewIsModal");

  assert.ok(backdropEnd >= 0);
  assert.ok(keyboardViewport > backdropEnd);
  assert.ok(sheet > keyboardViewport);
  assert.doesNotMatch(airportSheet.slice(0, keyboardViewport), /<Pressable[^>]*onPress=\{onClose\}>[\s\S]*<View accessibilityViewIsModal/);
});
...
test("backdrop dismissal adds no device-specific sizing or positioning hacks", () => {
  assert.match(panel, /modalRoot:\{flex:1\}/);
