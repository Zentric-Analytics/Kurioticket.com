  ...
  return <Modal transparent animationType="slide" visible onRequestClose={onCancel}>
    <View style={styles.modalRoot}>
      ...
      <SafeAreaView edges={["bottom"]} style={styles.safeLayer} pointerEvents="box-none"><View accessibilityViewIsModal style={[styles.sheet, { backgroundColor: ft.colors.surface }]}>
        ...
        <View style={styles.rangeHeader}><RangeValue label={startLabel} value={draftStart} active={activePart === "start"}/><RangeValue label={endLabel} value={draftEnd} active={activePart === "end"}/></View>
...
function RangeValue({label,value,active}:{label:string;value:string;active:boolean}) { const ft=useFlowTheme(); const displayValue=value ? localDateFromIso(value)?.toLocaleDateString(FLIGHT_DATE_LOCALE,{weekday:"short",month:"short",day:"numeric"}) : "Select"; return <View style={styles.rangeValue} accessible accessibilityState={{selected:active}} accessibilityLabel={`${label}, ${displayValue}${active ? ", currently selecting" : ""}`}><Text style={ft.styles.label}>{label}</Text><View style={[styles.valueIndicator,{borderBottomColor:active?ft.colors.selectedBorder:"transparent"}]}><Text style={ft.styles.value}>{displayValue}</Text></View></View>; }

const styles=StyleSheet.create({modalRoot:{flex:1,justifyContent:"flex-end",backgroundColor:SEARCH_PICKER_BACKDROP_COLOR},safeLayer:{flex:1,justifyContent:"flex-end"},sheet:{borderTopLeftRadius:24,borderTopRightRadius:24,padding:16,gap:10,maxHeight:"94%"},rangeHeader:{flexDirection:"row",gap:12},rangeValue:{flex:1,minWidth:0,padding:10},valueIndicator:{alignSelf:"flex-start",borderBottomWidth:1,paddingBottom:2},monthRow:{minHeight:48,flexDirection:"row",alignItems:"center",gap:4},month:{flex:1,textAlign:"center",fontSize:18,fontWeight:"800"},monthControl:{width:44,height:44,borderRadius:22,borderWidth:1,alignItems:"center",justifyContent:"center"},controlText:{fontSize:25},week:{flexDirection:"row"},weekday:{width:"14.285%",textAlign:"center",fontSize:11},grid:{flexDirection:"row",flexWrap:"wrap"},day:{width:"14.285%",minHeight:44,alignItems:"center",justifyContent:"center",borderRadius:8},dayText:{fontSize:13},selectedText:{fontWeight:"800"},disabled:{opacity:.35}});
