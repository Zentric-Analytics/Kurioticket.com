...
export function LocalCalendarModal({ visible, title, selected, minimum, onChoose, onClose, dismissOnBackdropPress = false }: LocalCalendarModalProps) {
  ...
  const context = useRetainedPickerContext(visible, { title, selected, minimum, dismissOnBackdropPress });
  const selectedDate = localDateFromIso(context.selected) ?? new Date();
  const [monthOffset, setMonthOffset] = useState(0);
  useEffect(() => { if (visible) setMonthOffset(0); }, [visible, context.selected]);
  const month = useMemo(() => new Date(selectedDate.getFullYear(), selectedDate.getMonth() + monthOffset, 1, 12), [context.selected, monthOffset]);
  ...
  return <Modal visible transparent animationType="none" onRequestClose={onClose}><SafeAreaView edges={["top", "left", "right"]} pointerEvents={motion.pointerEvents} style={styles.overlay}>
    <Animated.View pointerEvents="none" accessible={false} style={[StyleSheet.absoluteFill, styles.scrim, motion.backdropStyle]}/>
    {context.dismissOnBackdropPress ? <Pressable style={StyleSheet.absoluteFill} onPress={onClose} accessibilityRole="button" accessibilityLabel="Close calendar"/> : <View pointerEvents="none" style={StyleSheet.absoluteFill}/>}<Animated.View accessibilityViewIsModal onLayout={motion.onSheetLayout} style={[styles.modal, { backgroundColor: ft.colors.surface, paddingBottom: 16 + motion.bottomSafeAreaInset }, motion.sheetStyle]}>
