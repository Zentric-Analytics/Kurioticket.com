...
export function LocalCalendarModal({ visible, title, selected, minimum, onChoose, onClose, dismissOnBackdropPress = false }: LocalCalendarModalProps) {
  ...
  const retainedContext = useRef({ title, selected, minimum, dismissOnBackdropPress });
  if (visible) retainedContext.current = { title, selected, minimum, dismissOnBackdropPress };
  const presented = visible ? { title, selected, minimum, dismissOnBackdropPress } : retainedContext.current;
  const selectedDate = localDateFromIso(presented.selected) ?? new Date();
  const [monthOffset, setMonthOffset] = useState(0);
  const motion = useSearchPickerMotion(visible);
  useEffect(() => { if (visible) setMonthOffset(0); }, [visible, presented.selected]);
  const month = useMemo(() => new Date(selectedDate.getFullYear(), selectedDate.getMonth() + monthOffset, 1, 12), [presented.selected, monthOffset]);
