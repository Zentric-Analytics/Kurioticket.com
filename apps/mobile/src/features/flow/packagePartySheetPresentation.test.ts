...
const airportSheet = source.slice(source.indexOf("function AirportSheet"), source.indexOf("const PACKAGE_TRAVELER_ROWS"));
const partySheet = source.slice(source.indexOf("function PackagePartySheet"), source.indexOf("function PackagePartyRow"));
...
test("package airport and party sheets keep isolated height geometry", () => {
  ...
  assert.match(styles, /airportSheet:\{height:"82%"\}/);
  assert.match(styles, /partySheet:\{maxHeight:"64%",paddingHorizontal:15,paddingVertical:13,gap:8\}/);
  assert.match(airportSheet, /style=\{\[styles\.sheet,styles\.airportSheet,\{backgroundColor:ft\.colors\.surface,paddingBottom:18\+motion\.bottomSafeAreaInset\},motion\.sheetStyle\]\}/);
  assert.doesNotMatch(airportSheet, /styles\.partySheet/);
  assert.match(partySheet, /style=\{\[styles\.sheet, styles\.partySheet, \{ backgroundColor: ft\.colors\.surface, paddingBottom: 13 \+ motion\.bottomSafeAreaInset \},motion\.sheetStyle\]\}/);
});
