...
export function SearchResultProductIcons({ icons }: { icons: readonly SearchResultProductIconName[] }) {
  ...
  return <View accessible={false} accessibilityElementsHidden importantForAccessibility="no-hide-descendants" style={[styles.container, icons.length === 3 ? styles.threeIcons : styles.icons, { backgroundColor: ft.colors.input }]}>
    {icons.map(icon => <FlowIcon key={icon} name={icon} size={size} color={ft.colors.icon}/>)}
  </View>;
}

const styles = StyleSheet.create({
  container: { width: 46, height: 46, borderRadius: 12, flexShrink: 0, alignItems: "center", justifyContent: "center", flexDirection: "row" },
  icons: { gap: 2 },
  threeIcons: { gap: 1 },
});
