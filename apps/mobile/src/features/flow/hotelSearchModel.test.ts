...
test("guest and room counters clamp independently to website limits", () => {
  ...
  assert.deepEqual(changeGuests(form, -20), { ...form, guests: 1 });
  assert.equal(changeGuests({ ...form, guests: 12 }, 50).guests, 12);
  assert.equal(changeRooms(form, 50).rooms, 6);
  assert.equal(changeRooms({ ...form, rooms: 1 }, -50).rooms, 1);
  assert.deepEqual(HOTEL_LIMITS, { guests: { min: 1, max: 12 }, rooms: { min: 1, max: 6 } });
});
