...
export type FlightForm = { tripType: FlightTripType; from?: Airport; to?: Airport; departureDate: string; returnDate: string; multiCityLegs: FlightSearchLeg[]; adults: number; children: number; infants: number; cabin?: FlightCabin; currency?: string };
...
const normalizeCurrency = (value: string) => { const normalized=value.trim().toUpperCase(); return /^[A-Z]{3}$/.test(normalized)?normalized:undefined; };
...
export function flightEditSearchParams(params: Record<string, RouteValue>) {
  const tripType = firstFlightParam(params.tripType); const from = firstFlightParam(params.from) || firstFlightParam(params.origin); const to = firstFlightParam(params.to) || firstFlightParam(params.destination); const currency = normalizeCurrency(firstFlightParam(params.currency));
  const common = { tripType, from, to, departureDate: firstFlightParam(params.departureDate), adults: firstFlightParam(params.adults), children: firstFlightParam(params.children), infants: firstFlightParam(params.infants), travelers: firstFlightParam(params.travelers), cabin: firstFlightParam(params.cabin) || firstFlightParam(params.cabinClass), ...(currency ? { currency } : {}) };
