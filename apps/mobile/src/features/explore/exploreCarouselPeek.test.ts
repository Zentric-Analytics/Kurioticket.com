import assert from "node:assert/strict";
import test from "node:test";
import { readFileSync } from "node:fs";

const source = () => readFileSync("src/features/explore/ExploreScreen.tsx", "utf8");
const layoutSource = () => readFileSync("src/features/explore/regionPreviewLayout.ts", "utf8");

test("Explore region carousels reveal a clear next-card preview", () => {
  const screen = source();
  const layout = layoutSource();
  assert.match(layout, /REGION_PREVIEW_CARD_WIDTH_RATIO = 0\.928/);
  assert.match(layout, /REGION_PREVIEW_NEXT_CARD_PEEK_EXPANSION_RATIO = 0\.058/);
  assert.match(layout, /width = windowWidth \* \(REGION_PREVIEW_CARD_WIDTH_RATIO - REGION_PREVIEW_NEXT_CARD_PEEK_EXPANSION_RATIO\)/);
  assert.match(layout, /REGION_PREVIEW_INSET_RATIO = 0\.024/);
  assert.match(layout, /REGION_PREVIEW_GAP_RATIO = 0\.024/);
  assert.match(screen, /regionPreviewCardLayout\(windowWidth\)/);

  const cardWidthRatio = 0.928 - 0.058;
  const nextCardPeekRatio = 1 - 0.024 - cardWidthRatio - 0.024;

  assert.equal(cardWidthRatio, 0.87);
  assert.ok(nextCardPeekRatio >= 0.08);
  assert.ok(nextCardPeekRatio < 0.09);
  assert.match(screen, /snapToInterval={previewCardWidth \+ previewGap}/);
  assert.match(screen, /decelerationRate="fast"/);
});
