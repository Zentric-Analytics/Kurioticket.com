  ...
  return (
    <View style={styles.header}>
      <View style={styles.headerCopy}>
        ...
        <Text style={styles.subtitle}>Discover places from our current catalogue</Text>
      </View>
      <Pressable
      ...
      >
        ...
        <View style={styles.photoCardInfo}>
          <View style={styles.photoCardCopy}>
            <Text numberOfLines={1} style={styles.photoCardTitle}>{airport.city}</Text>
            <Text numberOfLines={1} style={styles.photoCardMeta}>{airport.country} · {airport.code}</Text>
          </View>
      ...
      >
        {image ? (
          ...
          <View style={[styles.savedImage, styles.savedFallback]}>
            <FlowIcon name="compass" color="white" size={26} />
      ...
      <View style={styles.discoveryGrid}>
        ...
        <DiscoveryShortcut
          ...
          onPress={() => onBrowse({
            ...
            subtitle: savedAirports.length
              ? "Destinations saved on this device."
              : "Saved destinations will appear here after you tap a heart.",
      ...
      <View style={styles.sectionBlock}>
        ...
        <View style={[styles.exploreActionGrid, { gap: actionLayout.gap }]}>
          <ExploreActionCard
            icon="globe"
            title="Browse countries"
            copy="View catalogue destinations by country."
            width={actionLayout.cardWidth}
            onPress={() => onBrowse({
              title: "Browse countries",
              subtitle: "Countries represented in our current catalogue.",
              groups: countries(),
          ...
          <ExploreActionCard
            icon="map"
            title="Browse regions"
            copy="Open maintained regional groupings."
            width={actionLayout.cardWidth}
            onPress={() => onBrowse({
              title: "Browse regions",
              subtitle: "Product-maintained groupings for the current catalogue.",
          ...
          <ExploreActionCard
            ...
            copy="Start a validated flight search."
            width={actionLayout.cardWidth}
          ...
          <ExploreActionCard
            icon="hotel"
            title="Search hotels"
            copy="Find stays for your destination."
            width={actionLayout.cardWidth}
        ...
        >
          {HERO_SLIDES.map((slide, index) => (
            <Pressable
              ...
              style={({ pressed }) => [styles.heroPressable, { width: heroWidth }, pressed && styles.cardPressed]}
            >
...
function CompareTab() {
  const actions: { icon: FlowIconName; title: string; copy: string; route: string; tone: object }[] = [
    { icon: "flight", title: "Flights", copy: "Compare provider-backed flight options.", route: "/flights", tone: styles.compareBlue },
    { icon: "hotel", title: "Hotels", copy: "Search stays for your destination.", route: "/hotels", tone: styles.comparePurple },
    { icon: "car", title: "Rental cars", copy: "Search available rental-car options.", route: "/cars", tone: styles.compareGreen },
    { icon: "bell", title: "Price alerts", copy: "Track a valid live flight search.", route: "/price-alerts", tone: styles.compareOrange },
  ];
...
function DestinationAction({
  ...
  const navigate = (product: DestinationProduct) => {
    ...
    navigateFromDestination(
      ...
      product,
      onClose,
      (route, destination) => router.push({ pathname: `/${route}`, params: { destination } }),
      navigating,
    );
  };
            ...
            {airport ? (
              <>
                ...
                <Pressable accessibilityRole="button" accessibilityLabel="Close destination actions" onPress={onClose} style={styles.sheetClose}>
                  <View style={styles.sheetCloseIcon}><FlowIcon name="back" color={MUTED} size={18} /></View>
        ...
        <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.browserList}>
          ...
          {browser?.destinations?.map((airport) => (
            <SearchResultRow
              key={airport.code}
              airport={airport}
              saved={saved.has(airport.code)}
              onSelect={() => choose(airport)}
              onToggle={() => toggle(airport.code)}
            />
          ))}
            ...
            return (
              <View key={group.name} style={styles.browserGroup}>
                ...
                >
                  ...
                  <View style={[styles.groupChevron, open && styles.groupChevronOpen]}>
                    <FlowIcon name="chevron" color={NAVY} size={18} />
                  </View>
                </Pressable>
                {open ? (
...
export function ExploreScreen() {
  ...
  useEffect(() => {
    if (queryActive) {
      void AccessibilityInfo.announceForAccessibility(`${results.length} ${results.length === 1 ? "result" : "results"}`);
    }
    ...
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <ScrollView
        ...
        contentContainerStyle={[
          styles.page,
          { paddingBottom: exploreBottomPadding(65, insets.bottom) + (showFloatingAction ? 66 : 8) },
        ]}
      >
        <Header />
          ...
          <View accessibilityLiveRegion="polite" accessibilityLabel={`${results.length} Explore results`} style={styles.searchResults}>
            ...
            {results.length ? results.map((result) => (
              <SearchResultRow
                ...
                onToggle={() => toggle(result.airport.code)}
              />
            )) : (
      ...
      <DestinationAction
        ...
        onToggle={() => { if (selected) void toggle(selected.code); }}
...
const styles = StyleSheet.create({
  ...
  title: { color: NAVY, fontSize: 31, lineHeight: 38, fontWeight: "900", letterSpacing: -0.6 },
  subtitle: { color: MUTED, fontSize: 12.5, lineHeight: 18 },
  ...
  fallbackOrbLarge: { position: "absolute", width: 190, height: 190, borderRadius: 95, backgroundColor: "rgba(255,255,255,.08)", top: -70, right: -30 },
  fallbackCode: { color: "white", fontSize: 27, fontWeight: "900", letterSpacing: 3 },
  photoCardInfo: { minHeight: 76, flexDirection: "row", alignItems: "center", paddingHorizontal: 15, paddingVertical: 12, gap: 12, backgroundColor: "white" },
  photoCardCopy: { flex: 1, gap: 3 },
  photoCardTitle: { color: NAVY, fontSize: 18, lineHeight: 23, fontWeight: "900" },
  photoCardMeta: { color: MUTED, fontSize: 12.5 },
  ...
  inspirationEyebrow: { color: BLUE, fontSize: 10.5, fontWeight: "900", letterSpacing: 1.6 },
  ...
  compareIntro: { borderRadius: 22, backgroundColor: NAVY, padding: 20, gap: 6, overflow: "hidden" },
  ...
  floatingBrowse: { position: "absolute", alignSelf: "center", minHeight: 52, borderRadius: 26, backgroundColor: NAVY, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingHorizontal: 24, shadowColor: "#071A48", shadowOpacity: 0.28, shadowRadius: 16, shadowOffset: { width: 0, height: 8 }, elevation: 8 },
  floatingPressed: { opacity: 0.9, transform: [{ scale: 0.98 }] },
  floatingBrowseText: { color: "white", fontSize: 13, fontWeight: "900" },
  ...
  sheetClose: { minHeight: 54, borderRadius: 17, backgroundColor: "white", borderWidth: 1, borderColor: BORDER, flexDirection: "row", alignItems: "center", paddingHorizontal: 14, gap: 11 },
  sheetCloseIcon: { width: 34, height: 34, borderRadius: 12, backgroundColor: "#F2F4F8", alignItems: "center", justifyContent: "center", transform: [{ rotate: "-90deg" }] },
