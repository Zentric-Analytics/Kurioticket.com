...
import { AndroidFavoriteButton, androidFavoriteColors } from "../home/AndroidFavoriteButton";
...
export function ExploreScreen() {
  ...
  return <SafeAreaView style={[s.safe, { backgroundColor: theme.background }]} edges={["top"]}>
    ...
    {view === "map" && <ExploreMap place={query.trim() ? (results.length === 1 ? `${results[0].destination.name}, ${results[0].destination.country}` : query.trim()) : ""} airportCode={results.length === 1 ? results[0].destination.primaryAirportCode : undefined} coordinates={results.length === 1 ? { latitude: results[0].destination.latitude, longitude: results[0].destination.longitude } : undefined} />}
...
function RegionPreviewCard({ destination, saved, onSelect, onToggle, width, height, imageHeight }: { destination: LiveExploreDestination; saved: boolean; onSelect: () => void; onToggle: () => void; width: number; height: number; imageHeight: number }) {
  ...
  </Pressable><AndroidFavoriteButton saved={saved} label={`${saved ? "Remove" : "Save"} ${destination.name}`} onPress={onToggle} style={s.heart} variant="webParity" />
  </View>;
}
