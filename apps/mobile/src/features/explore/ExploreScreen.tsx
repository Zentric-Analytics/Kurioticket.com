...
export function ExploreScreen() {
  ...
  return <SafeAreaView style={[s.safe, { backgroundColor: theme.background }]} edges={["top"]}>
    ...
    {view === "map" && <ExploreMap place={query.trim() ? (results.length === 1 ? `${results[0].destination.name}, ${results[0].destination.country}` : query.trim()) : ""} airportCode={results.length === 1 ? results[0].destination.primaryAirportCode : undefined} />}
