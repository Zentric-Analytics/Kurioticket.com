...
export function DestinationResultRow({ destination, saved, onSelect, onToggle }: { destination: Destination; saved: boolean; onSelect: () => void; onToggle: () => void }) {
  const { theme } = useAppTheme();
  return <View style={[s.resultRow, { backgroundColor: theme.surface, borderColor: theme.border }]}>
    <Pressable accessibilityRole="button" accessibilityLabel={`Open details for ${destination.name}, ${destination.country}, ${destination.primaryAirportCode}`} onPress={onSelect} style={s.resultMain}>
      ...
      <View style={s.resultCopy}><Text style={[s.resultTitle, { color: theme.text }]}>{destination.name}</Text><Text style={[s.resultMeta, { color: theme.muted }]}>{destination.country} · {destination.primaryAirportCode}{destination.airportCodes.length > 1 ? ` + ${destination.airportCodes.length - 1} airports` : ""}</Text></View>
    </Pressable>
...
function ExploreHeader({ query, setQuery, input, submit }: { query: string; setQuery: (value: string) => void; input: React.RefObject<TextInput | null>; submit: () => void }) {
  const { theme } = useAppTheme();
  return <><View style={s.header}><Text accessibilityRole="header" style={[s.title, { color: theme.text }]}>Explore</Text></View><View style={[s.search, { backgroundColor: theme.surface, borderColor: theme.border }]}>
    <FlowIcon name="search" color={theme.icon} size={22} />
    <TextInput ref={input} accessibilityLabel="Explore search" accessibilityHint="Search destinations or airports" value={query} onChangeText={setQuery} onSubmitEditing={submit} returnKeyType="search" placeholder="Search destinations or airports" placeholderTextColor={theme.muted} style={[s.searchInput, { color: theme.text }]} />
...
export function ExploreScreen() {
  ...
  return <SafeAreaView style={[s.safe, { backgroundColor: theme.background }]} edges={["top"]}>
    <View style={[s.stableHeader, { backgroundColor: theme.background }]}><ExploreHeader query={query} setQuery={setQuery} input={input} submit={submit} /></View>
    {query.trim() ? <FlatList data={results} keyExtractor={(item) => item.destination.id} keyboardShouldPersistTaps="handled" contentContainerStyle={[s.content, { paddingBottom: bottomPadding }]} ListHeaderComponent={<SectionHeading title={`${results.length} result${results.length === 1 ? "" : "s"}`} />} ListEmptyComponent={<Text style={[s.empty, { backgroundColor: theme.surface, color: theme.muted }]}>No destinations match “{query.trim()}”. Try a city, destination code, airport, or country.</Text>} renderItem={({ item }) => <DestinationResultRow destination={item.destination} saved={savedIds.has(item.destination.id)} onSelect={() => select(item.destination)} onToggle={() => toggle(item.destination.id)} />} />
...
function SectionHeading({ title }: { title: string }) {
  const { theme } = useAppTheme();
  return <View style={s.sectionHeader}><Text accessibilityRole="header" style={[s.sectionTitle, { color: theme.text }]}>{title}</Text></View>;
}
...
function RegionPreviewCard({ destination, saved, onSelect, onToggle, width, height, imageHeight }: { destination: Destination; saved: boolean; onSelect: () => void; onToggle: () => void; width: number; height: number; imageHeight: number }) {
  ...
  const media = destinationMedia(destination.id);
  const [failed, setFailed] = useState(false);
  return <View style={[s.previewCard, { width, height, backgroundColor: theme.surface, borderColor: theme.border }]}><Pressable accessibilityRole="button" accessibilityLabel={`Open details for ${destination.name}, ${destination.country}`} onPress={onSelect} style={s.previewMain}>
    ...
    <View style={[s.previewCopy, { height: height - imageHeight }]}><Text numberOfLines={1} style={[s.previewName, { color: theme.text }]}>{destination.name}</Text><Text numberOfLines={1} style={[s.previewCountry, { color: theme.muted }]}>{destination.country}</Text></View>
...
function ExploreDiscoveryContent({ bottomPadding, select }: { bottomPadding: number; select: (destination: Destination) => void }) {
  ...
  return <FlatList data={REGION_DISCOVERY} keyExtractor={({ region }) => region} contentContainerStyle={[s.discoveryContent, { paddingBottom: bottomPadding }]} renderItem={({ item }) => <View style={s.regionSection}>
    <View style={[s.regionHeader, { paddingHorizontal: previewInset }]}><View><Text accessibilityRole="header" style={[s.regionTitle, { color: theme.text }]}>{item.region}</Text><Text style={[s.regionCount, { color: theme.muted }]}>{item.destinations.length} destinations</Text></View><Pressable accessibilityRole="button" accessibilityLabel={`See all destinations in ${item.region}`} onPress={() => openRegion(item.region)} style={s.seeAll}><Text style={s.seeAllText}>See all</Text><FlowIcon name="chevron" color={BLUE} size={16} /></Pressable></View>
...
export const exploreScreenStyles = StyleSheet.create({
  ...
  regionSection: { marginBottom: 18 }, regionHeader: { minHeight: 58, paddingBottom: 6, flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between" }, regionTitle: { fontSize: 22, lineHeight: 28, fontWeight: "800" }, regionCount: { fontSize: 13, lineHeight: 18, marginTop: 1 }, seeAll: { minHeight: 44, flexDirection: "row", alignItems: "center", paddingLeft: 12 }, seeAllText: { color: BLUE, fontSize: 13, fontWeight: "700" }, previewRow: {}, previewCard: { borderRadius: 6, borderWidth: 1, overflow: "hidden", ...shadow }, previewMain: { flex: 1 }, previewImage: { width: "100%" }, previewCopy: { paddingHorizontal: 14, paddingVertical: 4, justifyContent: "center" }, previewName: { fontSize: 16, lineHeight: 20, fontWeight: "800" }, previewCountry: { fontSize: 12, lineHeight: 16, marginTop: 2 }, heart: {
    position: "absolute",
    right: 10,
    top: 10,
  },
