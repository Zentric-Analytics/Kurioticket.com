...
export function regionPreviewCardLayout(windowWidth: number) {
  const width = windowWidth * (REGION_PREVIEW_CARD_WIDTH_RATIO - REGION_PREVIEW_NEXT_CARD_PEEK_EXPANSION_RATIO);
  const previousHeight = width / REGION_PREVIEW_ASPECT_RATIO;
  const previousImageHeight = width / REGION_PREVIEW_IMAGE_ASPECT_RATIO;
  const imageHeight = previousImageHeight * REGION_PREVIEW_IMAGE_HEIGHT_SCALE;

  return {
    width,
    height: previousHeight + (imageHeight - previousImageHeight),
    imageHeight,
  };
}
