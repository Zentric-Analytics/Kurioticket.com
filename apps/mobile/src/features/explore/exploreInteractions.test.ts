...
test("search covers names, countries, ISO codes, airport codes, airport names and aliases", () => {
  ...
  assert.deepEqual(searchExplore("City skylines"), []);
});
...
test("Explore omits interest discovery and describes destination search accurately", () => {
  const source = screen();
  assert.doesNotMatch(source, /Explore by interest|RESOLVED_INTERESTS|Interest match|mapped to/);
  assert.match(source, /accessibilityHint="Search destinations, countries or airports"/);
  assert.match(source, /placeholder="Search destinations, countries or airports"/);
});
