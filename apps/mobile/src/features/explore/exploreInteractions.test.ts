...
test("Explore omits interest discovery, search, and accessibility copy", () => {
  const source = screen();
  assert.doesNotMatch(source, /Explore by interest|Interests|RESOLVED_INTERESTS|Interest match|mapped to/);
  assert.doesNotMatch(source, /countries or interests|maintained interests/);
  assert.equal(
