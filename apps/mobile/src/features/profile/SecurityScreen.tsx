  ...
  return <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]} edges={["top"]}>
    ...
    <ScreenModal visible={devicesOpen} title={c.activeSessions} closeLabel={c.close} onClose={closeDevices}>
      <View style={styles.devicesContent}>
        ...
        {sessions.length ? sessions.map((item) => <SessionRow key={item.id} item={item} copy={c} date={date(item.lastSeenAt)} onManage={setManagedSession} />) : <View style={[styles.devicesEmpty, { borderColor: theme.border, backgroundColor: theme.surface }]}><Smartphone size={22} color={theme.muted}/><Text style={[styles.rowLabel, { color: theme.text }]}>{c.noActiveSessions}</Text><Text style={[styles.rowDetail, { color: theme.muted, textAlign: "center" }]}>{c.noActiveSessionsHelp}</Text></View>}
      </View>
    </ScreenModal>
    <BottomSheet visible={Boolean(managedSession)} dismissLabel={c.close} onDismiss={() => setManagedSession(null)}>
      <Text style={[styles.sheetTitle, { color: theme.text }]}>{managedSession?.deviceLabel}</Text>
      <Text style={[styles.rowDetail, { color: theme.muted }]}>{c.manageSession}</Text>
      <Pressable accessibilityRole="button" onPress={() => { const item=managedSession;setManagedSession(null);if(item)remove(item); }} style={styles.sheetAction}><Text style={styles.danger}>{c.removeSession}</Text></Pressable>
    </BottomSheet>
...
function SessionRow({item,copy,date,onManage}:{item:SecuritySession;copy:SecurityCopy;date:string;onManage:(item:SecuritySession)=>void}) { const {theme}=useAppTheme();const mobile=item.client.toUpperCase()==="MOBILE";const details=sessionDetails(item,copy);return <View style={[styles.device,{borderColor:theme.border,backgroundColor:theme.surface}]}>{mobile?<Smartphone size={20} color={theme.icon}/>:<Monitor size={20} color={theme.icon}/>}<View style={styles.deviceCopy}><View style={styles.deviceHeading}><Text numberOfLines={1} style={[styles.rowLabel,{color:theme.text}]}>{item.deviceLabel}</Text>{item.isCurrent?<Text style={styles.current}>{copy.currentDevice}</Text>:null}</View>{details?<Text numberOfLines={1} style={[styles.rowDetail,{color:theme.muted}]}>{details}</Text>:null}{item.maskedIp?<Text style={[styles.deviceMeta,{color:theme.muted}]}>{item.maskedIp}</Text>:null}<Text style={[styles.deviceMeta,{color:theme.muted}]}>{copy.lastActive} {date}</Text></View>{!item.isCurrent?<Pressable accessibilityRole="button" accessibilityLabel={`${copy.manageSession}: ${item.deviceLabel}`} onPress={()=>onManage(item)} style={styles.manageSession}><MoreHorizontal size={22} color={theme.icon}/></Pressable>:null}</View>;}
