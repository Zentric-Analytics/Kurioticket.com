...
export function SecurityScreen() {
  ...
  return <SafeAreaView style={[styles.safe, { backgroundColor: theme.background }]} edges={["top"]}>
    ...
    <ScreenModal visible={devicesOpen} title={c.activeSessions} closeLabel={c.close} onClose={closeDevices} onRequestClose={() => managedSession ? setManagedSession(null) : closeDevices()} overlay={sessionSheet}>
      <View style={styles.devicesContent}>
        <SessionSuccessNotice message={devicesSuccess} sequence={devicesSuccessSequence} />
...
function SessionSuccessNotice({ message, sequence }: { message: string; sequence: number }) {
  ...
  if (!displayed) return null;
  return <Animated.View pointerEvents="none" style={[styles.sessionNoticePosition, { opacity }]}><View style={[styles.sessionNotice, { backgroundColor: theme.surface, borderColor: theme.border }]}><Check size={15} strokeWidth={2.5} color="#067647"/><Text style={styles.sessionNoticeText}>{displayed}</Text></View></Animated.View>;
}
