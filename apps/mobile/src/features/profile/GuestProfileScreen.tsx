...
export function GuestProfileScreen() {
  ...
  const sections: ProfileSection[] = [
    ...
    { title: "aboutLegal", items: [
      { label: "terms", icon: "document", destination: { kind: "preview-browser", path: "/mobile/legal/terms-of-service", productionHref: "/(tabs)/profile/terms-of-service" } },
      { label: "privacy", icon: "shield", destination: { kind: "preview-browser", path: "/mobile/legal/privacy-policy", productionHref: "/(tabs)/profile/privacy-policy" } },
    ] },
  ];
  return <SafeAreaView style={[styles.safe, { backgroundColor: theme.dark ? theme.background : "#F5F5F5" }]} edges={["top"]}><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}><View style={styles.header}><Text accessibilityRole="header" style={[styles.title, { color: theme.text }]}>{t("profile")}</Text></View><View style={styles.illustration}><TravelIllustration /></View><Text accessibilityRole="header" style={[styles.heroTitle, { color: theme.text }]}>{t("guestHeroTitle")}</Text><Text style={[styles.heroBody, { color: theme.muted }]}>{t("guestHeroBody")}</Text><Pressable accessibilityRole="button" accessibilityLabel={t("signIn")} accessibilityHint={t("signInHint")} onPress={() => router.push(signInHref("/(tabs)/profile"))} style={({ pressed }) => [styles.signIn, pressed && styles.pressed]}><FlowIcon strokeWidth={1.3} name="person" color="white" size={24} /><Text style={styles.signInText}>{t("signIn")}</Text><View style={styles.signInArrow}><FlowIcon strokeWidth={1.3} name="chevron" color="white" size={22} /></View></Pressable><View style={styles.sections}>{sections.map(section => <ProfileCardSection key={section.title} section={section} />)}</View><AppVersionFooter /></ScrollView></SafeAreaView>;
}
