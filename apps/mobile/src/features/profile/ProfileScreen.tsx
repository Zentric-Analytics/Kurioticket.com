...
function WelcomeCard({ name, email }: { name: string | null; email: string | null }) {
  ...
  return <View accessibilityRole="summary" accessibilityLabel={accessibilityCopy} style={[styles.welcomeCard, { backgroundColor: theme.dark ? "#102B4A" : "#F3F8FF", borderColor: theme.dark ? "#28577E" : "#D6E5F4" }]}>
...
export function AuthenticatedProfileScreen() {
  ...
  const load = useCallback(() => { void readSession().then(session => { setAuthenticated(Boolean(session)); setName(session?.user.name ?? null); setEmail(session?.user.email ?? null); }).catch(() => { setAuthenticated(false); setName(null); setEmail(null); }); void travelApi.notificationUnreadCount().then(({ count }) => setUnreadCount(count)).catch(() => undefined); }, []);
...
const styles = StyleSheet.create({ safe: { flex: 1 }, scroll: { paddingHorizontal: 18, paddingBottom: 24 }, header: { minHeight: 72, flexDirection: "row", alignItems: "center" }, title: { flex: 1, fontSize: 30, lineHeight: 38, fontWeight: "800" }, iconButton: { width: 48, height: 48, alignItems: "center", justifyContent: "center" }, badge: { position: "absolute", right: 0, top: 2, minWidth: 19, height: 19, borderRadius: 10, paddingHorizontal: 4, backgroundColor: "#D92D20", alignItems: "center", justifyContent: "center" }, badgeText: { color: "white", fontSize: 10, fontWeight: "800" }, welcomeCard: { minHeight: 94, borderRadius: 16, borderWidth: 1, paddingHorizontal: 16, paddingVertical: 13, flexDirection: "row", alignItems: "center", marginBottom: 18 }, avatar: { width: 58, height: 58, borderRadius: 29, alignItems: "center", justifyContent: "center", marginRight: 14 }, avatarText: { color: "#FFFFFF", fontSize: 22, lineHeight: 27, fontWeight: "800" }, welcomeCopy: { flex: 1, minWidth: 0 }, greeting: { fontSize: 19, lineHeight: 24, fontWeight: "800" }, welcomeEmail: { fontSize: 12, lineHeight: 17, marginTop: 1, fontWeight: "500" }, welcomeLine: { fontSize: 13, lineHeight: 18, marginTop: 1, fontWeight: "500" }, sections: { gap: 18 }, logout: { minHeight: 56, marginTop: 22, borderRadius: 14, borderWidth: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10 }, logoutText: { color: flowColors.red, fontSize: 16, fontWeight: "700" }, pressed: { opacity: .68 } });
