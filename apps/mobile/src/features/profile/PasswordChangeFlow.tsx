...
function PrimaryButton({ label, loading, onPress }: { label: string; loading: boolean; onPress: () => void }) {
  return (
    <Pressable accessibilityRole="button" accessibilityState={{ busy: loading, disabled: loading }} disabled={loading} onPress={onPress} style={({ pressed }) => [styles.primaryButton, loading && styles.disabled, pressed && !loading && styles.pressed]}>
...
export function PasswordChangeFlow({ active, copy, onRecovery, onUnauthorized, onSuccess }: Props) {
  ...
  const start = async () => {
    ...
    try {
      ...
      if (e instanceof TravelApiError) {
        ...
        if (details.field === "newPassword" || e.message.includes("different from your current")) {
          setFieldErrors((current) => ({ ...current, newPassword: f.newSame }));
        } else if (details.field === "currentPassword" || e.message.toLowerCase().includes("current password")) {
          const attempts = wrongCurrentAttempts + 1;
          setWrongCurrentAttempts(attempts);
          setFieldErrors((current) => ({ ...current, currentPassword: f.currentIncorrect }));
          if (attempts >= 3) setRecoveryAvailable(true);
        } else {
          setGeneralError(e.message);
        }
      } else {
        setGeneralError(copy.loadError);
      }
    } finally {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      else setGeneralError(e instanceof TravelApiError ? e.message : copy.loadError);
    } finally {
      if (generation === requestGeneration.current) setSubmitting(false);
    }
  };
    ...
    return (
      <View style={styles.form}>
        ...
        <View style={styles.fieldGroup}>
          ...
          <TextInput accessibilityLabel={f.codeLabel} keyboardType="number-pad" textContentType="oneTimeCode" maxLength={6} value={code} onChangeText={(value) => { setCode(value.replace(/\D/g, "")); setCodeError(""); setGeneralError(""); setMessage(""); }} autoFocus style={[styles.codeInput, { color: theme.text, borderColor: codeError ? "#B42318" : theme.border, backgroundColor: theme.surface }]} />
          <View style={styles.fieldFeedback}>{codeError ? <Text accessibilityRole="alert" style={styles.error}>{codeError}</Text> : null}</View>
        </View>
        <PrimaryButton label={f.verifyChangeAction} loading={submitting} onPress={() => void confirm()} />
        <Pressable accessibilityRole="button" accessibilityState={{ disabled: resendRemaining > 0 || submitting }} disabled={resendRemaining > 0 || submitting} onPress={() => void resend()} style={({ pressed }) => [styles.resendAction, pressed && resendRemaining === 0 && styles.pressed]}>
          <Text style={[styles.resendText, { color: resendRemaining > 0 ? theme.muted : "#1769E0" }]}>{resendRemaining > 0 ? f.resendIn(resendRemaining) : f.resend}</Text>
        </Pressable>
        <Text style={[styles.expiryHint, { color: theme.muted }]}>{f.expiryHint}</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.form}>
      ...
      {generalError ? <Text accessibilityRole="alert" style={styles.error}>{generalError}</Text> : null}
      <PasswordField label={copy.current} value={passwords.currentPassword} hidden={hidden.currentPassword} error={fieldErrors.currentPassword} onChange={(value) => patch("currentPassword", value)} onToggle={() => setHidden((current) => ({ ...current, currentPassword: !current.currentPassword }))} showLabel={copy.show} hideLabel={copy.hide} returnKeyType="next" />
      <PasswordField label={copy.next} value={passwords.newPassword} hidden={hidden.newPassword} error={fieldErrors.newPassword} onChange={(value) => patch("newPassword", value)} onToggle={() => setHidden((current) => ({ ...current, newPassword: !current.newPassword }))} showLabel={copy.show} hideLabel={copy.hide} returnKeyType="next" />
      <PasswordField label={copy.confirm} value={passwords.confirmPassword} hidden={hidden.confirmPassword} error={fieldErrors.confirmPassword} onChange={(value) => patch("confirmPassword", value)} onToggle={() => setHidden((current) => ({ ...current, confirmPassword: !current.confirmPassword }))} showLabel={copy.show} hideLabel={copy.hide} returnKeyType="done" />
      <PrimaryButton label={copy.change} loading={submitting} onPress={() => void start()} />
      {recoveryAvailable ? (
        <Pressable accessibilityRole="button" accessibilityLabel={f.tryAnotherWay} onPress={onRecovery} style={styles.recoveryAction}>
          <Text style={styles.link}>{f.tryAnotherWay}</Text>
        </Pressable>
      ) : null}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  primaryButton: { minHeight: 52, borderRadius: 11, backgroundColor: "#1769E0", alignItems: "center", justifyContent: "center", marginTop: 2 },
  ...
  recoveryAction: { minHeight: 44, alignSelf: "center", justifyContent: "center", paddingHorizontal: 10 },
  link: { color: "#1769E0", fontSize: 15, fontWeight: "800" },
});
