...
export function PersonalDetailsEmailEditor({
  ...
  const [currentEntry, setCurrentEntry] = useState(true);
  ...
  const continueRemaining = isCodeStep
    ...
    : remaining;
  useEffect(
    () => onDirtyChange(currentEmail.length > 0 || step > 1 || code.length > 0),
    [currentEmail, step, code, onDirtyChange],
  );
  useEffect(() => {
    if (!error) return;
  ...
  async function run(action: "request" | "confirm", isResend = false) {
    if (
      ...
      return;
    if (step === 1 && action === "request" && normalizedEmail(currentEmail) !== normalizedEmail(email)) {
      setError(c.emailCurrentMismatch);
      return;
    }
    ...
    try {
      if (action === "request") {
        ...
        if (step === 1) setCurrentEntry(false);
        if (step !== 1) {
  ...
  const borderColor = theme.dark ? "#75839B" : "#B8C0CC";
  ...
  return (
    <View style={[s.layout, { borderTopColor: theme.border }]}>
      <View style={s.content}>
        {isCodeStep && <Text style={[s.compactHelp, { color: theme.muted }]}>{c.emailCodeSent + " " + (step === 1 ? email : requestedEmail)}</Text>}
        {isCodeStep ? (
          <>
