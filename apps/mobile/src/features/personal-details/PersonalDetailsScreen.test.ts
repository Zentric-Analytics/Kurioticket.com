...
test("read-only and edit content have no outer card treatment",()=>{assert.doesNotMatch(screen,/\bs\.card\b|\bcard:\s*\{|formCard/);assert.doesNotMatch(screen,/backgroundColor:theme\.surface,borderColor:theme\.border/);assert.match(screen,/<View><Text style=\{\[s\.description/);assert.match(screen,/<View style=\{s\.formContent\}>/)});
test("country and nationality search selectors wait for an explicit search-field tap",()=>{assert.doesNotMatch(screen,/<TextInput autoFocus accessibilityLabel=\{c\.searchCountry\}/);assert.match(screen,/dismissKeyboard=\{selector==="nationality"\|\|selector==="addressCountry"\}/);assert.match(screen,/if\(visible&&dismissKeyboard\)Keyboard\.dismiss\(\)/);assert.match(screen,/if\(dismissKeyboard\)Keyboard\.dismiss\(\);onClose\(\)/)});
test("opening Address does not programmatically focus its fields",()=>{const addressSection=screen.slice(screen.indexOf("{c.addressSection}"),screen.indexOf("<View style={s.actions}>"));assert.ok(addressSection.length>0);assert.doesNotMatch(addressSection,/autoFocus|\.focus\(/);assert.match(addressSection,/<Field key=\{key\}/)});
