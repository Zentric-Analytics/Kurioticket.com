...
import { canActivateRouteWatch, canCreateOrReactivatePriceAlert, isMobileProductAvailable, loadFeatureAvailability, normalizeFeatureAvailability, resetFeatureAvailabilityCacheForTests, safeFeatureAvailability } from "./featureAvailabilityModel";

const disabled = { flightSearch: false, hotelSearch: false, carSearch: false, deals: false, priceAlerts: false, routeWatch: false };
...
test("disabled Route Watch prevents activation", () => assert.equal(canActivateRouteWatch(disabled), false));
