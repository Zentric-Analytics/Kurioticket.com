...
export function AuthWelcomeScreen({ onEmail, onGoogle, onGuest, busy }: { onEmail: () => void; onGoogle: () => void; onGuest: () => void; busy?: boolean }) {
  return <ImageBackground source={hero} resizeMode="cover" imageStyle={styles.image} style={styles.background}><View style={styles.overlay} /><SafeAreaView style={styles.safe}>
    ...
    <View style={styles.panel}><Text style={styles.title}>Continue your journey</Text><Text style={styles.body}>Sign in or create an account to get the{"\n"}best travel experience.</Text>
...
const styles = StyleSheet.create({
  background: { flex: 1 }, image: { resizeMode: "cover" }, overlay: { ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(255,255,255,.06)" }, safe: { flex: 1, justifyContent: "space-between" },
