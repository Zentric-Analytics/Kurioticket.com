...
export function AuthWelcomeScreen({ onEmail, onGoogle, onPasskey, onGuest, busy, error }: { onEmail: () => void; onGoogle: () => void; onPasskey?: () => void; onGuest: () => void; busy?: boolean; error?: string }) {
  ...
  return <View style={styles.background}><Image source={hero} resizeMode="cover" style={[styles.image, heroStyle]} /><View style={styles.overlay} /><SafeAreaView style={styles.safe} edges={["top"]}>
    ...
    <View style={[styles.panel, height < 700 && styles.compactPanel, { paddingBottom: (height < 700 ? 6 : 10) + insets.bottom }]}><Text style={styles.title}>Continue your journey</Text><Text style={styles.body}>Sign in or create an account to get the{"\n"}best travel experience.</Text>
      <View style={[styles.buttons, height < 700 && styles.compactButtons]}><AuthButton label="Continue with Email" onPress={onEmail} disabled={busy} icon={<AuthIcon name="mail" color="white" size={21} />} /><AuthButton label="Continue with Google" onPress={onGoogle} disabled={busy} secondary icon={<GoogleG />} />{onPasskey ? <AuthButton label="Continue with passkey" onPress={onPasskey} disabled={busy} secondary icon={<AuthIcon name="key" color={authColors.navy} size={21} />} /> : null}<AuthButton label="Continue as Guest" onPress={onGuest} disabled={busy} secondary icon={<AuthIcon name="user" color={authColors.navy} size={21} />} /></View>
...
const styles = StyleSheet.create({
  ...
  compactPanel: { paddingTop: 10 }, compactButtons: { gap: 6, marginTop: 8 },
