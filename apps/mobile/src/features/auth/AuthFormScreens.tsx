...
import { AuthButton, authColors, ErrorText, Field, FormHeading, FormShell, SecurityMessage, SuccessText } from "./AuthPrimitives";
...
export function PasswordScreen({ onBack, onSubmit, onForgot, loading, error, notice }: { onBack: () => void; onSubmit: (password: string) => void; onForgot: () => void; loading: boolean; error?: string; notice?: string }) {
  ...
  return <FormShell onBack={onBack}><FormHeading icon="lock" title="Welcome back!" body="Enter your password to continue." />
    ...
    <Pressable accessibilityRole="button" onPress={onForgot} style={styles.forgot}><Text style={styles.action}>Forgot password?</Text></Pressable><SuccessText>{notice}</SuccessText><ErrorText>{error}</ErrorText><AuthButton label="Sign in" onPress={() => onSubmit(password)} loading={loading} disabled={!password} /><SecurityMessage />
  </FormShell>;
}
