...
test("airport payload parsing accepts optional country, rejects malformed rows, deduplicates codes, and limits results", () => {
  const valid = Array.from({ length: 10 }, (_, index) => ({ code: `A${String.fromCharCode(65 + Math.floor(index / 26))}${String.fromCharCode(65 + index % 26)}`, airport: `Airport ${index}`, city: "City", country: "Country", type: "airport" as const }));
