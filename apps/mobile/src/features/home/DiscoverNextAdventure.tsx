...
function DiscoveryCard({
  ...
  return (
    ...
    >
      ...
      >
        <Text numberOfLines={2} style={[styles.title, { color: ft.colors.textPrimary }]}>{adventure.title}</Text>
        <Text style={[styles.route, { color: ft.colors.textSecondary }]}>{route}</Text>
        <Text style={[styles.tripSummary, { color: ft.colors.textMuted }]}>ONE WAY · ECONOMY · 1 TRAVELER</Text>
        <Text style={[styles.from, { color: ft.colors.textSecondary }]}>From</Text>
      </View>
    </Pressable>
  );
}
...
const styles = StyleSheet.create({
  ...
  grid: { flexDirection: "row", flexWrap: "wrap", columnGap: DISCOVERY_GRID_LAYOUT.columnGap, rowGap: DISCOVERY_GRID_LAYOUT.rowGap },
